  ...
  def jsonOf[A](implicit reader: Reader[A]): EntityDecoder[A] =
    json.flatMapR { json =>
      try DecodeResult.success(reader.read(json))
        ...
        case e: MappingException => DecodeResult.failure(InvalidRequestBodyFailure("Could not map JSON", Some(e)))
      }
    }
  ...
  def jsonExtract[A](implicit formats: Formats, manifest: Manifest[A]): EntityDecoder[A] =
    json.flatMapR { json =>
      try DecodeResult.success(json.extract[A])
        ...
        case NonFatal(e) => DecodeResult.failure(InvalidRequestBodyFailure("Could not extract JSON", Some(e)))
      }
    }
