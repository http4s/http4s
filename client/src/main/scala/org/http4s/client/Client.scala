...
object Client {
  /** Creates a mock client from the specified service.  Useful for generating
    ...
    * 
    * @param dispose action to dispose of responses generated by this client. 
    * Defaults to a no-op.
    * 
    * @param shutdown action when this client is shutdown.  Defaults to a no-op.
    */
  def mock(service: HttpService, 
           dispose: Task[Unit] = Task.now(()),
           shutdown: Task[Unit] = Task.now(())): Client =
    Client(
      service.map(resp => DisposableResponse(resp, dispose)),
      shutdown
    )
}
