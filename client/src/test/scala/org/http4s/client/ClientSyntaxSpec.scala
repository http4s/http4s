...
class ClientSyntaxSpec extends Http4sSpec with Http4sClientDsl[IO] with MustThrownMatchers {
  ...
  "Client" should {
    ...
    "fetch Uris with expectOr" in {
      client.expectOr[String](req.uri) { _ => IO.pure(SadTrombone) } must returnValue("hello") 
    }
    ...
    "fetch requests with expectOr" in {
      client.expectOr[String](req) { _ => IO.pure(SadTrombone) } must returnValue("hello")
    }
    ...
    "fetch request tasks with expectOr" in {
      client.expectOr[String](IO.pure(req)) { _ => IO.pure(SadTrombone) } must returnValue("hello")
    }
    ...
    "handle an unexpected status when calling a URI with expectOr" in {
      ...
      client.expectOr[String](uri("http://www.foo.com/status/500")) { resp =>
