...
// service: org.http4s.HttpService = Kleisli(org.http4s.package$HttpService$$$Lambda$6431/33542387@65976657)
```
...
task: scalaz.concurrent.Task[org.http4s.Response] = scalaz.concurrent.Task@379e276
```
...
```scala
scala> val okTask = Ok()
okTask: scalaz.concurrent.Task[org.http4s.Response] = scalaz.concurrent.Task@7d031a4a
...
scala> val task = Ok(Future {
     ...
     | })
task: scalaz.concurrent.Task[org.http4s.Response] = scalaz.concurrent.Task@301bf931
...
scala> val task = Ok(Task {
     ...
     | })
task: scalaz.concurrent.Task[org.http4s.Response] = scalaz.concurrent.Task@4b1296e5
