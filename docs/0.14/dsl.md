...
// service: org.http4s.HttpService = Kleisli(org.http4s.package$HttpService$$$Lambda$6484/769504188@7f8925bb)
```
...
task: scalaz.concurrent.Task[org.http4s.Response] = scalaz.concurrent.Task@65472132
```
...
```scala
scala> val okTask = Ok()
okTask: scalaz.concurrent.Task[org.http4s.Response] = scalaz.concurrent.Task@17c0e121
...
scala> val task = Ok(Future {
     ...
     | })
task: scalaz.concurrent.Task[org.http4s.Response] = scalaz.concurrent.Task@7ba3ae27
...
scala> val task = Ok(Task {
     ...
     | })
task: scalaz.concurrent.Task[org.http4s.Response] = scalaz.concurrent.Task@1b83b67c
