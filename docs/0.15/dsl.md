...
task: scalaz.concurrent.Task[org.http4s.Response] = scalaz.concurrent.Task@b1cd759
```
...
```scala
scala> val okTask = Ok()
okTask: scalaz.concurrent.Task[org.http4s.Response] = scalaz.concurrent.Task@41316c7e
...
scala> val task = Ok(Future {
     ...
     | })
task: scalaz.concurrent.Task[org.http4s.Response] = scalaz.concurrent.Task@713f2948
...
scala> val task = Ok(Task {
     ...
     | })
task: scalaz.concurrent.Task[org.http4s.Response] = scalaz.concurrent.Task@6a18818b
