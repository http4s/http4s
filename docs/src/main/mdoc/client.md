...
```
```scala mdoc
val registry2 = new CollectorRegistry()
val requestMethodClassifier2 = (r: Request[IO]) => Some(r.method.toString.toLowerCase)
val meteredClient2 = Prometheus[IO](registry2, "prefix").map(
  Metrics[IO](_, requestMethodClassifier2)(httpClient)
)
```
