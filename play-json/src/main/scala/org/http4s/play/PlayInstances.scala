...
import org.http4s.{DecodeResult, EntityDecoder, EntityEncoder, InvalidMessageBodyFailure, MediaType, Message, Uri, jawn}
...
trait PlayInstances {
  ...
  def jsonOf[F[_]: Sync, A](implicit decoder: Reads[A]): EntityDecoder[F, A] =
    jsonDecoder[F].flatMapR { json =>
      decoder
        .reads(json)
        .fold(
          _ => DecodeResult.failure(InvalidMessageBodyFailure(s"Could not decode JSON: $json", None)),
  ...
  def jsonEncoderOf[F[_]: EntityEncoder[?[_], String]: Applicative, A: Writes]: EntityEncoder[F, A] =
