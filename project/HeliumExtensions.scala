...
/** Extensions for the Laika toolkit to implement features not yet available in Laika Core.
  ...
  * used when it was generated by Hugo. It's implemented and documented in the inner `PrettyURLs`
  * type. The other one (`CodeVariables`) enables variable substitutions in string literals in code
  * blocks, which is currently used solely for injecting version numbers from the build.
  *
  * Finally, the `applyTo` method applies these two extensions to an instance of default Helium
  * configuration.
  */
object HeliumExtensions {

  /** Implements the Pretty-URL scheme previously used with the http4s site when it was generated by
    * Hugo.
    *
    * It consists of two parts: one (`treeProcessor`) re-arranges the document tree in a way that
    * all pages are moved into a sub-directory with that page becoming the only content of the
    * directory. The name of this document will always be `index.md`, meaning `someTitle.html`
    * changes to `someTitle/index.html`.
    *
    * The other part (`renderOverride`) overrides the default renderer for internal links to adjust
    * them for the changes performed by the `treeProcessor`.
    */
  object PrettyURLs {
  ...
  /** Implements variable substitutions for string literals in code blocks. Laika supports variable
    * substitutions out of the box for regular markup, but not yet for use inside code blocks.
    *
    * The syntax to be used inside code blocks is slightly different than the one for regular markup
    * substitions, as its syntax (`${foo.bar}`) would conflict with many languages, including Scala.
    * Instead, for code blocks, it has to written as `@{foo.bar}`.
    *
    * The extension matches on AST nodes representing string literals, which means that it only
    * works when Laika's internal syntax highlighters are active (by adding the `SyntaxHighlighting`
  ...
  /** Applies the two feature extensions implemented by this class to a default Helium configuration
    * instance.
    *
    * @param helium
    *   the Helium configuration as set up by other parts of this build
    * @param variables
    *   key/value mappings for variable substitutions that can be used in markup and code blocks
    * @param versionLinks
    *   all known paths of configured versions of the site, for link validation
    * @return
    *   a theme provider that merges the default Helium theme with the two extensions from this
    *   class
    */
  def applyTo(
