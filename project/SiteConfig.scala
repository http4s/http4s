...
/** Shared configuration for the `docs` and `website` projects.
  *
  * This class represents relatively trivial configuration code talking to well-documented public
  * APIs of the Laika toolkit. It is the only Laika-related build artifact that is intended to live
  * in this repo indefinitely (the others are temporary workarounds for features not yet part of
  * Laika Core).
  *
  * The configuration code is fairly verbose as Laika prefers code over HOCON for global config. It
  ...
  *   - Style adjustments to override the default Helium color theme for the orange/grey theme used
  *     by http4s.
  *   - Generators for variable substitutions that are used to inject version numbers in markup
  *     pages.
