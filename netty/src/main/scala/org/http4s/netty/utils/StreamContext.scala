...
import org.http4s.netty.spdy.{InboundWindow, ConnectionOutboundWindow, AbstractStream}
...
/** Manages the stream ID's for the SPDY protocol
  ...
  */
abstract class StreamContext[S <: AbstractStream](val isServer: Boolean)
                  extends InboundWindow
                  with ConnectionOutboundWindow
                  with Logging {
