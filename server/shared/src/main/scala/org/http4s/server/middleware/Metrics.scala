...
object Metrics {
  ...
  /** A server middleware capable of recording metrics
    *
    * Same as [[apply]], but can classify requests effectually, e.g. performing side-effects. Failed
    * attempt to classify the request (e.g. failing with `F.raiseError`) leads to not recording
