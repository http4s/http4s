...
sealed abstract class WebSocketBuilder[F[_]: Applicative] private (
  ...
  def withMaxMessageSize(maxMessageSize: Long): WebSocketBuilder[F] =
  ...
  def withMaxFragmentCount(maxFragmentCount: Int): WebSocketBuilder[F] =
  ...
  )(implicit F: Temporal[F]): WebSocketBuilder[F] =
    copy(heartbeat = Some(Heartbeat.start(every, frame)))

  /** Do not send pings, the default. */
  def withoutHeartbeat: WebSocketBuilder[F] =
    copy(heartbeat = None)
...
object WebSocketBuilder {
  ...
  private def withKey[F[_]: Applicative](
    ...
    impl(
      ...
      maxMessageSize = WebSocketBuilder.DefaultMaxMessageSize,
