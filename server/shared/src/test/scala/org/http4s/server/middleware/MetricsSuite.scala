...
class MetricsSuite extends Http4sSuite {
  ...
  test("Server Metrics middleware should record response body size when body is consumed") {
    ...
    for {
      ...
      metricsRoutes = Metrics(ops, classifierF = (_: Request[IO]) => Some("test"))(testRoutes)
      req = Request[IO](Method.GET, uri"/test")

      resp <- metricsRoutes.orNotFound(req)
  ...
  test("Server Metrics middleware should record correct body size for streamed response") {
    ...
    for {
      ...
      metricsRoutes = Metrics(ops)(testRoutes)
  ...
  test("Server Metrics middleware should not record body size if body is not consumed") {
    for {
      bodySizeRef <- Ref.of[IO, List[RecordedBodySize]](List.empty)
      ops = recordingMetricsOps(bodySizeRef)
      ...
      _ <- metricsRoutes.orNotFound(req)
      recorded <- bodySizeRef.get
    } yield assertEquals(
      recorded.length,
      0,
