...
class WebSocketBuilderSuite extends Http4sSuite {
  ...
  // this could be your HTTP application taking a WebSocketBuilder as an argument
  private def simpleWsApp(wsb: WebSocketBuilder[IO]) =
    HttpRoutes
  ...
  test("return web socket route default message") {
    WebSocketBuilder[IO].flatMap { wsb =>
      simpleWsApp(wsb)
  ...
  private def webSocket(build: WebSocketBuilder[IO] => IO[Response[IO]]): IO[WebSocket[IO]] =
    WebSocketBuilder[IO].flatMap { wsb =>
  ...
  private def outgoing(
      build: WebSocketBuilder[IO] => IO[Response[IO]],
