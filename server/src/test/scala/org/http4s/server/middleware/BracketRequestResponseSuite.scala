...
final class BracketRequestResponseSuite extends Http4sSuite {
  ...
  test("When no errors occur, acquire, release, or the middleware should yield the expected values") {
      for {
        acquireRef <- Ref.of[IO, Long](0L)
        releaseRef <- Ref.of[IO, Long](0L)
        middleware =
          BracketRequestResponse.bracketRequestResponseCaseRoutes[IO, Long](acquireRef.updateAndGet(_ + 1L)) {
            case (_, ec) =>
              IO(assertEquals(ec, ExitCase.Completed)) *> releaseRef.update(_ + 1L)
          }
        routes = middleware(
          Kleisli((contextRequest: ContextRequest[IO, Long]) =>
            OptionT.liftF(IO(Response(status = Status.Ok).withEntity(contextRequest.context.toString)))
          )
        )
  ...
  test("When more than one request is running at a time, release Refs should reflect the current state") {
      for {
        acquireRef <- Ref.of[IO, Long](0L)
        releaseRef <- Ref.of[IO, Long](0L)
        middleware =
  ...
  test("When an error occurs during processing of the Response body, acquire/release still execute") {
      for {
        acquireRef <- Ref.of[IO, Long](0L)
        releaseRef <- Ref.of[IO, Long](0L)
  ...
  test("When an error occurs during acquire, release should not run") {
    ...
    val middleware: ContextMiddleware[IO, Unit] =
      BracketRequestResponse.bracketRequestResponseCaseRoutes[IO, Unit](IO.raiseError[Unit](error)) { case _ =>
  ...
  test("When an error occurs during release, acquire should execute correctly and release should only be attempted once") {
    val error: Throwable = new AssertionError
    for {
        acquireRef <- Ref.of[IO, Long](0L)
        releaseRef <- Ref.of[IO, Long](0L)
        middleware =
          BracketRequestResponse.bracketRequestResponseCaseRoutes[IO, Long](acquireRef.updateAndGet(_ + 1L)) {
            case (_, ec) =>
              IO(assertEquals(ec, ExitCase.Completed)) *> releaseRef.update(_ + 1L) *> IO.raiseError[Unit](error)
          }
        routes = middleware(
          Kleisli((contextRequest: ContextRequest[IO, Long]) =>
            OptionT.liftF(IO(Response(status = Status.Ok).withEntity(contextRequest.context.toString)))
