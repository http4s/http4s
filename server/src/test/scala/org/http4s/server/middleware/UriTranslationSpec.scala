...
class UriTranslationSpec extends Http4sSpec {
  ...
  "UriTranslation" should {
    "match a matching request" in {
      ...
      trans1.orNotFound(req).unsafePerformSync.status must_== (Ok)
      trans2.orNotFound(req).unsafePerformSync.status must_== (Ok)
      service.orNotFound(req).unsafePerformSync.status must_== (NotFound)
    }

    "Not match a request missing the prefix" in {
      val req = Request(uri = Uri(path = "/foo"))
      trans1.orNotFound(req).unsafePerformSync.status must_== (NotFound)
      trans2.orNotFound(req).unsafePerformSync.status must_== (NotFound)
      service.orNotFound(req).unsafePerformSync.status must_== (Ok)
    }

    "Split the Uri into scriptName and pathInfo" in {
      val req = Request(uri = Uri(path = "/http4s/checkattr"))
      val resp = trans1.orNotFound(req).unsafePerformSync
      resp.status must_== (Ok)
      val s = new String(resp.body.runLog.unsafePerformSync.reduce(_ ++ _).toArray, StandardCharsets.UTF_8)
