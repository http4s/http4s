...
class UrlFormLifterSpec extends Http4sSpec {
  ...
  "UrlFormLifter" should {
    "Add application/x-www-form-urlencoded bodies to the query params" in {
      val req = Request(method = Method.POST).withBody(urlForm).unsafePerformSync
    ...
    "Add application/x-www-form-urlencoded bodies after query params" in {
      val req = Request(method = Method.POST, uri = Uri.uri("/foo?foo=biz")).withBody(urlForm).unsafePerformSync

      val resp = service.orNotFound(req).unsafePerformSync
      resp.status must_== Status.Ok
      resp.as[String].unsafePerformSync must_== "biz,bar"
    }

    "Ignore Requests that don't have application/x-www-form-urlencoded bodies" in {
      val req = Request(method = Method.POST).withBody("foo").unsafePerformSync
