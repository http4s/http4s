...
class TimeoutSpec extends Http4sSpec with NoTimeConversions {
  ...
  val myservice: HttpService = {
    case req if req.uri.path == "/fast" => Some(ResponseBuilder(Status.Ok, "Fast"))
    case req if req.uri.path == "/slow" => Some(Task(Thread.sleep(1000)).flatMap(_ => ResponseBuilder(Status.Ok, "Slow")))
    case _ => None
  }
  ...
  "Timeout Middleware" should {
    "Have no effect if the response is not delayed" in {
      ...
      timeoutService(req).get.run.status must_==(Status.Ok)
    }

    "return a timeout if the result takes too long" in {
      val req = Request(GET, uri("/slow"))

      timeoutService(req).get.run.status must_==(Status.RequestTimeout)
    }

    "Handle infinite durations" in {
      val service = Timeout(Duration.Inf)(myservice)

      service(Request(GET, uri("/slow"))).get.run.status must_==(Status.Ok)
    }
  }
