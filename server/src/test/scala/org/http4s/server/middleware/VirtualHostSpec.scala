  ...
  "VirtualHost" >> {
    ...
    "exact" should {
      "return a 400 BadRequest when no header is present on a NON HTTP/1.0 request" in {
        ...
        virtualServices.apply(req1) must runToStatus(BadRequest)
        virtualServices.apply(req2) must runToStatus(BadRequest)
      }
      ...
      "honor the Host header host" in {
        ...
        virtualServices.apply(req) must runToBody("servicea")
      }
      ...
      "honor the Host header port" in {
        ...
        virtualServices.apply(req) must runToBody("serviceb")
      }
      ...
      "ignore the Host header port if not specified" in {
        ...
        virtualServices.apply(good) must runToBody("servicea")
      }
      ...
      "result in a 404 if the hosts fail to match" in {
        ...
        virtualServices.apply(req) must runToStatus(NotFound)
      }
    }
    ...
    "wildcard" should {
      ...
      "allow for a dash in the service" in {
        ...
        virtualServices.apply(req) must runToBody("default")
      }
