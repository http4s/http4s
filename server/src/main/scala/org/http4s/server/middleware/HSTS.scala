...
object HSTS {
  ...
  def apply[F[_]: Functor](service: HttpService[F]): HttpService[F] = apply(service, defaultHSTSPolicy)
  ...
  def unsafeFromDuration[F[_]: Functor](service: HttpService[F], maxAge: FiniteDuration = 365.days, includeSubDomains: Boolean = true, preload: Boolean = false): HttpService[F] = {
