...
object ChunkAggregator {
  def apply[F[_]](service: HttpService[F])(implicit F: Effect[F]): HttpService[F] =
    service.flatMapF { response =>
      OptionT.liftF(response.body.chunks.runFold((Segment.empty[Byte], 0L)) {
        case ((seg, len), c) => (seg ++ c.toSegment, len + c.size)
      }.map { case (body, len) =>
        removeChunkedTransferEncoding(response.withBodyStream(Stream.segment(body)), len)
      })
    }

  private def removeChunkedTransferEncoding[F[_]: Functor](resp: Response[F], len: Long): Response[F] =
