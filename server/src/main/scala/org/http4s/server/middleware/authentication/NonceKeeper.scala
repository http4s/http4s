...
private[authentication] class NonceKeeper(
  ...
  /** Removes nonces that are older than staleTimeout Note: this _MUST_ be executed inside a block
    * synchronized on `nonces`
    */
  ...
  /** Checks if the nonce {@link data} is known and the {@link nc} value is correct. If this is so,
    * the nc value associated with the nonce is increased and the appropriate status is returned.
    * @param data
    *   The nonce.
    * @param nc
    *   The nonce counter.
    * @return
    *   A reply indicating the status of (data, nc).
    */
