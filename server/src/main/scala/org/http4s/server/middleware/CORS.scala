...
sealed class CORSPolicy(
  ...
  /** Allow credentials. Sends an `Access-Control-Allow-Credentials: *` on valid CORS requests if
    * true, and omits the header if false.
    *
  ...
  /** Returns a static value in `Access-Control-Allow-Headers` on preflight requests consisting of
