...
object CORS {
  ...
  def apply[F[_]](service: HttpService[F], config: CORSConfig = DefaultCORSConfig)
    ...
    Service.lift { req =>
      ...
      def corsHeaders(origin: String, acrm: String, isPreflight: Boolean)(resp: Response[F]): Response[F] = {
        val methodBasedHeader =
          if (isPreflight) config.allowedHeaders.map(headerFromStrings("Access-Control-Allow-Headers", _))
