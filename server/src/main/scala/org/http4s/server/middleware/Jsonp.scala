...
object Jsonp  {
  ...
  def apply[F[_]: Functor](callbackParam: String)(service: HttpService[F])
    ...
    HttpService.lift { req =>
      req.params.get(callbackParam) match {
        ...
        case Some(invalidCallback) =>
          ...
          Response[F](Status.BadRequest).withBody(s"Not a valid callback name.").widen[MaybeResponse[F]]
        case None =>
          service(req)
      }
    }
