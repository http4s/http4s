...
class Fs2ClientCall[F[_], Request, Response](
    private[client] val call: ClientCall[Request, Response])
    extends AnyVal {
  ...
  private def start(listener: ClientCall.Listener[Response],
                    metadata: Metadata)(implicit F: Sync[F]): F[Unit] =
    F.delay(call.start(listener, metadata))

  def startListener[A <: ClientCall.Listener[Response]](createListener: F[A])(
  ...
  def sendStream(stream: Stream[F, Request])(
      implicit F: Sync[F]): Stream[F, Unit] = {
    stream.evalMap(sendMessage) ++ Stream.eval(halfClose)
  }

  def unaryToUnaryCall(message: Request)(implicit F: Async[F],
  ...
  def streamingToUnaryCall(messages: Stream[F, Request])(
      implicit F: Effect[F],
      ec: ExecutionContext): F[Response] = {
    for {
      listener <- startListener(Fs2UnaryClientCallListener[F, Response])
      result <- Stream
        .eval(listener.getValue)
        .concurrently(sendStream(messages))
        .compile
        .last
    } yield result.get
  }

  def unaryToStreamingCall(message: Request)(
      implicit F: Async[F],
      ec: ExecutionContext): Stream[F, Response] = {
    for {
      listener <- Stream.eval(startListener(Fs2StreamClientCallListener[
        F,
        Response](call.request)) <* sendSingleMessage(message))
  ...
  def streamingToStreamingCall(messages: Stream[F, Request])(
    ...
    for {
      listener <- Stream.eval(
        startListener(Fs2StreamClientCallListener[F, Response](call.request)))
...
object Fs2ClientCall {
  class PartiallyAppliedClientCall[F[_]](val dummy: Boolean = false)
      extends AnyVal {
