...
private[server] trait Fs2ServerCallListener[F[_], G[_], Request, Response] {
  ...
  def handleUnaryResponse(headers: Metadata, response: F[Response])(
  ...
  def handleStreamResponse(headers: Metadata, response: Stream[F, Response])(
      implicit F: Sync[F]): F[Unit] =
    (call.sendHeaders(headers) *> call.request(1) *>
      (response.evalMap(call.sendMessage) ++
        Stream.eval(call.closeStream(Status.OK, new Metadata()))).compile.drain)
      .handleErrorWith(reportError)

  def unsafeRun(f: F[Unit])(implicit F: Effect[F]): Unit =
    F.runAsync(f)(_.fold(IO.raiseError, _ => IO.unit)).unsafeRunSync()

  def unsafeUnaryResponse(
      headers: Metadata,
      implementation: G[Request] => F[Response])(implicit F: Effect[F]): Unit =
    unsafeRun(handleUnaryResponse(headers, implementation(source)))

  def unsafeStreamResponse(headers: Metadata,
