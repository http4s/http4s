...
object JsonDebugErrorHandler {
  ...
  private[this] val messageFailureLogger = org.log4s.getLogger("org.http4s.circe.jsondebugerrorhandler.message-failures")
  private[this] val serviceErrorLogger = org.log4s.getLogger("org.http4s.circe.jsondebugerrorhandler.service-errors")
  ...
  )(implicit F: Sync[F]) : Kleisli[F, Request[F], Response[F]] = Kleisli{
    req =>
        ...
        case mf: MessageFailure =>
          messageFailureLogger.debug(mf)(
            s"""Message failure handling request: ${req.method} ${req.pathInfo} from ${req.remoteAddr
              .getOrElse("<unknown>")}""")
          mf.toHttpResponse[F](req.httpVersion).map{
            firstResp =>
            Response[F](
  ...
  private def encodeRequest[F[_]](req: Request[F], redactWhen: CaseInsensitiveString => Boolean): Json = {
    Json.obj(
      "method" -> req.method.name.asJson,
      "uri" -> Json.obj(
        "authority" -> req.uri.authority.map( auth => 
          Json.obj(
