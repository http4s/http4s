...
trait CirceInstances {
  ...
  def jsonDecoderByteBuffer[F[_]: Sync]: EntityDecoder[F, Json] =
    EntityDecoder.decodeBy(MediaType.application.`application/json`)(jsonDecoderByteBufferImpl[F])
  ...
  def jsonDecoderAdaptive[F[_]: Sync](cutoff: Long): EntityDecoder[F, Json] =
    EntityDecoder.decodeBy(MediaType.application.`application/json`) { msg =>
