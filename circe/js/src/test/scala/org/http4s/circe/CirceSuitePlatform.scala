...
trait CirceSuitePlatform extends Http4sSuite with CatsEffectAssertions {
  ...
  def testJsonDecoder(decoder: EntityDecoder[IO, Json]) = {
  ...
  def testJsonDecoderError(decoder: EntityDecoder[IO, Json])(
  ...
  private def testErrors(decoder: EntityDecoder[IO, Json])(
      emptyBody: PartialFunction[DecodeFailure, Boolean],
      parseError: PartialFunction[DecodeFailure, Boolean]
  ) = {
