      ...
      case None => 
        createConnection(host, port, useTLS, priorKnowledge).allocated.flatMap(tup => 
          connections.modify{map => 
            ...
            val out = current.fold(Either.left[H2Connection[F], (H2Connection[F], F[Unit])]((tup._1)))(r => Either.right((r._1, tup._2)))
            (newMap, out)
          }.flatMap{
      ...
      if (useTLS){
        for {
          ...
          socketType <- protocol match {
            ...
            case Some(other) => Resource.raiseError[F, SocketType, Throwable](new Throwable("Unknown Protocol Received"))
