...
private[h2] object PseudoHeaders {
  ...
  def headersToRequestNoBody(hI: NonEmptyList[(String, String)]): Option[Request[fs2.Pure]] = {
    ...
    val scheme = findWithNoDuplicates(pseudos)(_._1 === SCHEME).map(_._2).map(Uri.Scheme.unsafeFromString(_))
  ...
  def headersToResponseNoBody(headers: NonEmptyList[(String, String)]): Option[Response[fs2.Pure]] = {
    ...
    statusO.map(s =>
      Response(status = s, httpVersion = HttpVersion.`HTTP/2`, headers = h)
    )
  }
