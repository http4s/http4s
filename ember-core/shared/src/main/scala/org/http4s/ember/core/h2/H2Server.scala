        ...
        created <-
          ...
          .map{i =>
              val x: F[Unit] = for {
                ...
                streams <- (Alternative[Option].guard(pushEnabled) >> pp).fold(Applicative[F].pure(List.empty[(Request[fs2.Pure], H2Stream[F])])){ l => 
                  l.traverse{req => 
                    streamCreationLock.permit.use[(Request[Pure], H2Stream[F])](_ =>
                      h2.initiateLocalStream.flatMap{ stream => 
                        stream.sendPushPromise(i, PseudoHeaders.requestToHeaders(req))
                        .map(_ => (req, stream))
                      }
                    )
                  }
