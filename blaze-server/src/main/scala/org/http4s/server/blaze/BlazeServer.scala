  ...
  with server.WebSocketSupport
{
  ...
  private def copy(socketAddress: InetSocketAddress = socketAddress,
             ...
             serviceErrorHandler: ServiceErrorHandler = serviceErrorHandler): BlazeBuilder =
    new BlazeBuilder(socketAddress, executionContext, idleTimeout, isNio2, connectorPoolSize, bufferSize, enableWebSockets, sslBits, http2Support, maxRequestLineLen, maxHeadersLen, serviceMounts, serviceErrorHandler)
  ...
  def withServiceErrorHandler(serviceErrorHandler: ServiceErrorHandler): BlazeBuilder =
  ...
  def start: Task[Server] = Task.delay {
    ...
    val pipelineFactory = { conn: SocketConnection =>
      ...
      def http1Stage(secure: Boolean) =
        Http1ServerStage(aggregateService, requestAttributes(secure = secure), executionContext, enableWebSockets, maxRequestLineLen, maxHeadersLen, serviceErrorHandler)

      def http2Stage(engine: SSLEngine) =
        ProtocolSelector(engine, aggregateService, maxRequestLineLen, maxHeadersLen, requestAttributes(secure = true), executionContext, serviceErrorHandler)
