  ...
  with SSLSupport
{
  ...
  private def copy(socketAddress: InetSocketAddress = socketAddress,
                 serviceExecutor: ExecutorService = serviceExecutor,
                    ...
                    http2Support: Boolean = http2Support,
    ...
    val pipelineFactory = getContext() match {
      case Some((ctx, clientAuth)) =>
        (conn: SocketConnection) => {
          ...
          val l1 =
            if (http2Support) LeafBuilder(ProtocolSelector(eng, aggregateService, 4*1024, Some(conn), serviceExecutor))
      ...
      case None =>
        if (http2Support) logger.warn("Http2 support requires TLS.")
        (conn: SocketConnection) => {
