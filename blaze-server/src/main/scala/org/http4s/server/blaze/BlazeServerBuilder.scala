...
object BlazeServerBuilder {
  ...
  implicit def serverBuildableInstance[F[_]]: ServerBuildable[F, BlazeServerBuilder[F]] =
    new ServerBuildable[F, BlazeServerBuilder[F]] {
      ...
      override def withHttpHost(a: BlazeServerBuilder[F])(host: String): BlazeServerBuilder[F] =
        a.withHost(host)

      override def withHttpPort(a: BlazeServerBuilder[F])(port: Int): BlazeServerBuilder[F] =
        a.withPort(port)
