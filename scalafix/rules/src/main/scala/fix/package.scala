...
package object fix {
  ...
  object XTerm {
    ...
    object IsBlockOrPartFunc {
      def unapply(term: Term): Boolean = PartialFunction.cond(term) {
  ...
  object XSemanticType {
    ...
    /** Tries to infer a result [[SemanticType]] of a particular [[Stat]].
      ...
      */
    def unapply(stat: Stat)(implicit doc: SemanticDocument): Option[SemanticType] = PartialFunction.condOpt(stat) {
