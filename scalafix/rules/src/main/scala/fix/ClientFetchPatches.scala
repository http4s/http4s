...
object ClientFetchPatches {
    ...
    tree
      .collect {
        ...
        case subtree@
          ...
          Patch.replaceTree(subtree, s"$req.flatMap(${ asCalleeString(maybeClient) }run(_).use$applyTypesStr$fun)")
      }
  }
  ...
  private object RequestValueEffectTypeSymbol {
    def unapply(tpe: SemanticType): Option[Symbol] = PartialFunction.condOpt(tpe) {
      case TypeRef(_, `requestClassSymbol`, List(TypeRef(_, effectTypeSym, Nil))) => effectTypeSym
    }
    def unapply(symInfo: SymbolInformation): Option[Symbol] = PartialFunction.condOpt(symInfo.signature) {
  ...
  private object IsClientFetchReqMethod {
    def unapply(sig: MethodSignature): Boolean = PartialFunction.cond(sig.parameterLists) {
      case List(List(RequestValueEffectTypeSymbol(_)), _) => true
    }
  }
  private object IsClientFetchFReqMethod {
    def unapply(sig: MethodSignature)(implicit doc: Symtab): Boolean = PartialFunction.cond(sig.parameterLists) {
      case
        List(
        List(
        XSignature(
        ValueSignature(
        TypeRef(_, XSymbol.Owner(IsSymbolATypeOfClientClass()), List(RequestValueEffectTypeSymbol(_)))
        ))), _)
      =>
        true
    }
    ...
    def unapply(term: Term.Name)(implicit doc: SemanticDocument): Option[MethodSignature] =
      PartialFunction.condOpt(term) {
        case Term.Name("fetch") &
          XSymbol(XSymbol.Owner(IsSymbolATypeOfClientClass()) & XSignature(fetchSig: MethodSignature)) => fetchSig
      }
  }
    ...
    : Option[(MethodSignature, Option[Term], Option[(SemanticType, Type)], Term, Term)] =
      PartialFunction.condOpt(tree) {
        ...
        case // client.fetch(fun)(req)
          Term.Apply(
          Term.Apply(
          Term.Select(callee, ClientFetchMethod(methodSig)),
          List(req)),
          List(fun))
        =>
        ...
        case // fetch[A](fun)(req)
          Term.Apply(
          Term.Apply(
          Term.ApplyType(ClientFetchMethod(methodSig), List(_)),
          List(req)),
          List(fun))
        =>
          (methodSig, None, None, req, fun)
  ...
  private object ClientFetchReqFunCall {
    def unapply(tree: Tree)(implicit doc: SemanticDocument)
    : Option[(Option[Term], String, Term, Term)] =
      PartialFunction.condOpt(tree) {
        case
          ClientFetchCall(
  ...
  private def asCalleeString(maybeCalleeTerm: Option[Term]) = maybeCalleeTerm.fold("") { _ + "." }
  ...
  private object AsApplyParamString {
    def unapply(term: Term): Option[String] = PartialFunction.condOpt(term) {
