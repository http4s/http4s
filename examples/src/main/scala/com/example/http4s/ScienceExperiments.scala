...
object ScienceExperiments {
  ...
  case req @ POST -> Root / "challenge1" =>
    ...
    def newBodyP(h: Handle[IO, String]): Pull[IO, String, String] = {
  ...
  case req @ POST -> Root / "challenge2" =>
    def parser(h: Handle[IO, String]): Pull[IO, IO[Response[IO]], Unit] = {
      h.await1Option.flatMap{
  ...
  */
  ...
  case GET -> Root / "hanging-body" =>
    Ok(Stream.eval(IO.pure(ByteVector(Seq(' '.toByte))))
      .evalMap[IO, IO, Byte](_ => IO.async[Byte]{ cb => /* hang */}))
