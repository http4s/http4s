...
object EmberClientSimpleExample extends IOApp{
  ...
  def run(args: List[String]) : IO[ExitCode] = {
    EmberClientBuilder.default[IO]
      .build
      .use( client =>
        logTimed(logger, "Http Only - ChristopherDavenport Site", getRequestBufferedBody(client, githubReq)) >>
        ...
        logTimed(logger, "Small Response - HttpBin Get", getRequestBufferedBody(client, httpBinGet)) >>
        logTimed(logger, "Large Response - HttpBin PNG", getRequestBufferedBody(client, httpBinPng)) >>
        IO(println("Done"))
      )
  ...
  def getRequestBufferedBody[F[_]: Sync](client: Client[F], req: Request[F]): F[Response[F]] = {
    client.run(req)
      .use(resp => 
        resp.body.compile.to(ByteVector).map(bv => resp.copy(body = Stream.chunk(Chunk.ByteVectorChunk(bv))))
      )
  }
