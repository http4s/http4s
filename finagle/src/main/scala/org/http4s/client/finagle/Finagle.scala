...
object Finagle {
  ...
  def mkClient[F[_]](dest: String)(
    implicit F: ConcurrentEffect[F]): Resource[F, Client[F]] = mkClient(Http.newService(dest))
  ...
  private def fromFinagleReq[F[_]](req: Req): Request[F] = {
    import org.http4s.{Method, Header}
    ...
    val version = HttpVersion.fromVersion(req.version.major, req.version.minor).getOrElse(HttpVersion.`HTTP/1.1`)
  ...
  private def toFinagleResp[F[_]:Concurrent](resp: Response[F]): F[Resp] = {
    ...
    val writeBody = if (resp.isChunked) {
      ...
      resp.as[Array[Byte]].map{Buf.ByteArray.Owned(_)}
        .map(finagleResp.content = _).void
    }
  ...
  private def streamBody[F[_]:Async](body: Stream[F, Byte], writer: Writer[Buf]): Stream[F, Unit] ={
