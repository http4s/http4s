...
class QueryParamBench {
  ...
  @Benchmark def withQueryParam(in: QueryParamInput): QueryOps =
    in.queryParams.foldLeft[QueryOps](Query.noQuery){ case (query, (key, value)) => query.withQueryParam(key, value) }

  @Benchmark def setQueryParams(in: QueryParamInput): QueryOps =
    Query.noQuery.setQueryParams(in.queryParams.mapValues(Seq(_)))
