...
private[blaze] final class BlazeHttp1ClientParser(
  ...
  def getHttpVersion(): HttpVersion =
    if (httpVersion == null) HttpVersion.Http1_0 // TODO Questionable default
    else httpVersion
  ...
  override protected def submitResponseLine(
    ...
    httpVersion =
      if (majorversion == 1 && minorversion == 1) HttpVersion.Http1_1
      else if (majorversion == 1 && minorversion == 0) HttpVersion.Http1_0
      else HttpVersion.fromVersion(majorversion, minorversion).getOrElse(HttpVersion.Http1_0)
  }
