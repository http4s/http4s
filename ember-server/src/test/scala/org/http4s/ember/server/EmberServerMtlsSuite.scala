...
class EmberServerMtlsSuite extends Http4sSuite {
  ...
  def server(tlsParams: TLSParameters, receiveBufferSize: Int = 256 * 1024) = ResourceFixture(
    EmberServerBuilder
      .default[IO]
  ...
  server(TLSParameters(needClientAuth = true))
    .test("Server should handle mTLS request correctly") { server =>
  ...
  server(TLSParameters(wantClientAuth = true))
    .test("Server should handle mTLS request correctly with optional auth") { server =>
      assertEquals(get(server, "/dummy", true), "CN=Test,OU=Test,O=Test,L=CA,ST=CA,C=US")
    }

  server(TLSParameters(wantClientAuth = true))
    .test("Server should handle mTLS request correctly without clientAuth") { server =>
