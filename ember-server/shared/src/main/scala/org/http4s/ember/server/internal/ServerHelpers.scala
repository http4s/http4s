...
private[server] object ServerHelpers extends ServerHelpersPlatform {
  ...
  def server[F[_]](
      ...
      ready: Deferred[F, Either[Throwable, SocketAddress[Host]]],
      shutdown: Shutdown[F],
      // Defaults
      ...
      Stream
        .eval(
          ready.complete( // This is a lie, there isn't any signal from fs2 when the server is actually ready
            SocketAddress
              .fromStringHostname(unixSocketAddress.path)
              .toRight(new IllegalArgumentException("Illegal host name: " + unixSocketAddress.path))
              .map(_.copy(port = Port.fromInt(0).get))
          )
        ) // Sketchy
        .drain ++
        unixSockets
