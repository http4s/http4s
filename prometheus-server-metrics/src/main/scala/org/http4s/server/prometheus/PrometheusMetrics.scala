...
object PrometheusMetrics {
  ...
  private def onServiceError[F[_]: Sync](s: SingleMetricRun): F[Unit] =
    for {
      _ <- requestMetrics(s)
  ...
  private def requestHistogram[F[_]: Sync](
      rt: RequestHistograms,
      method: Method): F[Histogram.Timer] =
    Sync[F].delay(method match {
  ...
  private def startedRun[F[_]: Sync](
      serviceMetrics: ServiceMetrics,
      method: Method): F[SingleMetricRun] =
    for {
  ...
  def apply[F[_]: Effect](
    ...
    val generalServiceMetrics = GeneralServicMetrics(
      ...
      abnormalTerminations = Counter
        ...
        .register(c),
      serviceErrors =
        Counter.build().name(prefix + "_" + "service_errors").help("Service Errors.").register(c)
    )
    ...
    { service: HttpService[F] =>
      Kleisli(
        req =>
          OptionT
            .liftF(startedRun[F](serviceMetrics, req.method))
            .flatMap(singleMetricRun => metricsService[F](singleMetricRun, service)(req)))
    }
  }
