      ...
      _ <- emptyResponseHandler.traverse_(status =>
        Sync[F].delay {
          serviceMetrics.requestDuration
            .labels(
              Seq(reportMethod(m), ServingPhase.report(ServingPhase.HeaderPhase)) ++ serviceMetrics.tags.values.toSeq: _*)
            .observe(SimpleTimer.elapsedSecondsFromNanos(start, headerTime))

          serviceMetrics.requestDuration
            .labels(
              Seq(reportMethod(m), ServingPhase.report(ServingPhase.BodyPhase)) ++ serviceMetrics.tags.values.toSeq: _*)
            .observe(SimpleTimer.elapsedSecondsFromNanos(start, now))

          serviceMetrics.requestCounter
            .labels(
              Seq(reportMethod(m), reportStatus(status)) ++ serviceMetrics.tags.values.toSeq: _*)
            .inc()
    ...
    val newBody = r.body
      .onFinalize {
        Sync[F].delay {
          ...
          serviceMetrics.requestCounter
            .labels(
              Seq(reportMethod(m), reportStatus(r.status)) ++ serviceMetrics.tags.values.toSeq: _*)
            .inc()
      ...
      _ <- errorResponseHandler.traverse_(status =>
        Sync[F].delay {
          ...
          serviceMetrics.abnormalTerminations
            .labels(
              Seq(AbnormalTermination.report(AbnormalTermination.ServerError)) ++ serviceMetrics.tags.values.toSeq: _*)
            .inc()
