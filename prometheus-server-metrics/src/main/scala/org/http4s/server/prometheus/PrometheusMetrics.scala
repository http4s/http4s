...
object PrometheusMetrics {
  ...
  private def metricsService[F[_]: Sync](
    ...
    for {
      ...
      _ <- Sync[F].delay(
        serviceMetrics.generalMetrics.headers
          .observe(((headersElapsed - initialTime) / 1000000000.0).toDouble))
  ...
  private def metricsServiceHandler[F[_]: Sync](
    ...
    e.fold(
      ...
      _.fold(
        Sync[F]
          .delay(serviceMetrics.generalMetrics.activeRequests.dec())
  ...
  private def onResponse[F[_]: Sync](m: Method, start: Long, serviceMetrics: ServiceMetrics)(
      r: Response[F]): Response[F] = {
    val newBody = r.body
      .onFinalize {
        for {
  ...
  private def requestMetrics[F[_]: Sync](
      rh: RequestHistograms,
      totalReqs: Histogram,
      activeReqs: Gauge)(m: Method, begin: Long, end: Long): F[Unit] = Sync[F].delay {
  ...
  private def responseMetrics[F[_]: Sync](
      rh: ResponseHistograms,
      status: Status,
      begin: Long,
      end: Long): F[Unit] = Sync[F].delay {
    responseHistogram(rh, status).observe(SimpleTimer.elapsedSecondsFromNanos(begin, end))
  }

  private def requestHistogram[F[_]: Sync](rt: RequestHistograms, method: Method): Histogram =
    method match {
  ...
  def apply[F[_]: Effect](
      ...
      c: CollectorRegistry): F[HttpMiddleware[F]] = Sync[F].delay {
    val generalServiceMetrics = GeneralServicMetrics(
      activeRequests = Gauge
        ...
        .register(c),
      totalRequests =
        Histogram.build().name(prefix + "_" + "requests").help("Total Requests.").register(c),
      headers =
        Histogram.build().name(prefix + "_" + "headers_times").help("Header Timings.").register(c),
      abnormalTerminations = Histogram
        .build()
        .name(prefix + "_" + "abnormal_terminations")
        .help("Abnormal Terminations.")
        .register(c),
      serviceErrors =
        Histogram.build().name(prefix + "_" + "service_errors").help("Service Errors.").register(c)
    )
    val requestHistograms = RequestHistograms(
      getReq =
        Histogram.build().name(prefix + "_" + "get_requests").help("GET Requests.").register(c),
      postReq =
        Histogram.build().name(prefix + "_" + "post_requests").help("POST Requests.").register(c),
      putReq =
        Histogram.build().name(prefix + "_" + "put_requests").help("PUT Requests.").register(c),
      headReq =
        Histogram.build().name(prefix + "_" + "head_requests").help("HEAD Requests.").register(c),
      moveReq =
        Histogram.build().name(prefix + "_" + "move_requests").help("MOVE Requests.").register(c),
      optionsReq = Histogram
        .build()
        .name(prefix + "_" + "options_requests")
        .help("OPTIONS Requests.")
        .register(c),
      traceReq =
        Histogram.build().name(prefix + "_" + "trace_requests").help("TRACE Requests.").register(c),
      connectReq = Histogram
        .build()
        .name(prefix + "_" + "connect_requests")
        .help("CONNECT Requests.")
        .register(c),
      deleteReq = Histogram
        .build()
        .name(prefix + "_" + "delete_requests")
        .help("DELETE Requests.")
        .register(c),
      otherReq =
        Histogram.build().name(prefix + "_" + "other_requests").help("Other Requests.").register(c)
    )
    val responseHistograms = ResponseHistograms(
      resp1xx =
        Histogram.build().name(prefix + "_" + "1xx_responses").help("1xx Responses.").register(c),
      resp2xx =
        Histogram.build().name(prefix + "_" + "2xx_responses").help("2xx Responses.").register(c),
      resp3xx =
        Histogram.build().name(prefix + "_" + "3xx_responses").help("3xx Responses.").register(c),
      resp4xx =
        Histogram.build().name(prefix + "_" + "4xx_responses").help("4xx Responses.").register(c),
      resp5xx =
        Histogram.build().name(prefix + "_" + "5xx_responses").help("5xx Responses.").register(c)
    )
    val serviceMetrics =
      ServiceMetrics(generalServiceMetrics, requestHistograms, responseHistograms)
