...
class ArgonautSpec extends JawnDecodeSupportSpec[Json] with Argonauts {
  ...
  "json" should {
    "handle the optionality of jNumber" in {
      ...
      val req = Request().withBody(jNumberOrNull(157)).unsafePerformSync
      val body = req.decode { json: Json => Response(Ok).withBody(json.number.flatMap(_.toLong).getOrElse(0L).toString)}.unsafePerformSync.body
  ...
  "jsonOf" should {
    "decode JSON from an Argonaut decoder" in {
      val result = jsonOf[Foo].decode(Request().withBody(jObjectFields("bar" -> jNumberOrNull(42))).unsafePerformSync, strict = true)
