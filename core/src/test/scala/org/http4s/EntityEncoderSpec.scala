...
object EntityEncoderSpec {
  def writeToString[A](a: A)(implicit W: EntityEncoder[A]): Task[String] =
  ...
  def writeToByteVector[A](a: A)(implicit W: EntityEncoder[A]): Task[ByteVector] =
    Process.eval(W.toEntity(a))
      .collect { case EntityEncoder.Entity(body, _ ) => body }
      .flatMap(identity)
      .fold1Monoid
...
class EntityEncoderSpec extends Http4sSpec {
  ...
  "EntityEncoder" should {
    "render strings" in {
      writeToString("pong") must returnValue("pong")
    }

    "render single characters" in {
      prop { char: Char => writeToString(char) must returnValue(Character.toString(char)) }
    }
    ...
    "render byte arrays" in {
      val hello = "hello"
      writeToString(hello.getBytes(StandardCharsets.UTF_8)) must returnValue(hello)
    }

    "render bytes" in {
      prop { byte: Byte => writeToByteVector(byte) must returnValue(ByteVector(byte)) }
    }

    "render futures" in {
      import scala.concurrent.ExecutionContext.Implicits.global
      val hello = "Hello"
      writeToString(Future(hello)) must returnValue(hello)
    }

    "render Tasks" in {
      val hello = "Hello"
      writeToString(Task.now(hello)) must returnValue(hello)
    }

    "render processes" in {
      val helloWorld = Process("hello", "world")
      writeToString(helloWorld) must returnValue("helloworld")
    }
    ...
    "render files" in {
      ...
      try {
        ...
        writeToString(tmpFile) must returnValue("render files test")
      }
    ...
    "render input streams" in {
      ...
      writeToString(inputStream) must returnValue("input stream")
    }

    "render readers" in {
      val reader = new StringReader("string reader")
      writeToString(reader) must returnValue("string reader")
    }
