...
object HttpRoutes {
  ...
  /** Lifts a function into an [[HttpRoutes]]. The application of `run` is suspended in `F` to
  ...
  /** Transforms an [[HttpRoutes]] on its input. The application of the transformed function is
    * suspended in `F` to permit more efficient combination of routes via `SemigroupK`.
    *
    * @tparam F
    *   the base effect of the [[HttpRoutes]]
    * @param f
  ...
  /** Lifts a partial function into an [[HttpRoutes]]. The application of the partial function is
    * suspended in `F` to permit more efficient combination of routes via `SemigroupK`.
    *
    * @tparam F
    *   the base effect of the [[HttpRoutes]] - Defer suspends evaluation of routes, so only 1
    *   section of routes is checked at a time.
    * @param pf
    *   the partial function to lift
    * @return
    *   An [[HttpRoutes]] that returns some [[Response]] in an `OptionT[F, *]` wherever `pf` is
