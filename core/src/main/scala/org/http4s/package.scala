...
package object http4s {
  ...
  /** A kleisli with a [[Request]] input and a [[Response]] output. This type is useful for writing
    * middleware that are polymorphic over the return type F.
    *
    * @tparam F
  ...
  /** A kleisli with a [[Request]] input and a [[Response]] output, such that the response effect is
    * the same as the request and response bodies'. An HTTP app is total on its inputs. An HTTP app
    * may be run by a server, and a client can be converted to or from an HTTP app.
    *
    * @tparam F
    *   the effect type in which the [[Response]] is returned, and also of the [[Request]] and
    *   [[Response]] bodies.
    */
  type HttpApp[F[_]] = Http[F, F]

  /** A kleisli with a [[Request]] input and a [[Response]] output, such that the response effect is
    * an optional inside the effect of the request and response bodies. HTTP routes can conveniently
    * be constructed from a partial function and combined as a `SemigroupK`.
    *
    * @tparam F
    *   the effect type of the [[Request]] and [[Response]] bodies, and the base monad of the
    *   `OptionT` in which the response is returned.
    */
  type HttpRoutes[F[_]] = Http[OptionT[F, *], F]

  type AuthedRequest[F[_], T] = ContextRequest[F, T]

  /** The type parameters need to be in this order to make partial unification trigger. See
    * https://github.com/http4s/http4s/issues/1506
    */
