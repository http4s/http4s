...
trait Header[A] {
  ...
  def parse(s: String): Option[A]
    // some headers like Set-Cookie may emit multiple actual headers
  def toRawHeaders(a: A): List[Header.RawHeader]
  def isSingleton: Boolean
}

object Header {
  def apply[A](implicit A: Header[A]): Header[A] = A

  final case class RawHeader(name: CaseInsensitiveString, value: String)
