...
object HttpApp {
  ...
  /** Lifts a function into an [[HttpApp]]. The application of `run` is suspended in `F` to permit
    * more efficient combination of routes via `SemigroupK`.
    *
    * @tparam F
  ...
  /** Transforms an [[HttpApp]] on its input. The application of the transformed function is
