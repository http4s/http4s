...
object UrlForm {
  ...
  implicit def entityEncoder[F[_]](
    ...
    EntityEncoder
      ...
      .withContentType(
        `Content-Type`(MediaType.application.`application/x-www-form-urlencoded`, charset))

  implicit def entityDecoder[F[_]](
      implicit F: Sync[F],
      defaultCharset: Charset = DefaultCharset): EntityDecoder[F, UrlForm] =
    EntityDecoder.decodeBy(MediaType.application.`application/x-www-form-urlencoded`) { m =>
      DecodeResult(
        EntityDecoder
          .decodeString(m)
