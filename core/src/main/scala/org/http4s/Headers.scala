...
final class Headers private (private val headers: List[Header]) extends AnyVal {
  ...
  /** Make a new collection adding the specified headers, replacing existing headers of singleton
    * type The passed headers are assumed to contain no duplicate Singleton headers.
    *
    * @param in
  ...
  /** Concatenate the two collections If the resulting collection is of Headers type, duplicate
    * Singleton headers will be removed from this Headers collection.
    *
    * @param that
    *   collection to append
    * @tparam B
    *   type contained in collection `that`
    * @tparam That
    *   resulting type of the new collection
    */
