...
final case class Headers(raw: List[Header.RawHeader]){
  def get[A](implicit A: Header[A]): Option[A] = {
    if (A.isSingleton) {
      raw.find(_.name == A.name).flatMap(h => A.parse(h.value))
    } else {
      val values = raw.filter(_.name == A.name).map(_.value)
      if (values.nonEmpty) A.parse(values.mkString(", "))
      else None
    }
  }

  def getRepeatable(key: CaseInsensitiveString): Option[Header.RawHeader] = {
      val hs = raw.filter(_.name == key)
      if (hs.isEmpty) None
      else {
        Some(Header.RawHeader(key, hs.mkString(", ")))
      }
    }

    def getSingleton(key: CaseInsensitiveString): Option[Header.RawHeader] =
      raw.find(_.name == key)

    def put(headers0: Sigma[Header]*): Headers = {
      val rawHeaders0 = headers0.toVector.flatMap { i =>
        i.typeclass.toRawHeaders(i.value)
      }
      Headers(raw ++ rawHeaders0)
    }

    def remove[A: Header]: Headers =
      Headers(raw = raw.filter(_.name != Header[A].name))

    def remove(key: CaseInsensitiveString): Headers =
      Headers(raw = raw.filter(_.name != key))
}
