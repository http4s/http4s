...
sealed trait SessionScoe extends Scope {
  val rank = 500
}
case class ThisSession(id: String) extends SessionScoe

sealed trait RequestScope extends Scope
case class ThisRequest[T](request: Request[T]) extends RequestScope {
  val rank = 1000
}