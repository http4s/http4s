...
object Uri {
   ...
   *             / path-empty
   */
  def hierPart(cs: JCharset): P[(Option[Authority], Path)] = {
  ...
  private[http4s] def absoluteUri(cs: JCharset): P1[Uri] = {
  ...
  private[http4s] def parser(cs: JCharset): P1[Uri] = {
    import cats.parse.Parser.char
    import Uri.Scheme.{parser => scheme}
    import Query.{parser => query}
  ...
  private[http4s] def uriReference(cs: JCharset): P[Uri] = {
    import cats.parse.Parser.{char, string1}
    import Authority.{parser => authority}
    ...
    /* relative-part = "//" authority path-abempty
                     ...
                     / path-empty
     */
    def relativePart(cs: JCharset): P[(Option[Authority], Path)] =
      ((string1("//") *> authority(cs) ~ pathAbempty)
        .map { case (a, p) => (Some(a), p) })
    ...
    def relativeRef(cs: JCharset): P[Uri] =
  ...
  /* request-target = origin-form
                    ...
                    / asterisk-form
   */
  private lazy val requestTargetParser: P[Uri] = {
    import cats.parse.Parser.char
    import Authority.{parser => authority}
  ...
  object Scheme {
    ...
    private[http4s] val parser: P1[Scheme] = {
      import cats.parse.Parser.{charIn, not, rep, string1}
  ...
  object Fragment {
     ...
     * Not URL decoded.
     */
    private[http4s] val parser: P[Fragment] =
  ...
  object Authority {
    ...
    def parser(cs: JCharset): P[Authority] =
  ...
  object Path {
    ...
    object Segment extends (String => Segment) {
      ...
      import cats.parse.Parser.{char, rep, rep1}
      import Rfc3986.{pchar, pctEncoded, subDelims, unreserved}

      /* segment       = *pchar */
      lazy val segment: Parser[Segment] =
        rep(pchar).string.map(new Segment(_))

      /* segment-nz    = 1*pchar */
      lazy val segmentNz: Parser1[Segment] =
        rep1(pchar, 1).string.map(new Segment(_))

      /* segment-nz-nc = 1*( unreserved / pct-encoded / sub-delims / "@" )
                     ; non-zero-length segment without any colon ":" */
      lazy val segmentNzNc: Parser1[Segment] =
    ...
    lazy val pathAbsolute: cats.parse.Parser1[Path] =
      (char('/') *> (segmentNz ~ rep(char('/') *> segment)).?).map {
        case Some((head, tail)) =>
    ...
    lazy val pathRootless: cats.parse.Parser1[Path] =
      (segmentNz ~ rep(char('/') *> segment)).map { case (head, tail) =>
        val segmentsV = head +: tail.toVector
        if (segmentsV.last.isEmpty)
    ...
    /* path-empty    = 0<pchar> */
    lazy val pathEmpty: cats.parse.Parser[Path] =
      pure(Path.empty)

    /* path-noscheme = segment-nz-nc *( "/" segment ) */
    lazy val pathNoscheme: cats.parse.Parser1[Path] =
      (segmentNzNc ~ rep(char('/') *> segment)).map { case (head, tail) =>
        val segmentsV = head +: tail.toVector
        if (segmentsV.last.isEmpty)
          Path(segmentsV.dropRight(1), absolute = false, endsWithSlash = true)
        else
    ...
    lazy val absolutePath: cats.parse.Parser1[Path] =
      rep1(char('/') *> segment, 1).map {
  ...
  object UserInfo {
    ...
    private[http4s] def parser(cs: JCharset): P[UserInfo] = {
  ...
  object Host {
    ...
    val parser: P[Host] = {
      ...
      val ipVFuture: P1[Nothing] = P.fail
  ...
  object Ipv4Address {
    ...
    private[http4s] val parser: P1[Ipv4Address] =
  ...
  object Ipv6Address {
    ...
    private[http4s] val parser: P1[Ipv6Address] = {
      import cats.parse.Parser.{char, string1}
      ...
      def repN[A](n: Int, p: P[A], sep: P[Unit] = P.unit): P[List[A]] =
      ...
      def repExactly[A](n: Int, p: P[A], sep: P[Unit] = P.unit): P[List[A]] =
      ...
      (repN(6, h16Colon).with1 ~ ls32)
        ...
        .backtrack
        .orElse1((doubleColon *> repN(4, h16Colon, P.unit) ~ ls32)
  ...
  object RegName {
    ...
    val parser: P[RegName] = {
      import cats.parse.Parser.rep
      import Rfc3986.{pctEncoded, subDelims, unreserved}

      rep(unreserved.orElse1(pctEncoded).orElse1(subDelims)).string.map(s => RegName(CIString(decode(s))))
    }
  ...
  object Port {
     ...
     */
    private[http4s] val parser: P[Option[Int]] = {
      import cats.parse.Parser.{rep}
