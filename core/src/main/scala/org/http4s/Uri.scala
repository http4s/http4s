...
object Uri {
   ...
   *             / path-empty
   */
  def hierPart(cs: JCharset): P0[(Option[Authority], Path)] = {
  ...
  private[http4s] def uriReference(cs: JCharset): P0[Uri] = {
    ...
    /* relative-part = "//" authority path-abempty
                     ...
                     / path-empty
     */
    def relativePart(cs: JCharset): P0[(Option[Authority], Path)] =
      ((string("//") *> authority(cs) ~ pathAbempty)
        .map { case (a, p) => (Some(a), p) })
        .orElse(pathAbsolute.map((None, _)))
    ...
    def relativeRef(cs: JCharset): P0[Uri] =
  ...
  /* request-target = origin-form
                    ...
                    / asterisk-form
   */
  private val requestTargetParser: P0[Uri] = {
    import cats.parse.Parser.char
    import Authority.{parser => authority}
  ...
  object Authority
    ...
    def parser(cs: JCharset): P0[Authority] =
  ...
  object UserInfo {
    ...
    private[http4s] def parser(cs: JCharset): P0[UserInfo] = {
  ...
  object Ipv6Address {
    ...
    private[http4s] val parser: P[Ipv6Address] = {
      ...
      def repN[A](n: Int, p: P0[A], sep: P0[Unit] = P.unit): P0[List[A]] =
      ...
      def repExactly[A](n: Int, p: P0[A], sep: P0[Unit] = P.unit): P0[List[A]] =
  ...
  object Port {
     ...
     */
    private[http4s] val parser: P0[Option[Int]] = {
