...
trait QueryOps {
  ...
  /** Checks if a specified parameter exists in the [[Query]]. A parameter without a name can be
    * checked with an empty string.
    */
  ...
  /** Creates maybe a new `Self` without the specified parameter in query. If no parameter with the
    * given `key` exists then `this` will be returned.
    */
  ...
  /** Creates maybe a new `Self` with the specified parameters. The entire [[Query]] will be
  ...
  /** Creates a new `Self` with the specified parameter in the [[Query]]. If a parameter with the
    * given `QueryParam.key` already exists the values will be replaced with an empty list.
    */
  def withQueryParam[T: QueryParam]: Self =
    _withQueryParam(QueryParam[T].key, Nil)

  /** Creates a new `Self` with the specified parameter in the [[Query]]. If a parameter with the
    * given `key` already exists the values will be replaced with an empty list.
    */
  def withQueryParam[K: QueryParamKeyLike](key: K): Self =
    _withQueryParam(QueryParamKeyLike[K].getKey(key), Nil)

  /** Creates maybe a new `Self` with the specified parameter in the [[Query]]. If a parameter with
    * the given `key` already exists the values will be replaced. If the parameter to be added equal
    * the existing entry the same instance of `Self` will be returned.
    */
  ...
  /** Creates maybe a new `Self` with the specified parameters in the [[Query]]. If a parameter with
    * the given `key` already exists the values will be replaced.
    */
  ...
  /** Creates maybe a new `Self` with all the specified parameters in the [[Query]]. If any of the
    * given parameters' keys already exists, the value(s) will be replaced. Parameters from the
    * input map are added left-to-right, so if a parameter with a given key is specified more than
  ...
  /** Creates maybe a new `Self` with the specified parameter in the [[Query]]. If the value is
    * empty or if the parameter to be added equal the existing entry the same instance of `Self`
    * will be returned. If a parameter with the given `key` already exists the values will be
    * replaced.
    */
  ...
  /** Creates maybe a new `Self` with the specified parameter in the [[Query]]. If the value is
    * empty or if the parameter to be added equal the existing entry the same instance of `Self`
    * will be returned. If a parameter with the given `name` already exists the values will be
    * replaced.
    */
