...
class BodyParser[A] private (p: Process1[Chunk, A], req: Request) { parent =>
  def apply(f: PartialFunction[Try[A], Task[Response]]): Task[Response] = {
    req.body.pipe(p).runLast.attempt.flatMap { r =>
      val result: Try[A] = r match {
        case \/-(Some(a)) => Success(a)
        case -\/(t)       => Failure(t)
        case _            => Failure(new Exception("No Body generated by parser."))
      }

      f.lift(result).getOrElse(Status.InternalServerError())
    }
  }

  def defaultFailure(t: Try[A]): Task[Response] = Status.InternalServerError()
