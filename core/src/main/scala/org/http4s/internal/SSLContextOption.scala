...
/** Indicates how to resolve SSLContext. * NoSSL = do not use SSL/HTTPS * TryDefaultSSLContext =
  * `SSLContext.getDefault()`, or `None` on systems where the default is unavailable * Provided =
  * use the explicitly passed SSLContext
  */
