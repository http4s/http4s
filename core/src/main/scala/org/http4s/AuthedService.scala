...
object AuthedService {
  ...
  /** Lifts a total function to an `AuthedService`. The function is expected to handle all requests
    * it is given. If `f` is a `PartialFunction`, use `apply` instead.
    */
  ...
  /** Lifts a partial function to an `AuthedService`. Responds with
    * [[org.http4s.Response.notFoundFor]], which generates a 404, for any request where `pf` is not
    * defined.
    */
