// Autogenerated file, don't edit
package org.http4s

private[http4s] trait MimeDB {
  final lazy val allMediaTypes
    : List[MediaType] = Nil ++ x_shader.all ++ x_conference.all ++ video.all ++ text.all ++ multipart.all ++ model.all ++ message.all ++ image.all ++ font.all ++ chemical.all ++ audio.all ++ application.all
  final val Compressible: Boolean = true
  final val Uncompressible: Boolean = false
  final val Binary: Boolean = true
  final val NotBinary: Boolean = false
  private[http4s] object application_parts {
    trait application_0 {
      final lazy val `1d-interleaved-parityfec`: MediaType =
        new MediaType("application", "1d-interleaved-parityfec", Compressible, NotBinary)
      final lazy val `3gpdash-qoe-report+xml`: MediaType =
        new MediaType("application", "3gpdash-qoe-report+xml", Compressible, NotBinary)
      final lazy val `3gpp-ims+xml`: MediaType =
        new MediaType("application", "3gpp-ims+xml", Compressible, NotBinary)
      final lazy val `a2l`: MediaType = new MediaType("application", "a2l", Compressible, NotBinary)
      final lazy val `activemessage`: MediaType =
        new MediaType("application", "activemessage", Compressible, NotBinary)
      final lazy val `activity+json`: MediaType =
        new MediaType("application", "activity+json", Compressible, NotBinary)
      final lazy val `alto-costmap+json`: MediaType =
        new MediaType("application", "alto-costmap+json", Compressible, NotBinary)
      final lazy val `alto-costmapfilter+json`: MediaType =
        new MediaType("application", "alto-costmapfilter+json", Compressible, NotBinary)
      final lazy val `alto-directory+json`: MediaType =
        new MediaType("application", "alto-directory+json", Compressible, NotBinary)
      final lazy val `alto-endpointcost+json`: MediaType =
        new MediaType("application", "alto-endpointcost+json", Compressible, NotBinary)
      final lazy val `alto-endpointcostparams+json`: MediaType =
        new MediaType("application", "alto-endpointcostparams+json", Compressible, NotBinary)
      final lazy val `alto-endpointprop+json`: MediaType =
        new MediaType("application", "alto-endpointprop+json", Compressible, NotBinary)
      final lazy val `alto-endpointpropparams+json`: MediaType =
        new MediaType("application", "alto-endpointpropparams+json", Compressible, NotBinary)
      final lazy val `alto-error+json`: MediaType =
        new MediaType("application", "alto-error+json", Compressible, NotBinary)
      final lazy val `alto-networkmap+json`: MediaType =
        new MediaType("application", "alto-networkmap+json", Compressible, NotBinary)
      final lazy val `alto-networkmapfilter+json`: MediaType =
        new MediaType("application", "alto-networkmapfilter+json", Compressible, NotBinary)
      final lazy val `aml`: MediaType = new MediaType("application", "aml", Compressible, NotBinary)
      final lazy val `andrew-inset`: MediaType =
        new MediaType("application", "andrew-inset", Compressible, NotBinary, List("ez"))
      final lazy val `applefile`: MediaType =
        new MediaType("application", "applefile", Compressible, NotBinary)
      final lazy val `applixware`: MediaType =
        new MediaType("application", "applixware", Compressible, NotBinary, List("aw"))
      final lazy val `atf`: MediaType = new MediaType("application", "atf", Compressible, NotBinary)
      final lazy val `atfx`: MediaType = new MediaType("application", "atfx", Compressible, NotBinary)
      final lazy val `atom+xml`: MediaType =
        new MediaType("application", "atom+xml", Compressible, NotBinary, List("atom"))
      final lazy val `atomcat+xml`: MediaType =
        new MediaType("application", "atomcat+xml", Compressible, NotBinary, List("atomcat"))
      final lazy val `atomdeleted+xml`: MediaType =
        new MediaType("application", "atomdeleted+xml", Compressible, NotBinary)
      final lazy val `atomicmail`: MediaType =
        new MediaType("application", "atomicmail", Compressible, NotBinary)
      final lazy val `atomsvc+xml`: MediaType =
        new MediaType("application", "atomsvc+xml", Compressible, NotBinary, List("atomsvc"))
      final lazy val `atxml`: MediaType = new MediaType("application", "atxml", Compressible, NotBinary)
      final lazy val `auth-policy+xml`: MediaType =
        new MediaType("application", "auth-policy+xml", Compressible, NotBinary)
      final lazy val `bacnet-xdd+zip`: MediaType =
        new MediaType("application", "bacnet-xdd+zip", Uncompressible, NotBinary)
      final lazy val `batch-smtp`: MediaType =
        new MediaType("application", "batch-smtp", Compressible, NotBinary)
      final lazy val `bdoc`: MediaType =
        new MediaType("application", "bdoc", Uncompressible, NotBinary, List("bdoc"))
      final lazy val `beep+xml`: MediaType =
        new MediaType("application", "beep+xml", Compressible, NotBinary)
      final lazy val `calendar+json`: MediaType =
        new MediaType("application", "calendar+json", Compressible, NotBinary)
      final lazy val `calendar+xml`: MediaType =
        new MediaType("application", "calendar+xml", Compressible, NotBinary)
      final lazy val `call-completion`: MediaType =
        new MediaType("application", "call-completion", Compressible, NotBinary)
      final lazy val `cals-1840`: MediaType =
        new MediaType("application", "cals-1840", Compressible, NotBinary)
      final lazy val `cbor`: MediaType = new MediaType("application", "cbor", Compressible, NotBinary)
      final lazy val `cccex`: MediaType = new MediaType("application", "cccex", Compressible, NotBinary)
      final lazy val `ccmp+xml`: MediaType =
        new MediaType("application", "ccmp+xml", Compressible, NotBinary)
      final lazy val `ccxml+xml`: MediaType =
        new MediaType("application", "ccxml+xml", Compressible, NotBinary, List("ccxml"))
      final lazy val `cdfx+xml`: MediaType =
        new MediaType("application", "cdfx+xml", Compressible, NotBinary)
      final lazy val `cdmi-capability`: MediaType =
        new MediaType("application", "cdmi-capability", Compressible, NotBinary, List("cdmia"))
      final lazy val `cdmi-container`: MediaType =
        new MediaType("application", "cdmi-container", Compressible, NotBinary, List("cdmic"))
      final lazy val `cdmi-domain`: MediaType =
        new MediaType("application", "cdmi-domain", Compressible, NotBinary, List("cdmid"))
      final lazy val `cdmi-object`: MediaType =
        new MediaType("application", "cdmi-object", Compressible, NotBinary, List("cdmio"))
      final lazy val `cdmi-queue`: MediaType =
        new MediaType("application", "cdmi-queue", Compressible, NotBinary, List("cdmiq"))
      final lazy val `cdni`: MediaType = new MediaType("application", "cdni", Compressible, NotBinary)
      final lazy val `cea`: MediaType = new MediaType("application", "cea", Compressible, NotBinary)
      final lazy val `cea-2018+xml`: MediaType =
        new MediaType("application", "cea-2018+xml", Compressible, NotBinary)
      final lazy val `cellml+xml`: MediaType =
        new MediaType("application", "cellml+xml", Compressible, NotBinary)
      final lazy val `cfw`: MediaType = new MediaType("application", "cfw", Compressible, NotBinary)
      final lazy val `clue_info+xml`: MediaType =
        new MediaType("application", "clue_info+xml", Compressible, NotBinary)
      final lazy val `cms`: MediaType = new MediaType("application", "cms", Compressible, NotBinary)
      final lazy val `cnrp+xml`: MediaType =
        new MediaType("application", "cnrp+xml", Compressible, NotBinary)
      final lazy val `coap-group+json`: MediaType =
        new MediaType("application", "coap-group+json", Compressible, NotBinary)
      final lazy val `coap-payload`: MediaType =
        new MediaType("application", "coap-payload", Compressible, NotBinary)
      final lazy val `commonground`: MediaType =
        new MediaType("application", "commonground", Compressible, NotBinary)
      final lazy val `conference-info+xml`: MediaType =
        new MediaType("application", "conference-info+xml", Compressible, NotBinary)
      final lazy val `cose`: MediaType = new MediaType("application", "cose", Compressible, NotBinary)
      final lazy val `cose-key`: MediaType =
        new MediaType("application", "cose-key", Compressible, NotBinary)
      final lazy val `cose-key-set`: MediaType =
        new MediaType("application", "cose-key-set", Compressible, NotBinary)
      final lazy val `cpl+xml`: MediaType =
        new MediaType("application", "cpl+xml", Compressible, NotBinary)
      final lazy val `csrattrs`: MediaType =
        new MediaType("application", "csrattrs", Compressible, NotBinary)
      final lazy val `csta+xml`: MediaType =
        new MediaType("application", "csta+xml", Compressible, NotBinary)
      final lazy val `cstadata+xml`: MediaType =
        new MediaType("application", "cstadata+xml", Compressible, NotBinary)
      final lazy val `csvm+json`: MediaType =
        new MediaType("application", "csvm+json", Compressible, NotBinary)
      final lazy val `cu-seeme`: MediaType =
        new MediaType("application", "cu-seeme", Compressible, NotBinary, List("cu"))
      final lazy val `cwt`: MediaType = new MediaType("application", "cwt", Compressible, NotBinary)
      final lazy val `cybercash`: MediaType =
        new MediaType("application", "cybercash", Compressible, NotBinary)
      final lazy val `dart`: MediaType = new MediaType("application", "dart", Compressible, NotBinary)
      final lazy val `dash+xml`: MediaType =
        new MediaType("application", "dash+xml", Compressible, NotBinary, List("mpd"))
      final lazy val `dashdelta`: MediaType =
        new MediaType("application", "dashdelta", Compressible, NotBinary)
      final lazy val `davmount+xml`: MediaType =
        new MediaType("application", "davmount+xml", Compressible, NotBinary, List("davmount"))
      final lazy val `dca-rft`: MediaType =
        new MediaType("application", "dca-rft", Compressible, NotBinary)
      final lazy val `dcd`: MediaType = new MediaType("application", "dcd", Compressible, NotBinary)
      final lazy val `dec-dx`: MediaType = new MediaType("application", "dec-dx", Compressible, NotBinary)
      final lazy val `dialog-info+xml`: MediaType =
        new MediaType("application", "dialog-info+xml", Compressible, NotBinary)
      final lazy val `dicom`: MediaType = new MediaType("application", "dicom", Compressible, NotBinary)
      final lazy val `dicom+json`: MediaType =
        new MediaType("application", "dicom+json", Compressible, NotBinary)
      final lazy val `dicom+xml`: MediaType =
        new MediaType("application", "dicom+xml", Compressible, NotBinary)
      final lazy val `dii`: MediaType = new MediaType("application", "dii", Compressible, NotBinary)
      final lazy val `dit`: MediaType = new MediaType("application", "dit", Compressible, NotBinary)
      final lazy val `dns`: MediaType = new MediaType("application", "dns", Compressible, NotBinary)
      final lazy val `dns+json`: MediaType =
        new MediaType("application", "dns+json", Compressible, NotBinary)
      final lazy val `dns-message`: MediaType =
        new MediaType("application", "dns-message", Compressible, NotBinary)
      final lazy val `docbook+xml`: MediaType =
        new MediaType("application", "docbook+xml", Compressible, NotBinary, List("dbk"))
      final lazy val `dskpp+xml`: MediaType =
        new MediaType("application", "dskpp+xml", Compressible, NotBinary)
      final lazy val `dssc+der`: MediaType =
        new MediaType("application", "dssc+der", Compressible, NotBinary, List("dssc"))
      final lazy val `dssc+xml`: MediaType =
        new MediaType("application", "dssc+xml", Compressible, NotBinary, List("xdssc"))
      final lazy val `dvcs`: MediaType = new MediaType("application", "dvcs", Compressible, NotBinary)
      final lazy val `ecmascript`: MediaType =
        new MediaType("application", "ecmascript", Compressible, NotBinary, List("ecma", "es"))
      final lazy val `edi-consent`: MediaType =
        new MediaType("application", "edi-consent", Compressible, NotBinary)
      final lazy val `edi-x12`: MediaType =
        new MediaType("application", "edi-x12", Uncompressible, NotBinary)
      final lazy val `edifact`: MediaType =
        new MediaType("application", "edifact", Uncompressible, NotBinary)
      final lazy val `efi`: MediaType = new MediaType("application", "efi", Compressible, NotBinary)
      final lazy val `emergencycalldata.comment+xml`: MediaType =
        new MediaType("application", "emergencycalldata.comment+xml", Compressible, NotBinary)
      final lazy val `emergencycalldata.control+xml`: MediaType =
        new MediaType("application", "emergencycalldata.control+xml", Compressible, NotBinary)
      final lazy val `emergencycalldata.deviceinfo+xml`: MediaType =
        new MediaType("application", "emergencycalldata.deviceinfo+xml", Compressible, NotBinary)
      final lazy val `emergencycalldata.ecall.msd`: MediaType =
        new MediaType("application", "emergencycalldata.ecall.msd", Compressible, NotBinary)
      final lazy val `emergencycalldata.providerinfo+xml`: MediaType =
        new MediaType("application", "emergencycalldata.providerinfo+xml", Compressible, NotBinary)
      final lazy val `emergencycalldata.serviceinfo+xml`: MediaType =
        new MediaType("application", "emergencycalldata.serviceinfo+xml", Compressible, NotBinary)
      final lazy val `emergencycalldata.subscriberinfo+xml`: MediaType = new MediaType(
        "application",
        "emergencycalldata.subscriberinfo+xml",
        Compressible,
        NotBinary)
      final lazy val `emergencycalldata.veds+xml`: MediaType =
        new MediaType("application", "emergencycalldata.veds+xml", Compressible, NotBinary)
      final lazy val `emma+xml`: MediaType =
        new MediaType("application", "emma+xml", Compressible, NotBinary, List("emma"))
      final lazy val `emotionml+xml`: MediaType =
        new MediaType("application", "emotionml+xml", Compressible, NotBinary)
      final lazy val `encaprtp`: MediaType =
        new MediaType("application", "encaprtp", Compressible, NotBinary)
      final lazy val `epp+xml`: MediaType =
        new MediaType("application", "epp+xml", Compressible, NotBinary)
      final lazy val `epub+zip`: MediaType =
        new MediaType("application", "epub+zip", Uncompressible, NotBinary, List("epub"))
      final lazy val `eshop`: MediaType = new MediaType("application", "eshop", Compressible, NotBinary)
      final lazy val `exi`: MediaType =
        new MediaType("application", "exi", Compressible, NotBinary, List("exi"))
      final lazy val `fastinfoset`: MediaType =
        new MediaType("application", "fastinfoset", Compressible, NotBinary)
      final lazy val `fastsoap`: MediaType =
        new MediaType("application", "fastsoap", Compressible, NotBinary)
      final lazy val `fdt+xml`: MediaType =
        new MediaType("application", "fdt+xml", Compressible, NotBinary)
      final lazy val `fhir+json`: MediaType =
        new MediaType("application", "fhir+json", Compressible, NotBinary)
      final lazy val `fhir+xml`: MediaType =
        new MediaType("application", "fhir+xml", Compressible, NotBinary)
      final lazy val `fido.trusted-apps+json`: MediaType =
        new MediaType("application", "fido.trusted-apps+json", Compressible, NotBinary)
      final lazy val `fits`: MediaType = new MediaType("application", "fits", Compressible, NotBinary)
      final lazy val `font-sfnt`: MediaType =
        new MediaType("application", "font-sfnt", Compressible, NotBinary)
      final lazy val `font-tdpfr`: MediaType =
        new MediaType("application", "font-tdpfr", Compressible, NotBinary, List("pfr"))
      final lazy val `font-woff`: MediaType =
        new MediaType("application", "font-woff", Uncompressible, Binary)
      final lazy val `framework-attributes+xml`: MediaType =
        new MediaType("application", "framework-attributes+xml", Compressible, NotBinary)
      final lazy val `geo+json`: MediaType =
        new MediaType("application", "geo+json", Compressible, NotBinary, List("geojson"))
      final lazy val `geo+json-seq`: MediaType =
        new MediaType("application", "geo+json-seq", Compressible, NotBinary)
      final lazy val `geopackage+sqlite3`: MediaType =
        new MediaType("application", "geopackage+sqlite3", Compressible, NotBinary)
      final lazy val `geoxacml+xml`: MediaType =
        new MediaType("application", "geoxacml+xml", Compressible, NotBinary)
      final lazy val `gltf-buffer`: MediaType =
        new MediaType("application", "gltf-buffer", Compressible, NotBinary)
      final lazy val `gml+xml`: MediaType =
        new MediaType("application", "gml+xml", Compressible, NotBinary, List("gml"))
      final lazy val `gpx+xml`: MediaType =
        new MediaType("application", "gpx+xml", Compressible, NotBinary, List("gpx"))
      final lazy val `gxf`: MediaType =
        new MediaType("application", "gxf", Compressible, NotBinary, List("gxf"))
      final lazy val `gzip`: MediaType =
        new MediaType("application", "gzip", Uncompressible, Binary, List("gz"))
      final lazy val `h224`: MediaType = new MediaType("application", "h224", Compressible, NotBinary)
      final lazy val `held+xml`: MediaType =
        new MediaType("application", "held+xml", Compressible, NotBinary)
      final lazy val `hjson`: MediaType =
        new MediaType("application", "hjson", Compressible, NotBinary, List("hjson"))
      final lazy val `http`: MediaType = new MediaType("application", "http", Compressible, NotBinary)
      final lazy val `hyperstudio`: MediaType =
        new MediaType("application", "hyperstudio", Compressible, NotBinary, List("stk"))
      final lazy val `ibe-key-request+xml`: MediaType =
        new MediaType("application", "ibe-key-request+xml", Compressible, NotBinary)
      final lazy val `ibe-pkg-reply+xml`: MediaType =
        new MediaType("application", "ibe-pkg-reply+xml", Compressible, NotBinary)
      final lazy val `ibe-pp-data`: MediaType =
        new MediaType("application", "ibe-pp-data", Compressible, NotBinary)
      final lazy val `iges`: MediaType = new MediaType("application", "iges", Compressible, NotBinary)
      final lazy val `im-iscomposing+xml`: MediaType =
        new MediaType("application", "im-iscomposing+xml", Compressible, NotBinary)
      final lazy val `index`: MediaType = new MediaType("application", "index", Compressible, NotBinary)
      final lazy val `index.cmd`: MediaType =
        new MediaType("application", "index.cmd", Compressible, NotBinary)
      final lazy val `index.obj`: MediaType =
        new MediaType("application", "index.obj", Compressible, NotBinary)
      final lazy val `index.response`: MediaType =
        new MediaType("application", "index.response", Compressible, NotBinary)
      final lazy val `index.vnd`: MediaType =
        new MediaType("application", "index.vnd", Compressible, NotBinary)
      final lazy val `inkml+xml`: MediaType =
        new MediaType("application", "inkml+xml", Compressible, NotBinary, List("ink", "inkml"))
      final lazy val `iotp`: MediaType = new MediaType("application", "iotp", Compressible, NotBinary)
      final lazy val `ipfix`: MediaType =
        new MediaType("application", "ipfix", Compressible, NotBinary, List("ipfix"))
      final lazy val `ipp`: MediaType = new MediaType("application", "ipp", Compressible, NotBinary)
      final lazy val `isup`: MediaType = new MediaType("application", "isup", Compressible, NotBinary)
      final lazy val `its+xml`: MediaType =
        new MediaType("application", "its+xml", Compressible, NotBinary)
      final lazy val `java-archive`: MediaType = new MediaType(
        "application",
        "java-archive",
        Uncompressible,
        Binary,
        List("jar", "war", "ear"))
      final lazy val `java-serialized-object`: MediaType = new MediaType(
        "application",
        "java-serialized-object",
        Uncompressible,
        NotBinary,
        List("ser"))
      final lazy val `java-vm`: MediaType =
        new MediaType("application", "java-vm", Uncompressible, NotBinary, List("class"))
      final lazy val `javascript`: MediaType =
        new MediaType("application", "javascript", Compressible, NotBinary, List("js", "mjs"))
      final lazy val `jf2feed+json`: MediaType =
        new MediaType("application", "jf2feed+json", Compressible, NotBinary)
      final lazy val `jose`: MediaType = new MediaType("application", "jose", Compressible, NotBinary)
      final lazy val `jose+json`: MediaType =
        new MediaType("application", "jose+json", Compressible, NotBinary)
      final lazy val `jrd+json`: MediaType =
        new MediaType("application", "jrd+json", Compressible, NotBinary)
      final lazy val `json`: MediaType =
        new MediaType("application", "json", Compressible, Binary, List("json", "map"))
      final lazy val `json-patch+json`: MediaType =
        new MediaType("application", "json-patch+json", Compressible, NotBinary)
      final lazy val `json-seq`: MediaType =
        new MediaType("application", "json-seq", Compressible, NotBinary)
      final lazy val `json5`: MediaType =
        new MediaType("application", "json5", Compressible, NotBinary, List("json5"))
      final lazy val `jsonml+json`: MediaType =
        new MediaType("application", "jsonml+json", Compressible, NotBinary, List("jsonml"))
      final lazy val `jwk+json`: MediaType =
        new MediaType("application", "jwk+json", Compressible, NotBinary)
      final lazy val `jwk-set+json`: MediaType =
        new MediaType("application", "jwk-set+json", Compressible, NotBinary)
      final lazy val `jwt`: MediaType = new MediaType("application", "jwt", Compressible, NotBinary)
      final lazy val `kpml-request+xml`: MediaType =
        new MediaType("application", "kpml-request+xml", Compressible, NotBinary)
      final lazy val `kpml-response+xml`: MediaType =
        new MediaType("application", "kpml-response+xml", Compressible, NotBinary)
      final lazy val `ld+json`: MediaType =
        new MediaType("application", "ld+json", Compressible, NotBinary, List("jsonld"))
      final lazy val `lgr+xml`: MediaType =
        new MediaType("application", "lgr+xml", Compressible, NotBinary)
      final lazy val `link-format`: MediaType =
        new MediaType("application", "link-format", Compressible, NotBinary)
      final lazy val `load-control+xml`: MediaType =
        new MediaType("application", "load-control+xml", Compressible, NotBinary)
      final lazy val `lost+xml`: MediaType =
        new MediaType("application", "lost+xml", Compressible, NotBinary, List("lostxml"))
      final lazy val `lostsync+xml`: MediaType =
        new MediaType("application", "lostsync+xml", Compressible, NotBinary)
      final lazy val `lxf`: MediaType = new MediaType("application", "lxf", Compressible, NotBinary)
      final lazy val `mac-binhex40`: MediaType =
        new MediaType("application", "mac-binhex40", Compressible, NotBinary, List("hqx"))
      final lazy val `mac-compactpro`: MediaType =
        new MediaType("application", "mac-compactpro", Compressible, NotBinary, List("cpt"))
      final lazy val `macwriteii`: MediaType =
        new MediaType("application", "macwriteii", Compressible, NotBinary)
      final lazy val `mads+xml`: MediaType =
        new MediaType("application", "mads+xml", Compressible, NotBinary, List("mads"))
      final lazy val `manifest+json`: MediaType =
        new MediaType("application", "manifest+json", Compressible, NotBinary, List("webmanifest"))
      final lazy val `marc`: MediaType =
        new MediaType("application", "marc", Compressible, NotBinary, List("mrc"))
      final lazy val `marcxml+xml`: MediaType =
        new MediaType("application", "marcxml+xml", Compressible, NotBinary, List("mrcx"))
      final lazy val `mathematica`: MediaType =
        new MediaType("application", "mathematica", Compressible, NotBinary, List("ma", "nb", "mb"))
      final lazy val `mathml+xml`: MediaType =
        new MediaType("application", "mathml+xml", Compressible, NotBinary, List("mathml"))
      final lazy val `mathml-content+xml`: MediaType =
        new MediaType("application", "mathml-content+xml", Compressible, NotBinary)
      final lazy val `mathml-presentation+xml`: MediaType =
        new MediaType("application", "mathml-presentation+xml", Compressible, NotBinary)
      final lazy val `mbms-associated-procedure-description+xml`: MediaType = new MediaType(
        "application",
        "mbms-associated-procedure-description+xml",
        Compressible,
        NotBinary)
      final lazy val `mbms-deregister+xml`: MediaType =
        new MediaType("application", "mbms-deregister+xml", Compressible, NotBinary)
      final lazy val `mbms-envelope+xml`: MediaType =
        new MediaType("application", "mbms-envelope+xml", Compressible, NotBinary)
      final lazy val `mbms-msk+xml`: MediaType =
        new MediaType("application", "mbms-msk+xml", Compressible, NotBinary)
      final lazy val `mbms-msk-response+xml`: MediaType =
        new MediaType("application", "mbms-msk-response+xml", Compressible, NotBinary)
      final lazy val `mbms-protection-description+xml`: MediaType =
        new MediaType("application", "mbms-protection-description+xml", Compressible, NotBinary)
      final lazy val `mbms-reception-report+xml`: MediaType =
        new MediaType("application", "mbms-reception-report+xml", Compressible, NotBinary)
      final lazy val `mbms-register+xml`: MediaType =
        new MediaType("application", "mbms-register+xml", Compressible, NotBinary)
      final lazy val `mbms-register-response+xml`: MediaType =
        new MediaType("application", "mbms-register-response+xml", Compressible, NotBinary)
      final lazy val `mbms-schedule+xml`: MediaType =
        new MediaType("application", "mbms-schedule+xml", Compressible, NotBinary)
      final lazy val `mbms-user-service-description+xml`: MediaType =
        new MediaType("application", "mbms-user-service-description+xml", Compressible, NotBinary)
      final lazy val `mbox`: MediaType =
        new MediaType("application", "mbox", Compressible, NotBinary, List("mbox"))
      final lazy val `media-policy-dataset+xml`: MediaType =
        new MediaType("application", "media-policy-dataset+xml", Compressible, NotBinary)
      final lazy val `media_control+xml`: MediaType =
        new MediaType("application", "media_control+xml", Compressible, NotBinary)
      final lazy val `mediaservercontrol+xml`: MediaType = new MediaType(
        "application",
        "mediaservercontrol+xml",
        Compressible,
        NotBinary,
        List("mscml"))
      final lazy val `merge-patch+json`: MediaType =
        new MediaType("application", "merge-patch+json", Compressible, NotBinary)
      final lazy val `metalink+xml`: MediaType =
        new MediaType("application", "metalink+xml", Compressible, NotBinary, List("metalink"))
      final lazy val `metalink4+xml`: MediaType =
        new MediaType("application", "metalink4+xml", Compressible, NotBinary, List("meta4"))
      final lazy val `mets+xml`: MediaType =
        new MediaType("application", "mets+xml", Compressible, NotBinary, List("mets"))
      final lazy val `mf4`: MediaType = new MediaType("application", "mf4", Compressible, NotBinary)
      final lazy val `mikey`: MediaType = new MediaType("application", "mikey", Compressible, NotBinary)
      final lazy val `mmt-usd+xml`: MediaType =
        new MediaType("application", "mmt-usd+xml", Compressible, NotBinary)
      final lazy val `mods+xml`: MediaType =
        new MediaType("application", "mods+xml", Compressible, NotBinary, List("mods"))
      final lazy val `moss-keys`: MediaType =
        new MediaType("application", "moss-keys", Compressible, NotBinary)
      final lazy val `moss-signature`: MediaType =
        new MediaType("application", "moss-signature", Compressible, NotBinary)
      final lazy val `mosskey-data`: MediaType =
        new MediaType("application", "mosskey-data", Compressible, NotBinary)
      final lazy val `mosskey-request`: MediaType =
        new MediaType("application", "mosskey-request", Compressible, NotBinary)
      final lazy val `mp21`: MediaType =
        new MediaType("application", "mp21", Compressible, NotBinary, List("m21", "mp21"))
      final lazy val `mp4`: MediaType =
        new MediaType("application", "mp4", Compressible, NotBinary, List("mp4s", "m4p"))
      final lazy val `mpeg4-generic`: MediaType =
        new MediaType("application", "mpeg4-generic", Compressible, NotBinary)
      final lazy val `mpeg4-iod`: MediaType =
        new MediaType("application", "mpeg4-iod", Compressible, NotBinary)
      final lazy val `mpeg4-iod-xmt`: MediaType =
        new MediaType("application", "mpeg4-iod-xmt", Compressible, NotBinary)
      final lazy val `mrb-consumer+xml`: MediaType =
        new MediaType("application", "mrb-consumer+xml", Compressible, NotBinary)
      final lazy val `mrb-publish+xml`: MediaType =
        new MediaType("application", "mrb-publish+xml", Compressible, NotBinary)
      final lazy val `msc-ivr+xml`: MediaType =
        new MediaType("application", "msc-ivr+xml", Compressible, NotBinary)
      final lazy val `msc-mixer+xml`: MediaType =
        new MediaType("application", "msc-mixer+xml", Compressible, NotBinary)
      final lazy val `msword`: MediaType =
        new MediaType("application", "msword", Uncompressible, Binary, List("doc", "dot"))
      final lazy val `mud+json`: MediaType =
        new MediaType("application", "mud+json", Compressible, NotBinary)
      final lazy val `mxf`: MediaType =
        new MediaType("application", "mxf", Compressible, NotBinary, List("mxf"))
      final lazy val `n-quads`: MediaType =
        new MediaType("application", "n-quads", Compressible, NotBinary)
      final lazy val `n-triples`: MediaType =
        new MediaType("application", "n-triples", Compressible, NotBinary)
      final lazy val `nasdata`: MediaType =
        new MediaType("application", "nasdata", Compressible, NotBinary)
      final lazy val `news-checkgroups`: MediaType =
        new MediaType("application", "news-checkgroups", Compressible, NotBinary)
      final lazy val `news-groupinfo`: MediaType =
        new MediaType("application", "news-groupinfo", Compressible, NotBinary)
      final lazy val `news-transmission`: MediaType =
        new MediaType("application", "news-transmission", Compressible, NotBinary)
      final lazy val `nlsml+xml`: MediaType =
        new MediaType("application", "nlsml+xml", Compressible, NotBinary)
      final lazy val `node`: MediaType = new MediaType("application", "node", Compressible, NotBinary)
      final lazy val `nss`: MediaType = new MediaType("application", "nss", Compressible, NotBinary)
      final lazy val `ocsp-request`: MediaType =
        new MediaType("application", "ocsp-request", Compressible, NotBinary)
      final lazy val `ocsp-response`: MediaType =
        new MediaType("application", "ocsp-response", Compressible, NotBinary)
      final lazy val `octet-stream`: MediaType = new MediaType(
        "application",
        "octet-stream",
        Uncompressible,
        Binary,
        List(
          "bin",
          "dms",
          "lrf",
          "mar",
          "so",
          "dist",
          "distz",
          "pkg",
          "bpk",
          "dump",
          "elc",
          "deploy",
          "exe",
          "dll",
          "deb",
          "dmg",
          "iso",
          "img",
          "msi",
          "msp",
          "msm",
          "buffer")
      )
      final lazy val `oda`: MediaType =
        new MediaType("application", "oda", Compressible, NotBinary, List("oda"))
      final lazy val `odx`: MediaType = new MediaType("application", "odx", Compressible, NotBinary)
      final lazy val `oebps-package+xml`: MediaType =
        new MediaType("application", "oebps-package+xml", Compressible, NotBinary, List("opf"))
      final lazy val `ogg`: MediaType =
        new MediaType("application", "ogg", Uncompressible, NotBinary, List("ogx"))
      final lazy val `omdoc+xml`: MediaType =
        new MediaType("application", "omdoc+xml", Compressible, NotBinary, List("omdoc"))
      final lazy val `onenote`: MediaType = new MediaType(
        "application",
        "onenote",
        Compressible,
        NotBinary,
        List("onetoc", "onetoc2", "onetmp", "onepkg"))
      final lazy val `oxps`: MediaType =
        new MediaType("application", "oxps", Compressible, NotBinary, List("oxps"))
      final lazy val `p2p-overlay+xml`: MediaType =
        new MediaType("application", "p2p-overlay+xml", Compressible, NotBinary)
      final lazy val `parityfec`: MediaType =
        new MediaType("application", "parityfec", Compressible, NotBinary)
      final lazy val `passport`: MediaType =
        new MediaType("application", "passport", Compressible, NotBinary)
      final lazy val `patch-ops-error+xml`: MediaType =
        new MediaType("application", "patch-ops-error+xml", Compressible, NotBinary, List("xer"))
      final lazy val `pdf`: MediaType =
        new MediaType("application", "pdf", Uncompressible, Binary, List("pdf"))
      final lazy val `pdx`: MediaType = new MediaType("application", "pdx", Compressible, NotBinary)
      final lazy val `pgp-encrypted`: MediaType =
        new MediaType("application", "pgp-encrypted", Uncompressible, NotBinary, List("pgp"))
      final lazy val `pgp-keys`: MediaType =
        new MediaType("application", "pgp-keys", Compressible, NotBinary)
      final lazy val `pgp-signature`: MediaType =
        new MediaType("application", "pgp-signature", Compressible, NotBinary, List("asc", "sig"))
      final lazy val `pics-rules`: MediaType =
        new MediaType("application", "pics-rules", Compressible, NotBinary, List("prf"))
      final lazy val `pidf+xml`: MediaType =
        new MediaType("application", "pidf+xml", Compressible, NotBinary)
      final lazy val `pidf-diff+xml`: MediaType =
        new MediaType("application", "pidf-diff+xml", Compressible, NotBinary)
      final lazy val `pkcs10`: MediaType =
        new MediaType("application", "pkcs10", Compressible, NotBinary, List("p10"))
      final lazy val `pkcs12`: MediaType = new MediaType("application", "pkcs12", Compressible, NotBinary)
      final lazy val `pkcs7-mime`: MediaType =
        new MediaType("application", "pkcs7-mime", Compressible, NotBinary, List("p7m", "p7c"))
      final lazy val `pkcs7-signature`: MediaType =
        new MediaType("application", "pkcs7-signature", Compressible, NotBinary, List("p7s"))
      final lazy val `pkcs8`: MediaType =
        new MediaType("application", "pkcs8", Compressible, NotBinary, List("p8"))
      final lazy val `pkcs8-encrypted`: MediaType =
        new MediaType("application", "pkcs8-encrypted", Compressible, NotBinary)
      final lazy val `pkix-attr-cert`: MediaType =
        new MediaType("application", "pkix-attr-cert", Compressible, NotBinary, List("ac"))
      final lazy val `pkix-cert`: MediaType =
        new MediaType("application", "pkix-cert", Compressible, NotBinary, List("cer"))
      final lazy val `pkix-crl`: MediaType =
        new MediaType("application", "pkix-crl", Compressible, NotBinary, List("crl"))
      final lazy val `pkix-pkipath`: MediaType =
        new MediaType("application", "pkix-pkipath", Compressible, NotBinary, List("pkipath"))
      final lazy val `pkixcmp`: MediaType =
        new MediaType("application", "pkixcmp", Compressible, NotBinary, List("pki"))
      final lazy val `pls+xml`: MediaType =
        new MediaType("application", "pls+xml", Compressible, NotBinary, List("pls"))
      final lazy val `poc-settings+xml`: MediaType =
        new MediaType("application", "poc-settings+xml", Compressible, NotBinary)
      final lazy val `postscript`: MediaType =
        new MediaType("application", "postscript", Compressible, Binary, List("ai", "eps", "ps"))
      final lazy val `ppsp-tracker+json`: MediaType =
        new MediaType("application", "ppsp-tracker+json", Compressible, NotBinary)
      final lazy val `problem+json`: MediaType =
        new MediaType("application", "problem+json", Compressible, Binary)
      final lazy val `problem+xml`: MediaType =
        new MediaType("application", "problem+xml", Compressible, NotBinary)
      final lazy val `provenance+xml`: MediaType =
        new MediaType("application", "provenance+xml", Compressible, NotBinary)
      final lazy val `prs.alvestrand.titrax-sheet`: MediaType =
        new MediaType("application", "prs.alvestrand.titrax-sheet", Compressible, NotBinary)
      final lazy val `prs.cww`: MediaType =
        new MediaType("application", "prs.cww", Compressible, NotBinary, List("cww"))
      final lazy val `prs.hpub+zip`: MediaType =
        new MediaType("application", "prs.hpub+zip", Uncompressible, NotBinary)
      final lazy val `prs.nprend`: MediaType =
        new MediaType("application", "prs.nprend", Compressible, NotBinary)
      final lazy val `prs.plucker`: MediaType =
        new MediaType("application", "prs.plucker", Compressible, NotBinary)
      final lazy val `prs.rdf-xml-crypt`: MediaType =
        new MediaType("application", "prs.rdf-xml-crypt", Compressible, NotBinary)
      final lazy val `prs.xsf+xml`: MediaType =
        new MediaType("application", "prs.xsf+xml", Compressible, NotBinary)
      final lazy val `pskc+xml`: MediaType =
        new MediaType("application", "pskc+xml", Compressible, NotBinary, List("pskcxml"))
      final lazy val `qsig`: MediaType = new MediaType("application", "qsig", Compressible, NotBinary)
      final lazy val `raml+yaml`: MediaType =
        new MediaType("application", "raml+yaml", Compressible, NotBinary, List("raml"))
      final lazy val `raptorfec`: MediaType =
        new MediaType("application", "raptorfec", Compressible, NotBinary)
      final lazy val `rdap+json`: MediaType =
        new MediaType("application", "rdap+json", Compressible, NotBinary)
      final lazy val `rdf+xml`: MediaType =
        new MediaType("application", "rdf+xml", Compressible, NotBinary, List("rdf", "owl"))
      final lazy val `reginfo+xml`: MediaType =
        new MediaType("application", "reginfo+xml", Compressible, NotBinary, List("rif"))
      final lazy val `relax-ng-compact-syntax`: MediaType = new MediaType(
        "application",
        "relax-ng-compact-syntax",
        Compressible,
        NotBinary,
        List("rnc"))
      final lazy val `remote-printing`: MediaType =
        new MediaType("application", "remote-printing", Compressible, NotBinary)
      final lazy val `reputon+json`: MediaType =
        new MediaType("application", "reputon+json", Compressible, NotBinary)
      final lazy val `resource-lists+xml`: MediaType =
        new MediaType("application", "resource-lists+xml", Compressible, NotBinary, List("rl"))
      final lazy val `resource-lists-diff+xml`: MediaType = new MediaType(
        "application",
        "resource-lists-diff+xml",
        Compressible,
        NotBinary,
        List("rld"))
      final lazy val `rfc+xml`: MediaType =
        new MediaType("application", "rfc+xml", Compressible, NotBinary)
      final lazy val `riscos`: MediaType = new MediaType("application", "riscos", Compressible, NotBinary)
      final lazy val `rlmi+xml`: MediaType =
        new MediaType("application", "rlmi+xml", Compressible, NotBinary)
      final lazy val `rls-services+xml`: MediaType =
        new MediaType("application", "rls-services+xml", Compressible, NotBinary, List("rs"))
      final lazy val `route-apd+xml`: MediaType =
        new MediaType("application", "route-apd+xml", Compressible, NotBinary)
      final lazy val `route-s-tsid+xml`: MediaType =
        new MediaType("application", "route-s-tsid+xml", Compressible, NotBinary)
      final lazy val `route-usd+xml`: MediaType =
        new MediaType("application", "route-usd+xml", Compressible, NotBinary)
      final lazy val `rpki-ghostbusters`: MediaType =
        new MediaType("application", "rpki-ghostbusters", Compressible, NotBinary, List("gbr"))
      final lazy val `rpki-manifest`: MediaType =
        new MediaType("application", "rpki-manifest", Compressible, NotBinary, List("mft"))
      final lazy val `rpki-publication`: MediaType =
        new MediaType("application", "rpki-publication", Compressible, NotBinary)
      final lazy val `rpki-roa`: MediaType =
        new MediaType("application", "rpki-roa", Compressible, NotBinary, List("roa"))
      final lazy val `rpki-updown`: MediaType =
        new MediaType("application", "rpki-updown", Compressible, NotBinary)
      final lazy val `rsd+xml`: MediaType =
        new MediaType("application", "rsd+xml", Compressible, NotBinary, List("rsd"))
      final lazy val `rss+xml`: MediaType =
        new MediaType("application", "rss+xml", Compressible, NotBinary, List("rss"))
      final lazy val `rtf`: MediaType =
        new MediaType("application", "rtf", Compressible, NotBinary, List("rtf"))
      final lazy val `rtploopback`: MediaType =
        new MediaType("application", "rtploopback", Compressible, NotBinary)
      final lazy val `rtx`: MediaType = new MediaType("application", "rtx", Compressible, NotBinary)
      final lazy val `samlassertion+xml`: MediaType =
        new MediaType("application", "samlassertion+xml", Compressible, NotBinary)
      final lazy val `samlmetadata+xml`: MediaType =
        new MediaType("application", "samlmetadata+xml", Compressible, NotBinary)
      final lazy val `sbml+xml`: MediaType =
        new MediaType("application", "sbml+xml", Compressible, NotBinary, List("sbml"))
      final lazy val `scaip+xml`: MediaType =
        new MediaType("application", "scaip+xml", Compressible, NotBinary)
      final lazy val `scim+json`: MediaType =
        new MediaType("application", "scim+json", Compressible, NotBinary)
      final lazy val `scvp-cv-request`: MediaType =
        new MediaType("application", "scvp-cv-request", Compressible, NotBinary, List("scq"))
      final lazy val `scvp-cv-response`: MediaType =
        new MediaType("application", "scvp-cv-response", Compressible, NotBinary, List("scs"))
      final lazy val `scvp-vp-request`: MediaType =
        new MediaType("application", "scvp-vp-request", Compressible, NotBinary, List("spq"))
      final lazy val `scvp-vp-response`: MediaType =
        new MediaType("application", "scvp-vp-response", Compressible, NotBinary, List("spp"))
      final lazy val `sdp`: MediaType =
        new MediaType("application", "sdp", Compressible, NotBinary, List("sdp"))
      final lazy val `secevent+jwt`: MediaType =
        new MediaType("application", "secevent+jwt", Compressible, NotBinary)
      final lazy val `senml+cbor`: MediaType =
        new MediaType("application", "senml+cbor", Compressible, NotBinary)
      final lazy val `senml+json`: MediaType =
        new MediaType("application", "senml+json", Compressible, NotBinary)
      final lazy val `senml+xml`: MediaType =
        new MediaType("application", "senml+xml", Compressible, NotBinary)
      final lazy val `senml-exi`: MediaType =
        new MediaType("application", "senml-exi", Compressible, NotBinary)
      final lazy val `sensml+cbor`: MediaType =
        new MediaType("application", "sensml+cbor", Compressible, NotBinary)
      final lazy val `sensml+json`: MediaType =
        new MediaType("application", "sensml+json", Compressible, NotBinary)
      final lazy val `sensml+xml`: MediaType =
        new MediaType("application", "sensml+xml", Compressible, NotBinary)
      final lazy val `sensml-exi`: MediaType =
        new MediaType("application", "sensml-exi", Compressible, NotBinary)
      final lazy val `sep+xml`: MediaType =
        new MediaType("application", "sep+xml", Compressible, NotBinary)
      final lazy val `sep-exi`: MediaType =
        new MediaType("application", "sep-exi", Compressible, NotBinary)
      final lazy val `session-info`: MediaType =
        new MediaType("application", "session-info", Compressible, NotBinary)
      final lazy val `set-payment`: MediaType =
        new MediaType("application", "set-payment", Compressible, NotBinary)
      final lazy val `set-payment-initiation`: MediaType = new MediaType(
        "application",
        "set-payment-initiation",
        Compressible,
        NotBinary,
        List("setpay"))
      final lazy val `set-registration`: MediaType =
        new MediaType("application", "set-registration", Compressible, NotBinary)
      final lazy val `set-registration-initiation`: MediaType = new MediaType(
        "application",
        "set-registration-initiation",
        Compressible,
        NotBinary,
        List("setreg"))
      final lazy val `sgml`: MediaType = new MediaType("application", "sgml", Compressible, NotBinary)
      final lazy val `sgml-open-catalog`: MediaType =
        new MediaType("application", "sgml-open-catalog", Compressible, NotBinary)
      final lazy val `shf+xml`: MediaType =
        new MediaType("application", "shf+xml", Compressible, NotBinary, List("shf"))
      final lazy val `sieve`: MediaType = new MediaType("application", "sieve", Compressible, NotBinary)
      final lazy val `simple-filter+xml`: MediaType =
        new MediaType("application", "simple-filter+xml", Compressible, NotBinary)
      final lazy val `simple-message-summary`: MediaType =
        new MediaType("application", "simple-message-summary", Compressible, NotBinary)
      final lazy val `simplesymbolcontainer`: MediaType =
        new MediaType("application", "simplesymbolcontainer", Compressible, NotBinary)
      final lazy val `slate`: MediaType = new MediaType("application", "slate", Compressible, NotBinary)
      final lazy val `smil`: MediaType = new MediaType("application", "smil", Compressible, NotBinary)
      final lazy val `smil+xml`: MediaType =
        new MediaType("application", "smil+xml", Compressible, NotBinary, List("smi", "smil"))
      final lazy val `smpte336m`: MediaType =
        new MediaType("application", "smpte336m", Compressible, NotBinary)
      final lazy val `soap+fastinfoset`: MediaType =
        new MediaType("application", "soap+fastinfoset", Compressible, NotBinary)
      final lazy val `soap+xml`: MediaType =
        new MediaType("application", "soap+xml", Compressible, NotBinary)
      final lazy val `sparql-query`: MediaType =
        new MediaType("application", "sparql-query", Compressible, NotBinary, List("rq"))
      final lazy val `sparql-results+xml`: MediaType =
        new MediaType("application", "sparql-results+xml", Compressible, NotBinary, List("srx"))
      final lazy val `spirits-event+xml`: MediaType =
        new MediaType("application", "spirits-event+xml", Compressible, NotBinary)
      final lazy val `sql`: MediaType = new MediaType("application", "sql", Compressible, NotBinary)
      final lazy val `srgs`: MediaType =
        new MediaType("application", "srgs", Compressible, NotBinary, List("gram"))
      final lazy val `srgs+xml`: MediaType =
        new MediaType("application", "srgs+xml", Compressible, NotBinary, List("grxml"))
      final lazy val `sru+xml`: MediaType =
        new MediaType("application", "sru+xml", Compressible, NotBinary, List("sru"))
      final lazy val `ssdl+xml`: MediaType =
        new MediaType("application", "ssdl+xml", Compressible, NotBinary, List("ssdl"))
      final lazy val `ssml+xml`: MediaType =
        new MediaType("application", "ssml+xml", Compressible, NotBinary, List("ssml"))
      final lazy val `stix+json`: MediaType =
        new MediaType("application", "stix+json", Compressible, NotBinary)
      final lazy val `tamp-apex-update`: MediaType =
        new MediaType("application", "tamp-apex-update", Compressible, NotBinary)
      final lazy val `tamp-apex-update-confirm`: MediaType =
        new MediaType("application", "tamp-apex-update-confirm", Compressible, NotBinary)
      final lazy val `tamp-community-update`: MediaType =
        new MediaType("application", "tamp-community-update", Compressible, NotBinary)
      final lazy val `tamp-community-update-confirm`: MediaType =
        new MediaType("application", "tamp-community-update-confirm", Compressible, NotBinary)
      final lazy val `tamp-error`: MediaType =
        new MediaType("application", "tamp-error", Compressible, NotBinary)
      final lazy val `tamp-sequence-adjust`: MediaType =
        new MediaType("application", "tamp-sequence-adjust", Compressible, NotBinary)
      final lazy val `tamp-sequence-adjust-confirm`: MediaType =
        new MediaType("application", "tamp-sequence-adjust-confirm", Compressible, NotBinary)
      final lazy val `tamp-status-query`: MediaType =
        new MediaType("application", "tamp-status-query", Compressible, NotBinary)
      final lazy val `tamp-status-response`: MediaType =
        new MediaType("application", "tamp-status-response", Compressible, NotBinary)
      final lazy val `tamp-update`: MediaType =
        new MediaType("application", "tamp-update", Compressible, NotBinary)
      final lazy val `tamp-update-confirm`: MediaType =
        new MediaType("application", "tamp-update-confirm", Compressible, NotBinary)
      final lazy val `tar`: MediaType = new MediaType("application", "tar", Compressible, NotBinary)
      final lazy val `taxii+json`: MediaType =
        new MediaType("application", "taxii+json", Compressible, NotBinary)
      final lazy val `tei+xml`: MediaType =
        new MediaType("application", "tei+xml", Compressible, NotBinary, List("tei", "teicorpus"))
      final lazy val `thraud+xml`: MediaType =
        new MediaType("application", "thraud+xml", Compressible, NotBinary, List("tfi"))
      final lazy val `timestamp-query`: MediaType =
        new MediaType("application", "timestamp-query", Compressible, NotBinary)
      final lazy val `timestamp-reply`: MediaType =
        new MediaType("application", "timestamp-reply", Compressible, NotBinary)
      final lazy val `timestamped-data`: MediaType =
        new MediaType("application", "timestamped-data", Compressible, NotBinary, List("tsd"))
      final lazy val `tlsrpt+gzip`: MediaType =
        new MediaType("application", "tlsrpt+gzip", Compressible, NotBinary)
      final lazy val `tlsrpt+json`: MediaType =
        new MediaType("application", "tlsrpt+json", Compressible, NotBinary)
      final lazy val `tnauthlist`: MediaType =
        new MediaType("application", "tnauthlist", Compressible, NotBinary)
      final lazy val `trickle-ice-sdpfrag`: MediaType =
        new MediaType("application", "trickle-ice-sdpfrag", Compressible, NotBinary)
      final lazy val `trig`: MediaType = new MediaType("application", "trig", Compressible, NotBinary)
      final lazy val `ttml+xml`: MediaType =
        new MediaType("application", "ttml+xml", Compressible, NotBinary)
      final lazy val `tve-trigger`: MediaType =
        new MediaType("application", "tve-trigger", Compressible, NotBinary)
      final lazy val `ulpfec`: MediaType = new MediaType("application", "ulpfec", Compressible, NotBinary)
      final lazy val `urc-grpsheet+xml`: MediaType =
        new MediaType("application", "urc-grpsheet+xml", Compressible, NotBinary)
      final lazy val `urc-ressheet+xml`: MediaType =
        new MediaType("application", "urc-ressheet+xml", Compressible, NotBinary)
      final lazy val `urc-targetdesc+xml`: MediaType =
        new MediaType("application", "urc-targetdesc+xml", Compressible, NotBinary)
      final lazy val `urc-uisocketdesc+xml`: MediaType =
        new MediaType("application", "urc-uisocketdesc+xml", Compressible, NotBinary)
      final lazy val `vcard+json`: MediaType =
        new MediaType("application", "vcard+json", Compressible, NotBinary)
      final lazy val `vcard+xml`: MediaType =
        new MediaType("application", "vcard+xml", Compressible, NotBinary)
      final lazy val `vemmi`: MediaType = new MediaType("application", "vemmi", Compressible, NotBinary)
      final lazy val `vividence.scriptfile`: MediaType =
        new MediaType("application", "vividence.scriptfile", Compressible, NotBinary)
      final lazy val `vnd.1000minds.decision-model+xml`: MediaType =
        new MediaType("application", "vnd.1000minds.decision-model+xml", Compressible, NotBinary)
      final lazy val `vnd.3gpp-prose+xml`: MediaType =
        new MediaType("application", "vnd.3gpp-prose+xml", Compressible, NotBinary)
      final lazy val `vnd.3gpp-prose-pc3ch+xml`: MediaType =
        new MediaType("application", "vnd.3gpp-prose-pc3ch+xml", Compressible, NotBinary)
      final lazy val `vnd.3gpp-v2x-local-service-information`: MediaType = new MediaType(
        "application",
        "vnd.3gpp-v2x-local-service-information",
        Compressible,
        NotBinary)
      final lazy val `vnd.3gpp.access-transfer-events+xml`: MediaType =
        new MediaType("application", "vnd.3gpp.access-transfer-events+xml", Compressible, NotBinary)
      final lazy val `vnd.3gpp.bsf+xml`: MediaType =
        new MediaType("application", "vnd.3gpp.bsf+xml", Compressible, NotBinary)
      final lazy val `vnd.3gpp.gmop+xml`: MediaType =
        new MediaType("application", "vnd.3gpp.gmop+xml", Compressible, NotBinary)
      final lazy val `vnd.3gpp.mc-signalling-ear`: MediaType =
        new MediaType("application", "vnd.3gpp.mc-signalling-ear", Compressible, NotBinary)
      final lazy val `vnd.3gpp.mcdata-payload`: MediaType =
        new MediaType("application", "vnd.3gpp.mcdata-payload", Compressible, NotBinary)
      final lazy val `vnd.3gpp.mcdata-signalling`: MediaType =
        new MediaType("application", "vnd.3gpp.mcdata-signalling", Compressible, NotBinary)
      final lazy val `vnd.3gpp.mcptt-affiliation-command+xml`: MediaType = new MediaType(
        "application",
        "vnd.3gpp.mcptt-affiliation-command+xml",
        Compressible,
        NotBinary)
      final lazy val `vnd.3gpp.mcptt-floor-request+xml`: MediaType =
        new MediaType("application", "vnd.3gpp.mcptt-floor-request+xml", Compressible, NotBinary)
      final lazy val `vnd.3gpp.mcptt-info+xml`: MediaType =
        new MediaType("application", "vnd.3gpp.mcptt-info+xml", Compressible, NotBinary)
      final lazy val `vnd.3gpp.mcptt-location-info+xml`: MediaType =
        new MediaType("application", "vnd.3gpp.mcptt-location-info+xml", Compressible, NotBinary)
      final lazy val `vnd.3gpp.mcptt-mbms-usage-info+xml`: MediaType =
        new MediaType("application", "vnd.3gpp.mcptt-mbms-usage-info+xml", Compressible, NotBinary)
      final lazy val `vnd.3gpp.mcptt-signed+xml`: MediaType =
        new MediaType("application", "vnd.3gpp.mcptt-signed+xml", Compressible, NotBinary)
      final lazy val `vnd.3gpp.mid-call+xml`: MediaType =
        new MediaType("application", "vnd.3gpp.mid-call+xml", Compressible, NotBinary)
      final lazy val `vnd.3gpp.pic-bw-large`: MediaType =
        new MediaType("application", "vnd.3gpp.pic-bw-large", Compressible, NotBinary, List("plb"))
      final lazy val `vnd.3gpp.pic-bw-small`: MediaType =
        new MediaType("application", "vnd.3gpp.pic-bw-small", Compressible, NotBinary, List("psb"))
      final lazy val `vnd.3gpp.pic-bw-var`: MediaType =
        new MediaType("application", "vnd.3gpp.pic-bw-var", Compressible, NotBinary, List("pvb"))
      final lazy val `vnd.3gpp.sms`: MediaType =
        new MediaType("application", "vnd.3gpp.sms", Compressible, NotBinary)
      final lazy val `vnd.3gpp.sms+xml`: MediaType =
        new MediaType("application", "vnd.3gpp.sms+xml", Compressible, NotBinary)
      final lazy val `vnd.3gpp.srvcc-ext+xml`: MediaType =
        new MediaType("application", "vnd.3gpp.srvcc-ext+xml", Compressible, NotBinary)
      final lazy val `vnd.3gpp.srvcc-info+xml`: MediaType =
        new MediaType("application", "vnd.3gpp.srvcc-info+xml", Compressible, NotBinary)
      final lazy val `vnd.3gpp.state-and-event-info+xml`: MediaType =
        new MediaType("application", "vnd.3gpp.state-and-event-info+xml", Compressible, NotBinary)
      final lazy val `vnd.3gpp.ussd+xml`: MediaType =
        new MediaType("application", "vnd.3gpp.ussd+xml", Compressible, NotBinary)
      final lazy val `vnd.3gpp2.bcmcsinfo+xml`: MediaType =
        new MediaType("application", "vnd.3gpp2.bcmcsinfo+xml", Compressible, NotBinary)
      final lazy val `vnd.3gpp2.sms`: MediaType =
        new MediaType("application", "vnd.3gpp2.sms", Compressible, NotBinary)
      final lazy val `vnd.3gpp2.tcap`: MediaType =
        new MediaType("application", "vnd.3gpp2.tcap", Compressible, NotBinary, List("tcap"))
      final lazy val `vnd.3lightssoftware.imagescal`: MediaType =
        new MediaType("application", "vnd.3lightssoftware.imagescal", Compressible, NotBinary)
      final lazy val `vnd.3m.post-it-notes`: MediaType =
        new MediaType("application", "vnd.3m.post-it-notes", Compressible, NotBinary, List("pwn"))
      final lazy val `vnd.accpac.simply.aso`: MediaType =
        new MediaType("application", "vnd.accpac.simply.aso", Compressible, NotBinary, List("aso"))
      final lazy val `vnd.accpac.simply.imp`: MediaType =
        new MediaType("application", "vnd.accpac.simply.imp", Compressible, NotBinary, List("imp"))
      final lazy val `vnd.acucobol`: MediaType =
        new MediaType("application", "vnd.acucobol", Compressible, NotBinary, List("acu"))
      final lazy val `vnd.acucorp`: MediaType =
        new MediaType("application", "vnd.acucorp", Compressible, NotBinary, List("atc", "acutc"))
      final lazy val `vnd.adobe.air-application-installer-package+zip`: MediaType = new MediaType(
        "application",
        "vnd.adobe.air-application-installer-package+zip",
        Uncompressible,
        NotBinary,
        List("air"))
      final lazy val `vnd.adobe.flash.movie`: MediaType =
        new MediaType("application", "vnd.adobe.flash.movie", Compressible, NotBinary)
      final lazy val `vnd.adobe.formscentral.fcdt`: MediaType = new MediaType(
        "application",
        "vnd.adobe.formscentral.fcdt",
        Compressible,
        NotBinary,
        List("fcdt"))
      final lazy val `vnd.adobe.fxp`: MediaType =
        new MediaType("application", "vnd.adobe.fxp", Compressible, NotBinary, List("fxp", "fxpl"))
      final lazy val `vnd.adobe.partial-upload`: MediaType =
        new MediaType("application", "vnd.adobe.partial-upload", Compressible, NotBinary)
      final lazy val `vnd.adobe.xdp+xml`: MediaType =
        new MediaType("application", "vnd.adobe.xdp+xml", Compressible, NotBinary, List("xdp"))
      final lazy val `vnd.adobe.xfdf`: MediaType =
        new MediaType("application", "vnd.adobe.xfdf", Compressible, NotBinary, List("xfdf"))
      final lazy val `vnd.aether.imp`: MediaType =
        new MediaType("application", "vnd.aether.imp", Compressible, NotBinary)
      final lazy val `vnd.afpc.afplinedata`: MediaType =
        new MediaType("application", "vnd.afpc.afplinedata", Compressible, NotBinary)
      final lazy val `vnd.afpc.modca`: MediaType =
        new MediaType("application", "vnd.afpc.modca", Compressible, NotBinary)
      final lazy val `vnd.ah-barcode`: MediaType =
        new MediaType("application", "vnd.ah-barcode", Compressible, NotBinary)
      final lazy val `vnd.ahead.space`: MediaType =
        new MediaType("application", "vnd.ahead.space", Compressible, NotBinary, List("ahead"))
      final lazy val `vnd.airzip.filesecure.azf`: MediaType = new MediaType(
        "application",
        "vnd.airzip.filesecure.azf",
        Compressible,
        NotBinary,
        List("azf"))
      final lazy val `vnd.airzip.filesecure.azs`: MediaType = new MediaType(
        "application",
        "vnd.airzip.filesecure.azs",
        Compressible,
        NotBinary,
        List("azs"))
      final lazy val `vnd.amadeus+json`: MediaType =
        new MediaType("application", "vnd.amadeus+json", Compressible, NotBinary)
      final lazy val `vnd.amazon.ebook`: MediaType =
        new MediaType("application", "vnd.amazon.ebook", Compressible, NotBinary, List("azw"))
      final lazy val `vnd.amazon.mobi8-ebook`: MediaType =
        new MediaType("application", "vnd.amazon.mobi8-ebook", Compressible, NotBinary)
      final lazy val `vnd.americandynamics.acc`: MediaType = new MediaType(
        "application",
        "vnd.americandynamics.acc",
        Compressible,
        NotBinary,
        List("acc"))
      final lazy val `vnd.amiga.ami`: MediaType =
        new MediaType("application", "vnd.amiga.ami", Compressible, NotBinary, List("ami"))
      final lazy val `vnd.amundsen.maze+xml`: MediaType =
        new MediaType("application", "vnd.amundsen.maze+xml", Compressible, NotBinary)
      final lazy val `vnd.android.package-archive`: MediaType = new MediaType(
        "application",
        "vnd.android.package-archive",
        Uncompressible,
        NotBinary,
        List("apk"))
      final lazy val `vnd.anki`: MediaType =
        new MediaType("application", "vnd.anki", Compressible, NotBinary)
      final lazy val `vnd.anser-web-certificate-issue-initiation`: MediaType = new MediaType(
        "application",
        "vnd.anser-web-certificate-issue-initiation",
        Compressible,
        NotBinary,
        List("cii"))
      final lazy val `vnd.anser-web-funds-transfer-initiation`: MediaType = new MediaType(
        "application",
        "vnd.anser-web-funds-transfer-initiation",
        Compressible,
        NotBinary,
        List("fti"))
      final lazy val `vnd.antix.game-component`: MediaType = new MediaType(
        "application",
        "vnd.antix.game-component",
        Compressible,
        NotBinary,
        List("atx"))
      final lazy val `vnd.apache.thrift.binary`: MediaType =
        new MediaType("application", "vnd.apache.thrift.binary", Compressible, NotBinary)
      final lazy val `vnd.apache.thrift.compact`: MediaType =
        new MediaType("application", "vnd.apache.thrift.compact", Compressible, NotBinary)
      final lazy val `vnd.apache.thrift.json`: MediaType =
        new MediaType("application", "vnd.apache.thrift.json", Compressible, NotBinary)
      final lazy val `vnd.api+json`: MediaType =
        new MediaType("application", "vnd.api+json", Compressible, Binary)
      final lazy val `vnd.apothekende.reservation+json`: MediaType =
        new MediaType("application", "vnd.apothekende.reservation+json", Compressible, NotBinary)
      final lazy val `vnd.apple.installer+xml`: MediaType = new MediaType(
        "application",
        "vnd.apple.installer+xml",
        Compressible,
        NotBinary,
        List("mpkg"))
      final lazy val `vnd.apple.keynote`: MediaType =
        new MediaType("application", "vnd.apple.keynote", Compressible, NotBinary, List("keynote"))
      final lazy val `vnd.apple.mpegurl`: MediaType =
        new MediaType("application", "vnd.apple.mpegurl", Compressible, NotBinary, List("m3u8"))
      final lazy val `vnd.apple.numbers`: MediaType =
        new MediaType("application", "vnd.apple.numbers", Compressible, NotBinary, List("numbers"))
      final lazy val `vnd.apple.pages`: MediaType =
        new MediaType("application", "vnd.apple.pages", Compressible, NotBinary, List("pages"))
      final lazy val `vnd.apple.pkpass`: MediaType =
        new MediaType("application", "vnd.apple.pkpass", Uncompressible, NotBinary, List("pkpass"))
      final lazy val `vnd.arastra.swi`: MediaType =
        new MediaType("application", "vnd.arastra.swi", Compressible, NotBinary)
      final lazy val `vnd.aristanetworks.swi`: MediaType =
        new MediaType("application", "vnd.aristanetworks.swi", Compressible, NotBinary, List("swi"))
      final lazy val `vnd.artisan+json`: MediaType =
        new MediaType("application", "vnd.artisan+json", Compressible, NotBinary)
      final lazy val `vnd.artsquare`: MediaType =
        new MediaType("application", "vnd.artsquare", Compressible, NotBinary)
      final lazy val `vnd.astraea-software.iota`: MediaType = new MediaType(
        "application",
        "vnd.astraea-software.iota",
        Compressible,
        NotBinary,
        List("iota"))
      final lazy val `vnd.audiograph`: MediaType =
        new MediaType("application", "vnd.audiograph", Compressible, NotBinary, List("aep"))
      final lazy val `vnd.autopackage`: MediaType =
        new MediaType("application", "vnd.autopackage", Compressible, NotBinary)
      final lazy val `vnd.avalon+json`: MediaType =
        new MediaType("application", "vnd.avalon+json", Compressible, NotBinary)
      final lazy val `vnd.avistar+xml`: MediaType =
        new MediaType("application", "vnd.avistar+xml", Compressible, NotBinary)
      final lazy val `vnd.balsamiq.bmml+xml`: MediaType =
        new MediaType("application", "vnd.balsamiq.bmml+xml", Compressible, NotBinary)
      final lazy val `vnd.balsamiq.bmpr`: MediaType =
        new MediaType("application", "vnd.balsamiq.bmpr", Compressible, NotBinary)
      final lazy val `vnd.banana-accounting`: MediaType =
        new MediaType("application", "vnd.banana-accounting", Compressible, NotBinary)
      final lazy val `vnd.bbf.usp.msg`: MediaType =
        new MediaType("application", "vnd.bbf.usp.msg", Compressible, NotBinary)
      final lazy val `vnd.bbf.usp.msg+json`: MediaType =
        new MediaType("application", "vnd.bbf.usp.msg+json", Compressible, NotBinary)
      final lazy val `vnd.bekitzur-stech+json`: MediaType =
        new MediaType("application", "vnd.bekitzur-stech+json", Compressible, NotBinary)
      final lazy val `vnd.bint.med-content`: MediaType =
        new MediaType("application", "vnd.bint.med-content", Compressible, NotBinary)
      final lazy val `vnd.biopax.rdf+xml`: MediaType =
        new MediaType("application", "vnd.biopax.rdf+xml", Compressible, NotBinary)
      final lazy val `vnd.blink-idb-value-wrapper`: MediaType =
        new MediaType("application", "vnd.blink-idb-value-wrapper", Compressible, NotBinary)
      final lazy val `vnd.blueice.multipass`: MediaType =
        new MediaType("application", "vnd.blueice.multipass", Compressible, NotBinary, List("mpm"))
      final lazy val `vnd.bluetooth.ep.oob`: MediaType =
        new MediaType("application", "vnd.bluetooth.ep.oob", Compressible, NotBinary)
      final lazy val `vnd.bluetooth.le.oob`: MediaType =
        new MediaType("application", "vnd.bluetooth.le.oob", Compressible, NotBinary)
      final lazy val `vnd.bmi`: MediaType =
        new MediaType("application", "vnd.bmi", Compressible, NotBinary, List("bmi"))
      final lazy val `vnd.businessobjects`: MediaType =
        new MediaType("application", "vnd.businessobjects", Compressible, NotBinary, List("rep"))
      final lazy val `vnd.byu.uapi+json`: MediaType =
        new MediaType("application", "vnd.byu.uapi+json", Compressible, NotBinary)
      final lazy val `vnd.cab-jscript`: MediaType =
        new MediaType("application", "vnd.cab-jscript", Compressible, NotBinary)
      final lazy val `vnd.canon-cpdl`: MediaType =
        new MediaType("application", "vnd.canon-cpdl", Compressible, NotBinary)
      final lazy val `vnd.canon-lips`: MediaType =
        new MediaType("application", "vnd.canon-lips", Compressible, NotBinary)
      final lazy val `vnd.capasystems-pg+json`: MediaType =
        new MediaType("application", "vnd.capasystems-pg+json", Compressible, NotBinary)
      final lazy val `vnd.cendio.thinlinc.clientconf`: MediaType =
        new MediaType("application", "vnd.cendio.thinlinc.clientconf", Compressible, NotBinary)
      final lazy val `vnd.century-systems.tcp_stream`: MediaType =
        new MediaType("application", "vnd.century-systems.tcp_stream", Compressible, NotBinary)
      final lazy val `vnd.chemdraw+xml`: MediaType =
        new MediaType("application", "vnd.chemdraw+xml", Compressible, NotBinary, List("cdxml"))
      final lazy val `vnd.chess-pgn`: MediaType =
        new MediaType("application", "vnd.chess-pgn", Compressible, NotBinary)
      final lazy val `vnd.chipnuts.karaoke-mmd`: MediaType = new MediaType(
        "application",
        "vnd.chipnuts.karaoke-mmd",
        Compressible,
        NotBinary,
        List("mmd"))
      final lazy val `vnd.cinderella`: MediaType =
        new MediaType("application", "vnd.cinderella", Compressible, NotBinary, List("cdy"))
      final lazy val part_0: List[MediaType] = List(
        `1d-interleaved-parityfec`,
        `3gpdash-qoe-report+xml`,
        `3gpp-ims+xml`,
        `a2l`,
        `activemessage`,
        `activity+json`,
        `alto-costmap+json`,
        `alto-costmapfilter+json`,
        `alto-directory+json`,
        `alto-endpointcost+json`,
        `alto-endpointcostparams+json`,
        `alto-endpointprop+json`,
        `alto-endpointpropparams+json`,
        `alto-error+json`,
        `alto-networkmap+json`,
        `alto-networkmapfilter+json`,
        `aml`,
        `andrew-inset`,
        `applefile`,
        `applixware`,
        `atf`,
        `atfx`,
        `atom+xml`,
        `atomcat+xml`,
        `atomdeleted+xml`,
        `atomicmail`,
        `atomsvc+xml`,
        `atxml`,
        `auth-policy+xml`,
        `bacnet-xdd+zip`,
        `batch-smtp`,
        `bdoc`,
        `beep+xml`,
        `calendar+json`,
        `calendar+xml`,
        `call-completion`,
        `cals-1840`,
        `cbor`,
        `cccex`,
        `ccmp+xml`,
        `ccxml+xml`,
        `cdfx+xml`,
        `cdmi-capability`,
        `cdmi-container`,
        `cdmi-domain`,
        `cdmi-object`,
        `cdmi-queue`,
        `cdni`,
        `cea`,
        `cea-2018+xml`,
        `cellml+xml`,
        `cfw`,
        `clue_info+xml`,
        `cms`,
        `cnrp+xml`,
        `coap-group+json`,
        `coap-payload`,
        `commonground`,
        `conference-info+xml`,
        `cose`,
        `cose-key`,
        `cose-key-set`,
        `cpl+xml`,
        `csrattrs`,
        `csta+xml`,
        `cstadata+xml`,
        `csvm+json`,
        `cu-seeme`,
        `cwt`,
        `cybercash`,
        `dart`,
        `dash+xml`,
        `dashdelta`,
        `davmount+xml`,
        `dca-rft`,
        `dcd`,
        `dec-dx`,
        `dialog-info+xml`,
        `dicom`,
        `dicom+json`,
        `dicom+xml`,
        `dii`,
        `dit`,
        `dns`,
        `dns+json`,
        `dns-message`,
        `docbook+xml`,
        `dskpp+xml`,
        `dssc+der`,
        `dssc+xml`,
        `dvcs`,
        `ecmascript`,
        `edi-consent`,
        `edi-x12`,
        `edifact`,
        `efi`,
        `emergencycalldata.comment+xml`,
        `emergencycalldata.control+xml`,
        `emergencycalldata.deviceinfo+xml`,
        `emergencycalldata.ecall.msd`,
        `emergencycalldata.providerinfo+xml`,
        `emergencycalldata.serviceinfo+xml`,
        `emergencycalldata.subscriberinfo+xml`,
        `emergencycalldata.veds+xml`,
        `emma+xml`,
        `emotionml+xml`,
        `encaprtp`,
        `epp+xml`,
        `epub+zip`,
        `eshop`,
        `exi`,
        `fastinfoset`,
        `fastsoap`,
        `fdt+xml`,
        `fhir+json`,
        `fhir+xml`,
        `fido.trusted-apps+json`,
        `fits`,
        `font-sfnt`,
        `font-tdpfr`,
        `font-woff`,
        `framework-attributes+xml`,
        `geo+json`,
        `geo+json-seq`,
        `geopackage+sqlite3`,
        `geoxacml+xml`,
        `gltf-buffer`,
        `gml+xml`,
        `gpx+xml`,
        `gxf`,
        `gzip`,
        `h224`,
        `held+xml`,
        `hjson`,
        `http`,
        `hyperstudio`,
        `ibe-key-request+xml`,
        `ibe-pkg-reply+xml`,
        `ibe-pp-data`,
        `iges`,
        `im-iscomposing+xml`,
        `index`,
        `index.cmd`,
        `index.obj`,
        `index.response`,
        `index.vnd`,
        `inkml+xml`,
        `iotp`,
        `ipfix`,
        `ipp`,
        `isup`,
        `its+xml`,
        `java-archive`,
        `java-serialized-object`,
        `java-vm`,
        `javascript`,
        `jf2feed+json`,
        `jose`,
        `jose+json`,
        `jrd+json`,
        `json`,
        `json-patch+json`,
        `json-seq`,
        `json5`,
        `jsonml+json`,
        `jwk+json`,
        `jwk-set+json`,
        `jwt`,
        `kpml-request+xml`,
        `kpml-response+xml`,
        `ld+json`,
        `lgr+xml`,
        `link-format`,
        `load-control+xml`,
        `lost+xml`,
        `lostsync+xml`,
        `lxf`,
        `mac-binhex40`,
        `mac-compactpro`,
        `macwriteii`,
        `mads+xml`,
        `manifest+json`,
        `marc`,
        `marcxml+xml`,
        `mathematica`,
        `mathml+xml`,
        `mathml-content+xml`,
        `mathml-presentation+xml`,
        `mbms-associated-procedure-description+xml`,
        `mbms-deregister+xml`,
        `mbms-envelope+xml`,
        `mbms-msk+xml`,
        `mbms-msk-response+xml`,
        `mbms-protection-description+xml`,
        `mbms-reception-report+xml`,
        `mbms-register+xml`,
        `mbms-register-response+xml`,
        `mbms-schedule+xml`,
        `mbms-user-service-description+xml`,
        `mbox`,
        `media-policy-dataset+xml`,
        `media_control+xml`,
        `mediaservercontrol+xml`,
        `merge-patch+json`,
        `metalink+xml`,
        `metalink4+xml`,
        `mets+xml`,
        `mf4`,
        `mikey`,
        `mmt-usd+xml`,
        `mods+xml`,
        `moss-keys`,
        `moss-signature`,
        `mosskey-data`,
        `mosskey-request`,
        `mp21`,
        `mp4`,
        `mpeg4-generic`,
        `mpeg4-iod`,
        `mpeg4-iod-xmt`,
        `mrb-consumer+xml`,
        `mrb-publish+xml`,
        `msc-ivr+xml`,
        `msc-mixer+xml`,
        `msword`,
        `mud+json`,
        `mxf`,
        `n-quads`,
        `n-triples`,
        `nasdata`,
        `news-checkgroups`,
        `news-groupinfo`,
        `news-transmission`,
        `nlsml+xml`,
        `node`,
        `nss`,
        `ocsp-request`,
        `ocsp-response`,
        `octet-stream`,
        `oda`,
        `odx`,
        `oebps-package+xml`,
        `ogg`,
        `omdoc+xml`,
        `onenote`,
        `oxps`,
        `p2p-overlay+xml`,
        `parityfec`,
        `passport`,
        `patch-ops-error+xml`,
        `pdf`,
        `pdx`,
        `pgp-encrypted`,
        `pgp-keys`,
        `pgp-signature`,
        `pics-rules`,
        `pidf+xml`,
        `pidf-diff+xml`,
        `pkcs10`,
        `pkcs12`,
        `pkcs7-mime`,
        `pkcs7-signature`,
        `pkcs8`,
        `pkcs8-encrypted`,
        `pkix-attr-cert`,
        `pkix-cert`,
        `pkix-crl`,
        `pkix-pkipath`,
        `pkixcmp`,
        `pls+xml`,
        `poc-settings+xml`,
        `postscript`,
        `ppsp-tracker+json`,
        `problem+json`,
        `problem+xml`,
        `provenance+xml`,
        `prs.alvestrand.titrax-sheet`,
        `prs.cww`,
        `prs.hpub+zip`,
        `prs.nprend`,
        `prs.plucker`,
        `prs.rdf-xml-crypt`,
        `prs.xsf+xml`,
        `pskc+xml`,
        `qsig`,
        `raml+yaml`,
        `raptorfec`,
        `rdap+json`,
        `rdf+xml`,
        `reginfo+xml`,
        `relax-ng-compact-syntax`,
        `remote-printing`,
        `reputon+json`,
        `resource-lists+xml`,
        `resource-lists-diff+xml`,
        `rfc+xml`,
        `riscos`,
        `rlmi+xml`,
        `rls-services+xml`,
        `route-apd+xml`,
        `route-s-tsid+xml`,
        `route-usd+xml`,
        `rpki-ghostbusters`,
        `rpki-manifest`,
        `rpki-publication`,
        `rpki-roa`,
        `rpki-updown`,
        `rsd+xml`,
        `rss+xml`,
        `rtf`,
        `rtploopback`,
        `rtx`,
        `samlassertion+xml`,
        `samlmetadata+xml`,
        `sbml+xml`,
        `scaip+xml`,
        `scim+json`,
        `scvp-cv-request`,
        `scvp-cv-response`,
        `scvp-vp-request`,
        `scvp-vp-response`,
        `sdp`,
        `secevent+jwt`,
        `senml+cbor`,
        `senml+json`,
        `senml+xml`,
        `senml-exi`,
        `sensml+cbor`,
        `sensml+json`,
        `sensml+xml`,
        `sensml-exi`,
        `sep+xml`,
        `sep-exi`,
        `session-info`,
        `set-payment`,
        `set-payment-initiation`,
        `set-registration`,
        `set-registration-initiation`,
        `sgml`,
        `sgml-open-catalog`,
        `shf+xml`,
        `sieve`,
        `simple-filter+xml`,
        `simple-message-summary`,
        `simplesymbolcontainer`,
        `slate`,
        `smil`,
        `smil+xml`,
        `smpte336m`,
        `soap+fastinfoset`,
        `soap+xml`,
        `sparql-query`,
        `sparql-results+xml`,
        `spirits-event+xml`,
        `sql`,
        `srgs`,
        `srgs+xml`,
        `sru+xml`,
        `ssdl+xml`,
        `ssml+xml`,
        `stix+json`,
        `tamp-apex-update`,
        `tamp-apex-update-confirm`,
        `tamp-community-update`,
        `tamp-community-update-confirm`,
        `tamp-error`,
        `tamp-sequence-adjust`,
        `tamp-sequence-adjust-confirm`,
        `tamp-status-query`,
        `tamp-status-response`,
        `tamp-update`,
        `tamp-update-confirm`,
        `tar`,
        `taxii+json`,
        `tei+xml`,
        `thraud+xml`,
        `timestamp-query`,
        `timestamp-reply`,
        `timestamped-data`,
        `tlsrpt+gzip`,
        `tlsrpt+json`,
        `tnauthlist`,
        `trickle-ice-sdpfrag`,
        `trig`,
        `ttml+xml`,
        `tve-trigger`,
        `ulpfec`,
        `urc-grpsheet+xml`,
        `urc-ressheet+xml`,
        `urc-targetdesc+xml`,
        `urc-uisocketdesc+xml`,
        `vcard+json`,
        `vcard+xml`,
        `vemmi`,
        `vividence.scriptfile`,
        `vnd.1000minds.decision-model+xml`,
        `vnd.3gpp-prose+xml`,
        `vnd.3gpp-prose-pc3ch+xml`,
        `vnd.3gpp-v2x-local-service-information`,
        `vnd.3gpp.access-transfer-events+xml`,
        `vnd.3gpp.bsf+xml`,
        `vnd.3gpp.gmop+xml`,
        `vnd.3gpp.mc-signalling-ear`,
        `vnd.3gpp.mcdata-payload`,
        `vnd.3gpp.mcdata-signalling`,
        `vnd.3gpp.mcptt-affiliation-command+xml`,
        `vnd.3gpp.mcptt-floor-request+xml`,
        `vnd.3gpp.mcptt-info+xml`,
        `vnd.3gpp.mcptt-location-info+xml`,
        `vnd.3gpp.mcptt-mbms-usage-info+xml`,
        `vnd.3gpp.mcptt-signed+xml`,
        `vnd.3gpp.mid-call+xml`,
        `vnd.3gpp.pic-bw-large`,
        `vnd.3gpp.pic-bw-small`,
        `vnd.3gpp.pic-bw-var`,
        `vnd.3gpp.sms`,
        `vnd.3gpp.sms+xml`,
        `vnd.3gpp.srvcc-ext+xml`,
        `vnd.3gpp.srvcc-info+xml`,
        `vnd.3gpp.state-and-event-info+xml`,
        `vnd.3gpp.ussd+xml`,
        `vnd.3gpp2.bcmcsinfo+xml`,
        `vnd.3gpp2.sms`,
        `vnd.3gpp2.tcap`,
        `vnd.3lightssoftware.imagescal`,
        `vnd.3m.post-it-notes`,
        `vnd.accpac.simply.aso`,
        `vnd.accpac.simply.imp`,
        `vnd.acucobol`,
        `vnd.acucorp`,
        `vnd.adobe.air-application-installer-package+zip`,
        `vnd.adobe.flash.movie`,
        `vnd.adobe.formscentral.fcdt`,
        `vnd.adobe.fxp`,
        `vnd.adobe.partial-upload`,
        `vnd.adobe.xdp+xml`,
        `vnd.adobe.xfdf`,
        `vnd.aether.imp`,
        `vnd.afpc.afplinedata`,
        `vnd.afpc.modca`,
        `vnd.ah-barcode`,
        `vnd.ahead.space`,
        `vnd.airzip.filesecure.azf`,
        `vnd.airzip.filesecure.azs`,
        `vnd.amadeus+json`,
        `vnd.amazon.ebook`,
        `vnd.amazon.mobi8-ebook`,
        `vnd.americandynamics.acc`,
        `vnd.amiga.ami`,
        `vnd.amundsen.maze+xml`,
        `vnd.android.package-archive`,
        `vnd.anki`,
        `vnd.anser-web-certificate-issue-initiation`,
        `vnd.anser-web-funds-transfer-initiation`,
        `vnd.antix.game-component`,
        `vnd.apache.thrift.binary`,
        `vnd.apache.thrift.compact`,
        `vnd.apache.thrift.json`,
        `vnd.api+json`,
        `vnd.apothekende.reservation+json`,
        `vnd.apple.installer+xml`,
        `vnd.apple.keynote`,
        `vnd.apple.mpegurl`,
        `vnd.apple.numbers`,
        `vnd.apple.pages`,
        `vnd.apple.pkpass`,
        `vnd.arastra.swi`,
        `vnd.aristanetworks.swi`,
        `vnd.artisan+json`,
        `vnd.artsquare`,
        `vnd.astraea-software.iota`,
        `vnd.audiograph`,
        `vnd.autopackage`,
        `vnd.avalon+json`,
        `vnd.avistar+xml`,
        `vnd.balsamiq.bmml+xml`,
        `vnd.balsamiq.bmpr`,
        `vnd.banana-accounting`,
        `vnd.bbf.usp.msg`,
        `vnd.bbf.usp.msg+json`,
        `vnd.bekitzur-stech+json`,
        `vnd.bint.med-content`,
        `vnd.biopax.rdf+xml`,
        `vnd.blink-idb-value-wrapper`,
        `vnd.blueice.multipass`,
        `vnd.bluetooth.ep.oob`,
        `vnd.bluetooth.le.oob`,
        `vnd.bmi`,
        `vnd.businessobjects`,
        `vnd.byu.uapi+json`,
        `vnd.cab-jscript`,
        `vnd.canon-cpdl`,
        `vnd.canon-lips`,
        `vnd.capasystems-pg+json`,
        `vnd.cendio.thinlinc.clientconf`,
        `vnd.century-systems.tcp_stream`,
        `vnd.chemdraw+xml`,
        `vnd.chess-pgn`,
        `vnd.chipnuts.karaoke-mmd`,
        `vnd.cinderella`
      )
    }
    trait application_1 {
      final lazy val `vnd.cirpack.isdn-ext`: MediaType =
        new MediaType("application", "vnd.cirpack.isdn-ext", Compressible, NotBinary)
      final lazy val `vnd.citationstyles.style+xml`: MediaType = new MediaType(
        "application",
        "vnd.citationstyles.style+xml",
        Compressible,
        NotBinary,
        List("csl"))
      final lazy val `vnd.claymore`: MediaType =
        new MediaType("application", "vnd.claymore", Compressible, NotBinary, List("cla"))
      final lazy val `vnd.cloanto.rp9`: MediaType =
        new MediaType("application", "vnd.cloanto.rp9", Compressible, NotBinary, List("rp9"))
      final lazy val `vnd.clonk.c4group`: MediaType = new MediaType(
        "application",
        "vnd.clonk.c4group",
        Compressible,
        NotBinary,
        List("c4g", "c4d", "c4f", "c4p", "c4u"))
      final lazy val `vnd.cluetrust.cartomobile-config`: MediaType = new MediaType(
        "application",
        "vnd.cluetrust.cartomobile-config",
        Compressible,
        NotBinary,
        List("c11amc"))
      final lazy val `vnd.cluetrust.cartomobile-config-pkg`: MediaType = new MediaType(
        "application",
        "vnd.cluetrust.cartomobile-config-pkg",
        Compressible,
        NotBinary,
        List("c11amz"))
      final lazy val `vnd.coffeescript`: MediaType =
        new MediaType("application", "vnd.coffeescript", Compressible, NotBinary)
      final lazy val `vnd.collabio.xodocuments.document`: MediaType =
        new MediaType("application", "vnd.collabio.xodocuments.document", Compressible, NotBinary)
      final lazy val `vnd.collabio.xodocuments.document-template`: MediaType = new MediaType(
        "application",
        "vnd.collabio.xodocuments.document-template",
        Compressible,
        NotBinary)
      final lazy val `vnd.collabio.xodocuments.presentation`: MediaType = new MediaType(
        "application",
        "vnd.collabio.xodocuments.presentation",
        Compressible,
        NotBinary)
      final lazy val `vnd.collabio.xodocuments.presentation-template`: MediaType = new MediaType(
        "application",
        "vnd.collabio.xodocuments.presentation-template",
        Compressible,
        NotBinary)
      final lazy val `vnd.collabio.xodocuments.spreadsheet`: MediaType = new MediaType(
        "application",
        "vnd.collabio.xodocuments.spreadsheet",
        Compressible,
        NotBinary)
      final lazy val `vnd.collabio.xodocuments.spreadsheet-template`: MediaType = new MediaType(
        "application",
        "vnd.collabio.xodocuments.spreadsheet-template",
        Compressible,
        NotBinary)
      final lazy val `vnd.collection+json`: MediaType =
        new MediaType("application", "vnd.collection+json", Compressible, NotBinary)
      final lazy val `vnd.collection.doc+json`: MediaType =
        new MediaType("application", "vnd.collection.doc+json", Compressible, NotBinary)
      final lazy val `vnd.collection.next+json`: MediaType =
        new MediaType("application", "vnd.collection.next+json", Compressible, NotBinary)
      final lazy val `vnd.comicbook+zip`: MediaType =
        new MediaType("application", "vnd.comicbook+zip", Uncompressible, NotBinary)
      final lazy val `vnd.comicbook-rar`: MediaType =
        new MediaType("application", "vnd.comicbook-rar", Compressible, NotBinary)
      final lazy val `vnd.commerce-battelle`: MediaType =
        new MediaType("application", "vnd.commerce-battelle", Compressible, NotBinary)
      final lazy val `vnd.commonspace`: MediaType =
        new MediaType("application", "vnd.commonspace", Compressible, NotBinary, List("csp"))
      final lazy val `vnd.contact.cmsg`: MediaType =
        new MediaType("application", "vnd.contact.cmsg", Compressible, NotBinary, List("cdbcmsg"))
      final lazy val `vnd.coreos.ignition+json`: MediaType =
        new MediaType("application", "vnd.coreos.ignition+json", Compressible, NotBinary)
      final lazy val `vnd.cosmocaller`: MediaType =
        new MediaType("application", "vnd.cosmocaller", Compressible, NotBinary, List("cmc"))
      final lazy val `vnd.crick.clicker`: MediaType =
        new MediaType("application", "vnd.crick.clicker", Compressible, NotBinary, List("clkx"))
      final lazy val `vnd.crick.clicker.keyboard`: MediaType = new MediaType(
        "application",
        "vnd.crick.clicker.keyboard",
        Compressible,
        NotBinary,
        List("clkk"))
      final lazy val `vnd.crick.clicker.palette`: MediaType = new MediaType(
        "application",
        "vnd.crick.clicker.palette",
        Compressible,
        NotBinary,
        List("clkp"))
      final lazy val `vnd.crick.clicker.template`: MediaType = new MediaType(
        "application",
        "vnd.crick.clicker.template",
        Compressible,
        NotBinary,
        List("clkt"))
      final lazy val `vnd.crick.clicker.wordbank`: MediaType = new MediaType(
        "application",
        "vnd.crick.clicker.wordbank",
        Compressible,
        NotBinary,
        List("clkw"))
      final lazy val `vnd.criticaltools.wbs+xml`: MediaType = new MediaType(
        "application",
        "vnd.criticaltools.wbs+xml",
        Compressible,
        NotBinary,
        List("wbs"))
      final lazy val `vnd.ctc-posml`: MediaType =
        new MediaType("application", "vnd.ctc-posml", Compressible, NotBinary, List("pml"))
      final lazy val `vnd.ctct.ws+xml`: MediaType =
        new MediaType("application", "vnd.ctct.ws+xml", Compressible, NotBinary)
      final lazy val `vnd.cups-pdf`: MediaType =
        new MediaType("application", "vnd.cups-pdf", Compressible, NotBinary)
      final lazy val `vnd.cups-postscript`: MediaType =
        new MediaType("application", "vnd.cups-postscript", Compressible, NotBinary)
      final lazy val `vnd.cups-ppd`: MediaType =
        new MediaType("application", "vnd.cups-ppd", Compressible, NotBinary, List("ppd"))
      final lazy val `vnd.cups-raster`: MediaType =
        new MediaType("application", "vnd.cups-raster", Compressible, NotBinary)
      final lazy val `vnd.cups-raw`: MediaType =
        new MediaType("application", "vnd.cups-raw", Compressible, NotBinary)
      final lazy val `vnd.curl`: MediaType =
        new MediaType("application", "vnd.curl", Compressible, NotBinary)
      final lazy val `vnd.curl.car`: MediaType =
        new MediaType("application", "vnd.curl.car", Compressible, NotBinary, List("car"))
      final lazy val `vnd.curl.pcurl`: MediaType =
        new MediaType("application", "vnd.curl.pcurl", Compressible, NotBinary, List("pcurl"))
      final lazy val `vnd.cyan.dean.root+xml`: MediaType =
        new MediaType("application", "vnd.cyan.dean.root+xml", Compressible, NotBinary)
      final lazy val `vnd.cybank`: MediaType =
        new MediaType("application", "vnd.cybank", Compressible, NotBinary)
      final lazy val `vnd.d2l.coursepackage1p0+zip`: MediaType =
        new MediaType("application", "vnd.d2l.coursepackage1p0+zip", Uncompressible, NotBinary)
      final lazy val `vnd.dart`: MediaType =
        new MediaType("application", "vnd.dart", Compressible, NotBinary, List("dart"))
      final lazy val `vnd.data-vision.rdz`: MediaType =
        new MediaType("application", "vnd.data-vision.rdz", Compressible, NotBinary, List("rdz"))
      final lazy val `vnd.datapackage+json`: MediaType =
        new MediaType("application", "vnd.datapackage+json", Compressible, NotBinary)
      final lazy val `vnd.dataresource+json`: MediaType =
        new MediaType("application", "vnd.dataresource+json", Compressible, NotBinary)
      final lazy val `vnd.debian.binary-package`: MediaType =
        new MediaType("application", "vnd.debian.binary-package", Compressible, NotBinary)
      final lazy val `vnd.dece.data`: MediaType = new MediaType(
        "application",
        "vnd.dece.data",
        Compressible,
        NotBinary,
        List("uvf", "uvvf", "uvd", "uvvd"))
      final lazy val `vnd.dece.ttml+xml`: MediaType = new MediaType(
        "application",
        "vnd.dece.ttml+xml",
        Compressible,
        NotBinary,
        List("uvt", "uvvt"))
      final lazy val `vnd.dece.unspecified`: MediaType = new MediaType(
        "application",
        "vnd.dece.unspecified",
        Compressible,
        NotBinary,
        List("uvx", "uvvx"))
      final lazy val `vnd.dece.zip`: MediaType =
        new MediaType("application", "vnd.dece.zip", Compressible, NotBinary, List("uvz", "uvvz"))
      final lazy val `vnd.denovo.fcselayout-link`: MediaType = new MediaType(
        "application",
        "vnd.denovo.fcselayout-link",
        Compressible,
        NotBinary,
        List("fe_launch"))
      final lazy val `vnd.desmume.movie`: MediaType =
        new MediaType("application", "vnd.desmume.movie", Compressible, NotBinary)
      final lazy val `vnd.dir-bi.plate-dl-nosuffix`: MediaType =
        new MediaType("application", "vnd.dir-bi.plate-dl-nosuffix", Compressible, NotBinary)
      final lazy val `vnd.dm.delegation+xml`: MediaType =
        new MediaType("application", "vnd.dm.delegation+xml", Compressible, NotBinary)
      final lazy val `vnd.dna`: MediaType =
        new MediaType("application", "vnd.dna", Compressible, NotBinary, List("dna"))
      final lazy val `vnd.document+json`: MediaType =
        new MediaType("application", "vnd.document+json", Compressible, NotBinary)
      final lazy val `vnd.dolby.mlp`: MediaType =
        new MediaType("application", "vnd.dolby.mlp", Compressible, NotBinary, List("mlp"))
      final lazy val `vnd.dolby.mobile.1`: MediaType =
        new MediaType("application", "vnd.dolby.mobile.1", Compressible, NotBinary)
      final lazy val `vnd.dolby.mobile.2`: MediaType =
        new MediaType("application", "vnd.dolby.mobile.2", Compressible, NotBinary)
      final lazy val `vnd.doremir.scorecloud-binary-document`: MediaType = new MediaType(
        "application",
        "vnd.doremir.scorecloud-binary-document",
        Compressible,
        NotBinary)
      final lazy val `vnd.dpgraph`: MediaType =
        new MediaType("application", "vnd.dpgraph", Compressible, NotBinary, List("dpg"))
      final lazy val `vnd.dreamfactory`: MediaType =
        new MediaType("application", "vnd.dreamfactory", Compressible, NotBinary, List("dfac"))
      final lazy val `vnd.drive+json`: MediaType =
        new MediaType("application", "vnd.drive+json", Compressible, NotBinary)
      final lazy val `vnd.ds-keypoint`: MediaType =
        new MediaType("application", "vnd.ds-keypoint", Compressible, NotBinary, List("kpxx"))
      final lazy val `vnd.dtg.local`: MediaType =
        new MediaType("application", "vnd.dtg.local", Compressible, NotBinary)
      final lazy val `vnd.dtg.local.flash`: MediaType =
        new MediaType("application", "vnd.dtg.local.flash", Compressible, NotBinary)
      final lazy val `vnd.dtg.local.html`: MediaType =
        new MediaType("application", "vnd.dtg.local.html", Compressible, NotBinary)
      final lazy val `vnd.dvb.ait`: MediaType =
        new MediaType("application", "vnd.dvb.ait", Compressible, NotBinary, List("ait"))
      final lazy val `vnd.dvb.dvbj`: MediaType =
        new MediaType("application", "vnd.dvb.dvbj", Compressible, NotBinary)
      final lazy val `vnd.dvb.esgcontainer`: MediaType =
        new MediaType("application", "vnd.dvb.esgcontainer", Compressible, NotBinary)
      final lazy val `vnd.dvb.ipdcdftnotifaccess`: MediaType =
        new MediaType("application", "vnd.dvb.ipdcdftnotifaccess", Compressible, NotBinary)
      final lazy val `vnd.dvb.ipdcesgaccess`: MediaType =
        new MediaType("application", "vnd.dvb.ipdcesgaccess", Compressible, NotBinary)
      final lazy val `vnd.dvb.ipdcesgaccess2`: MediaType =
        new MediaType("application", "vnd.dvb.ipdcesgaccess2", Compressible, NotBinary)
      final lazy val `vnd.dvb.ipdcesgpdd`: MediaType =
        new MediaType("application", "vnd.dvb.ipdcesgpdd", Compressible, NotBinary)
      final lazy val `vnd.dvb.ipdcroaming`: MediaType =
        new MediaType("application", "vnd.dvb.ipdcroaming", Compressible, NotBinary)
      final lazy val `vnd.dvb.iptv.alfec-base`: MediaType =
        new MediaType("application", "vnd.dvb.iptv.alfec-base", Compressible, NotBinary)
      final lazy val `vnd.dvb.iptv.alfec-enhancement`: MediaType =
        new MediaType("application", "vnd.dvb.iptv.alfec-enhancement", Compressible, NotBinary)
      final lazy val `vnd.dvb.notif-aggregate-root+xml`: MediaType =
        new MediaType("application", "vnd.dvb.notif-aggregate-root+xml", Compressible, NotBinary)
      final lazy val `vnd.dvb.notif-container+xml`: MediaType =
        new MediaType("application", "vnd.dvb.notif-container+xml", Compressible, NotBinary)
      final lazy val `vnd.dvb.notif-generic+xml`: MediaType =
        new MediaType("application", "vnd.dvb.notif-generic+xml", Compressible, NotBinary)
      final lazy val `vnd.dvb.notif-ia-msglist+xml`: MediaType =
        new MediaType("application", "vnd.dvb.notif-ia-msglist+xml", Compressible, NotBinary)
      final lazy val `vnd.dvb.notif-ia-registration-request+xml`: MediaType = new MediaType(
        "application",
        "vnd.dvb.notif-ia-registration-request+xml",
        Compressible,
        NotBinary)
      final lazy val `vnd.dvb.notif-ia-registration-response+xml`: MediaType = new MediaType(
        "application",
        "vnd.dvb.notif-ia-registration-response+xml",
        Compressible,
        NotBinary)
      final lazy val `vnd.dvb.notif-init+xml`: MediaType =
        new MediaType("application", "vnd.dvb.notif-init+xml", Compressible, NotBinary)
      final lazy val `vnd.dvb.pfr`: MediaType =
        new MediaType("application", "vnd.dvb.pfr", Compressible, NotBinary)
      final lazy val `vnd.dvb.service`: MediaType =
        new MediaType("application", "vnd.dvb.service", Compressible, NotBinary, List("svc"))
      final lazy val `vnd.dxr`: MediaType =
        new MediaType("application", "vnd.dxr", Compressible, NotBinary)
      final lazy val `vnd.dynageo`: MediaType =
        new MediaType("application", "vnd.dynageo", Compressible, NotBinary, List("geo"))
      final lazy val `vnd.dzr`: MediaType =
        new MediaType("application", "vnd.dzr", Compressible, NotBinary)
      final lazy val `vnd.easykaraoke.cdgdownload`: MediaType =
        new MediaType("application", "vnd.easykaraoke.cdgdownload", Compressible, NotBinary)
      final lazy val `vnd.ecdis-update`: MediaType =
        new MediaType("application", "vnd.ecdis-update", Compressible, NotBinary)
      final lazy val `vnd.ecip.rlp`: MediaType =
        new MediaType("application", "vnd.ecip.rlp", Compressible, NotBinary)
      final lazy val `vnd.ecowin.chart`: MediaType =
        new MediaType("application", "vnd.ecowin.chart", Compressible, NotBinary, List("mag"))
      final lazy val `vnd.ecowin.filerequest`: MediaType =
        new MediaType("application", "vnd.ecowin.filerequest", Compressible, NotBinary)
      final lazy val `vnd.ecowin.fileupdate`: MediaType =
        new MediaType("application", "vnd.ecowin.fileupdate", Compressible, NotBinary)
      final lazy val `vnd.ecowin.series`: MediaType =
        new MediaType("application", "vnd.ecowin.series", Compressible, NotBinary)
      final lazy val `vnd.ecowin.seriesrequest`: MediaType =
        new MediaType("application", "vnd.ecowin.seriesrequest", Compressible, NotBinary)
      final lazy val `vnd.ecowin.seriesupdate`: MediaType =
        new MediaType("application", "vnd.ecowin.seriesupdate", Compressible, NotBinary)
      final lazy val `vnd.efi.img`: MediaType =
        new MediaType("application", "vnd.efi.img", Compressible, NotBinary)
      final lazy val `vnd.efi.iso`: MediaType =
        new MediaType("application", "vnd.efi.iso", Compressible, NotBinary)
      final lazy val `vnd.emclient.accessrequest+xml`: MediaType =
        new MediaType("application", "vnd.emclient.accessrequest+xml", Compressible, NotBinary)
      final lazy val `vnd.enliven`: MediaType =
        new MediaType("application", "vnd.enliven", Compressible, NotBinary, List("nml"))
      final lazy val `vnd.enphase.envoy`: MediaType =
        new MediaType("application", "vnd.enphase.envoy", Compressible, NotBinary)
      final lazy val `vnd.eprints.data+xml`: MediaType =
        new MediaType("application", "vnd.eprints.data+xml", Compressible, NotBinary)
      final lazy val `vnd.epson.esf`: MediaType =
        new MediaType("application", "vnd.epson.esf", Compressible, NotBinary, List("esf"))
      final lazy val `vnd.epson.msf`: MediaType =
        new MediaType("application", "vnd.epson.msf", Compressible, NotBinary, List("msf"))
      final lazy val `vnd.epson.quickanime`: MediaType =
        new MediaType("application", "vnd.epson.quickanime", Compressible, NotBinary, List("qam"))
      final lazy val `vnd.epson.salt`: MediaType =
        new MediaType("application", "vnd.epson.salt", Compressible, NotBinary, List("slt"))
      final lazy val `vnd.epson.ssf`: MediaType =
        new MediaType("application", "vnd.epson.ssf", Compressible, NotBinary, List("ssf"))
      final lazy val `vnd.ericsson.quickcall`: MediaType =
        new MediaType("application", "vnd.ericsson.quickcall", Compressible, NotBinary)
      final lazy val `vnd.espass-espass+zip`: MediaType =
        new MediaType("application", "vnd.espass-espass+zip", Uncompressible, NotBinary)
      final lazy val `vnd.eszigno3+xml`: MediaType = new MediaType(
        "application",
        "vnd.eszigno3+xml",
        Compressible,
        NotBinary,
        List("es3", "et3"))
      final lazy val `vnd.etsi.aoc+xml`: MediaType =
        new MediaType("application", "vnd.etsi.aoc+xml", Compressible, NotBinary)
      final lazy val `vnd.etsi.asic-e+zip`: MediaType =
        new MediaType("application", "vnd.etsi.asic-e+zip", Uncompressible, NotBinary)
      final lazy val `vnd.etsi.asic-s+zip`: MediaType =
        new MediaType("application", "vnd.etsi.asic-s+zip", Uncompressible, NotBinary)
      final lazy val `vnd.etsi.cug+xml`: MediaType =
        new MediaType("application", "vnd.etsi.cug+xml", Compressible, NotBinary)
      final lazy val `vnd.etsi.iptvcommand+xml`: MediaType =
        new MediaType("application", "vnd.etsi.iptvcommand+xml", Compressible, NotBinary)
      final lazy val `vnd.etsi.iptvdiscovery+xml`: MediaType =
        new MediaType("application", "vnd.etsi.iptvdiscovery+xml", Compressible, NotBinary)
      final lazy val `vnd.etsi.iptvprofile+xml`: MediaType =
        new MediaType("application", "vnd.etsi.iptvprofile+xml", Compressible, NotBinary)
      final lazy val `vnd.etsi.iptvsad-bc+xml`: MediaType =
        new MediaType("application", "vnd.etsi.iptvsad-bc+xml", Compressible, NotBinary)
      final lazy val `vnd.etsi.iptvsad-cod+xml`: MediaType =
        new MediaType("application", "vnd.etsi.iptvsad-cod+xml", Compressible, NotBinary)
      final lazy val `vnd.etsi.iptvsad-npvr+xml`: MediaType =
        new MediaType("application", "vnd.etsi.iptvsad-npvr+xml", Compressible, NotBinary)
      final lazy val `vnd.etsi.iptvservice+xml`: MediaType =
        new MediaType("application", "vnd.etsi.iptvservice+xml", Compressible, NotBinary)
      final lazy val `vnd.etsi.iptvsync+xml`: MediaType =
        new MediaType("application", "vnd.etsi.iptvsync+xml", Compressible, NotBinary)
      final lazy val `vnd.etsi.iptvueprofile+xml`: MediaType =
        new MediaType("application", "vnd.etsi.iptvueprofile+xml", Compressible, NotBinary)
      final lazy val `vnd.etsi.mcid+xml`: MediaType =
        new MediaType("application", "vnd.etsi.mcid+xml", Compressible, NotBinary)
      final lazy val `vnd.etsi.mheg5`: MediaType =
        new MediaType("application", "vnd.etsi.mheg5", Compressible, NotBinary)
      final lazy val `vnd.etsi.overload-control-policy-dataset+xml`: MediaType = new MediaType(
        "application",
        "vnd.etsi.overload-control-policy-dataset+xml",
        Compressible,
        NotBinary)
      final lazy val `vnd.etsi.pstn+xml`: MediaType =
        new MediaType("application", "vnd.etsi.pstn+xml", Compressible, NotBinary)
      final lazy val `vnd.etsi.sci+xml`: MediaType =
        new MediaType("application", "vnd.etsi.sci+xml", Compressible, NotBinary)
      final lazy val `vnd.etsi.simservs+xml`: MediaType =
        new MediaType("application", "vnd.etsi.simservs+xml", Compressible, NotBinary)
      final lazy val `vnd.etsi.timestamp-token`: MediaType =
        new MediaType("application", "vnd.etsi.timestamp-token", Compressible, NotBinary)
      final lazy val `vnd.etsi.tsl+xml`: MediaType =
        new MediaType("application", "vnd.etsi.tsl+xml", Compressible, NotBinary)
      final lazy val `vnd.etsi.tsl.der`: MediaType =
        new MediaType("application", "vnd.etsi.tsl.der", Compressible, NotBinary)
      final lazy val `vnd.eudora.data`: MediaType =
        new MediaType("application", "vnd.eudora.data", Compressible, NotBinary)
      final lazy val `vnd.evolv.ecig.profile`: MediaType =
        new MediaType("application", "vnd.evolv.ecig.profile", Compressible, NotBinary)
      final lazy val `vnd.evolv.ecig.settings`: MediaType =
        new MediaType("application", "vnd.evolv.ecig.settings", Compressible, NotBinary)
      final lazy val `vnd.evolv.ecig.theme`: MediaType =
        new MediaType("application", "vnd.evolv.ecig.theme", Compressible, NotBinary)
      final lazy val `vnd.exstream-empower+zip`: MediaType =
        new MediaType("application", "vnd.exstream-empower+zip", Uncompressible, NotBinary)
      final lazy val `vnd.ezpix-album`: MediaType =
        new MediaType("application", "vnd.ezpix-album", Compressible, NotBinary, List("ez2"))
      final lazy val `vnd.ezpix-package`: MediaType =
        new MediaType("application", "vnd.ezpix-package", Compressible, NotBinary, List("ez3"))
      final lazy val `vnd.f-secure.mobile`: MediaType =
        new MediaType("application", "vnd.f-secure.mobile", Compressible, NotBinary)
      final lazy val `vnd.fastcopy-disk-image`: MediaType =
        new MediaType("application", "vnd.fastcopy-disk-image", Compressible, NotBinary)
      final lazy val `vnd.fdf`: MediaType =
        new MediaType("application", "vnd.fdf", Compressible, NotBinary, List("fdf"))
      final lazy val `vnd.fdsn.mseed`: MediaType =
        new MediaType("application", "vnd.fdsn.mseed", Compressible, NotBinary, List("mseed"))
      final lazy val `vnd.fdsn.seed`: MediaType = new MediaType(
        "application",
        "vnd.fdsn.seed",
        Compressible,
        NotBinary,
        List("seed", "dataless"))
      final lazy val `vnd.ffsns`: MediaType =
        new MediaType("application", "vnd.ffsns", Compressible, NotBinary)
      final lazy val `vnd.filmit.zfc`: MediaType =
        new MediaType("application", "vnd.filmit.zfc", Compressible, NotBinary)
      final lazy val `vnd.fints`: MediaType =
        new MediaType("application", "vnd.fints", Compressible, NotBinary)
      final lazy val `vnd.firemonkeys.cloudcell`: MediaType =
        new MediaType("application", "vnd.firemonkeys.cloudcell", Compressible, NotBinary)
      final lazy val `vnd.flographit`: MediaType =
        new MediaType("application", "vnd.flographit", Compressible, NotBinary, List("gph"))
      final lazy val `vnd.fluxtime.clip`: MediaType =
        new MediaType("application", "vnd.fluxtime.clip", Compressible, NotBinary, List("ftc"))
      final lazy val `vnd.font-fontforge-sfd`: MediaType =
        new MediaType("application", "vnd.font-fontforge-sfd", Compressible, NotBinary)
      final lazy val `vnd.framemaker`: MediaType = new MediaType(
        "application",
        "vnd.framemaker",
        Compressible,
        NotBinary,
        List("fm", "frame", "maker", "book"))
      final lazy val `vnd.frogans.fnc`: MediaType =
        new MediaType("application", "vnd.frogans.fnc", Compressible, NotBinary, List("fnc"))
      final lazy val `vnd.frogans.ltf`: MediaType =
        new MediaType("application", "vnd.frogans.ltf", Compressible, NotBinary, List("ltf"))
      final lazy val `vnd.fsc.weblaunch`: MediaType =
        new MediaType("application", "vnd.fsc.weblaunch", Compressible, NotBinary, List("fsc"))
      final lazy val `vnd.fujitsu.oasys`: MediaType =
        new MediaType("application", "vnd.fujitsu.oasys", Compressible, NotBinary, List("oas"))
      final lazy val `vnd.fujitsu.oasys2`: MediaType =
        new MediaType("application", "vnd.fujitsu.oasys2", Compressible, NotBinary, List("oa2"))
      final lazy val `vnd.fujitsu.oasys3`: MediaType =
        new MediaType("application", "vnd.fujitsu.oasys3", Compressible, NotBinary, List("oa3"))
      final lazy val `vnd.fujitsu.oasysgp`: MediaType =
        new MediaType("application", "vnd.fujitsu.oasysgp", Compressible, NotBinary, List("fg5"))
      final lazy val `vnd.fujitsu.oasysprs`: MediaType =
        new MediaType("application", "vnd.fujitsu.oasysprs", Compressible, NotBinary, List("bh2"))
      final lazy val `vnd.fujixerox.art-ex`: MediaType =
        new MediaType("application", "vnd.fujixerox.art-ex", Compressible, NotBinary)
      final lazy val `vnd.fujixerox.art4`: MediaType =
        new MediaType("application", "vnd.fujixerox.art4", Compressible, NotBinary)
      final lazy val `vnd.fujixerox.ddd`: MediaType =
        new MediaType("application", "vnd.fujixerox.ddd", Compressible, NotBinary, List("ddd"))
      final lazy val `vnd.fujixerox.docuworks`: MediaType = new MediaType(
        "application",
        "vnd.fujixerox.docuworks",
        Compressible,
        NotBinary,
        List("xdw"))
      final lazy val `vnd.fujixerox.docuworks.binder`: MediaType = new MediaType(
        "application",
        "vnd.fujixerox.docuworks.binder",
        Compressible,
        NotBinary,
        List("xbd"))
      final lazy val `vnd.fujixerox.docuworks.container`: MediaType =
        new MediaType("application", "vnd.fujixerox.docuworks.container", Compressible, NotBinary)
      final lazy val `vnd.fujixerox.hbpl`: MediaType =
        new MediaType("application", "vnd.fujixerox.hbpl", Compressible, NotBinary)
      final lazy val `vnd.fut-misnet`: MediaType =
        new MediaType("application", "vnd.fut-misnet", Compressible, NotBinary)
      final lazy val `vnd.futoin+cbor`: MediaType =
        new MediaType("application", "vnd.futoin+cbor", Compressible, NotBinary)
      final lazy val `vnd.futoin+json`: MediaType =
        new MediaType("application", "vnd.futoin+json", Compressible, NotBinary)
      final lazy val `vnd.fuzzysheet`: MediaType =
        new MediaType("application", "vnd.fuzzysheet", Compressible, NotBinary, List("fzs"))
      final lazy val `vnd.genomatix.tuxedo`: MediaType =
        new MediaType("application", "vnd.genomatix.tuxedo", Compressible, NotBinary, List("txd"))
      final lazy val `vnd.geo+json`: MediaType =
        new MediaType("application", "vnd.geo+json", Compressible, NotBinary)
      final lazy val `vnd.geocube+xml`: MediaType =
        new MediaType("application", "vnd.geocube+xml", Compressible, NotBinary)
      final lazy val `vnd.geogebra.file`: MediaType =
        new MediaType("application", "vnd.geogebra.file", Compressible, NotBinary, List("ggb"))
      final lazy val `vnd.geogebra.tool`: MediaType =
        new MediaType("application", "vnd.geogebra.tool", Compressible, NotBinary, List("ggt"))
      final lazy val `vnd.geometry-explorer`: MediaType = new MediaType(
        "application",
        "vnd.geometry-explorer",
        Compressible,
        NotBinary,
        List("gex", "gre"))
      final lazy val `vnd.geonext`: MediaType =
        new MediaType("application", "vnd.geonext", Compressible, NotBinary, List("gxt"))
      final lazy val `vnd.geoplan`: MediaType =
        new MediaType("application", "vnd.geoplan", Compressible, NotBinary, List("g2w"))
      final lazy val `vnd.geospace`: MediaType =
        new MediaType("application", "vnd.geospace", Compressible, NotBinary, List("g3w"))
      final lazy val `vnd.gerber`: MediaType =
        new MediaType("application", "vnd.gerber", Compressible, NotBinary)
      final lazy val `vnd.globalplatform.card-content-mgt`: MediaType =
        new MediaType("application", "vnd.globalplatform.card-content-mgt", Compressible, NotBinary)
      final lazy val `vnd.globalplatform.card-content-mgt-response`: MediaType = new MediaType(
        "application",
        "vnd.globalplatform.card-content-mgt-response",
        Compressible,
        NotBinary)
      final lazy val `vnd.gmx`: MediaType =
        new MediaType("application", "vnd.gmx", Compressible, NotBinary, List("gmx"))
      final lazy val `vnd.google-apps.document`: MediaType = new MediaType(
        "application",
        "vnd.google-apps.document",
        Uncompressible,
        NotBinary,
        List("gdoc"))
      final lazy val `vnd.google-apps.presentation`: MediaType = new MediaType(
        "application",
        "vnd.google-apps.presentation",
        Uncompressible,
        NotBinary,
        List("gslides"))
      final lazy val `vnd.google-apps.spreadsheet`: MediaType = new MediaType(
        "application",
        "vnd.google-apps.spreadsheet",
        Uncompressible,
        NotBinary,
        List("gsheet"))
      final lazy val `vnd.google-earth.kml+xml`: MediaType = new MediaType(
        "application",
        "vnd.google-earth.kml+xml",
        Compressible,
        NotBinary,
        List("kml"))
      final lazy val `vnd.google-earth.kmz`: MediaType =
        new MediaType("application", "vnd.google-earth.kmz", Uncompressible, Binary, List("kmz"))
      final lazy val `vnd.gov.sk.e-form+xml`: MediaType =
        new MediaType("application", "vnd.gov.sk.e-form+xml", Compressible, NotBinary)
      final lazy val `vnd.gov.sk.e-form+zip`: MediaType =
        new MediaType("application", "vnd.gov.sk.e-form+zip", Uncompressible, NotBinary)
      final lazy val `vnd.gov.sk.xmldatacontainer+xml`: MediaType =
        new MediaType("application", "vnd.gov.sk.xmldatacontainer+xml", Compressible, NotBinary)
      final lazy val `vnd.grafeq`: MediaType =
        new MediaType("application", "vnd.grafeq", Compressible, NotBinary, List("gqf", "gqs"))
      final lazy val `vnd.gridmp`: MediaType =
        new MediaType("application", "vnd.gridmp", Compressible, NotBinary)
      final lazy val `vnd.groove-account`: MediaType =
        new MediaType("application", "vnd.groove-account", Compressible, NotBinary, List("gac"))
      final lazy val `vnd.groove-help`: MediaType =
        new MediaType("application", "vnd.groove-help", Compressible, NotBinary, List("ghf"))
      final lazy val `vnd.groove-identity-message`: MediaType = new MediaType(
        "application",
        "vnd.groove-identity-message",
        Compressible,
        NotBinary,
        List("gim"))
      final lazy val `vnd.groove-injector`: MediaType =
        new MediaType("application", "vnd.groove-injector", Compressible, NotBinary, List("grv"))
      final lazy val `vnd.groove-tool-message`: MediaType = new MediaType(
        "application",
        "vnd.groove-tool-message",
        Compressible,
        NotBinary,
        List("gtm"))
      final lazy val `vnd.groove-tool-template`: MediaType = new MediaType(
        "application",
        "vnd.groove-tool-template",
        Compressible,
        NotBinary,
        List("tpl"))
      final lazy val `vnd.groove-vcard`: MediaType =
        new MediaType("application", "vnd.groove-vcard", Compressible, NotBinary, List("vcg"))
      final lazy val `vnd.hal+json`: MediaType =
        new MediaType("application", "vnd.hal+json", Compressible, NotBinary)
      final lazy val `vnd.hal+xml`: MediaType =
        new MediaType("application", "vnd.hal+xml", Compressible, NotBinary, List("hal"))
      final lazy val `vnd.handheld-entertainment+xml`: MediaType = new MediaType(
        "application",
        "vnd.handheld-entertainment+xml",
        Compressible,
        NotBinary,
        List("zmm"))
      final lazy val `vnd.hbci`: MediaType =
        new MediaType("application", "vnd.hbci", Compressible, NotBinary, List("hbci"))
      final lazy val `vnd.hc+json`: MediaType =
        new MediaType("application", "vnd.hc+json", Compressible, NotBinary)
      final lazy val `vnd.hcl-bireports`: MediaType =
        new MediaType("application", "vnd.hcl-bireports", Compressible, NotBinary)
      final lazy val `vnd.hdt`: MediaType =
        new MediaType("application", "vnd.hdt", Compressible, NotBinary)
      final lazy val `vnd.heroku+json`: MediaType =
        new MediaType("application", "vnd.heroku+json", Compressible, NotBinary)
      final lazy val `vnd.hhe.lesson-player`: MediaType =
        new MediaType("application", "vnd.hhe.lesson-player", Compressible, NotBinary, List("les"))
      final lazy val `vnd.hp-hpgl`: MediaType =
        new MediaType("application", "vnd.hp-hpgl", Compressible, NotBinary, List("hpgl"))
      final lazy val `vnd.hp-hpid`: MediaType =
        new MediaType("application", "vnd.hp-hpid", Compressible, NotBinary, List("hpid"))
      final lazy val `vnd.hp-hps`: MediaType =
        new MediaType("application", "vnd.hp-hps", Compressible, NotBinary, List("hps"))
      final lazy val `vnd.hp-jlyt`: MediaType =
        new MediaType("application", "vnd.hp-jlyt", Compressible, NotBinary, List("jlt"))
      final lazy val `vnd.hp-pcl`: MediaType =
        new MediaType("application", "vnd.hp-pcl", Compressible, NotBinary, List("pcl"))
      final lazy val `vnd.hp-pclxl`: MediaType =
        new MediaType("application", "vnd.hp-pclxl", Compressible, NotBinary, List("pclxl"))
      final lazy val `vnd.httphone`: MediaType =
        new MediaType("application", "vnd.httphone", Compressible, NotBinary)
      final lazy val `vnd.hydrostatix.sof-data`: MediaType = new MediaType(
        "application",
        "vnd.hydrostatix.sof-data",
        Compressible,
        NotBinary,
        List("sfd-hdstx"))
      final lazy val `vnd.hyper+json`: MediaType =
        new MediaType("application", "vnd.hyper+json", Compressible, NotBinary)
      final lazy val `vnd.hyper-item+json`: MediaType =
        new MediaType("application", "vnd.hyper-item+json", Compressible, NotBinary)
      final lazy val `vnd.hyperdrive+json`: MediaType =
        new MediaType("application", "vnd.hyperdrive+json", Compressible, NotBinary)
      final lazy val `vnd.hzn-3d-crossword`: MediaType =
        new MediaType("application", "vnd.hzn-3d-crossword", Compressible, NotBinary)
      final lazy val `vnd.ibm.afplinedata`: MediaType =
        new MediaType("application", "vnd.ibm.afplinedata", Compressible, NotBinary)
      final lazy val `vnd.ibm.electronic-media`: MediaType =
        new MediaType("application", "vnd.ibm.electronic-media", Compressible, NotBinary)
      final lazy val `vnd.ibm.minipay`: MediaType =
        new MediaType("application", "vnd.ibm.minipay", Compressible, NotBinary, List("mpy"))
      final lazy val `vnd.ibm.modcap`: MediaType = new MediaType(
        "application",
        "vnd.ibm.modcap",
        Compressible,
        NotBinary,
        List("afp", "listafp", "list3820"))
      final lazy val `vnd.ibm.rights-management`: MediaType = new MediaType(
        "application",
        "vnd.ibm.rights-management",
        Compressible,
        NotBinary,
        List("irm"))
      final lazy val `vnd.ibm.secure-container`: MediaType = new MediaType(
        "application",
        "vnd.ibm.secure-container",
        Compressible,
        NotBinary,
        List("sc"))
      final lazy val `vnd.iccprofile`: MediaType =
        new MediaType("application", "vnd.iccprofile", Compressible, NotBinary, List("icc", "icm"))
      final lazy val `vnd.ieee.1905`: MediaType =
        new MediaType("application", "vnd.ieee.1905", Compressible, NotBinary)
      final lazy val `vnd.igloader`: MediaType =
        new MediaType("application", "vnd.igloader", Compressible, NotBinary, List("igl"))
      final lazy val `vnd.imagemeter.folder+zip`: MediaType =
        new MediaType("application", "vnd.imagemeter.folder+zip", Uncompressible, NotBinary)
      final lazy val `vnd.imagemeter.image+zip`: MediaType =
        new MediaType("application", "vnd.imagemeter.image+zip", Uncompressible, NotBinary)
      final lazy val `vnd.immervision-ivp`: MediaType =
        new MediaType("application", "vnd.immervision-ivp", Compressible, NotBinary, List("ivp"))
      final lazy val `vnd.immervision-ivu`: MediaType =
        new MediaType("application", "vnd.immervision-ivu", Compressible, NotBinary, List("ivu"))
      final lazy val `vnd.ims.imsccv1p1`: MediaType =
        new MediaType("application", "vnd.ims.imsccv1p1", Compressible, NotBinary)
      final lazy val `vnd.ims.imsccv1p2`: MediaType =
        new MediaType("application", "vnd.ims.imsccv1p2", Compressible, NotBinary)
      final lazy val `vnd.ims.imsccv1p3`: MediaType =
        new MediaType("application", "vnd.ims.imsccv1p3", Compressible, NotBinary)
      final lazy val `vnd.ims.lis.v2.result+json`: MediaType =
        new MediaType("application", "vnd.ims.lis.v2.result+json", Compressible, NotBinary)
      final lazy val `vnd.ims.lti.v2.toolconsumerprofile+json`: MediaType = new MediaType(
        "application",
        "vnd.ims.lti.v2.toolconsumerprofile+json",
        Compressible,
        NotBinary)
      final lazy val `vnd.ims.lti.v2.toolproxy+json`: MediaType =
        new MediaType("application", "vnd.ims.lti.v2.toolproxy+json", Compressible, NotBinary)
      final lazy val `vnd.ims.lti.v2.toolproxy.id+json`: MediaType =
        new MediaType("application", "vnd.ims.lti.v2.toolproxy.id+json", Compressible, NotBinary)
      final lazy val `vnd.ims.lti.v2.toolsettings+json`: MediaType =
        new MediaType("application", "vnd.ims.lti.v2.toolsettings+json", Compressible, NotBinary)
      final lazy val `vnd.ims.lti.v2.toolsettings.simple+json`: MediaType = new MediaType(
        "application",
        "vnd.ims.lti.v2.toolsettings.simple+json",
        Compressible,
        NotBinary)
      final lazy val `vnd.informedcontrol.rms+xml`: MediaType =
        new MediaType("application", "vnd.informedcontrol.rms+xml", Compressible, NotBinary)
      final lazy val `vnd.informix-visionary`: MediaType =
        new MediaType("application", "vnd.informix-visionary", Compressible, NotBinary)
      final lazy val `vnd.infotech.project`: MediaType =
        new MediaType("application", "vnd.infotech.project", Compressible, NotBinary)
      final lazy val `vnd.infotech.project+xml`: MediaType =
        new MediaType("application", "vnd.infotech.project+xml", Compressible, NotBinary)
      final lazy val `vnd.innopath.wamp.notification`: MediaType =
        new MediaType("application", "vnd.innopath.wamp.notification", Compressible, NotBinary)
      final lazy val `vnd.insors.igm`: MediaType =
        new MediaType("application", "vnd.insors.igm", Compressible, NotBinary, List("igm"))
      final lazy val `vnd.intercon.formnet`: MediaType = new MediaType(
        "application",
        "vnd.intercon.formnet",
        Compressible,
        NotBinary,
        List("xpw", "xpx"))
      final lazy val `vnd.intergeo`: MediaType =
        new MediaType("application", "vnd.intergeo", Compressible, NotBinary, List("i2g"))
      final lazy val `vnd.intertrust.digibox`: MediaType =
        new MediaType("application", "vnd.intertrust.digibox", Compressible, NotBinary)
      final lazy val `vnd.intertrust.nncp`: MediaType =
        new MediaType("application", "vnd.intertrust.nncp", Compressible, NotBinary)
      final lazy val `vnd.intu.qbo`: MediaType =
        new MediaType("application", "vnd.intu.qbo", Compressible, NotBinary, List("qbo"))
      final lazy val `vnd.intu.qfx`: MediaType =
        new MediaType("application", "vnd.intu.qfx", Compressible, NotBinary, List("qfx"))
      final lazy val `vnd.iptc.g2.catalogitem+xml`: MediaType =
        new MediaType("application", "vnd.iptc.g2.catalogitem+xml", Compressible, NotBinary)
      final lazy val `vnd.iptc.g2.conceptitem+xml`: MediaType =
        new MediaType("application", "vnd.iptc.g2.conceptitem+xml", Compressible, NotBinary)
      final lazy val `vnd.iptc.g2.knowledgeitem+xml`: MediaType =
        new MediaType("application", "vnd.iptc.g2.knowledgeitem+xml", Compressible, NotBinary)
      final lazy val `vnd.iptc.g2.newsitem+xml`: MediaType =
        new MediaType("application", "vnd.iptc.g2.newsitem+xml", Compressible, NotBinary)
      final lazy val `vnd.iptc.g2.newsmessage+xml`: MediaType =
        new MediaType("application", "vnd.iptc.g2.newsmessage+xml", Compressible, NotBinary)
      final lazy val `vnd.iptc.g2.packageitem+xml`: MediaType =
        new MediaType("application", "vnd.iptc.g2.packageitem+xml", Compressible, NotBinary)
      final lazy val `vnd.iptc.g2.planningitem+xml`: MediaType =
        new MediaType("application", "vnd.iptc.g2.planningitem+xml", Compressible, NotBinary)
      final lazy val `vnd.ipunplugged.rcprofile`: MediaType = new MediaType(
        "application",
        "vnd.ipunplugged.rcprofile",
        Compressible,
        NotBinary,
        List("rcprofile"))
      final lazy val `vnd.irepository.package+xml`: MediaType = new MediaType(
        "application",
        "vnd.irepository.package+xml",
        Compressible,
        NotBinary,
        List("irp"))
      final lazy val `vnd.is-xpr`: MediaType =
        new MediaType("application", "vnd.is-xpr", Compressible, NotBinary, List("xpr"))
      final lazy val `vnd.isac.fcs`: MediaType =
        new MediaType("application", "vnd.isac.fcs", Compressible, NotBinary, List("fcs"))
      final lazy val `vnd.jam`: MediaType =
        new MediaType("application", "vnd.jam", Compressible, NotBinary, List("jam"))
      final lazy val `vnd.japannet-directory-service`: MediaType =
        new MediaType("application", "vnd.japannet-directory-service", Compressible, NotBinary)
      final lazy val `vnd.japannet-jpnstore-wakeup`: MediaType =
        new MediaType("application", "vnd.japannet-jpnstore-wakeup", Compressible, NotBinary)
      final lazy val `vnd.japannet-payment-wakeup`: MediaType =
        new MediaType("application", "vnd.japannet-payment-wakeup", Compressible, NotBinary)
      final lazy val `vnd.japannet-registration`: MediaType =
        new MediaType("application", "vnd.japannet-registration", Compressible, NotBinary)
      final lazy val `vnd.japannet-registration-wakeup`: MediaType =
        new MediaType("application", "vnd.japannet-registration-wakeup", Compressible, NotBinary)
      final lazy val `vnd.japannet-setstore-wakeup`: MediaType =
        new MediaType("application", "vnd.japannet-setstore-wakeup", Compressible, NotBinary)
      final lazy val `vnd.japannet-verification`: MediaType =
        new MediaType("application", "vnd.japannet-verification", Compressible, NotBinary)
      final lazy val `vnd.japannet-verification-wakeup`: MediaType =
        new MediaType("application", "vnd.japannet-verification-wakeup", Compressible, NotBinary)
      final lazy val `vnd.jcp.javame.midlet-rms`: MediaType = new MediaType(
        "application",
        "vnd.jcp.javame.midlet-rms",
        Compressible,
        NotBinary,
        List("rms"))
      final lazy val `vnd.jisp`: MediaType =
        new MediaType("application", "vnd.jisp", Compressible, NotBinary, List("jisp"))
      final lazy val `vnd.joost.joda-archive`: MediaType = new MediaType(
        "application",
        "vnd.joost.joda-archive",
        Compressible,
        NotBinary,
        List("joda"))
      final lazy val `vnd.jsk.isdn-ngn`: MediaType =
        new MediaType("application", "vnd.jsk.isdn-ngn", Compressible, NotBinary)
      final lazy val `vnd.kahootz`: MediaType =
        new MediaType("application", "vnd.kahootz", Compressible, NotBinary, List("ktz", "ktr"))
      final lazy val `vnd.kde.karbon`: MediaType =
        new MediaType("application", "vnd.kde.karbon", Compressible, NotBinary, List("karbon"))
      final lazy val `vnd.kde.kchart`: MediaType =
        new MediaType("application", "vnd.kde.kchart", Compressible, NotBinary, List("chrt"))
      final lazy val `vnd.kde.kformula`: MediaType =
        new MediaType("application", "vnd.kde.kformula", Compressible, NotBinary, List("kfo"))
      final lazy val `vnd.kde.kivio`: MediaType =
        new MediaType("application", "vnd.kde.kivio", Compressible, NotBinary, List("flw"))
      final lazy val `vnd.kde.kontour`: MediaType =
        new MediaType("application", "vnd.kde.kontour", Compressible, NotBinary, List("kon"))
      final lazy val `vnd.kde.kpresenter`: MediaType = new MediaType(
        "application",
        "vnd.kde.kpresenter",
        Compressible,
        NotBinary,
        List("kpr", "kpt"))
      final lazy val `vnd.kde.kspread`: MediaType =
        new MediaType("application", "vnd.kde.kspread", Compressible, NotBinary, List("ksp"))
      final lazy val `vnd.kde.kword`: MediaType =
        new MediaType("application", "vnd.kde.kword", Compressible, NotBinary, List("kwd", "kwt"))
      final lazy val `vnd.kenameaapp`: MediaType =
        new MediaType("application", "vnd.kenameaapp", Compressible, NotBinary, List("htke"))
      final lazy val `vnd.kidspiration`: MediaType =
        new MediaType("application", "vnd.kidspiration", Compressible, NotBinary, List("kia"))
      final lazy val `vnd.kinar`: MediaType =
        new MediaType("application", "vnd.kinar", Compressible, NotBinary, List("kne", "knp"))
      final lazy val `vnd.koan`: MediaType = new MediaType(
        "application",
        "vnd.koan",
        Compressible,
        NotBinary,
        List("skp", "skd", "skt", "skm"))
      final lazy val `vnd.kodak-descriptor`: MediaType =
        new MediaType("application", "vnd.kodak-descriptor", Compressible, NotBinary, List("sse"))
      final lazy val `vnd.las.las+json`: MediaType =
        new MediaType("application", "vnd.las.las+json", Compressible, NotBinary)
      final lazy val `vnd.las.las+xml`: MediaType =
        new MediaType("application", "vnd.las.las+xml", Compressible, NotBinary, List("lasxml"))
      final lazy val `vnd.leap+json`: MediaType =
        new MediaType("application", "vnd.leap+json", Compressible, NotBinary)
      final lazy val `vnd.liberty-request+xml`: MediaType =
        new MediaType("application", "vnd.liberty-request+xml", Compressible, NotBinary)
      final lazy val `vnd.llamagraphics.life-balance.desktop`: MediaType = new MediaType(
        "application",
        "vnd.llamagraphics.life-balance.desktop",
        Compressible,
        NotBinary,
        List("lbd"))
      final lazy val `vnd.llamagraphics.life-balance.exchange+xml`: MediaType = new MediaType(
        "application",
        "vnd.llamagraphics.life-balance.exchange+xml",
        Compressible,
        NotBinary,
        List("lbe"))
      final lazy val `vnd.lotus-1-2-3`: MediaType =
        new MediaType("application", "vnd.lotus-1-2-3", Compressible, NotBinary, List("123"))
      final lazy val `vnd.lotus-approach`: MediaType =
        new MediaType("application", "vnd.lotus-approach", Compressible, NotBinary, List("apr"))
      final lazy val `vnd.lotus-freelance`: MediaType =
        new MediaType("application", "vnd.lotus-freelance", Compressible, NotBinary, List("pre"))
      final lazy val `vnd.lotus-notes`: MediaType =
        new MediaType("application", "vnd.lotus-notes", Compressible, NotBinary, List("nsf"))
      final lazy val `vnd.lotus-organizer`: MediaType =
        new MediaType("application", "vnd.lotus-organizer", Compressible, NotBinary, List("org"))
      final lazy val `vnd.lotus-screencam`: MediaType =
        new MediaType("application", "vnd.lotus-screencam", Compressible, NotBinary, List("scm"))
      final lazy val `vnd.lotus-wordpro`: MediaType =
        new MediaType("application", "vnd.lotus-wordpro", Compressible, NotBinary, List("lwp"))
      final lazy val `vnd.macports.portpkg`: MediaType = new MediaType(
        "application",
        "vnd.macports.portpkg",
        Compressible,
        NotBinary,
        List("portpkg"))
      final lazy val `vnd.mapbox-vector-tile`: MediaType =
        new MediaType("application", "vnd.mapbox-vector-tile", Compressible, NotBinary)
      final lazy val `vnd.marlin.drm.actiontoken+xml`: MediaType =
        new MediaType("application", "vnd.marlin.drm.actiontoken+xml", Compressible, NotBinary)
      final lazy val `vnd.marlin.drm.conftoken+xml`: MediaType =
        new MediaType("application", "vnd.marlin.drm.conftoken+xml", Compressible, NotBinary)
      final lazy val `vnd.marlin.drm.license+xml`: MediaType =
        new MediaType("application", "vnd.marlin.drm.license+xml", Compressible, NotBinary)
      final lazy val `vnd.marlin.drm.mdcf`: MediaType =
        new MediaType("application", "vnd.marlin.drm.mdcf", Compressible, NotBinary)
      final lazy val `vnd.mason+json`: MediaType =
        new MediaType("application", "vnd.mason+json", Compressible, NotBinary)
      final lazy val `vnd.maxmind.maxmind-db`: MediaType =
        new MediaType("application", "vnd.maxmind.maxmind-db", Compressible, NotBinary)
      final lazy val `vnd.mcd`: MediaType =
        new MediaType("application", "vnd.mcd", Compressible, NotBinary, List("mcd"))
      final lazy val `vnd.medcalcdata`: MediaType =
        new MediaType("application", "vnd.medcalcdata", Compressible, NotBinary, List("mc1"))
      final lazy val `vnd.mediastation.cdkey`: MediaType = new MediaType(
        "application",
        "vnd.mediastation.cdkey",
        Compressible,
        NotBinary,
        List("cdkey"))
      final lazy val `vnd.meridian-slingshot`: MediaType =
        new MediaType("application", "vnd.meridian-slingshot", Compressible, NotBinary)
      final lazy val `vnd.mfer`: MediaType =
        new MediaType("application", "vnd.mfer", Compressible, NotBinary, List("mwf"))
      final lazy val `vnd.mfmp`: MediaType =
        new MediaType("application", "vnd.mfmp", Compressible, NotBinary, List("mfm"))
      final lazy val `vnd.micro+json`: MediaType =
        new MediaType("application", "vnd.micro+json", Compressible, NotBinary)
      final lazy val `vnd.micrografx.flo`: MediaType =
        new MediaType("application", "vnd.micrografx.flo", Compressible, NotBinary, List("flo"))
      final lazy val `vnd.micrografx.igx`: MediaType =
        new MediaType("application", "vnd.micrografx.igx", Compressible, NotBinary, List("igx"))
      final lazy val `vnd.microsoft.portable-executable`: MediaType =
        new MediaType("application", "vnd.microsoft.portable-executable", Compressible, NotBinary)
      final lazy val `vnd.microsoft.windows.thumbnail-cache`: MediaType = new MediaType(
        "application",
        "vnd.microsoft.windows.thumbnail-cache",
        Compressible,
        NotBinary)
      final lazy val `vnd.miele+json`: MediaType =
        new MediaType("application", "vnd.miele+json", Compressible, NotBinary)
      final lazy val `vnd.mif`: MediaType =
        new MediaType("application", "vnd.mif", Compressible, NotBinary, List("mif"))
      final lazy val `vnd.minisoft-hp3000-save`: MediaType =
        new MediaType("application", "vnd.minisoft-hp3000-save", Compressible, NotBinary)
      final lazy val `vnd.mitsubishi.misty-guard.trustweb`: MediaType =
        new MediaType("application", "vnd.mitsubishi.misty-guard.trustweb", Compressible, NotBinary)
      final lazy val `vnd.mobius.daf`: MediaType =
        new MediaType("application", "vnd.mobius.daf", Compressible, NotBinary, List("daf"))
      final lazy val `vnd.mobius.dis`: MediaType =
        new MediaType("application", "vnd.mobius.dis", Compressible, NotBinary, List("dis"))
      final lazy val `vnd.mobius.mbk`: MediaType =
        new MediaType("application", "vnd.mobius.mbk", Compressible, NotBinary, List("mbk"))
      final lazy val `vnd.mobius.mqy`: MediaType =
        new MediaType("application", "vnd.mobius.mqy", Compressible, NotBinary, List("mqy"))
      final lazy val `vnd.mobius.msl`: MediaType =
        new MediaType("application", "vnd.mobius.msl", Compressible, NotBinary, List("msl"))
      final lazy val `vnd.mobius.plc`: MediaType =
        new MediaType("application", "vnd.mobius.plc", Compressible, NotBinary, List("plc"))
      final lazy val `vnd.mobius.txf`: MediaType =
        new MediaType("application", "vnd.mobius.txf", Compressible, NotBinary, List("txf"))
      final lazy val `vnd.mophun.application`: MediaType =
        new MediaType("application", "vnd.mophun.application", Compressible, NotBinary, List("mpn"))
      final lazy val `vnd.mophun.certificate`: MediaType =
        new MediaType("application", "vnd.mophun.certificate", Compressible, NotBinary, List("mpc"))
      final lazy val `vnd.motorola.flexsuite`: MediaType =
        new MediaType("application", "vnd.motorola.flexsuite", Compressible, NotBinary)
      final lazy val `vnd.motorola.flexsuite.adsi`: MediaType =
        new MediaType("application", "vnd.motorola.flexsuite.adsi", Compressible, NotBinary)
      final lazy val `vnd.motorola.flexsuite.fis`: MediaType =
        new MediaType("application", "vnd.motorola.flexsuite.fis", Compressible, NotBinary)
      final lazy val `vnd.motorola.flexsuite.gotap`: MediaType =
        new MediaType("application", "vnd.motorola.flexsuite.gotap", Compressible, NotBinary)
      final lazy val `vnd.motorola.flexsuite.kmr`: MediaType =
        new MediaType("application", "vnd.motorola.flexsuite.kmr", Compressible, NotBinary)
      final lazy val `vnd.motorola.flexsuite.ttc`: MediaType =
        new MediaType("application", "vnd.motorola.flexsuite.ttc", Compressible, NotBinary)
      final lazy val `vnd.motorola.flexsuite.wem`: MediaType =
        new MediaType("application", "vnd.motorola.flexsuite.wem", Compressible, NotBinary)
      final lazy val `vnd.motorola.iprm`: MediaType =
        new MediaType("application", "vnd.motorola.iprm", Compressible, NotBinary)
      final lazy val `vnd.mozilla.xul+xml`: MediaType =
        new MediaType("application", "vnd.mozilla.xul+xml", Compressible, NotBinary, List("xul"))
      final lazy val `vnd.ms-3mfdocument`: MediaType =
        new MediaType("application", "vnd.ms-3mfdocument", Compressible, NotBinary)
      final lazy val `vnd.ms-artgalry`: MediaType =
        new MediaType("application", "vnd.ms-artgalry", Compressible, NotBinary, List("cil"))
      final lazy val `vnd.ms-asf`: MediaType =
        new MediaType("application", "vnd.ms-asf", Compressible, NotBinary)
      final lazy val `vnd.ms-cab-compressed`: MediaType =
        new MediaType("application", "vnd.ms-cab-compressed", Compressible, NotBinary, List("cab"))
      final lazy val `vnd.ms-color.iccprofile`: MediaType =
        new MediaType("application", "vnd.ms-color.iccprofile", Compressible, NotBinary)
      final lazy val `vnd.ms-excel`: MediaType = new MediaType(
        "application",
        "vnd.ms-excel",
        Uncompressible,
        NotBinary,
        List("xls", "xlm", "xla", "xlc", "xlt", "xlw"))
      final lazy val `vnd.ms-excel.addin.macroenabled.12`: MediaType = new MediaType(
        "application",
        "vnd.ms-excel.addin.macroenabled.12",
        Compressible,
        NotBinary,
        List("xlam"))
      final lazy val `vnd.ms-excel.sheet.binary.macroenabled.12`: MediaType = new MediaType(
        "application",
        "vnd.ms-excel.sheet.binary.macroenabled.12",
        Compressible,
        NotBinary,
        List("xlsb"))
      final lazy val `vnd.ms-excel.sheet.macroenabled.12`: MediaType = new MediaType(
        "application",
        "vnd.ms-excel.sheet.macroenabled.12",
        Compressible,
        NotBinary,
        List("xlsm"))
      final lazy val `vnd.ms-excel.template.macroenabled.12`: MediaType = new MediaType(
        "application",
        "vnd.ms-excel.template.macroenabled.12",
        Compressible,
        NotBinary,
        List("xltm"))
      final lazy val `vnd.ms-fontobject`: MediaType =
        new MediaType("application", "vnd.ms-fontobject", Compressible, Binary, List("eot"))
      final lazy val `vnd.ms-htmlhelp`: MediaType =
        new MediaType("application", "vnd.ms-htmlhelp", Compressible, NotBinary, List("chm"))
      final lazy val `vnd.ms-ims`: MediaType =
        new MediaType("application", "vnd.ms-ims", Compressible, NotBinary, List("ims"))
      final lazy val `vnd.ms-lrm`: MediaType =
        new MediaType("application", "vnd.ms-lrm", Compressible, NotBinary, List("lrm"))
      final lazy val `vnd.ms-office.activex+xml`: MediaType =
        new MediaType("application", "vnd.ms-office.activex+xml", Compressible, NotBinary)
      final lazy val `vnd.ms-officetheme`: MediaType =
        new MediaType("application", "vnd.ms-officetheme", Compressible, NotBinary, List("thmx"))
      final lazy val `vnd.ms-opentype`: MediaType =
        new MediaType("application", "vnd.ms-opentype", Compressible, NotBinary)
      final lazy val `vnd.ms-outlook`: MediaType =
        new MediaType("application", "vnd.ms-outlook", Uncompressible, NotBinary, List("msg"))
      final lazy val `vnd.ms-package.obfuscated-opentype`: MediaType =
        new MediaType("application", "vnd.ms-package.obfuscated-opentype", Compressible, NotBinary)
      final lazy val `vnd.ms-pki.seccat`: MediaType =
        new MediaType("application", "vnd.ms-pki.seccat", Compressible, NotBinary, List("cat"))
      final lazy val `vnd.ms-pki.stl`: MediaType =
        new MediaType("application", "vnd.ms-pki.stl", Compressible, NotBinary, List("stl"))
      final lazy val `vnd.ms-playready.initiator+xml`: MediaType =
        new MediaType("application", "vnd.ms-playready.initiator+xml", Compressible, NotBinary)
      final lazy val `vnd.ms-powerpoint`: MediaType = new MediaType(
        "application",
        "vnd.ms-powerpoint",
        Uncompressible,
        NotBinary,
        List("ppt", "pps", "pot"))
      final lazy val `vnd.ms-powerpoint.addin.macroenabled.12`: MediaType = new MediaType(
        "application",
        "vnd.ms-powerpoint.addin.macroenabled.12",
        Compressible,
        NotBinary,
        List("ppam"))
      final lazy val `vnd.ms-powerpoint.presentation.macroenabled.12`: MediaType = new MediaType(
        "application",
        "vnd.ms-powerpoint.presentation.macroenabled.12",
        Compressible,
        NotBinary,
        List("pptm"))
      final lazy val `vnd.ms-powerpoint.slide.macroenabled.12`: MediaType = new MediaType(
        "application",
        "vnd.ms-powerpoint.slide.macroenabled.12",
        Compressible,
        NotBinary,
        List("sldm"))
      final lazy val `vnd.ms-powerpoint.slideshow.macroenabled.12`: MediaType = new MediaType(
        "application",
        "vnd.ms-powerpoint.slideshow.macroenabled.12",
        Compressible,
        NotBinary,
        List("ppsm"))
      final lazy val `vnd.ms-powerpoint.template.macroenabled.12`: MediaType = new MediaType(
        "application",
        "vnd.ms-powerpoint.template.macroenabled.12",
        Compressible,
        NotBinary,
        List("potm"))
      final lazy val `vnd.ms-printdevicecapabilities+xml`: MediaType =
        new MediaType("application", "vnd.ms-printdevicecapabilities+xml", Compressible, NotBinary)
      final lazy val `vnd.ms-printing.printticket+xml`: MediaType =
        new MediaType("application", "vnd.ms-printing.printticket+xml", Compressible, NotBinary)
      final lazy val `vnd.ms-printschematicket+xml`: MediaType =
        new MediaType("application", "vnd.ms-printschematicket+xml", Compressible, NotBinary)
      final lazy val `vnd.ms-project`: MediaType =
        new MediaType("application", "vnd.ms-project", Compressible, NotBinary, List("mpp", "mpt"))
      final lazy val `vnd.ms-tnef`: MediaType =
        new MediaType("application", "vnd.ms-tnef", Compressible, NotBinary)
      final lazy val `vnd.ms-windows.devicepairing`: MediaType =
        new MediaType("application", "vnd.ms-windows.devicepairing", Compressible, NotBinary)
      final lazy val `vnd.ms-windows.nwprinting.oob`: MediaType =
        new MediaType("application", "vnd.ms-windows.nwprinting.oob", Compressible, NotBinary)
      final lazy val `vnd.ms-windows.printerpairing`: MediaType =
        new MediaType("application", "vnd.ms-windows.printerpairing", Compressible, NotBinary)
      final lazy val `vnd.ms-windows.wsd.oob`: MediaType =
        new MediaType("application", "vnd.ms-windows.wsd.oob", Compressible, NotBinary)
      final lazy val `vnd.ms-wmdrm.lic-chlg-req`: MediaType =
        new MediaType("application", "vnd.ms-wmdrm.lic-chlg-req", Compressible, NotBinary)
      final lazy val `vnd.ms-wmdrm.lic-resp`: MediaType =
        new MediaType("application", "vnd.ms-wmdrm.lic-resp", Compressible, NotBinary)
      final lazy val `vnd.ms-wmdrm.meter-chlg-req`: MediaType =
        new MediaType("application", "vnd.ms-wmdrm.meter-chlg-req", Compressible, NotBinary)
      final lazy val `vnd.ms-wmdrm.meter-resp`: MediaType =
        new MediaType("application", "vnd.ms-wmdrm.meter-resp", Compressible, NotBinary)
      final lazy val `vnd.ms-word.document.macroenabled.12`: MediaType = new MediaType(
        "application",
        "vnd.ms-word.document.macroenabled.12",
        Compressible,
        NotBinary,
        List("docm"))
      final lazy val `vnd.ms-word.template.macroenabled.12`: MediaType = new MediaType(
        "application",
        "vnd.ms-word.template.macroenabled.12",
        Compressible,
        NotBinary,
        List("dotm"))
      final lazy val `vnd.ms-works`: MediaType = new MediaType(
        "application",
        "vnd.ms-works",
        Compressible,
        NotBinary,
        List("wps", "wks", "wcm", "wdb"))
      final lazy val `vnd.ms-wpl`: MediaType =
        new MediaType("application", "vnd.ms-wpl", Compressible, NotBinary, List("wpl"))
      final lazy val `vnd.ms-xpsdocument`: MediaType =
        new MediaType("application", "vnd.ms-xpsdocument", Uncompressible, NotBinary, List("xps"))
      final lazy val `vnd.msa-disk-image`: MediaType =
        new MediaType("application", "vnd.msa-disk-image", Compressible, NotBinary)
      final lazy val `vnd.mseq`: MediaType =
        new MediaType("application", "vnd.mseq", Compressible, NotBinary, List("mseq"))
      final lazy val `vnd.msign`: MediaType =
        new MediaType("application", "vnd.msign", Compressible, NotBinary)
      final lazy val `vnd.multiad.creator`: MediaType =
        new MediaType("application", "vnd.multiad.creator", Compressible, NotBinary)
      final lazy val `vnd.multiad.creator.cif`: MediaType =
        new MediaType("application", "vnd.multiad.creator.cif", Compressible, NotBinary)
      final lazy val `vnd.music-niff`: MediaType =
        new MediaType("application", "vnd.music-niff", Compressible, NotBinary)
      final lazy val `vnd.musician`: MediaType =
        new MediaType("application", "vnd.musician", Compressible, NotBinary, List("mus"))
      final lazy val `vnd.muvee.style`: MediaType =
        new MediaType("application", "vnd.muvee.style", Compressible, NotBinary, List("msty"))
      final lazy val `vnd.mynfc`: MediaType =
        new MediaType("application", "vnd.mynfc", Compressible, NotBinary, List("taglet"))
      final lazy val `vnd.ncd.control`: MediaType =
        new MediaType("application", "vnd.ncd.control", Compressible, NotBinary)
      final lazy val `vnd.ncd.reference`: MediaType =
        new MediaType("application", "vnd.ncd.reference", Compressible, NotBinary)
      final lazy val `vnd.nearst.inv+json`: MediaType =
        new MediaType("application", "vnd.nearst.inv+json", Compressible, NotBinary)
      final lazy val `vnd.nervana`: MediaType =
        new MediaType("application", "vnd.nervana", Compressible, NotBinary)
      final lazy val `vnd.netfpx`: MediaType =
        new MediaType("application", "vnd.netfpx", Compressible, NotBinary)
      final lazy val `vnd.neurolanguage.nlu`: MediaType =
        new MediaType("application", "vnd.neurolanguage.nlu", Compressible, NotBinary, List("nlu"))
      final lazy val `vnd.nimn`: MediaType =
        new MediaType("application", "vnd.nimn", Compressible, NotBinary)
      final lazy val `vnd.nintendo.nitro.rom`: MediaType =
        new MediaType("application", "vnd.nintendo.nitro.rom", Compressible, NotBinary)
      final lazy val `vnd.nintendo.snes.rom`: MediaType =
        new MediaType("application", "vnd.nintendo.snes.rom", Compressible, NotBinary)
      final lazy val `vnd.nitf`: MediaType =
        new MediaType("application", "vnd.nitf", Compressible, NotBinary, List("ntf", "nitf"))
      final lazy val `vnd.noblenet-directory`: MediaType =
        new MediaType("application", "vnd.noblenet-directory", Compressible, NotBinary, List("nnd"))
      final lazy val `vnd.noblenet-sealer`: MediaType =
        new MediaType("application", "vnd.noblenet-sealer", Compressible, NotBinary, List("nns"))
      final lazy val `vnd.noblenet-web`: MediaType =
        new MediaType("application", "vnd.noblenet-web", Compressible, NotBinary, List("nnw"))
      final lazy val `vnd.nokia.catalogs`: MediaType =
        new MediaType("application", "vnd.nokia.catalogs", Compressible, NotBinary)
      final lazy val `vnd.nokia.conml+wbxml`: MediaType =
        new MediaType("application", "vnd.nokia.conml+wbxml", Compressible, NotBinary)
      final lazy val `vnd.nokia.conml+xml`: MediaType =
        new MediaType("application", "vnd.nokia.conml+xml", Compressible, NotBinary)
      final lazy val `vnd.nokia.iptv.config+xml`: MediaType =
        new MediaType("application", "vnd.nokia.iptv.config+xml", Compressible, NotBinary)
      final lazy val `vnd.nokia.isds-radio-presets`: MediaType =
        new MediaType("application", "vnd.nokia.isds-radio-presets", Compressible, NotBinary)
      final lazy val `vnd.nokia.landmark+wbxml`: MediaType =
        new MediaType("application", "vnd.nokia.landmark+wbxml", Compressible, NotBinary)
      final lazy val `vnd.nokia.landmark+xml`: MediaType =
        new MediaType("application", "vnd.nokia.landmark+xml", Compressible, NotBinary)
      final lazy val `vnd.nokia.landmarkcollection+xml`: MediaType =
        new MediaType("application", "vnd.nokia.landmarkcollection+xml", Compressible, NotBinary)
      final lazy val `vnd.nokia.n-gage.ac+xml`: MediaType =
        new MediaType("application", "vnd.nokia.n-gage.ac+xml", Compressible, NotBinary)
      final lazy val `vnd.nokia.n-gage.data`: MediaType = new MediaType(
        "application",
        "vnd.nokia.n-gage.data",
        Compressible,
        NotBinary,
        List("ngdat"))
      final lazy val `vnd.nokia.n-gage.symbian.install`: MediaType = new MediaType(
        "application",
        "vnd.nokia.n-gage.symbian.install",
        Compressible,
        NotBinary,
        List("n-gage"))
      final lazy val `vnd.nokia.ncd`: MediaType =
        new MediaType("application", "vnd.nokia.ncd", Compressible, NotBinary)
      final lazy val `vnd.nokia.pcd+wbxml`: MediaType =
        new MediaType("application", "vnd.nokia.pcd+wbxml", Compressible, NotBinary)
      final lazy val `vnd.nokia.pcd+xml`: MediaType =
        new MediaType("application", "vnd.nokia.pcd+xml", Compressible, NotBinary)
      final lazy val `vnd.nokia.radio-preset`: MediaType = new MediaType(
        "application",
        "vnd.nokia.radio-preset",
        Compressible,
        NotBinary,
        List("rpst"))
      final lazy val `vnd.nokia.radio-presets`: MediaType = new MediaType(
        "application",
        "vnd.nokia.radio-presets",
        Compressible,
        NotBinary,
        List("rpss"))
      final lazy val `vnd.novadigm.edm`: MediaType =
        new MediaType("application", "vnd.novadigm.edm", Compressible, NotBinary, List("edm"))
      final lazy val `vnd.novadigm.edx`: MediaType =
        new MediaType("application", "vnd.novadigm.edx", Compressible, NotBinary, List("edx"))
      final lazy val `vnd.novadigm.ext`: MediaType =
        new MediaType("application", "vnd.novadigm.ext", Compressible, NotBinary, List("ext"))
      final lazy val `vnd.ntt-local.content-share`: MediaType =
        new MediaType("application", "vnd.ntt-local.content-share", Compressible, NotBinary)
      final lazy val `vnd.ntt-local.file-transfer`: MediaType =
        new MediaType("application", "vnd.ntt-local.file-transfer", Compressible, NotBinary)
      final lazy val `vnd.ntt-local.ogw_remote-access`: MediaType =
        new MediaType("application", "vnd.ntt-local.ogw_remote-access", Compressible, NotBinary)
      final lazy val `vnd.ntt-local.sip-ta_remote`: MediaType =
        new MediaType("application", "vnd.ntt-local.sip-ta_remote", Compressible, NotBinary)
      final lazy val `vnd.ntt-local.sip-ta_tcp_stream`: MediaType =
        new MediaType("application", "vnd.ntt-local.sip-ta_tcp_stream", Compressible, NotBinary)
      final lazy val `vnd.oasis.opendocument.chart`: MediaType = new MediaType(
        "application",
        "vnd.oasis.opendocument.chart",
        Compressible,
        Binary,
        List("odc"))
      final lazy val `vnd.oasis.opendocument.chart-template`: MediaType = new MediaType(
        "application",
        "vnd.oasis.opendocument.chart-template",
        Compressible,
        NotBinary,
        List("otc"))
      final lazy val `vnd.oasis.opendocument.database`: MediaType = new MediaType(
        "application",
        "vnd.oasis.opendocument.database",
        Compressible,
        Binary,
        List("odb"))
      final lazy val `vnd.oasis.opendocument.formula`: MediaType = new MediaType(
        "application",
        "vnd.oasis.opendocument.formula",
        Compressible,
        Binary,
        List("odf"))
      final lazy val `vnd.oasis.opendocument.formula-template`: MediaType = new MediaType(
        "application",
        "vnd.oasis.opendocument.formula-template",
        Compressible,
        NotBinary,
        List("odft"))
      final lazy val `vnd.oasis.opendocument.graphics`: MediaType = new MediaType(
        "application",
        "vnd.oasis.opendocument.graphics",
        Uncompressible,
        Binary,
        List("odg"))
      final lazy val `vnd.oasis.opendocument.graphics-template`: MediaType = new MediaType(
        "application",
        "vnd.oasis.opendocument.graphics-template",
        Compressible,
        NotBinary,
        List("otg"))
      final lazy val `vnd.oasis.opendocument.image`: MediaType = new MediaType(
        "application",
        "vnd.oasis.opendocument.image",
        Compressible,
        Binary,
        List("odi"))
      final lazy val `vnd.oasis.opendocument.image-template`: MediaType = new MediaType(
        "application",
        "vnd.oasis.opendocument.image-template",
        Compressible,
        NotBinary,
        List("oti"))
      final lazy val `vnd.oasis.opendocument.presentation`: MediaType = new MediaType(
        "application",
        "vnd.oasis.opendocument.presentation",
        Uncompressible,
        Binary,
        List("odp"))
      final lazy val `vnd.oasis.opendocument.presentation-template`: MediaType = new MediaType(
        "application",
        "vnd.oasis.opendocument.presentation-template",
        Compressible,
        NotBinary,
        List("otp"))
      final lazy val `vnd.oasis.opendocument.spreadsheet`: MediaType = new MediaType(
        "application",
        "vnd.oasis.opendocument.spreadsheet",
        Uncompressible,
        Binary,
        List("ods"))
      final lazy val `vnd.oasis.opendocument.spreadsheet-template`: MediaType = new MediaType(
        "application",
        "vnd.oasis.opendocument.spreadsheet-template",
        Compressible,
        NotBinary,
        List("ots"))
      final lazy val `vnd.oasis.opendocument.text`: MediaType = new MediaType(
        "application",
        "vnd.oasis.opendocument.text",
        Uncompressible,
        Binary,
        List("odt"))
      final lazy val `vnd.oasis.opendocument.text-master`: MediaType = new MediaType(
        "application",
        "vnd.oasis.opendocument.text-master",
        Compressible,
        Binary,
        List("odm"))
      final lazy val `vnd.oasis.opendocument.text-template`: MediaType = new MediaType(
        "application",
        "vnd.oasis.opendocument.text-template",
        Compressible,
        NotBinary,
        List("ott"))
      final lazy val `vnd.oasis.opendocument.text-web`: MediaType = new MediaType(
        "application",
        "vnd.oasis.opendocument.text-web",
        Compressible,
        Binary,
        List("oth"))
      final lazy val `vnd.obn`: MediaType =
        new MediaType("application", "vnd.obn", Compressible, NotBinary)
      final lazy val `vnd.ocf+cbor`: MediaType =
        new MediaType("application", "vnd.ocf+cbor", Compressible, NotBinary)
      final lazy val `vnd.oftn.l10n+json`: MediaType =
        new MediaType("application", "vnd.oftn.l10n+json", Compressible, NotBinary)
      final lazy val `vnd.oipf.contentaccessdownload+xml`: MediaType =
        new MediaType("application", "vnd.oipf.contentaccessdownload+xml", Compressible, NotBinary)
      final lazy val `vnd.oipf.contentaccessstreaming+xml`: MediaType =
        new MediaType("application", "vnd.oipf.contentaccessstreaming+xml", Compressible, NotBinary)
      final lazy val `vnd.oipf.cspg-hexbinary`: MediaType =
        new MediaType("application", "vnd.oipf.cspg-hexbinary", Compressible, NotBinary)
      final lazy val `vnd.oipf.dae.svg+xml`: MediaType =
        new MediaType("application", "vnd.oipf.dae.svg+xml", Compressible, NotBinary)
      final lazy val `vnd.oipf.dae.xhtml+xml`: MediaType =
        new MediaType("application", "vnd.oipf.dae.xhtml+xml", Compressible, NotBinary)
      final lazy val `vnd.oipf.mippvcontrolmessage+xml`: MediaType =
        new MediaType("application", "vnd.oipf.mippvcontrolmessage+xml", Compressible, NotBinary)
      final lazy val `vnd.oipf.pae.gem`: MediaType =
        new MediaType("application", "vnd.oipf.pae.gem", Compressible, NotBinary)
      final lazy val `vnd.oipf.spdiscovery+xml`: MediaType =
        new MediaType("application", "vnd.oipf.spdiscovery+xml", Compressible, NotBinary)
      final lazy val `vnd.oipf.spdlist+xml`: MediaType =
        new MediaType("application", "vnd.oipf.spdlist+xml", Compressible, NotBinary)
      final lazy val `vnd.oipf.ueprofile+xml`: MediaType =
        new MediaType("application", "vnd.oipf.ueprofile+xml", Compressible, NotBinary)
      final lazy val `vnd.oipf.userprofile+xml`: MediaType =
        new MediaType("application", "vnd.oipf.userprofile+xml", Compressible, NotBinary)
      final lazy val `vnd.olpc-sugar`: MediaType =
        new MediaType("application", "vnd.olpc-sugar", Compressible, NotBinary, List("xo"))
      final lazy val `vnd.oma-scws-config`: MediaType =
        new MediaType("application", "vnd.oma-scws-config", Compressible, NotBinary)
      final lazy val `vnd.oma-scws-http-request`: MediaType =
        new MediaType("application", "vnd.oma-scws-http-request", Compressible, NotBinary)
      final lazy val `vnd.oma-scws-http-response`: MediaType =
        new MediaType("application", "vnd.oma-scws-http-response", Compressible, NotBinary)
      final lazy val `vnd.oma.bcast.associated-procedure-parameter+xml`: MediaType = new MediaType(
        "application",
        "vnd.oma.bcast.associated-procedure-parameter+xml",
        Compressible,
        NotBinary)
      final lazy val `vnd.oma.bcast.drm-trigger+xml`: MediaType =
        new MediaType("application", "vnd.oma.bcast.drm-trigger+xml", Compressible, NotBinary)
      final lazy val `vnd.oma.bcast.imd+xml`: MediaType =
        new MediaType("application", "vnd.oma.bcast.imd+xml", Compressible, NotBinary)
      final lazy val `vnd.oma.bcast.ltkm`: MediaType =
        new MediaType("application", "vnd.oma.bcast.ltkm", Compressible, NotBinary)
      final lazy val `vnd.oma.bcast.notification+xml`: MediaType =
        new MediaType("application", "vnd.oma.bcast.notification+xml", Compressible, NotBinary)
      final lazy val `vnd.oma.bcast.provisioningtrigger`: MediaType =
        new MediaType("application", "vnd.oma.bcast.provisioningtrigger", Compressible, NotBinary)
      final lazy val `vnd.oma.bcast.sgboot`: MediaType =
        new MediaType("application", "vnd.oma.bcast.sgboot", Compressible, NotBinary)
      final lazy val `vnd.oma.bcast.sgdd+xml`: MediaType =
        new MediaType("application", "vnd.oma.bcast.sgdd+xml", Compressible, NotBinary)
      final lazy val `vnd.oma.bcast.sgdu`: MediaType =
        new MediaType("application", "vnd.oma.bcast.sgdu", Compressible, NotBinary)
      final lazy val `vnd.oma.bcast.simple-symbol-container`: MediaType = new MediaType(
        "application",
        "vnd.oma.bcast.simple-symbol-container",
        Compressible,
        NotBinary)
      final lazy val `vnd.oma.bcast.smartcard-trigger+xml`: MediaType =
        new MediaType("application", "vnd.oma.bcast.smartcard-trigger+xml", Compressible, NotBinary)
      final lazy val `vnd.oma.bcast.sprov+xml`: MediaType =
        new MediaType("application", "vnd.oma.bcast.sprov+xml", Compressible, NotBinary)
      final lazy val `vnd.oma.bcast.stkm`: MediaType =
        new MediaType("application", "vnd.oma.bcast.stkm", Compressible, NotBinary)
      final lazy val `vnd.oma.cab-address-book+xml`: MediaType =
        new MediaType("application", "vnd.oma.cab-address-book+xml", Compressible, NotBinary)
      final lazy val `vnd.oma.cab-feature-handler+xml`: MediaType =
        new MediaType("application", "vnd.oma.cab-feature-handler+xml", Compressible, NotBinary)
      final lazy val `vnd.oma.cab-pcc+xml`: MediaType =
        new MediaType("application", "vnd.oma.cab-pcc+xml", Compressible, NotBinary)
      final lazy val `vnd.oma.cab-subs-invite+xml`: MediaType =
        new MediaType("application", "vnd.oma.cab-subs-invite+xml", Compressible, NotBinary)
      final lazy val `vnd.oma.cab-user-prefs+xml`: MediaType =
        new MediaType("application", "vnd.oma.cab-user-prefs+xml", Compressible, NotBinary)
      final lazy val `vnd.oma.dcd`: MediaType =
        new MediaType("application", "vnd.oma.dcd", Compressible, NotBinary)
      final lazy val `vnd.oma.dcdc`: MediaType =
        new MediaType("application", "vnd.oma.dcdc", Compressible, NotBinary)
      final lazy val `vnd.oma.dd2+xml`: MediaType =
        new MediaType("application", "vnd.oma.dd2+xml", Compressible, NotBinary, List("dd2"))
      final lazy val part_1: List[MediaType] = List(
        `vnd.cirpack.isdn-ext`,
        `vnd.citationstyles.style+xml`,
        `vnd.claymore`,
        `vnd.cloanto.rp9`,
        `vnd.clonk.c4group`,
        `vnd.cluetrust.cartomobile-config`,
        `vnd.cluetrust.cartomobile-config-pkg`,
        `vnd.coffeescript`,
        `vnd.collabio.xodocuments.document`,
        `vnd.collabio.xodocuments.document-template`,
        `vnd.collabio.xodocuments.presentation`,
        `vnd.collabio.xodocuments.presentation-template`,
        `vnd.collabio.xodocuments.spreadsheet`,
        `vnd.collabio.xodocuments.spreadsheet-template`,
        `vnd.collection+json`,
        `vnd.collection.doc+json`,
        `vnd.collection.next+json`,
        `vnd.comicbook+zip`,
        `vnd.comicbook-rar`,
        `vnd.commerce-battelle`,
        `vnd.commonspace`,
        `vnd.contact.cmsg`,
        `vnd.coreos.ignition+json`,
        `vnd.cosmocaller`,
        `vnd.crick.clicker`,
        `vnd.crick.clicker.keyboard`,
        `vnd.crick.clicker.palette`,
        `vnd.crick.clicker.template`,
        `vnd.crick.clicker.wordbank`,
        `vnd.criticaltools.wbs+xml`,
        `vnd.ctc-posml`,
        `vnd.ctct.ws+xml`,
        `vnd.cups-pdf`,
        `vnd.cups-postscript`,
        `vnd.cups-ppd`,
        `vnd.cups-raster`,
        `vnd.cups-raw`,
        `vnd.curl`,
        `vnd.curl.car`,
        `vnd.curl.pcurl`,
        `vnd.cyan.dean.root+xml`,
        `vnd.cybank`,
        `vnd.d2l.coursepackage1p0+zip`,
        `vnd.dart`,
        `vnd.data-vision.rdz`,
        `vnd.datapackage+json`,
        `vnd.dataresource+json`,
        `vnd.debian.binary-package`,
        `vnd.dece.data`,
        `vnd.dece.ttml+xml`,
        `vnd.dece.unspecified`,
        `vnd.dece.zip`,
        `vnd.denovo.fcselayout-link`,
        `vnd.desmume.movie`,
        `vnd.dir-bi.plate-dl-nosuffix`,
        `vnd.dm.delegation+xml`,
        `vnd.dna`,
        `vnd.document+json`,
        `vnd.dolby.mlp`,
        `vnd.dolby.mobile.1`,
        `vnd.dolby.mobile.2`,
        `vnd.doremir.scorecloud-binary-document`,
        `vnd.dpgraph`,
        `vnd.dreamfactory`,
        `vnd.drive+json`,
        `vnd.ds-keypoint`,
        `vnd.dtg.local`,
        `vnd.dtg.local.flash`,
        `vnd.dtg.local.html`,
        `vnd.dvb.ait`,
        `vnd.dvb.dvbj`,
        `vnd.dvb.esgcontainer`,
        `vnd.dvb.ipdcdftnotifaccess`,
        `vnd.dvb.ipdcesgaccess`,
        `vnd.dvb.ipdcesgaccess2`,
        `vnd.dvb.ipdcesgpdd`,
        `vnd.dvb.ipdcroaming`,
        `vnd.dvb.iptv.alfec-base`,
        `vnd.dvb.iptv.alfec-enhancement`,
        `vnd.dvb.notif-aggregate-root+xml`,
        `vnd.dvb.notif-container+xml`,
        `vnd.dvb.notif-generic+xml`,
        `vnd.dvb.notif-ia-msglist+xml`,
        `vnd.dvb.notif-ia-registration-request+xml`,
        `vnd.dvb.notif-ia-registration-response+xml`,
        `vnd.dvb.notif-init+xml`,
        `vnd.dvb.pfr`,
        `vnd.dvb.service`,
        `vnd.dxr`,
        `vnd.dynageo`,
        `vnd.dzr`,
        `vnd.easykaraoke.cdgdownload`,
        `vnd.ecdis-update`,
        `vnd.ecip.rlp`,
        `vnd.ecowin.chart`,
        `vnd.ecowin.filerequest`,
        `vnd.ecowin.fileupdate`,
        `vnd.ecowin.series`,
        `vnd.ecowin.seriesrequest`,
        `vnd.ecowin.seriesupdate`,
        `vnd.efi.img`,
        `vnd.efi.iso`,
        `vnd.emclient.accessrequest+xml`,
        `vnd.enliven`,
        `vnd.enphase.envoy`,
        `vnd.eprints.data+xml`,
        `vnd.epson.esf`,
        `vnd.epson.msf`,
        `vnd.epson.quickanime`,
        `vnd.epson.salt`,
        `vnd.epson.ssf`,
        `vnd.ericsson.quickcall`,
        `vnd.espass-espass+zip`,
        `vnd.eszigno3+xml`,
        `vnd.etsi.aoc+xml`,
        `vnd.etsi.asic-e+zip`,
        `vnd.etsi.asic-s+zip`,
        `vnd.etsi.cug+xml`,
        `vnd.etsi.iptvcommand+xml`,
        `vnd.etsi.iptvdiscovery+xml`,
        `vnd.etsi.iptvprofile+xml`,
        `vnd.etsi.iptvsad-bc+xml`,
        `vnd.etsi.iptvsad-cod+xml`,
        `vnd.etsi.iptvsad-npvr+xml`,
        `vnd.etsi.iptvservice+xml`,
        `vnd.etsi.iptvsync+xml`,
        `vnd.etsi.iptvueprofile+xml`,
        `vnd.etsi.mcid+xml`,
        `vnd.etsi.mheg5`,
        `vnd.etsi.overload-control-policy-dataset+xml`,
        `vnd.etsi.pstn+xml`,
        `vnd.etsi.sci+xml`,
        `vnd.etsi.simservs+xml`,
        `vnd.etsi.timestamp-token`,
        `vnd.etsi.tsl+xml`,
        `vnd.etsi.tsl.der`,
        `vnd.eudora.data`,
        `vnd.evolv.ecig.profile`,
        `vnd.evolv.ecig.settings`,
        `vnd.evolv.ecig.theme`,
        `vnd.exstream-empower+zip`,
        `vnd.ezpix-album`,
        `vnd.ezpix-package`,
        `vnd.f-secure.mobile`,
        `vnd.fastcopy-disk-image`,
        `vnd.fdf`,
        `vnd.fdsn.mseed`,
        `vnd.fdsn.seed`,
        `vnd.ffsns`,
        `vnd.filmit.zfc`,
        `vnd.fints`,
        `vnd.firemonkeys.cloudcell`,
        `vnd.flographit`,
        `vnd.fluxtime.clip`,
        `vnd.font-fontforge-sfd`,
        `vnd.framemaker`,
        `vnd.frogans.fnc`,
        `vnd.frogans.ltf`,
        `vnd.fsc.weblaunch`,
        `vnd.fujitsu.oasys`,
        `vnd.fujitsu.oasys2`,
        `vnd.fujitsu.oasys3`,
        `vnd.fujitsu.oasysgp`,
        `vnd.fujitsu.oasysprs`,
        `vnd.fujixerox.art-ex`,
        `vnd.fujixerox.art4`,
        `vnd.fujixerox.ddd`,
        `vnd.fujixerox.docuworks`,
        `vnd.fujixerox.docuworks.binder`,
        `vnd.fujixerox.docuworks.container`,
        `vnd.fujixerox.hbpl`,
        `vnd.fut-misnet`,
        `vnd.futoin+cbor`,
        `vnd.futoin+json`,
        `vnd.fuzzysheet`,
        `vnd.genomatix.tuxedo`,
        `vnd.geo+json`,
        `vnd.geocube+xml`,
        `vnd.geogebra.file`,
        `vnd.geogebra.tool`,
        `vnd.geometry-explorer`,
        `vnd.geonext`,
        `vnd.geoplan`,
        `vnd.geospace`,
        `vnd.gerber`,
        `vnd.globalplatform.card-content-mgt`,
        `vnd.globalplatform.card-content-mgt-response`,
        `vnd.gmx`,
        `vnd.google-apps.document`,
        `vnd.google-apps.presentation`,
        `vnd.google-apps.spreadsheet`,
        `vnd.google-earth.kml+xml`,
        `vnd.google-earth.kmz`,
        `vnd.gov.sk.e-form+xml`,
        `vnd.gov.sk.e-form+zip`,
        `vnd.gov.sk.xmldatacontainer+xml`,
        `vnd.grafeq`,
        `vnd.gridmp`,
        `vnd.groove-account`,
        `vnd.groove-help`,
        `vnd.groove-identity-message`,
        `vnd.groove-injector`,
        `vnd.groove-tool-message`,
        `vnd.groove-tool-template`,
        `vnd.groove-vcard`,
        `vnd.hal+json`,
        `vnd.hal+xml`,
        `vnd.handheld-entertainment+xml`,
        `vnd.hbci`,
        `vnd.hc+json`,
        `vnd.hcl-bireports`,
        `vnd.hdt`,
        `vnd.heroku+json`,
        `vnd.hhe.lesson-player`,
        `vnd.hp-hpgl`,
        `vnd.hp-hpid`,
        `vnd.hp-hps`,
        `vnd.hp-jlyt`,
        `vnd.hp-pcl`,
        `vnd.hp-pclxl`,
        `vnd.httphone`,
        `vnd.hydrostatix.sof-data`,
        `vnd.hyper+json`,
        `vnd.hyper-item+json`,
        `vnd.hyperdrive+json`,
        `vnd.hzn-3d-crossword`,
        `vnd.ibm.afplinedata`,
        `vnd.ibm.electronic-media`,
        `vnd.ibm.minipay`,
        `vnd.ibm.modcap`,
        `vnd.ibm.rights-management`,
        `vnd.ibm.secure-container`,
        `vnd.iccprofile`,
        `vnd.ieee.1905`,
        `vnd.igloader`,
        `vnd.imagemeter.folder+zip`,
        `vnd.imagemeter.image+zip`,
        `vnd.immervision-ivp`,
        `vnd.immervision-ivu`,
        `vnd.ims.imsccv1p1`,
        `vnd.ims.imsccv1p2`,
        `vnd.ims.imsccv1p3`,
        `vnd.ims.lis.v2.result+json`,
        `vnd.ims.lti.v2.toolconsumerprofile+json`,
        `vnd.ims.lti.v2.toolproxy+json`,
        `vnd.ims.lti.v2.toolproxy.id+json`,
        `vnd.ims.lti.v2.toolsettings+json`,
        `vnd.ims.lti.v2.toolsettings.simple+json`,
        `vnd.informedcontrol.rms+xml`,
        `vnd.informix-visionary`,
        `vnd.infotech.project`,
        `vnd.infotech.project+xml`,
        `vnd.innopath.wamp.notification`,
        `vnd.insors.igm`,
        `vnd.intercon.formnet`,
        `vnd.intergeo`,
        `vnd.intertrust.digibox`,
        `vnd.intertrust.nncp`,
        `vnd.intu.qbo`,
        `vnd.intu.qfx`,
        `vnd.iptc.g2.catalogitem+xml`,
        `vnd.iptc.g2.conceptitem+xml`,
        `vnd.iptc.g2.knowledgeitem+xml`,
        `vnd.iptc.g2.newsitem+xml`,
        `vnd.iptc.g2.newsmessage+xml`,
        `vnd.iptc.g2.packageitem+xml`,
        `vnd.iptc.g2.planningitem+xml`,
        `vnd.ipunplugged.rcprofile`,
        `vnd.irepository.package+xml`,
        `vnd.is-xpr`,
        `vnd.isac.fcs`,
        `vnd.jam`,
        `vnd.japannet-directory-service`,
        `vnd.japannet-jpnstore-wakeup`,
        `vnd.japannet-payment-wakeup`,
        `vnd.japannet-registration`,
        `vnd.japannet-registration-wakeup`,
        `vnd.japannet-setstore-wakeup`,
        `vnd.japannet-verification`,
        `vnd.japannet-verification-wakeup`,
        `vnd.jcp.javame.midlet-rms`,
        `vnd.jisp`,
        `vnd.joost.joda-archive`,
        `vnd.jsk.isdn-ngn`,
        `vnd.kahootz`,
        `vnd.kde.karbon`,
        `vnd.kde.kchart`,
        `vnd.kde.kformula`,
        `vnd.kde.kivio`,
        `vnd.kde.kontour`,
        `vnd.kde.kpresenter`,
        `vnd.kde.kspread`,
        `vnd.kde.kword`,
        `vnd.kenameaapp`,
        `vnd.kidspiration`,
        `vnd.kinar`,
        `vnd.koan`,
        `vnd.kodak-descriptor`,
        `vnd.las.las+json`,
        `vnd.las.las+xml`,
        `vnd.leap+json`,
        `vnd.liberty-request+xml`,
        `vnd.llamagraphics.life-balance.desktop`,
        `vnd.llamagraphics.life-balance.exchange+xml`,
        `vnd.lotus-1-2-3`,
        `vnd.lotus-approach`,
        `vnd.lotus-freelance`,
        `vnd.lotus-notes`,
        `vnd.lotus-organizer`,
        `vnd.lotus-screencam`,
        `vnd.lotus-wordpro`,
        `vnd.macports.portpkg`,
        `vnd.mapbox-vector-tile`,
        `vnd.marlin.drm.actiontoken+xml`,
        `vnd.marlin.drm.conftoken+xml`,
        `vnd.marlin.drm.license+xml`,
        `vnd.marlin.drm.mdcf`,
        `vnd.mason+json`,
        `vnd.maxmind.maxmind-db`,
        `vnd.mcd`,
        `vnd.medcalcdata`,
        `vnd.mediastation.cdkey`,
        `vnd.meridian-slingshot`,
        `vnd.mfer`,
        `vnd.mfmp`,
        `vnd.micro+json`,
        `vnd.micrografx.flo`,
        `vnd.micrografx.igx`,
        `vnd.microsoft.portable-executable`,
        `vnd.microsoft.windows.thumbnail-cache`,
        `vnd.miele+json`,
        `vnd.mif`,
        `vnd.minisoft-hp3000-save`,
        `vnd.mitsubishi.misty-guard.trustweb`,
        `vnd.mobius.daf`,
        `vnd.mobius.dis`,
        `vnd.mobius.mbk`,
        `vnd.mobius.mqy`,
        `vnd.mobius.msl`,
        `vnd.mobius.plc`,
        `vnd.mobius.txf`,
        `vnd.mophun.application`,
        `vnd.mophun.certificate`,
        `vnd.motorola.flexsuite`,
        `vnd.motorola.flexsuite.adsi`,
        `vnd.motorola.flexsuite.fis`,
        `vnd.motorola.flexsuite.gotap`,
        `vnd.motorola.flexsuite.kmr`,
        `vnd.motorola.flexsuite.ttc`,
        `vnd.motorola.flexsuite.wem`,
        `vnd.motorola.iprm`,
        `vnd.mozilla.xul+xml`,
        `vnd.ms-3mfdocument`,
        `vnd.ms-artgalry`,
        `vnd.ms-asf`,
        `vnd.ms-cab-compressed`,
        `vnd.ms-color.iccprofile`,
        `vnd.ms-excel`,
        `vnd.ms-excel.addin.macroenabled.12`,
        `vnd.ms-excel.sheet.binary.macroenabled.12`,
        `vnd.ms-excel.sheet.macroenabled.12`,
        `vnd.ms-excel.template.macroenabled.12`,
        `vnd.ms-fontobject`,
        `vnd.ms-htmlhelp`,
        `vnd.ms-ims`,
        `vnd.ms-lrm`,
        `vnd.ms-office.activex+xml`,
        `vnd.ms-officetheme`,
        `vnd.ms-opentype`,
        `vnd.ms-outlook`,
        `vnd.ms-package.obfuscated-opentype`,
        `vnd.ms-pki.seccat`,
        `vnd.ms-pki.stl`,
        `vnd.ms-playready.initiator+xml`,
        `vnd.ms-powerpoint`,
        `vnd.ms-powerpoint.addin.macroenabled.12`,
        `vnd.ms-powerpoint.presentation.macroenabled.12`,
        `vnd.ms-powerpoint.slide.macroenabled.12`,
        `vnd.ms-powerpoint.slideshow.macroenabled.12`,
        `vnd.ms-powerpoint.template.macroenabled.12`,
        `vnd.ms-printdevicecapabilities+xml`,
        `vnd.ms-printing.printticket+xml`,
        `vnd.ms-printschematicket+xml`,
        `vnd.ms-project`,
        `vnd.ms-tnef`,
        `vnd.ms-windows.devicepairing`,
        `vnd.ms-windows.nwprinting.oob`,
        `vnd.ms-windows.printerpairing`,
        `vnd.ms-windows.wsd.oob`,
        `vnd.ms-wmdrm.lic-chlg-req`,
        `vnd.ms-wmdrm.lic-resp`,
        `vnd.ms-wmdrm.meter-chlg-req`,
        `vnd.ms-wmdrm.meter-resp`,
        `vnd.ms-word.document.macroenabled.12`,
        `vnd.ms-word.template.macroenabled.12`,
        `vnd.ms-works`,
        `vnd.ms-wpl`,
        `vnd.ms-xpsdocument`,
        `vnd.msa-disk-image`,
        `vnd.mseq`,
        `vnd.msign`,
        `vnd.multiad.creator`,
        `vnd.multiad.creator.cif`,
        `vnd.music-niff`,
        `vnd.musician`,
        `vnd.muvee.style`,
        `vnd.mynfc`,
        `vnd.ncd.control`,
        `vnd.ncd.reference`,
        `vnd.nearst.inv+json`,
        `vnd.nervana`,
        `vnd.netfpx`,
        `vnd.neurolanguage.nlu`,
        `vnd.nimn`,
        `vnd.nintendo.nitro.rom`,
        `vnd.nintendo.snes.rom`,
        `vnd.nitf`,
        `vnd.noblenet-directory`,
        `vnd.noblenet-sealer`,
        `vnd.noblenet-web`,
        `vnd.nokia.catalogs`,
        `vnd.nokia.conml+wbxml`,
        `vnd.nokia.conml+xml`,
        `vnd.nokia.iptv.config+xml`,
        `vnd.nokia.isds-radio-presets`,
        `vnd.nokia.landmark+wbxml`,
        `vnd.nokia.landmark+xml`,
        `vnd.nokia.landmarkcollection+xml`,
        `vnd.nokia.n-gage.ac+xml`,
        `vnd.nokia.n-gage.data`,
        `vnd.nokia.n-gage.symbian.install`,
        `vnd.nokia.ncd`,
        `vnd.nokia.pcd+wbxml`,
        `vnd.nokia.pcd+xml`,
        `vnd.nokia.radio-preset`,
        `vnd.nokia.radio-presets`,
        `vnd.novadigm.edm`,
        `vnd.novadigm.edx`,
        `vnd.novadigm.ext`,
        `vnd.ntt-local.content-share`,
        `vnd.ntt-local.file-transfer`,
        `vnd.ntt-local.ogw_remote-access`,
        `vnd.ntt-local.sip-ta_remote`,
        `vnd.ntt-local.sip-ta_tcp_stream`,
        `vnd.oasis.opendocument.chart`,
        `vnd.oasis.opendocument.chart-template`,
        `vnd.oasis.opendocument.database`,
        `vnd.oasis.opendocument.formula`,
        `vnd.oasis.opendocument.formula-template`,
        `vnd.oasis.opendocument.graphics`,
        `vnd.oasis.opendocument.graphics-template`,
        `vnd.oasis.opendocument.image`,
        `vnd.oasis.opendocument.image-template`,
        `vnd.oasis.opendocument.presentation`,
        `vnd.oasis.opendocument.presentation-template`,
        `vnd.oasis.opendocument.spreadsheet`,
        `vnd.oasis.opendocument.spreadsheet-template`,
        `vnd.oasis.opendocument.text`,
        `vnd.oasis.opendocument.text-master`,
        `vnd.oasis.opendocument.text-template`,
        `vnd.oasis.opendocument.text-web`,
        `vnd.obn`,
        `vnd.ocf+cbor`,
        `vnd.oftn.l10n+json`,
        `vnd.oipf.contentaccessdownload+xml`,
        `vnd.oipf.contentaccessstreaming+xml`,
        `vnd.oipf.cspg-hexbinary`,
        `vnd.oipf.dae.svg+xml`,
        `vnd.oipf.dae.xhtml+xml`,
        `vnd.oipf.mippvcontrolmessage+xml`,
        `vnd.oipf.pae.gem`,
        `vnd.oipf.spdiscovery+xml`,
        `vnd.oipf.spdlist+xml`,
        `vnd.oipf.ueprofile+xml`,
        `vnd.oipf.userprofile+xml`,
        `vnd.olpc-sugar`,
        `vnd.oma-scws-config`,
        `vnd.oma-scws-http-request`,
        `vnd.oma-scws-http-response`,
        `vnd.oma.bcast.associated-procedure-parameter+xml`,
        `vnd.oma.bcast.drm-trigger+xml`,
        `vnd.oma.bcast.imd+xml`,
        `vnd.oma.bcast.ltkm`,
        `vnd.oma.bcast.notification+xml`,
        `vnd.oma.bcast.provisioningtrigger`,
        `vnd.oma.bcast.sgboot`,
        `vnd.oma.bcast.sgdd+xml`,
        `vnd.oma.bcast.sgdu`,
        `vnd.oma.bcast.simple-symbol-container`,
        `vnd.oma.bcast.smartcard-trigger+xml`,
        `vnd.oma.bcast.sprov+xml`,
        `vnd.oma.bcast.stkm`,
        `vnd.oma.cab-address-book+xml`,
        `vnd.oma.cab-feature-handler+xml`,
        `vnd.oma.cab-pcc+xml`,
        `vnd.oma.cab-subs-invite+xml`,
        `vnd.oma.cab-user-prefs+xml`,
        `vnd.oma.dcd`,
        `vnd.oma.dcdc`,
        `vnd.oma.dd2+xml`
      )
    }
    trait application_2 {
      final lazy val `vnd.oma.drm.risd+xml`: MediaType =
        new MediaType("application", "vnd.oma.drm.risd+xml", Compressible, NotBinary)
      final lazy val `vnd.oma.group-usage-list+xml`: MediaType =
        new MediaType("application", "vnd.oma.group-usage-list+xml", Compressible, NotBinary)
      final lazy val `vnd.oma.lwm2m+json`: MediaType =
        new MediaType("application", "vnd.oma.lwm2m+json", Compressible, NotBinary)
      final lazy val `vnd.oma.lwm2m+tlv`: MediaType =
        new MediaType("application", "vnd.oma.lwm2m+tlv", Compressible, NotBinary)
      final lazy val `vnd.oma.pal+xml`: MediaType =
        new MediaType("application", "vnd.oma.pal+xml", Compressible, NotBinary)
      final lazy val `vnd.oma.poc.detailed-progress-report+xml`: MediaType = new MediaType(
        "application",
        "vnd.oma.poc.detailed-progress-report+xml",
        Compressible,
        NotBinary)
      final lazy val `vnd.oma.poc.final-report+xml`: MediaType =
        new MediaType("application", "vnd.oma.poc.final-report+xml", Compressible, NotBinary)
      final lazy val `vnd.oma.poc.groups+xml`: MediaType =
        new MediaType("application", "vnd.oma.poc.groups+xml", Compressible, NotBinary)
      final lazy val `vnd.oma.poc.invocation-descriptor+xml`: MediaType = new MediaType(
        "application",
        "vnd.oma.poc.invocation-descriptor+xml",
        Compressible,
        NotBinary)
      final lazy val `vnd.oma.poc.optimized-progress-report+xml`: MediaType = new MediaType(
        "application",
        "vnd.oma.poc.optimized-progress-report+xml",
        Compressible,
        NotBinary)
      final lazy val `vnd.oma.push`: MediaType =
        new MediaType("application", "vnd.oma.push", Compressible, NotBinary)
      final lazy val `vnd.oma.scidm.messages+xml`: MediaType =
        new MediaType("application", "vnd.oma.scidm.messages+xml", Compressible, NotBinary)
      final lazy val `vnd.oma.xcap-directory+xml`: MediaType =
        new MediaType("application", "vnd.oma.xcap-directory+xml", Compressible, NotBinary)
      final lazy val `vnd.omads-email+xml`: MediaType =
        new MediaType("application", "vnd.omads-email+xml", Compressible, NotBinary)
      final lazy val `vnd.omads-file+xml`: MediaType =
        new MediaType("application", "vnd.omads-file+xml", Compressible, NotBinary)
      final lazy val `vnd.omads-folder+xml`: MediaType =
        new MediaType("application", "vnd.omads-folder+xml", Compressible, NotBinary)
      final lazy val `vnd.omaloc-supl-init`: MediaType =
        new MediaType("application", "vnd.omaloc-supl-init", Compressible, NotBinary)
      final lazy val `vnd.onepager`: MediaType =
        new MediaType("application", "vnd.onepager", Compressible, NotBinary)
      final lazy val `vnd.onepagertamp`: MediaType =
        new MediaType("application", "vnd.onepagertamp", Compressible, NotBinary)
      final lazy val `vnd.onepagertamx`: MediaType =
        new MediaType("application", "vnd.onepagertamx", Compressible, NotBinary)
      final lazy val `vnd.onepagertat`: MediaType =
        new MediaType("application", "vnd.onepagertat", Compressible, NotBinary)
      final lazy val `vnd.onepagertatp`: MediaType =
        new MediaType("application", "vnd.onepagertatp", Compressible, NotBinary)
      final lazy val `vnd.onepagertatx`: MediaType =
        new MediaType("application", "vnd.onepagertatx", Compressible, NotBinary)
      final lazy val `vnd.openblox.game+xml`: MediaType =
        new MediaType("application", "vnd.openblox.game+xml", Compressible, NotBinary)
      final lazy val `vnd.openblox.game-binary`: MediaType =
        new MediaType("application", "vnd.openblox.game-binary", Compressible, NotBinary)
      final lazy val `vnd.openeye.oeb`: MediaType =
        new MediaType("application", "vnd.openeye.oeb", Compressible, NotBinary)
      final lazy val `vnd.openofficeorg.extension`: MediaType = new MediaType(
        "application",
        "vnd.openofficeorg.extension",
        Compressible,
        NotBinary,
        List("oxt"))
      final lazy val `vnd.openstreetmap.data+xml`: MediaType =
        new MediaType("application", "vnd.openstreetmap.data+xml", Compressible, NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.custom-properties+xml`: MediaType = new MediaType(
        "application",
        "vnd.openxmlformats-officedocument.custom-properties+xml",
        Compressible,
        NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.customxmlproperties+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.customxmlproperties+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.drawing+xml`: MediaType = new MediaType(
        "application",
        "vnd.openxmlformats-officedocument.drawing+xml",
        Compressible,
        NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.drawingml.chart+xml`: MediaType = new MediaType(
        "application",
        "vnd.openxmlformats-officedocument.drawingml.chart+xml",
        Compressible,
        NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.drawingml.chartshapes+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.drawingml.chartshapes+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.drawingml.diagramcolors+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.drawingml.diagramcolors+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.drawingml.diagramdata+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.drawingml.diagramdata+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.drawingml.diagramlayout+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.drawingml.diagramlayout+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.drawingml.diagramstyle+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.drawingml.diagramstyle+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.extended-properties+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.extended-properties+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.presentationml.commentauthors+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.presentationml.commentauthors+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.presentationml.comments+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.presentationml.comments+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.presentationml.handoutmaster+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.presentationml.handoutmaster+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.presentationml.notesmaster+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.presentationml.notesmaster+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.presentationml.notesslide+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.presentationml.notesslide+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.presentationml.presentation`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.presentationml.presentation",
          Uncompressible,
          Binary,
          List("pptx"))
      final lazy val `vnd.openxmlformats-officedocument.presentationml.presentation.main+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.presentationml.presentation.main+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.presentationml.presprops+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.presentationml.presprops+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.presentationml.slide`: MediaType = new MediaType(
        "application",
        "vnd.openxmlformats-officedocument.presentationml.slide",
        Compressible,
        Binary,
        List("sldx"))
      final lazy val `vnd.openxmlformats-officedocument.presentationml.slide+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.presentationml.slide+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.presentationml.slidelayout+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.presentationml.slidelayout+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.presentationml.slidemaster+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.presentationml.slidemaster+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.presentationml.slideshow`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.presentationml.slideshow",
          Compressible,
          Binary,
          List("ppsx"))
      final lazy val `vnd.openxmlformats-officedocument.presentationml.slideshow.main+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.presentationml.slideshow.main+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.presentationml.slideupdateinfo+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.presentationml.slideupdateinfo+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.presentationml.tablestyles+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.presentationml.tablestyles+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.presentationml.tags+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.presentationml.tags+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.presentationml.template`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.presentationml.template",
          Compressible,
          Binary,
          List("potx"))
      final lazy val `vnd.openxmlformats-officedocument.presentationml.template.main+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.presentationml.template.main+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.presentationml.viewprops+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.presentationml.viewprops+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.calcchain+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.spreadsheetml.calcchain+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.chartsheet+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.spreadsheetml.chartsheet+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.comments+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.spreadsheetml.comments+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.connections+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.spreadsheetml.connections+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.dialogsheet+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.spreadsheetml.dialogsheet+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.externallink+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.spreadsheetml.externallink+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.pivotcachedefinition+xml`
        : MediaType = new MediaType(
        "application",
        "vnd.openxmlformats-officedocument.spreadsheetml.pivotcachedefinition+xml",
        Compressible,
        NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.pivotcacherecords+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.spreadsheetml.pivotcacherecords+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.pivottable+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.spreadsheetml.pivottable+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.querytable+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.spreadsheetml.querytable+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.revisionheaders+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.spreadsheetml.revisionheaders+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.revisionlog+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.spreadsheetml.revisionlog+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.sharedstrings+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.spreadsheetml.sharedstrings+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.sheet`: MediaType = new MediaType(
        "application",
        "vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        Uncompressible,
        Binary,
        List("xlsx"))
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.sheetmetadata+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.spreadsheetml.sheetmetadata+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.styles+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.spreadsheetml.styles+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.table+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.spreadsheetml.table+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.tablesinglecells+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.spreadsheetml.tablesinglecells+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.template`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.spreadsheetml.template",
          Compressible,
          Binary,
          List("xltx"))
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.template.main+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.spreadsheetml.template.main+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.usernames+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.spreadsheetml.usernames+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.volatiledependencies+xml`
        : MediaType = new MediaType(
        "application",
        "vnd.openxmlformats-officedocument.spreadsheetml.volatiledependencies+xml",
        Compressible,
        NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.theme+xml`: MediaType = new MediaType(
        "application",
        "vnd.openxmlformats-officedocument.theme+xml",
        Compressible,
        NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.themeoverride+xml`: MediaType = new MediaType(
        "application",
        "vnd.openxmlformats-officedocument.themeoverride+xml",
        Compressible,
        NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.vmldrawing`: MediaType = new MediaType(
        "application",
        "vnd.openxmlformats-officedocument.vmldrawing",
        Compressible,
        NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.wordprocessingml.comments+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.wordprocessingml.comments+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.wordprocessingml.document`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.wordprocessingml.document",
          Uncompressible,
          Binary,
          List("docx"))
      final lazy val `vnd.openxmlformats-officedocument.wordprocessingml.document.glossary+xml`
        : MediaType = new MediaType(
        "application",
        "vnd.openxmlformats-officedocument.wordprocessingml.document.glossary+xml",
        Compressible,
        NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.wordprocessingml.endnotes+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.wordprocessingml.endnotes+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.wordprocessingml.fonttable+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.wordprocessingml.fonttable+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.wordprocessingml.footer+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.wordprocessingml.footer+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.wordprocessingml.footnotes+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.wordprocessingml.footnotes+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.wordprocessingml.numbering+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.wordprocessingml.numbering+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.wordprocessingml.settings+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.wordprocessingml.settings+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.wordprocessingml.styles+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.wordprocessingml.styles+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.wordprocessingml.template`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.wordprocessingml.template",
          Compressible,
          Binary,
          List("dotx"))
      final lazy val `vnd.openxmlformats-officedocument.wordprocessingml.template.main+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.wordprocessingml.template.main+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-officedocument.wordprocessingml.websettings+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-officedocument.wordprocessingml.websettings+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-package.core-properties+xml`: MediaType = new MediaType(
        "application",
        "vnd.openxmlformats-package.core-properties+xml",
        Compressible,
        NotBinary)
      final lazy val `vnd.openxmlformats-package.digital-signature-xmlsignature+xml`: MediaType =
        new MediaType(
          "application",
          "vnd.openxmlformats-package.digital-signature-xmlsignature+xml",
          Compressible,
          NotBinary)
      final lazy val `vnd.openxmlformats-package.relationships+xml`: MediaType = new MediaType(
        "application",
        "vnd.openxmlformats-package.relationships+xml",
        Compressible,
        NotBinary)
      final lazy val `vnd.oracle.resource+json`: MediaType =
        new MediaType("application", "vnd.oracle.resource+json", Compressible, NotBinary)
      final lazy val `vnd.orange.indata`: MediaType =
        new MediaType("application", "vnd.orange.indata", Compressible, NotBinary)
      final lazy val `vnd.osa.netdeploy`: MediaType =
        new MediaType("application", "vnd.osa.netdeploy", Compressible, NotBinary)
      final lazy val `vnd.osgeo.mapguide.package`: MediaType = new MediaType(
        "application",
        "vnd.osgeo.mapguide.package",
        Compressible,
        NotBinary,
        List("mgp"))
      final lazy val `vnd.osgi.bundle`: MediaType =
        new MediaType("application", "vnd.osgi.bundle", Compressible, NotBinary)
      final lazy val `vnd.osgi.dp`: MediaType =
        new MediaType("application", "vnd.osgi.dp", Compressible, NotBinary, List("dp"))
      final lazy val `vnd.osgi.subsystem`: MediaType =
        new MediaType("application", "vnd.osgi.subsystem", Compressible, NotBinary, List("esa"))
      final lazy val `vnd.otps.ct-kip+xml`: MediaType =
        new MediaType("application", "vnd.otps.ct-kip+xml", Compressible, NotBinary)
      final lazy val `vnd.oxli.countgraph`: MediaType =
        new MediaType("application", "vnd.oxli.countgraph", Compressible, NotBinary)
      final lazy val `vnd.pagerduty+json`: MediaType =
        new MediaType("application", "vnd.pagerduty+json", Compressible, NotBinary)
      final lazy val `vnd.palm`: MediaType = new MediaType(
        "application",
        "vnd.palm",
        Compressible,
        NotBinary,
        List("pdb", "pqa", "oprc"))
      final lazy val `vnd.panoply`: MediaType =
        new MediaType("application", "vnd.panoply", Compressible, NotBinary)
      final lazy val `vnd.paos.xml`: MediaType =
        new MediaType("application", "vnd.paos.xml", Compressible, NotBinary)
      final lazy val `vnd.patentdive`: MediaType =
        new MediaType("application", "vnd.patentdive", Compressible, NotBinary)
      final lazy val `vnd.pawaafile`: MediaType =
        new MediaType("application", "vnd.pawaafile", Compressible, NotBinary, List("paw"))
      final lazy val `vnd.pcos`: MediaType =
        new MediaType("application", "vnd.pcos", Compressible, NotBinary)
      final lazy val `vnd.pg.format`: MediaType =
        new MediaType("application", "vnd.pg.format", Compressible, NotBinary, List("str"))
      final lazy val `vnd.pg.osasli`: MediaType =
        new MediaType("application", "vnd.pg.osasli", Compressible, NotBinary, List("ei6"))
      final lazy val `vnd.piaccess.application-licence`: MediaType =
        new MediaType("application", "vnd.piaccess.application-licence", Compressible, NotBinary)
      final lazy val `vnd.picsel`: MediaType =
        new MediaType("application", "vnd.picsel", Compressible, NotBinary, List("efif"))
      final lazy val `vnd.pmi.widget`: MediaType =
        new MediaType("application", "vnd.pmi.widget", Compressible, NotBinary, List("wg"))
      final lazy val `vnd.poc.group-advertisement+xml`: MediaType =
        new MediaType("application", "vnd.poc.group-advertisement+xml", Compressible, NotBinary)
      final lazy val `vnd.pocketlearn`: MediaType =
        new MediaType("application", "vnd.pocketlearn", Compressible, NotBinary, List("plf"))
      final lazy val `vnd.powerbuilder6`: MediaType =
        new MediaType("application", "vnd.powerbuilder6", Compressible, NotBinary, List("pbd"))
      final lazy val `vnd.powerbuilder6-s`: MediaType =
        new MediaType("application", "vnd.powerbuilder6-s", Compressible, NotBinary)
      final lazy val `vnd.powerbuilder7`: MediaType =
        new MediaType("application", "vnd.powerbuilder7", Compressible, NotBinary)
      final lazy val `vnd.powerbuilder7-s`: MediaType =
        new MediaType("application", "vnd.powerbuilder7-s", Compressible, NotBinary)
      final lazy val `vnd.powerbuilder75`: MediaType =
        new MediaType("application", "vnd.powerbuilder75", Compressible, NotBinary)
      final lazy val `vnd.powerbuilder75-s`: MediaType =
        new MediaType("application", "vnd.powerbuilder75-s", Compressible, NotBinary)
      final lazy val `vnd.preminet`: MediaType =
        new MediaType("application", "vnd.preminet", Compressible, NotBinary)
      final lazy val `vnd.previewsystems.box`: MediaType =
        new MediaType("application", "vnd.previewsystems.box", Compressible, NotBinary, List("box"))
      final lazy val `vnd.proteus.magazine`: MediaType =
        new MediaType("application", "vnd.proteus.magazine", Compressible, NotBinary, List("mgz"))
      final lazy val `vnd.psfs`: MediaType =
        new MediaType("application", "vnd.psfs", Compressible, NotBinary)
      final lazy val `vnd.publishare-delta-tree`: MediaType = new MediaType(
        "application",
        "vnd.publishare-delta-tree",
        Compressible,
        NotBinary,
        List("qps"))
      final lazy val `vnd.pvi.ptid1`: MediaType =
        new MediaType("application", "vnd.pvi.ptid1", Compressible, NotBinary, List("ptid"))
      final lazy val `vnd.pwg-multiplexed`: MediaType =
        new MediaType("application", "vnd.pwg-multiplexed", Compressible, NotBinary)
      final lazy val `vnd.pwg-xhtml-print+xml`: MediaType =
        new MediaType("application", "vnd.pwg-xhtml-print+xml", Compressible, NotBinary)
      final lazy val `vnd.qualcomm.brew-app-res`: MediaType =
        new MediaType("application", "vnd.qualcomm.brew-app-res", Compressible, NotBinary)
      final lazy val `vnd.quarantainenet`: MediaType =
        new MediaType("application", "vnd.quarantainenet", Compressible, NotBinary)
      final lazy val `vnd.quark.quarkxpress`: MediaType = new MediaType(
        "application",
        "vnd.quark.quarkxpress",
        Compressible,
        NotBinary,
        List("qxd", "qxt", "qwd", "qwt", "qxl", "qxb"))
      final lazy val `vnd.quobject-quoxdocument`: MediaType =
        new MediaType("application", "vnd.quobject-quoxdocument", Compressible, NotBinary)
      final lazy val `vnd.radisys.moml+xml`: MediaType =
        new MediaType("application", "vnd.radisys.moml+xml", Compressible, NotBinary)
      final lazy val `vnd.radisys.msml+xml`: MediaType =
        new MediaType("application", "vnd.radisys.msml+xml", Compressible, NotBinary)
      final lazy val `vnd.radisys.msml-audit+xml`: MediaType =
        new MediaType("application", "vnd.radisys.msml-audit+xml", Compressible, NotBinary)
      final lazy val `vnd.radisys.msml-audit-conf+xml`: MediaType =
        new MediaType("application", "vnd.radisys.msml-audit-conf+xml", Compressible, NotBinary)
      final lazy val `vnd.radisys.msml-audit-conn+xml`: MediaType =
        new MediaType("application", "vnd.radisys.msml-audit-conn+xml", Compressible, NotBinary)
      final lazy val `vnd.radisys.msml-audit-dialog+xml`: MediaType =
        new MediaType("application", "vnd.radisys.msml-audit-dialog+xml", Compressible, NotBinary)
      final lazy val `vnd.radisys.msml-audit-stream+xml`: MediaType =
        new MediaType("application", "vnd.radisys.msml-audit-stream+xml", Compressible, NotBinary)
      final lazy val `vnd.radisys.msml-conf+xml`: MediaType =
        new MediaType("application", "vnd.radisys.msml-conf+xml", Compressible, NotBinary)
      final lazy val `vnd.radisys.msml-dialog+xml`: MediaType =
        new MediaType("application", "vnd.radisys.msml-dialog+xml", Compressible, NotBinary)
      final lazy val `vnd.radisys.msml-dialog-base+xml`: MediaType =
        new MediaType("application", "vnd.radisys.msml-dialog-base+xml", Compressible, NotBinary)
      final lazy val `vnd.radisys.msml-dialog-fax-detect+xml`: MediaType = new MediaType(
        "application",
        "vnd.radisys.msml-dialog-fax-detect+xml",
        Compressible,
        NotBinary)
      final lazy val `vnd.radisys.msml-dialog-fax-sendrecv+xml`: MediaType = new MediaType(
        "application",
        "vnd.radisys.msml-dialog-fax-sendrecv+xml",
        Compressible,
        NotBinary)
      final lazy val `vnd.radisys.msml-dialog-group+xml`: MediaType =
        new MediaType("application", "vnd.radisys.msml-dialog-group+xml", Compressible, NotBinary)
      final lazy val `vnd.radisys.msml-dialog-speech+xml`: MediaType =
        new MediaType("application", "vnd.radisys.msml-dialog-speech+xml", Compressible, NotBinary)
      final lazy val `vnd.radisys.msml-dialog-transform+xml`: MediaType = new MediaType(
        "application",
        "vnd.radisys.msml-dialog-transform+xml",
        Compressible,
        NotBinary)
      final lazy val `vnd.rainstor.data`: MediaType =
        new MediaType("application", "vnd.rainstor.data", Compressible, NotBinary)
      final lazy val `vnd.rapid`: MediaType =
        new MediaType("application", "vnd.rapid", Compressible, NotBinary)
      final lazy val `vnd.rar`: MediaType =
        new MediaType("application", "vnd.rar", Compressible, NotBinary)
      final lazy val `vnd.realvnc.bed`: MediaType =
        new MediaType("application", "vnd.realvnc.bed", Compressible, NotBinary, List("bed"))
      final lazy val `vnd.recordare.musicxml`: MediaType =
        new MediaType("application", "vnd.recordare.musicxml", Compressible, NotBinary, List("mxl"))
      final lazy val `vnd.recordare.musicxml+xml`: MediaType = new MediaType(
        "application",
        "vnd.recordare.musicxml+xml",
        Compressible,
        NotBinary,
        List("musicxml"))
      final lazy val `vnd.renlearn.rlprint`: MediaType =
        new MediaType("application", "vnd.renlearn.rlprint", Compressible, NotBinary)
      final lazy val `vnd.restful+json`: MediaType =
        new MediaType("application", "vnd.restful+json", Compressible, NotBinary)
      final lazy val `vnd.rig.cryptonote`: MediaType = new MediaType(
        "application",
        "vnd.rig.cryptonote",
        Compressible,
        NotBinary,
        List("cryptonote"))
      final lazy val `vnd.rim.cod`: MediaType =
        new MediaType("application", "vnd.rim.cod", Compressible, NotBinary, List("cod"))
      final lazy val `vnd.rn-realmedia`: MediaType =
        new MediaType("application", "vnd.rn-realmedia", Compressible, NotBinary, List("rm"))
      final lazy val `vnd.rn-realmedia-vbr`: MediaType =
        new MediaType("application", "vnd.rn-realmedia-vbr", Compressible, NotBinary, List("rmvb"))
      final lazy val `vnd.route66.link66+xml`: MediaType = new MediaType(
        "application",
        "vnd.route66.link66+xml",
        Compressible,
        NotBinary,
        List("link66"))
      final lazy val `vnd.rs-274x`: MediaType =
        new MediaType("application", "vnd.rs-274x", Compressible, NotBinary)
      final lazy val `vnd.ruckus.download`: MediaType =
        new MediaType("application", "vnd.ruckus.download", Compressible, NotBinary)
      final lazy val `vnd.s3sms`: MediaType =
        new MediaType("application", "vnd.s3sms", Compressible, NotBinary)
      final lazy val `vnd.sailingtracker.track`: MediaType = new MediaType(
        "application",
        "vnd.sailingtracker.track",
        Compressible,
        NotBinary,
        List("st"))
      final lazy val `vnd.sbm.cid`: MediaType =
        new MediaType("application", "vnd.sbm.cid", Compressible, NotBinary)
      final lazy val `vnd.sbm.mid2`: MediaType =
        new MediaType("application", "vnd.sbm.mid2", Compressible, NotBinary)
      final lazy val `vnd.scribus`: MediaType =
        new MediaType("application", "vnd.scribus", Compressible, NotBinary)
      final lazy val `vnd.sealed.3df`: MediaType =
        new MediaType("application", "vnd.sealed.3df", Compressible, NotBinary)
      final lazy val `vnd.sealed.csf`: MediaType =
        new MediaType("application", "vnd.sealed.csf", Compressible, NotBinary)
      final lazy val `vnd.sealed.doc`: MediaType =
        new MediaType("application", "vnd.sealed.doc", Compressible, NotBinary)
      final lazy val `vnd.sealed.eml`: MediaType =
        new MediaType("application", "vnd.sealed.eml", Compressible, NotBinary)
      final lazy val `vnd.sealed.mht`: MediaType =
        new MediaType("application", "vnd.sealed.mht", Compressible, NotBinary)
      final lazy val `vnd.sealed.net`: MediaType =
        new MediaType("application", "vnd.sealed.net", Compressible, NotBinary)
      final lazy val `vnd.sealed.ppt`: MediaType =
        new MediaType("application", "vnd.sealed.ppt", Compressible, NotBinary)
      final lazy val `vnd.sealed.tiff`: MediaType =
        new MediaType("application", "vnd.sealed.tiff", Compressible, NotBinary)
      final lazy val `vnd.sealed.xls`: MediaType =
        new MediaType("application", "vnd.sealed.xls", Compressible, NotBinary)
      final lazy val `vnd.sealedmedia.softseal.html`: MediaType =
        new MediaType("application", "vnd.sealedmedia.softseal.html", Compressible, NotBinary)
      final lazy val `vnd.sealedmedia.softseal.pdf`: MediaType =
        new MediaType("application", "vnd.sealedmedia.softseal.pdf", Compressible, NotBinary)
      final lazy val `vnd.seemail`: MediaType =
        new MediaType("application", "vnd.seemail", Compressible, NotBinary, List("see"))
      final lazy val `vnd.sema`: MediaType =
        new MediaType("application", "vnd.sema", Compressible, NotBinary, List("sema"))
      final lazy val `vnd.semd`: MediaType =
        new MediaType("application", "vnd.semd", Compressible, NotBinary, List("semd"))
      final lazy val `vnd.semf`: MediaType =
        new MediaType("application", "vnd.semf", Compressible, NotBinary, List("semf"))
      final lazy val `vnd.shana.informed.formdata`: MediaType = new MediaType(
        "application",
        "vnd.shana.informed.formdata",
        Compressible,
        NotBinary,
        List("ifm"))
      final lazy val `vnd.shana.informed.formtemplate`: MediaType = new MediaType(
        "application",
        "vnd.shana.informed.formtemplate",
        Compressible,
        NotBinary,
        List("itp"))
      final lazy val `vnd.shana.informed.interchange`: MediaType = new MediaType(
        "application",
        "vnd.shana.informed.interchange",
        Compressible,
        NotBinary,
        List("iif"))
      final lazy val `vnd.shana.informed.package`: MediaType = new MediaType(
        "application",
        "vnd.shana.informed.package",
        Compressible,
        NotBinary,
        List("ipk"))
      final lazy val `vnd.shootproof+json`: MediaType =
        new MediaType("application", "vnd.shootproof+json", Compressible, NotBinary)
      final lazy val `vnd.sigrok.session`: MediaType =
        new MediaType("application", "vnd.sigrok.session", Compressible, NotBinary)
      final lazy val `vnd.simtech-mindmapper`: MediaType = new MediaType(
        "application",
        "vnd.simtech-mindmapper",
        Compressible,
        NotBinary,
        List("twd", "twds"))
      final lazy val `vnd.siren+json`: MediaType =
        new MediaType("application", "vnd.siren+json", Compressible, NotBinary)
      final lazy val `vnd.smaf`: MediaType =
        new MediaType("application", "vnd.smaf", Compressible, NotBinary, List("mmf"))
      final lazy val `vnd.smart.notebook`: MediaType =
        new MediaType("application", "vnd.smart.notebook", Compressible, NotBinary)
      final lazy val `vnd.smart.teacher`: MediaType =
        new MediaType("application", "vnd.smart.teacher", Compressible, NotBinary, List("teacher"))
      final lazy val `vnd.software602.filler.form+xml`: MediaType =
        new MediaType("application", "vnd.software602.filler.form+xml", Compressible, NotBinary)
      final lazy val `vnd.software602.filler.form-xml-zip`: MediaType =
        new MediaType("application", "vnd.software602.filler.form-xml-zip", Compressible, NotBinary)
      final lazy val `vnd.solent.sdkm+xml`: MediaType = new MediaType(
        "application",
        "vnd.solent.sdkm+xml",
        Compressible,
        NotBinary,
        List("sdkm", "sdkd"))
      final lazy val `vnd.spotfire.dxp`: MediaType =
        new MediaType("application", "vnd.spotfire.dxp", Compressible, NotBinary, List("dxp"))
      final lazy val `vnd.spotfire.sfs`: MediaType =
        new MediaType("application", "vnd.spotfire.sfs", Compressible, NotBinary, List("sfs"))
      final lazy val `vnd.sqlite3`: MediaType =
        new MediaType("application", "vnd.sqlite3", Compressible, NotBinary)
      final lazy val `vnd.sss-cod`: MediaType =
        new MediaType("application", "vnd.sss-cod", Compressible, NotBinary)
      final lazy val `vnd.sss-dtf`: MediaType =
        new MediaType("application", "vnd.sss-dtf", Compressible, NotBinary)
      final lazy val `vnd.sss-ntf`: MediaType =
        new MediaType("application", "vnd.sss-ntf", Compressible, NotBinary)
      final lazy val `vnd.stardivision.calc`: MediaType =
        new MediaType("application", "vnd.stardivision.calc", Compressible, NotBinary, List("sdc"))
      final lazy val `vnd.stardivision.draw`: MediaType =
        new MediaType("application", "vnd.stardivision.draw", Compressible, NotBinary, List("sda"))
      final lazy val `vnd.stardivision.impress`: MediaType = new MediaType(
        "application",
        "vnd.stardivision.impress",
        Compressible,
        NotBinary,
        List("sdd"))
      final lazy val `vnd.stardivision.math`: MediaType =
        new MediaType("application", "vnd.stardivision.math", Compressible, NotBinary, List("smf"))
      final lazy val `vnd.stardivision.writer`: MediaType = new MediaType(
        "application",
        "vnd.stardivision.writer",
        Compressible,
        NotBinary,
        List("sdw", "vor"))
      final lazy val `vnd.stardivision.writer-global`: MediaType = new MediaType(
        "application",
        "vnd.stardivision.writer-global",
        Compressible,
        NotBinary,
        List("sgl"))
      final lazy val `vnd.stepmania.package`: MediaType = new MediaType(
        "application",
        "vnd.stepmania.package",
        Compressible,
        NotBinary,
        List("smzip"))
      final lazy val `vnd.stepmania.stepchart`: MediaType =
        new MediaType("application", "vnd.stepmania.stepchart", Compressible, NotBinary, List("sm"))
      final lazy val `vnd.street-stream`: MediaType =
        new MediaType("application", "vnd.street-stream", Compressible, NotBinary)
      final lazy val `vnd.sun.wadl+xml`: MediaType =
        new MediaType("application", "vnd.sun.wadl+xml", Compressible, NotBinary, List("wadl"))
      final lazy val `vnd.sun.xml.calc`: MediaType =
        new MediaType("application", "vnd.sun.xml.calc", Compressible, NotBinary, List("sxc"))
      final lazy val `vnd.sun.xml.calc.template`: MediaType = new MediaType(
        "application",
        "vnd.sun.xml.calc.template",
        Compressible,
        NotBinary,
        List("stc"))
      final lazy val `vnd.sun.xml.draw`: MediaType =
        new MediaType("application", "vnd.sun.xml.draw", Compressible, NotBinary, List("sxd"))
      final lazy val `vnd.sun.xml.draw.template`: MediaType = new MediaType(
        "application",
        "vnd.sun.xml.draw.template",
        Compressible,
        NotBinary,
        List("std"))
      final lazy val `vnd.sun.xml.impress`: MediaType =
        new MediaType("application", "vnd.sun.xml.impress", Compressible, NotBinary, List("sxi"))
      final lazy val `vnd.sun.xml.impress.template`: MediaType = new MediaType(
        "application",
        "vnd.sun.xml.impress.template",
        Compressible,
        NotBinary,
        List("sti"))
      final lazy val `vnd.sun.xml.math`: MediaType =
        new MediaType("application", "vnd.sun.xml.math", Compressible, NotBinary, List("sxm"))
      final lazy val `vnd.sun.xml.writer`: MediaType =
        new MediaType("application", "vnd.sun.xml.writer", Compressible, NotBinary, List("sxw"))
      final lazy val `vnd.sun.xml.writer.global`: MediaType = new MediaType(
        "application",
        "vnd.sun.xml.writer.global",
        Compressible,
        NotBinary,
        List("sxg"))
      final lazy val `vnd.sun.xml.writer.template`: MediaType = new MediaType(
        "application",
        "vnd.sun.xml.writer.template",
        Compressible,
        NotBinary,
        List("stw"))
      final lazy val `vnd.sus-calendar`: MediaType = new MediaType(
        "application",
        "vnd.sus-calendar",
        Compressible,
        NotBinary,
        List("sus", "susp"))
      final lazy val `vnd.svd`: MediaType =
        new MediaType("application", "vnd.svd", Compressible, NotBinary, List("svd"))
      final lazy val `vnd.swiftview-ics`: MediaType =
        new MediaType("application", "vnd.swiftview-ics", Compressible, NotBinary)
      final lazy val `vnd.symbian.install`: MediaType = new MediaType(
        "application",
        "vnd.symbian.install",
        Compressible,
        NotBinary,
        List("sis", "sisx"))
      final lazy val `vnd.syncml+xml`: MediaType =
        new MediaType("application", "vnd.syncml+xml", Compressible, NotBinary, List("xsm"))
      final lazy val `vnd.syncml.dm+wbxml`: MediaType =
        new MediaType("application", "vnd.syncml.dm+wbxml", Compressible, NotBinary, List("bdm"))
      final lazy val `vnd.syncml.dm+xml`: MediaType =
        new MediaType("application", "vnd.syncml.dm+xml", Compressible, NotBinary, List("xdm"))
      final lazy val `vnd.syncml.dm.notification`: MediaType =
        new MediaType("application", "vnd.syncml.dm.notification", Compressible, NotBinary)
      final lazy val `vnd.syncml.dmddf+wbxml`: MediaType =
        new MediaType("application", "vnd.syncml.dmddf+wbxml", Compressible, NotBinary)
      final lazy val `vnd.syncml.dmddf+xml`: MediaType =
        new MediaType("application", "vnd.syncml.dmddf+xml", Compressible, NotBinary)
      final lazy val `vnd.syncml.dmtnds+wbxml`: MediaType =
        new MediaType("application", "vnd.syncml.dmtnds+wbxml", Compressible, NotBinary)
      final lazy val `vnd.syncml.dmtnds+xml`: MediaType =
        new MediaType("application", "vnd.syncml.dmtnds+xml", Compressible, NotBinary)
      final lazy val `vnd.syncml.ds.notification`: MediaType =
        new MediaType("application", "vnd.syncml.ds.notification", Compressible, NotBinary)
      final lazy val `vnd.tableschema+json`: MediaType =
        new MediaType("application", "vnd.tableschema+json", Compressible, NotBinary)
      final lazy val `vnd.tao.intent-module-archive`: MediaType = new MediaType(
        "application",
        "vnd.tao.intent-module-archive",
        Compressible,
        NotBinary,
        List("tao"))
      final lazy val `vnd.tcpdump.pcap`: MediaType = new MediaType(
        "application",
        "vnd.tcpdump.pcap",
        Compressible,
        NotBinary,
        List("pcap", "cap", "dmp"))
      final lazy val `vnd.think-cell.ppttc+json`: MediaType =
        new MediaType("application", "vnd.think-cell.ppttc+json", Compressible, NotBinary)
      final lazy val `vnd.tmd.mediaflex.api+xml`: MediaType =
        new MediaType("application", "vnd.tmd.mediaflex.api+xml", Compressible, NotBinary)
      final lazy val `vnd.tml`: MediaType =
        new MediaType("application", "vnd.tml", Compressible, NotBinary)
      final lazy val `vnd.tmobile-livetv`: MediaType =
        new MediaType("application", "vnd.tmobile-livetv", Compressible, NotBinary, List("tmo"))
      final lazy val `vnd.tri.onesource`: MediaType =
        new MediaType("application", "vnd.tri.onesource", Compressible, NotBinary)
      final lazy val `vnd.trid.tpt`: MediaType =
        new MediaType("application", "vnd.trid.tpt", Compressible, NotBinary, List("tpt"))
      final lazy val `vnd.triscape.mxs`: MediaType =
        new MediaType("application", "vnd.triscape.mxs", Compressible, NotBinary, List("mxs"))
      final lazy val `vnd.trueapp`: MediaType =
        new MediaType("application", "vnd.trueapp", Compressible, NotBinary, List("tra"))
      final lazy val `vnd.truedoc`: MediaType =
        new MediaType("application", "vnd.truedoc", Compressible, NotBinary)
      final lazy val `vnd.ubisoft.webplayer`: MediaType =
        new MediaType("application", "vnd.ubisoft.webplayer", Compressible, NotBinary)
      final lazy val `vnd.ufdl`: MediaType =
        new MediaType("application", "vnd.ufdl", Compressible, NotBinary, List("ufd", "ufdl"))
      final lazy val `vnd.uiq.theme`: MediaType =
        new MediaType("application", "vnd.uiq.theme", Compressible, NotBinary, List("utz"))
      final lazy val `vnd.umajin`: MediaType =
        new MediaType("application", "vnd.umajin", Compressible, NotBinary, List("umj"))
      final lazy val `vnd.unity`: MediaType =
        new MediaType("application", "vnd.unity", Compressible, NotBinary, List("unityweb"))
      final lazy val `vnd.uoml+xml`: MediaType =
        new MediaType("application", "vnd.uoml+xml", Compressible, NotBinary, List("uoml"))
      final lazy val `vnd.uplanet.alert`: MediaType =
        new MediaType("application", "vnd.uplanet.alert", Compressible, NotBinary)
      final lazy val `vnd.uplanet.alert-wbxml`: MediaType =
        new MediaType("application", "vnd.uplanet.alert-wbxml", Compressible, NotBinary)
      final lazy val `vnd.uplanet.bearer-choice`: MediaType =
        new MediaType("application", "vnd.uplanet.bearer-choice", Compressible, NotBinary)
      final lazy val `vnd.uplanet.bearer-choice-wbxml`: MediaType =
        new MediaType("application", "vnd.uplanet.bearer-choice-wbxml", Compressible, NotBinary)
      final lazy val `vnd.uplanet.cacheop`: MediaType =
        new MediaType("application", "vnd.uplanet.cacheop", Compressible, NotBinary)
      final lazy val `vnd.uplanet.cacheop-wbxml`: MediaType =
        new MediaType("application", "vnd.uplanet.cacheop-wbxml", Compressible, NotBinary)
      final lazy val `vnd.uplanet.channel`: MediaType =
        new MediaType("application", "vnd.uplanet.channel", Compressible, NotBinary)
      final lazy val `vnd.uplanet.channel-wbxml`: MediaType =
        new MediaType("application", "vnd.uplanet.channel-wbxml", Compressible, NotBinary)
      final lazy val `vnd.uplanet.list`: MediaType =
        new MediaType("application", "vnd.uplanet.list", Compressible, NotBinary)
      final lazy val `vnd.uplanet.list-wbxml`: MediaType =
        new MediaType("application", "vnd.uplanet.list-wbxml", Compressible, NotBinary)
      final lazy val `vnd.uplanet.listcmd`: MediaType =
        new MediaType("application", "vnd.uplanet.listcmd", Compressible, NotBinary)
      final lazy val `vnd.uplanet.listcmd-wbxml`: MediaType =
        new MediaType("application", "vnd.uplanet.listcmd-wbxml", Compressible, NotBinary)
      final lazy val `vnd.uplanet.signal`: MediaType =
        new MediaType("application", "vnd.uplanet.signal", Compressible, NotBinary)
      final lazy val `vnd.uri-map`: MediaType =
        new MediaType("application", "vnd.uri-map", Compressible, NotBinary)
      final lazy val `vnd.valve.source.material`: MediaType =
        new MediaType("application", "vnd.valve.source.material", Compressible, NotBinary)
      final lazy val `vnd.vcx`: MediaType =
        new MediaType("application", "vnd.vcx", Compressible, NotBinary, List("vcx"))
      final lazy val `vnd.vd-study`: MediaType =
        new MediaType("application", "vnd.vd-study", Compressible, NotBinary)
      final lazy val `vnd.vectorworks`: MediaType =
        new MediaType("application", "vnd.vectorworks", Compressible, NotBinary)
      final lazy val `vnd.vel+json`: MediaType =
        new MediaType("application", "vnd.vel+json", Compressible, NotBinary)
      final lazy val `vnd.verimatrix.vcas`: MediaType =
        new MediaType("application", "vnd.verimatrix.vcas", Compressible, NotBinary)
      final lazy val `vnd.vidsoft.vidconference`: MediaType =
        new MediaType("application", "vnd.vidsoft.vidconference", Compressible, NotBinary)
      final lazy val `vnd.visio`: MediaType = new MediaType(
        "application",
        "vnd.visio",
        Compressible,
        NotBinary,
        List("vsd", "vst", "vss", "vsw"))
      final lazy val `vnd.visionary`: MediaType =
        new MediaType("application", "vnd.visionary", Compressible, NotBinary, List("vis"))
      final lazy val `vnd.vividence.scriptfile`: MediaType =
        new MediaType("application", "vnd.vividence.scriptfile", Compressible, NotBinary)
      final lazy val `vnd.vsf`: MediaType =
        new MediaType("application", "vnd.vsf", Compressible, NotBinary, List("vsf"))
      final lazy val `vnd.wap.sic`: MediaType =
        new MediaType("application", "vnd.wap.sic", Compressible, NotBinary)
      final lazy val `vnd.wap.slc`: MediaType =
        new MediaType("application", "vnd.wap.slc", Compressible, NotBinary)
      final lazy val `vnd.wap.wbxml`: MediaType =
        new MediaType("application", "vnd.wap.wbxml", Compressible, NotBinary, List("wbxml"))
      final lazy val `vnd.wap.wmlc`: MediaType =
        new MediaType("application", "vnd.wap.wmlc", Compressible, NotBinary, List("wmlc"))
      final lazy val `vnd.wap.wmlscriptc`: MediaType =
        new MediaType("application", "vnd.wap.wmlscriptc", Compressible, NotBinary, List("wmlsc"))
      final lazy val `vnd.webturbo`: MediaType =
        new MediaType("application", "vnd.webturbo", Compressible, NotBinary, List("wtb"))
      final lazy val `vnd.wfa.p2p`: MediaType =
        new MediaType("application", "vnd.wfa.p2p", Compressible, NotBinary)
      final lazy val `vnd.wfa.wsc`: MediaType =
        new MediaType("application", "vnd.wfa.wsc", Compressible, NotBinary)
      final lazy val `vnd.windows.devicepairing`: MediaType =
        new MediaType("application", "vnd.windows.devicepairing", Compressible, NotBinary)
      final lazy val `vnd.wmc`: MediaType =
        new MediaType("application", "vnd.wmc", Compressible, NotBinary)
      final lazy val `vnd.wmf.bootstrap`: MediaType =
        new MediaType("application", "vnd.wmf.bootstrap", Compressible, NotBinary)
      final lazy val `vnd.wolfram.mathematica`: MediaType =
        new MediaType("application", "vnd.wolfram.mathematica", Compressible, NotBinary)
      final lazy val `vnd.wolfram.mathematica.package`: MediaType =
        new MediaType("application", "vnd.wolfram.mathematica.package", Compressible, NotBinary)
      final lazy val `vnd.wolfram.player`: MediaType =
        new MediaType("application", "vnd.wolfram.player", Compressible, NotBinary, List("nbp"))
      final lazy val `vnd.wordperfect`: MediaType =
        new MediaType("application", "vnd.wordperfect", Compressible, NotBinary, List("wpd"))
      final lazy val `vnd.wqd`: MediaType =
        new MediaType("application", "vnd.wqd", Compressible, NotBinary, List("wqd"))
      final lazy val `vnd.wrq-hp3000-labelled`: MediaType =
        new MediaType("application", "vnd.wrq-hp3000-labelled", Compressible, NotBinary)
      final lazy val `vnd.wt.stf`: MediaType =
        new MediaType("application", "vnd.wt.stf", Compressible, NotBinary, List("stf"))
      final lazy val `vnd.wv.csp+wbxml`: MediaType =
        new MediaType("application", "vnd.wv.csp+wbxml", Compressible, NotBinary)
      final lazy val `vnd.wv.csp+xml`: MediaType =
        new MediaType("application", "vnd.wv.csp+xml", Compressible, NotBinary)
      final lazy val `vnd.wv.ssp+xml`: MediaType =
        new MediaType("application", "vnd.wv.ssp+xml", Compressible, NotBinary)
      final lazy val `vnd.xacml+json`: MediaType =
        new MediaType("application", "vnd.xacml+json", Compressible, NotBinary)
      final lazy val `vnd.xara`: MediaType =
        new MediaType("application", "vnd.xara", Compressible, NotBinary, List("xar"))
      final lazy val `vnd.xfdl`: MediaType =
        new MediaType("application", "vnd.xfdl", Compressible, NotBinary, List("xfdl"))
      final lazy val `vnd.xfdl.webform`: MediaType =
        new MediaType("application", "vnd.xfdl.webform", Compressible, NotBinary)
      final lazy val `vnd.xmi+xml`: MediaType =
        new MediaType("application", "vnd.xmi+xml", Compressible, NotBinary)
      final lazy val `vnd.xmpie.cpkg`: MediaType =
        new MediaType("application", "vnd.xmpie.cpkg", Compressible, NotBinary)
      final lazy val `vnd.xmpie.dpkg`: MediaType =
        new MediaType("application", "vnd.xmpie.dpkg", Compressible, NotBinary)
      final lazy val `vnd.xmpie.plan`: MediaType =
        new MediaType("application", "vnd.xmpie.plan", Compressible, NotBinary)
      final lazy val `vnd.xmpie.ppkg`: MediaType =
        new MediaType("application", "vnd.xmpie.ppkg", Compressible, NotBinary)
      final lazy val `vnd.xmpie.xlim`: MediaType =
        new MediaType("application", "vnd.xmpie.xlim", Compressible, NotBinary)
      final lazy val `vnd.yamaha.hv-dic`: MediaType =
        new MediaType("application", "vnd.yamaha.hv-dic", Compressible, NotBinary, List("hvd"))
      final lazy val `vnd.yamaha.hv-script`: MediaType =
        new MediaType("application", "vnd.yamaha.hv-script", Compressible, NotBinary, List("hvs"))
      final lazy val `vnd.yamaha.hv-voice`: MediaType =
        new MediaType("application", "vnd.yamaha.hv-voice", Compressible, NotBinary, List("hvp"))
      final lazy val `vnd.yamaha.openscoreformat`: MediaType = new MediaType(
        "application",
        "vnd.yamaha.openscoreformat",
        Compressible,
        NotBinary,
        List("osf"))
      final lazy val `vnd.yamaha.openscoreformat.osfpvg+xml`: MediaType = new MediaType(
        "application",
        "vnd.yamaha.openscoreformat.osfpvg+xml",
        Compressible,
        NotBinary,
        List("osfpvg"))
      final lazy val `vnd.yamaha.remote-setup`: MediaType =
        new MediaType("application", "vnd.yamaha.remote-setup", Compressible, NotBinary)
      final lazy val `vnd.yamaha.smaf-audio`: MediaType =
        new MediaType("application", "vnd.yamaha.smaf-audio", Compressible, NotBinary, List("saf"))
      final lazy val `vnd.yamaha.smaf-phrase`: MediaType =
        new MediaType("application", "vnd.yamaha.smaf-phrase", Compressible, NotBinary, List("spf"))
      final lazy val `vnd.yamaha.through-ngn`: MediaType =
        new MediaType("application", "vnd.yamaha.through-ngn", Compressible, NotBinary)
      final lazy val `vnd.yamaha.tunnel-udpencap`: MediaType =
        new MediaType("application", "vnd.yamaha.tunnel-udpencap", Compressible, NotBinary)
      final lazy val `vnd.yaoweme`: MediaType =
        new MediaType("application", "vnd.yaoweme", Compressible, NotBinary)
      final lazy val `vnd.yellowriver-custom-menu`: MediaType = new MediaType(
        "application",
        "vnd.yellowriver-custom-menu",
        Compressible,
        NotBinary,
        List("cmp"))
      final lazy val `vnd.youtube.yt`: MediaType =
        new MediaType("application", "vnd.youtube.yt", Compressible, NotBinary)
      final lazy val `vnd.zul`: MediaType =
        new MediaType("application", "vnd.zul", Compressible, NotBinary, List("zir", "zirz"))
      final lazy val `vnd.zzazz.deck+xml`: MediaType =
        new MediaType("application", "vnd.zzazz.deck+xml", Compressible, NotBinary, List("zaz"))
      final lazy val `voicexml+xml`: MediaType =
        new MediaType("application", "voicexml+xml", Compressible, NotBinary, List("vxml"))
      final lazy val `voucher-cms+json`: MediaType =
        new MediaType("application", "voucher-cms+json", Compressible, NotBinary)
      final lazy val `vq-rtcpxr`: MediaType =
        new MediaType("application", "vq-rtcpxr", Compressible, NotBinary)
      final lazy val `wasm`: MediaType =
        new MediaType("application", "wasm", Compressible, NotBinary, List("wasm"))
      final lazy val `watcherinfo+xml`: MediaType =
        new MediaType("application", "watcherinfo+xml", Compressible, NotBinary)
      final lazy val `webpush-options+json`: MediaType =
        new MediaType("application", "webpush-options+json", Compressible, NotBinary)
      final lazy val `whoispp-query`: MediaType =
        new MediaType("application", "whoispp-query", Compressible, NotBinary)
      final lazy val `whoispp-response`: MediaType =
        new MediaType("application", "whoispp-response", Compressible, NotBinary)
      final lazy val `widget`: MediaType =
        new MediaType("application", "widget", Compressible, NotBinary, List("wgt"))
      final lazy val `winhlp`: MediaType =
        new MediaType("application", "winhlp", Compressible, NotBinary, List("hlp"))
      final lazy val `wita`: MediaType = new MediaType("application", "wita", Compressible, NotBinary)
      final lazy val `wordperfect5.1`: MediaType =
        new MediaType("application", "wordperfect5.1", Compressible, NotBinary)
      final lazy val `wsdl+xml`: MediaType =
        new MediaType("application", "wsdl+xml", Compressible, NotBinary, List("wsdl"))
      final lazy val `wspolicy+xml`: MediaType =
        new MediaType("application", "wspolicy+xml", Compressible, NotBinary, List("wspolicy"))
      final lazy val `x-7z-compressed`: MediaType =
        new MediaType("application", "x-7z-compressed", Uncompressible, Binary, List("7z"))
      final lazy val `x-abiword`: MediaType =
        new MediaType("application", "x-abiword", Compressible, NotBinary, List("abw"))
      final lazy val `x-ace-compressed`: MediaType =
        new MediaType("application", "x-ace-compressed", Compressible, Binary, List("ace"))
      final lazy val `x-amf`: MediaType = new MediaType("application", "x-amf", Compressible, NotBinary)
      final lazy val `x-apple-diskimage`: MediaType =
        new MediaType("application", "x-apple-diskimage", Compressible, Binary, List("dmg"))
      final lazy val `x-arj`: MediaType =
        new MediaType("application", "x-arj", Uncompressible, NotBinary, List("arj"))
      final lazy val `x-authorware-bin`: MediaType = new MediaType(
        "application",
        "x-authorware-bin",
        Compressible,
        NotBinary,
        List("aab", "x32", "u32", "vox"))
      final lazy val `x-authorware-map`: MediaType =
        new MediaType("application", "x-authorware-map", Compressible, NotBinary, List("aam"))
      final lazy val `x-authorware-seg`: MediaType =
        new MediaType("application", "x-authorware-seg", Compressible, NotBinary, List("aas"))
      final lazy val `x-bcpio`: MediaType =
        new MediaType("application", "x-bcpio", Compressible, NotBinary, List("bcpio"))
      final lazy val `x-bdoc`: MediaType =
        new MediaType("application", "x-bdoc", Uncompressible, NotBinary, List("bdoc"))
      final lazy val `x-bittorrent`: MediaType =
        new MediaType("application", "x-bittorrent", Compressible, NotBinary, List("torrent"))
      final lazy val `x-blorb`: MediaType =
        new MediaType("application", "x-blorb", Compressible, NotBinary, List("blb", "blorb"))
      final lazy val `x-bzip`: MediaType =
        new MediaType("application", "x-bzip", Uncompressible, Binary, List("bz"))
      final lazy val `x-bzip2`: MediaType =
        new MediaType("application", "x-bzip2", Uncompressible, Binary, List("bz2", "boz"))
      final lazy val `x-cbr`: MediaType = new MediaType(
        "application",
        "x-cbr",
        Compressible,
        NotBinary,
        List("cbr", "cba", "cbt", "cbz", "cb7"))
      final lazy val `x-cdlink`: MediaType =
        new MediaType("application", "x-cdlink", Compressible, NotBinary, List("vcd"))
      final lazy val `x-cfs-compressed`: MediaType =
        new MediaType("application", "x-cfs-compressed", Compressible, NotBinary, List("cfs"))
      final lazy val `x-chat`: MediaType =
        new MediaType("application", "x-chat", Compressible, NotBinary, List("chat"))
      final lazy val `x-chess-pgn`: MediaType =
        new MediaType("application", "x-chess-pgn", Compressible, NotBinary, List("pgn"))
      final lazy val `x-chrome-extension`: MediaType =
        new MediaType("application", "x-chrome-extension", Compressible, Binary, List("crx"))
      final lazy val `x-cocoa`: MediaType =
        new MediaType("application", "x-cocoa", Compressible, NotBinary, List("cco"))
      final lazy val `x-compress`: MediaType =
        new MediaType("application", "x-compress", Compressible, Binary)
      final lazy val `x-conference`: MediaType =
        new MediaType("application", "x-conference", Compressible, NotBinary, List("nsc"))
      final lazy val `x-cpio`: MediaType =
        new MediaType("application", "x-cpio", Compressible, NotBinary, List("cpio"))
      final lazy val `x-csh`: MediaType =
        new MediaType("application", "x-csh", Compressible, NotBinary, List("csh"))
      final lazy val `x-deb`: MediaType = new MediaType("application", "x-deb", Uncompressible, NotBinary)
      final lazy val `x-debian-package`: MediaType =
        new MediaType("application", "x-debian-package", Compressible, Binary, List("deb", "udeb"))
      final lazy val `x-dgc-compressed`: MediaType =
        new MediaType("application", "x-dgc-compressed", Compressible, NotBinary, List("dgc"))
      final lazy val `x-director`: MediaType = new MediaType(
        "application",
        "x-director",
        Compressible,
        NotBinary,
        List("dir", "dcr", "dxr", "cst", "cct", "cxt", "w3d", "fgd", "swa"))
      final lazy val `x-doom`: MediaType =
        new MediaType("application", "x-doom", Compressible, NotBinary, List("wad"))
      final lazy val `x-dtbncx+xml`: MediaType =
        new MediaType("application", "x-dtbncx+xml", Compressible, NotBinary, List("ncx"))
      final lazy val `x-dtbook+xml`: MediaType =
        new MediaType("application", "x-dtbook+xml", Compressible, NotBinary, List("dtb"))
      final lazy val `x-dtbresource+xml`: MediaType =
        new MediaType("application", "x-dtbresource+xml", Compressible, NotBinary, List("res"))
      final lazy val `x-dvi`: MediaType =
        new MediaType("application", "x-dvi", Uncompressible, Binary, List("dvi"))
      final lazy val `x-envoy`: MediaType =
        new MediaType("application", "x-envoy", Compressible, NotBinary, List("evy"))
      final lazy val `x-eva`: MediaType =
        new MediaType("application", "x-eva", Compressible, NotBinary, List("eva"))
      final lazy val `x-font-bdf`: MediaType =
        new MediaType("application", "x-font-bdf", Compressible, NotBinary, List("bdf"))
      final lazy val `x-font-dos`: MediaType =
        new MediaType("application", "x-font-dos", Compressible, NotBinary)
      final lazy val `x-font-framemaker`: MediaType =
        new MediaType("application", "x-font-framemaker", Compressible, NotBinary)
      final lazy val `x-font-ghostscript`: MediaType =
        new MediaType("application", "x-font-ghostscript", Compressible, NotBinary, List("gsf"))
      final lazy val `x-font-libgrx`: MediaType =
        new MediaType("application", "x-font-libgrx", Compressible, NotBinary)
      final lazy val `x-font-linux-psf`: MediaType =
        new MediaType("application", "x-font-linux-psf", Compressible, NotBinary, List("psf"))
      final lazy val `x-font-pcf`: MediaType =
        new MediaType("application", "x-font-pcf", Compressible, NotBinary, List("pcf"))
      final lazy val `x-font-snf`: MediaType =
        new MediaType("application", "x-font-snf", Compressible, NotBinary, List("snf"))
      final lazy val `x-font-speedo`: MediaType =
        new MediaType("application", "x-font-speedo", Compressible, NotBinary)
      final lazy val `x-font-sunos-news`: MediaType =
        new MediaType("application", "x-font-sunos-news", Compressible, NotBinary)
      final lazy val `x-font-type1`: MediaType = new MediaType(
        "application",
        "x-font-type1",
        Compressible,
        NotBinary,
        List("pfa", "pfb", "pfm", "afm"))
      final lazy val `x-font-vfont`: MediaType =
        new MediaType("application", "x-font-vfont", Compressible, NotBinary)
      final lazy val `x-freearc`: MediaType =
        new MediaType("application", "x-freearc", Compressible, NotBinary, List("arc"))
      final lazy val `x-futuresplash`: MediaType =
        new MediaType("application", "x-futuresplash", Compressible, NotBinary, List("spl"))
      final lazy val `x-gca-compressed`: MediaType =
        new MediaType("application", "x-gca-compressed", Compressible, NotBinary, List("gca"))
      final lazy val `x-glulx`: MediaType =
        new MediaType("application", "x-glulx", Compressible, NotBinary, List("ulx"))
      final lazy val `x-gnumeric`: MediaType =
        new MediaType("application", "x-gnumeric", Compressible, NotBinary, List("gnumeric"))
      final lazy val `x-gramps-xml`: MediaType =
        new MediaType("application", "x-gramps-xml", Compressible, NotBinary, List("gramps"))
      final lazy val `x-gtar`: MediaType =
        new MediaType("application", "x-gtar", Compressible, Binary, List("gtar"))
      final lazy val `x-gzip`: MediaType = new MediaType("application", "x-gzip", Compressible, Binary)
      final lazy val `x-hdf`: MediaType =
        new MediaType("application", "x-hdf", Compressible, NotBinary, List("hdf"))
      final lazy val `x-httpd-php`: MediaType =
        new MediaType("application", "x-httpd-php", Compressible, NotBinary, List("php"))
      final lazy val `x-install-instructions`: MediaType = new MediaType(
        "application",
        "x-install-instructions",
        Compressible,
        NotBinary,
        List("install"))
      final lazy val `x-iso9660-image`: MediaType =
        new MediaType("application", "x-iso9660-image", Compressible, NotBinary, List("iso"))
      final lazy val `x-java-archive-diff`: MediaType = new MediaType(
        "application",
        "x-java-archive-diff",
        Compressible,
        NotBinary,
        List("jardiff"))
      final lazy val `x-java-jnlp-file`: MediaType =
        new MediaType("application", "x-java-jnlp-file", Uncompressible, NotBinary, List("jnlp"))
      final lazy val `x-javascript`: MediaType =
        new MediaType("application", "x-javascript", Compressible, NotBinary)
      final lazy val `x-latex`: MediaType =
        new MediaType("application", "x-latex", Uncompressible, Binary, List("latex"))
      final lazy val `x-lua-bytecode`: MediaType =
        new MediaType("application", "x-lua-bytecode", Compressible, NotBinary, List("luac"))
      final lazy val `x-lzh-compressed`: MediaType = new MediaType(
        "application",
        "x-lzh-compressed",
        Compressible,
        NotBinary,
        List("lzh", "lha"))
      final lazy val `x-makeself`: MediaType =
        new MediaType("application", "x-makeself", Compressible, NotBinary, List("run"))
      final lazy val `x-mie`: MediaType =
        new MediaType("application", "x-mie", Compressible, NotBinary, List("mie"))
      final lazy val `x-mobipocket-ebook`: MediaType = new MediaType(
        "application",
        "x-mobipocket-ebook",
        Compressible,
        NotBinary,
        List("prc", "mobi"))
      final lazy val `x-mpegurl`: MediaType =
        new MediaType("application", "x-mpegurl", Uncompressible, NotBinary)
      final lazy val `x-ms-application`: MediaType = new MediaType(
        "application",
        "x-ms-application",
        Compressible,
        NotBinary,
        List("application"))
      final lazy val `x-ms-shortcut`: MediaType =
        new MediaType("application", "x-ms-shortcut", Compressible, NotBinary, List("lnk"))
      final lazy val `x-ms-wmd`: MediaType =
        new MediaType("application", "x-ms-wmd", Compressible, NotBinary, List("wmd"))
      final lazy val `x-ms-wmz`: MediaType =
        new MediaType("application", "x-ms-wmz", Compressible, NotBinary, List("wmz"))
      final lazy val `x-ms-xbap`: MediaType =
        new MediaType("application", "x-ms-xbap", Compressible, NotBinary, List("xbap"))
      final lazy val `x-msaccess`: MediaType =
        new MediaType("application", "x-msaccess", Compressible, NotBinary, List("mdb"))
      final lazy val `x-msbinder`: MediaType =
        new MediaType("application", "x-msbinder", Compressible, NotBinary, List("obd"))
      final lazy val `x-mscardfile`: MediaType =
        new MediaType("application", "x-mscardfile", Compressible, NotBinary, List("crd"))
      final lazy val `x-msclip`: MediaType =
        new MediaType("application", "x-msclip", Compressible, NotBinary, List("clp"))
      final lazy val `x-msdos-program`: MediaType =
        new MediaType("application", "x-msdos-program", Compressible, NotBinary, List("exe"))
      final lazy val `x-msdownload`: MediaType = new MediaType(
        "application",
        "x-msdownload",
        Compressible,
        NotBinary,
        List("exe", "dll", "com", "bat", "msi"))
      final lazy val `x-msmediaview`: MediaType = new MediaType(
        "application",
        "x-msmediaview",
        Compressible,
        NotBinary,
        List("mvb", "m13", "m14"))
      final lazy val `x-msmetafile`: MediaType = new MediaType(
        "application",
        "x-msmetafile",
        Compressible,
        NotBinary,
        List("wmf", "wmz", "emf", "emz"))
      final lazy val `x-msmoney`: MediaType =
        new MediaType("application", "x-msmoney", Compressible, NotBinary, List("mny"))
      final lazy val `x-mspublisher`: MediaType =
        new MediaType("application", "x-mspublisher", Compressible, NotBinary, List("pub"))
      final lazy val `x-msschedule`: MediaType =
        new MediaType("application", "x-msschedule", Compressible, NotBinary, List("scd"))
      final lazy val `x-msterminal`: MediaType =
        new MediaType("application", "x-msterminal", Compressible, NotBinary, List("trm"))
      final lazy val `x-mswrite`: MediaType =
        new MediaType("application", "x-mswrite", Compressible, NotBinary, List("wri"))
      final lazy val `x-netcdf`: MediaType =
        new MediaType("application", "x-netcdf", Compressible, NotBinary, List("nc", "cdf"))
      final lazy val `x-ns-proxy-autoconfig`: MediaType =
        new MediaType("application", "x-ns-proxy-autoconfig", Compressible, NotBinary, List("pac"))
      final lazy val `x-nzb`: MediaType =
        new MediaType("application", "x-nzb", Compressible, NotBinary, List("nzb"))
      final lazy val `x-perl`: MediaType =
        new MediaType("application", "x-perl", Compressible, NotBinary, List("pl", "pm"))
      final lazy val `x-pilot`: MediaType =
        new MediaType("application", "x-pilot", Compressible, NotBinary, List("prc", "pdb"))
      final lazy val `x-pkcs12`: MediaType =
        new MediaType("application", "x-pkcs12", Uncompressible, NotBinary, List("p12", "pfx"))
      final lazy val `x-pkcs7-certificates`: MediaType = new MediaType(
        "application",
        "x-pkcs7-certificates",
        Compressible,
        NotBinary,
        List("p7b", "spc"))
      final lazy val `x-pkcs7-certreqresp`: MediaType =
        new MediaType("application", "x-pkcs7-certreqresp", Compressible, NotBinary, List("p7r"))
      final lazy val `x-rar-compressed`: MediaType =
        new MediaType("application", "x-rar-compressed", Uncompressible, Binary, List("rar"))
      final lazy val `x-redhat-package-manager`: MediaType =
        new MediaType("application", "x-redhat-package-manager", Compressible, Binary, List("rpm"))
      final lazy val `x-research-info-systems`: MediaType = new MediaType(
        "application",
        "x-research-info-systems",
        Compressible,
        NotBinary,
        List("ris"))
      final lazy val `x-sea`: MediaType =
        new MediaType("application", "x-sea", Compressible, NotBinary, List("sea"))
      final lazy val `x-sh`: MediaType =
        new MediaType("application", "x-sh", Compressible, NotBinary, List("sh"))
      final lazy val `x-shar`: MediaType =
        new MediaType("application", "x-shar", Compressible, NotBinary, List("shar"))
      final lazy val `x-shockwave-flash`: MediaType =
        new MediaType("application", "x-shockwave-flash", Uncompressible, Binary, List("swf"))
      final lazy val `x-silverlight-app`: MediaType =
        new MediaType("application", "x-silverlight-app", Compressible, NotBinary, List("xap"))
      final lazy val `x-sql`: MediaType =
        new MediaType("application", "x-sql", Compressible, NotBinary, List("sql"))
      final lazy val `x-stuffit`: MediaType =
        new MediaType("application", "x-stuffit", Uncompressible, NotBinary, List("sit"))
      final lazy val `x-stuffitx`: MediaType =
        new MediaType("application", "x-stuffitx", Compressible, NotBinary, List("sitx"))
      final lazy val `x-subrip`: MediaType =
        new MediaType("application", "x-subrip", Compressible, NotBinary, List("srt"))
      final lazy val `x-sv4cpio`: MediaType =
        new MediaType("application", "x-sv4cpio", Compressible, NotBinary, List("sv4cpio"))
      final lazy val `x-sv4crc`: MediaType =
        new MediaType("application", "x-sv4crc", Compressible, NotBinary, List("sv4crc"))
      final lazy val `x-t3vm-image`: MediaType =
        new MediaType("application", "x-t3vm-image", Compressible, NotBinary, List("t3"))
      final lazy val `x-tads`: MediaType =
        new MediaType("application", "x-tads", Compressible, NotBinary, List("gam"))
      final lazy val `x-tar`: MediaType =
        new MediaType("application", "x-tar", Compressible, Binary, List("tar"))
      final lazy val `x-tcl`: MediaType =
        new MediaType("application", "x-tcl", Compressible, NotBinary, List("tcl", "tk"))
      final lazy val `x-tex`: MediaType =
        new MediaType("application", "x-tex", Compressible, Binary, List("tex"))
      final lazy val `x-tex-tfm`: MediaType =
        new MediaType("application", "x-tex-tfm", Compressible, NotBinary, List("tfm"))
      final lazy val `x-texinfo`: MediaType =
        new MediaType("application", "x-texinfo", Compressible, Binary, List("texinfo", "texi"))
      final lazy val `x-tgif`: MediaType =
        new MediaType("application", "x-tgif", Compressible, NotBinary, List("obj"))
      final lazy val `x-ustar`: MediaType =
        new MediaType("application", "x-ustar", Compressible, NotBinary, List("ustar"))
      final lazy val `x-virtualbox-hdd`: MediaType =
        new MediaType("application", "x-virtualbox-hdd", Compressible, NotBinary, List("hdd"))
      final lazy val `x-virtualbox-ova`: MediaType =
        new MediaType("application", "x-virtualbox-ova", Compressible, NotBinary, List("ova"))
      final lazy val `x-virtualbox-ovf`: MediaType =
        new MediaType("application", "x-virtualbox-ovf", Compressible, NotBinary, List("ovf"))
      final lazy val `x-virtualbox-vbox`: MediaType =
        new MediaType("application", "x-virtualbox-vbox", Compressible, NotBinary, List("vbox"))
      final lazy val `x-virtualbox-vbox-extpack`: MediaType = new MediaType(
        "application",
        "x-virtualbox-vbox-extpack",
        Uncompressible,
        NotBinary,
        List("vbox-extpack"))
      final lazy val `x-virtualbox-vdi`: MediaType =
        new MediaType("application", "x-virtualbox-vdi", Compressible, NotBinary, List("vdi"))
      final lazy val `x-virtualbox-vhd`: MediaType =
        new MediaType("application", "x-virtualbox-vhd", Compressible, NotBinary, List("vhd"))
      final lazy val `x-virtualbox-vmdk`: MediaType =
        new MediaType("application", "x-virtualbox-vmdk", Compressible, NotBinary, List("vmdk"))
      final lazy val `x-wais-source`: MediaType =
        new MediaType("application", "x-wais-source", Compressible, NotBinary, List("src"))
      final lazy val `x-web-app-manifest+json`: MediaType = new MediaType(
        "application",
        "x-web-app-manifest+json",
        Compressible,
        NotBinary,
        List("webapp"))
      final lazy val `x-www-form-urlencoded`: MediaType =
        new MediaType("application", "x-www-form-urlencoded", Compressible, NotBinary)
      final lazy val `x-x509-ca-cert`: MediaType = new MediaType(
        "application",
        "x-x509-ca-cert",
        Compressible,
        Binary,
        List("der", "crt", "pem"))
      final lazy val `x-xfig`: MediaType =
        new MediaType("application", "x-xfig", Compressible, NotBinary, List("fig"))
      final lazy val `x-xliff+xml`: MediaType =
        new MediaType("application", "x-xliff+xml", Compressible, NotBinary, List("xlf"))
      final lazy val `x-xpinstall`: MediaType =
        new MediaType("application", "x-xpinstall", Uncompressible, Binary, List("xpi"))
      final lazy val `x-xz`: MediaType =
        new MediaType("application", "x-xz", Compressible, NotBinary, List("xz"))
      final lazy val `x-zmachine`: MediaType = new MediaType(
        "application",
        "x-zmachine",
        Compressible,
        NotBinary,
        List("z1", "z2", "z3", "z4", "z5", "z6", "z7", "z8"))
      final lazy val `x400-bp`: MediaType =
        new MediaType("application", "x400-bp", Compressible, NotBinary)
      final lazy val `xacml+xml`: MediaType =
        new MediaType("application", "xacml+xml", Compressible, NotBinary)
      final lazy val `xaml+xml`: MediaType =
        new MediaType("application", "xaml+xml", Compressible, NotBinary, List("xaml"))
      final lazy val `xcap-att+xml`: MediaType =
        new MediaType("application", "xcap-att+xml", Compressible, NotBinary)
      final lazy val `xcap-caps+xml`: MediaType =
        new MediaType("application", "xcap-caps+xml", Compressible, NotBinary)
      final lazy val `xcap-diff+xml`: MediaType =
        new MediaType("application", "xcap-diff+xml", Compressible, NotBinary, List("xdf"))
      final lazy val `xcap-el+xml`: MediaType =
        new MediaType("application", "xcap-el+xml", Compressible, NotBinary)
      final lazy val `xcap-error+xml`: MediaType =
        new MediaType("application", "xcap-error+xml", Compressible, NotBinary)
      final lazy val `xcap-ns+xml`: MediaType =
        new MediaType("application", "xcap-ns+xml", Compressible, NotBinary)
      final lazy val `xcon-conference-info+xml`: MediaType =
        new MediaType("application", "xcon-conference-info+xml", Compressible, NotBinary)
      final lazy val `xcon-conference-info-diff+xml`: MediaType =
        new MediaType("application", "xcon-conference-info-diff+xml", Compressible, NotBinary)
      final lazy val `xenc+xml`: MediaType =
        new MediaType("application", "xenc+xml", Compressible, NotBinary, List("xenc"))
      final lazy val `xhtml+xml`: MediaType =
        new MediaType("application", "xhtml+xml", Compressible, NotBinary, List("xhtml", "xht"))
      final lazy val `xhtml-voice+xml`: MediaType =
        new MediaType("application", "xhtml-voice+xml", Compressible, NotBinary)
      final lazy val part_2: List[MediaType] = List(
        `vnd.oma.drm.risd+xml`,
        `vnd.oma.group-usage-list+xml`,
        `vnd.oma.lwm2m+json`,
        `vnd.oma.lwm2m+tlv`,
        `vnd.oma.pal+xml`,
        `vnd.oma.poc.detailed-progress-report+xml`,
        `vnd.oma.poc.final-report+xml`,
        `vnd.oma.poc.groups+xml`,
        `vnd.oma.poc.invocation-descriptor+xml`,
        `vnd.oma.poc.optimized-progress-report+xml`,
        `vnd.oma.push`,
        `vnd.oma.scidm.messages+xml`,
        `vnd.oma.xcap-directory+xml`,
        `vnd.omads-email+xml`,
        `vnd.omads-file+xml`,
        `vnd.omads-folder+xml`,
        `vnd.omaloc-supl-init`,
        `vnd.onepager`,
        `vnd.onepagertamp`,
        `vnd.onepagertamx`,
        `vnd.onepagertat`,
        `vnd.onepagertatp`,
        `vnd.onepagertatx`,
        `vnd.openblox.game+xml`,
        `vnd.openblox.game-binary`,
        `vnd.openeye.oeb`,
        `vnd.openofficeorg.extension`,
        `vnd.openstreetmap.data+xml`,
        `vnd.openxmlformats-officedocument.custom-properties+xml`,
        `vnd.openxmlformats-officedocument.customxmlproperties+xml`,
        `vnd.openxmlformats-officedocument.drawing+xml`,
        `vnd.openxmlformats-officedocument.drawingml.chart+xml`,
        `vnd.openxmlformats-officedocument.drawingml.chartshapes+xml`,
        `vnd.openxmlformats-officedocument.drawingml.diagramcolors+xml`,
        `vnd.openxmlformats-officedocument.drawingml.diagramdata+xml`,
        `vnd.openxmlformats-officedocument.drawingml.diagramlayout+xml`,
        `vnd.openxmlformats-officedocument.drawingml.diagramstyle+xml`,
        `vnd.openxmlformats-officedocument.extended-properties+xml`,
        `vnd.openxmlformats-officedocument.presentationml.commentauthors+xml`,
        `vnd.openxmlformats-officedocument.presentationml.comments+xml`,
        `vnd.openxmlformats-officedocument.presentationml.handoutmaster+xml`,
        `vnd.openxmlformats-officedocument.presentationml.notesmaster+xml`,
        `vnd.openxmlformats-officedocument.presentationml.notesslide+xml`,
        `vnd.openxmlformats-officedocument.presentationml.presentation`,
        `vnd.openxmlformats-officedocument.presentationml.presentation.main+xml`,
        `vnd.openxmlformats-officedocument.presentationml.presprops+xml`,
        `vnd.openxmlformats-officedocument.presentationml.slide`,
        `vnd.openxmlformats-officedocument.presentationml.slide+xml`,
        `vnd.openxmlformats-officedocument.presentationml.slidelayout+xml`,
        `vnd.openxmlformats-officedocument.presentationml.slidemaster+xml`,
        `vnd.openxmlformats-officedocument.presentationml.slideshow`,
        `vnd.openxmlformats-officedocument.presentationml.slideshow.main+xml`,
        `vnd.openxmlformats-officedocument.presentationml.slideupdateinfo+xml`,
        `vnd.openxmlformats-officedocument.presentationml.tablestyles+xml`,
        `vnd.openxmlformats-officedocument.presentationml.tags+xml`,
        `vnd.openxmlformats-officedocument.presentationml.template`,
        `vnd.openxmlformats-officedocument.presentationml.template.main+xml`,
        `vnd.openxmlformats-officedocument.presentationml.viewprops+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.calcchain+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.chartsheet+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.comments+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.connections+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.dialogsheet+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.externallink+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.pivotcachedefinition+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.pivotcacherecords+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.pivottable+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.querytable+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.revisionheaders+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.revisionlog+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.sharedstrings+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.sheet`,
        `vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.sheetmetadata+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.styles+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.table+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.tablesinglecells+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.template`,
        `vnd.openxmlformats-officedocument.spreadsheetml.template.main+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.usernames+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.volatiledependencies+xml`,
        `vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml`,
        `vnd.openxmlformats-officedocument.theme+xml`,
        `vnd.openxmlformats-officedocument.themeoverride+xml`,
        `vnd.openxmlformats-officedocument.vmldrawing`,
        `vnd.openxmlformats-officedocument.wordprocessingml.comments+xml`,
        `vnd.openxmlformats-officedocument.wordprocessingml.document`,
        `vnd.openxmlformats-officedocument.wordprocessingml.document.glossary+xml`,
        `vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml`,
        `vnd.openxmlformats-officedocument.wordprocessingml.endnotes+xml`,
        `vnd.openxmlformats-officedocument.wordprocessingml.fonttable+xml`,
        `vnd.openxmlformats-officedocument.wordprocessingml.footer+xml`,
        `vnd.openxmlformats-officedocument.wordprocessingml.footnotes+xml`,
        `vnd.openxmlformats-officedocument.wordprocessingml.numbering+xml`,
        `vnd.openxmlformats-officedocument.wordprocessingml.settings+xml`,
        `vnd.openxmlformats-officedocument.wordprocessingml.styles+xml`,
        `vnd.openxmlformats-officedocument.wordprocessingml.template`,
        `vnd.openxmlformats-officedocument.wordprocessingml.template.main+xml`,
        `vnd.openxmlformats-officedocument.wordprocessingml.websettings+xml`,
        `vnd.openxmlformats-package.core-properties+xml`,
        `vnd.openxmlformats-package.digital-signature-xmlsignature+xml`,
        `vnd.openxmlformats-package.relationships+xml`,
        `vnd.oracle.resource+json`,
        `vnd.orange.indata`,
        `vnd.osa.netdeploy`,
        `vnd.osgeo.mapguide.package`,
        `vnd.osgi.bundle`,
        `vnd.osgi.dp`,
        `vnd.osgi.subsystem`,
        `vnd.otps.ct-kip+xml`,
        `vnd.oxli.countgraph`,
        `vnd.pagerduty+json`,
        `vnd.palm`,
        `vnd.panoply`,
        `vnd.paos.xml`,
        `vnd.patentdive`,
        `vnd.pawaafile`,
        `vnd.pcos`,
        `vnd.pg.format`,
        `vnd.pg.osasli`,
        `vnd.piaccess.application-licence`,
        `vnd.picsel`,
        `vnd.pmi.widget`,
        `vnd.poc.group-advertisement+xml`,
        `vnd.pocketlearn`,
        `vnd.powerbuilder6`,
        `vnd.powerbuilder6-s`,
        `vnd.powerbuilder7`,
        `vnd.powerbuilder7-s`,
        `vnd.powerbuilder75`,
        `vnd.powerbuilder75-s`,
        `vnd.preminet`,
        `vnd.previewsystems.box`,
        `vnd.proteus.magazine`,
        `vnd.psfs`,
        `vnd.publishare-delta-tree`,
        `vnd.pvi.ptid1`,
        `vnd.pwg-multiplexed`,
        `vnd.pwg-xhtml-print+xml`,
        `vnd.qualcomm.brew-app-res`,
        `vnd.quarantainenet`,
        `vnd.quark.quarkxpress`,
        `vnd.quobject-quoxdocument`,
        `vnd.radisys.moml+xml`,
        `vnd.radisys.msml+xml`,
        `vnd.radisys.msml-audit+xml`,
        `vnd.radisys.msml-audit-conf+xml`,
        `vnd.radisys.msml-audit-conn+xml`,
        `vnd.radisys.msml-audit-dialog+xml`,
        `vnd.radisys.msml-audit-stream+xml`,
        `vnd.radisys.msml-conf+xml`,
        `vnd.radisys.msml-dialog+xml`,
        `vnd.radisys.msml-dialog-base+xml`,
        `vnd.radisys.msml-dialog-fax-detect+xml`,
        `vnd.radisys.msml-dialog-fax-sendrecv+xml`,
        `vnd.radisys.msml-dialog-group+xml`,
        `vnd.radisys.msml-dialog-speech+xml`,
        `vnd.radisys.msml-dialog-transform+xml`,
        `vnd.rainstor.data`,
        `vnd.rapid`,
        `vnd.rar`,
        `vnd.realvnc.bed`,
        `vnd.recordare.musicxml`,
        `vnd.recordare.musicxml+xml`,
        `vnd.renlearn.rlprint`,
        `vnd.restful+json`,
        `vnd.rig.cryptonote`,
        `vnd.rim.cod`,
        `vnd.rn-realmedia`,
        `vnd.rn-realmedia-vbr`,
        `vnd.route66.link66+xml`,
        `vnd.rs-274x`,
        `vnd.ruckus.download`,
        `vnd.s3sms`,
        `vnd.sailingtracker.track`,
        `vnd.sbm.cid`,
        `vnd.sbm.mid2`,
        `vnd.scribus`,
        `vnd.sealed.3df`,
        `vnd.sealed.csf`,
        `vnd.sealed.doc`,
        `vnd.sealed.eml`,
        `vnd.sealed.mht`,
        `vnd.sealed.net`,
        `vnd.sealed.ppt`,
        `vnd.sealed.tiff`,
        `vnd.sealed.xls`,
        `vnd.sealedmedia.softseal.html`,
        `vnd.sealedmedia.softseal.pdf`,
        `vnd.seemail`,
        `vnd.sema`,
        `vnd.semd`,
        `vnd.semf`,
        `vnd.shana.informed.formdata`,
        `vnd.shana.informed.formtemplate`,
        `vnd.shana.informed.interchange`,
        `vnd.shana.informed.package`,
        `vnd.shootproof+json`,
        `vnd.sigrok.session`,
        `vnd.simtech-mindmapper`,
        `vnd.siren+json`,
        `vnd.smaf`,
        `vnd.smart.notebook`,
        `vnd.smart.teacher`,
        `vnd.software602.filler.form+xml`,
        `vnd.software602.filler.form-xml-zip`,
        `vnd.solent.sdkm+xml`,
        `vnd.spotfire.dxp`,
        `vnd.spotfire.sfs`,
        `vnd.sqlite3`,
        `vnd.sss-cod`,
        `vnd.sss-dtf`,
        `vnd.sss-ntf`,
        `vnd.stardivision.calc`,
        `vnd.stardivision.draw`,
        `vnd.stardivision.impress`,
        `vnd.stardivision.math`,
        `vnd.stardivision.writer`,
        `vnd.stardivision.writer-global`,
        `vnd.stepmania.package`,
        `vnd.stepmania.stepchart`,
        `vnd.street-stream`,
        `vnd.sun.wadl+xml`,
        `vnd.sun.xml.calc`,
        `vnd.sun.xml.calc.template`,
        `vnd.sun.xml.draw`,
        `vnd.sun.xml.draw.template`,
        `vnd.sun.xml.impress`,
        `vnd.sun.xml.impress.template`,
        `vnd.sun.xml.math`,
        `vnd.sun.xml.writer`,
        `vnd.sun.xml.writer.global`,
        `vnd.sun.xml.writer.template`,
        `vnd.sus-calendar`,
        `vnd.svd`,
        `vnd.swiftview-ics`,
        `vnd.symbian.install`,
        `vnd.syncml+xml`,
        `vnd.syncml.dm+wbxml`,
        `vnd.syncml.dm+xml`,
        `vnd.syncml.dm.notification`,
        `vnd.syncml.dmddf+wbxml`,
        `vnd.syncml.dmddf+xml`,
        `vnd.syncml.dmtnds+wbxml`,
        `vnd.syncml.dmtnds+xml`,
        `vnd.syncml.ds.notification`,
        `vnd.tableschema+json`,
        `vnd.tao.intent-module-archive`,
        `vnd.tcpdump.pcap`,
        `vnd.think-cell.ppttc+json`,
        `vnd.tmd.mediaflex.api+xml`,
        `vnd.tml`,
        `vnd.tmobile-livetv`,
        `vnd.tri.onesource`,
        `vnd.trid.tpt`,
        `vnd.triscape.mxs`,
        `vnd.trueapp`,
        `vnd.truedoc`,
        `vnd.ubisoft.webplayer`,
        `vnd.ufdl`,
        `vnd.uiq.theme`,
        `vnd.umajin`,
        `vnd.unity`,
        `vnd.uoml+xml`,
        `vnd.uplanet.alert`,
        `vnd.uplanet.alert-wbxml`,
        `vnd.uplanet.bearer-choice`,
        `vnd.uplanet.bearer-choice-wbxml`,
        `vnd.uplanet.cacheop`,
        `vnd.uplanet.cacheop-wbxml`,
        `vnd.uplanet.channel`,
        `vnd.uplanet.channel-wbxml`,
        `vnd.uplanet.list`,
        `vnd.uplanet.list-wbxml`,
        `vnd.uplanet.listcmd`,
        `vnd.uplanet.listcmd-wbxml`,
        `vnd.uplanet.signal`,
        `vnd.uri-map`,
        `vnd.valve.source.material`,
        `vnd.vcx`,
        `vnd.vd-study`,
        `vnd.vectorworks`,
        `vnd.vel+json`,
        `vnd.verimatrix.vcas`,
        `vnd.vidsoft.vidconference`,
        `vnd.visio`,
        `vnd.visionary`,
        `vnd.vividence.scriptfile`,
        `vnd.vsf`,
        `vnd.wap.sic`,
        `vnd.wap.slc`,
        `vnd.wap.wbxml`,
        `vnd.wap.wmlc`,
        `vnd.wap.wmlscriptc`,
        `vnd.webturbo`,
        `vnd.wfa.p2p`,
        `vnd.wfa.wsc`,
        `vnd.windows.devicepairing`,
        `vnd.wmc`,
        `vnd.wmf.bootstrap`,
        `vnd.wolfram.mathematica`,
        `vnd.wolfram.mathematica.package`,
        `vnd.wolfram.player`,
        `vnd.wordperfect`,
        `vnd.wqd`,
        `vnd.wrq-hp3000-labelled`,
        `vnd.wt.stf`,
        `vnd.wv.csp+wbxml`,
        `vnd.wv.csp+xml`,
        `vnd.wv.ssp+xml`,
        `vnd.xacml+json`,
        `vnd.xara`,
        `vnd.xfdl`,
        `vnd.xfdl.webform`,
        `vnd.xmi+xml`,
        `vnd.xmpie.cpkg`,
        `vnd.xmpie.dpkg`,
        `vnd.xmpie.plan`,
        `vnd.xmpie.ppkg`,
        `vnd.xmpie.xlim`,
        `vnd.yamaha.hv-dic`,
        `vnd.yamaha.hv-script`,
        `vnd.yamaha.hv-voice`,
        `vnd.yamaha.openscoreformat`,
        `vnd.yamaha.openscoreformat.osfpvg+xml`,
        `vnd.yamaha.remote-setup`,
        `vnd.yamaha.smaf-audio`,
        `vnd.yamaha.smaf-phrase`,
        `vnd.yamaha.through-ngn`,
        `vnd.yamaha.tunnel-udpencap`,
        `vnd.yaoweme`,
        `vnd.yellowriver-custom-menu`,
        `vnd.youtube.yt`,
        `vnd.zul`,
        `vnd.zzazz.deck+xml`,
        `voicexml+xml`,
        `voucher-cms+json`,
        `vq-rtcpxr`,
        `wasm`,
        `watcherinfo+xml`,
        `webpush-options+json`,
        `whoispp-query`,
        `whoispp-response`,
        `widget`,
        `winhlp`,
        `wita`,
        `wordperfect5.1`,
        `wsdl+xml`,
        `wspolicy+xml`,
        `x-7z-compressed`,
        `x-abiword`,
        `x-ace-compressed`,
        `x-amf`,
        `x-apple-diskimage`,
        `x-arj`,
        `x-authorware-bin`,
        `x-authorware-map`,
        `x-authorware-seg`,
        `x-bcpio`,
        `x-bdoc`,
        `x-bittorrent`,
        `x-blorb`,
        `x-bzip`,
        `x-bzip2`,
        `x-cbr`,
        `x-cdlink`,
        `x-cfs-compressed`,
        `x-chat`,
        `x-chess-pgn`,
        `x-chrome-extension`,
        `x-cocoa`,
        `x-compress`,
        `x-conference`,
        `x-cpio`,
        `x-csh`,
        `x-deb`,
        `x-debian-package`,
        `x-dgc-compressed`,
        `x-director`,
        `x-doom`,
        `x-dtbncx+xml`,
        `x-dtbook+xml`,
        `x-dtbresource+xml`,
        `x-dvi`,
        `x-envoy`,
        `x-eva`,
        `x-font-bdf`,
        `x-font-dos`,
        `x-font-framemaker`,
        `x-font-ghostscript`,
        `x-font-libgrx`,
        `x-font-linux-psf`,
        `x-font-pcf`,
        `x-font-snf`,
        `x-font-speedo`,
        `x-font-sunos-news`,
        `x-font-type1`,
        `x-font-vfont`,
        `x-freearc`,
        `x-futuresplash`,
        `x-gca-compressed`,
        `x-glulx`,
        `x-gnumeric`,
        `x-gramps-xml`,
        `x-gtar`,
        `x-gzip`,
        `x-hdf`,
        `x-httpd-php`,
        `x-install-instructions`,
        `x-iso9660-image`,
        `x-java-archive-diff`,
        `x-java-jnlp-file`,
        `x-javascript`,
        `x-latex`,
        `x-lua-bytecode`,
        `x-lzh-compressed`,
        `x-makeself`,
        `x-mie`,
        `x-mobipocket-ebook`,
        `x-mpegurl`,
        `x-ms-application`,
        `x-ms-shortcut`,
        `x-ms-wmd`,
        `x-ms-wmz`,
        `x-ms-xbap`,
        `x-msaccess`,
        `x-msbinder`,
        `x-mscardfile`,
        `x-msclip`,
        `x-msdos-program`,
        `x-msdownload`,
        `x-msmediaview`,
        `x-msmetafile`,
        `x-msmoney`,
        `x-mspublisher`,
        `x-msschedule`,
        `x-msterminal`,
        `x-mswrite`,
        `x-netcdf`,
        `x-ns-proxy-autoconfig`,
        `x-nzb`,
        `x-perl`,
        `x-pilot`,
        `x-pkcs12`,
        `x-pkcs7-certificates`,
        `x-pkcs7-certreqresp`,
        `x-rar-compressed`,
        `x-redhat-package-manager`,
        `x-research-info-systems`,
        `x-sea`,
        `x-sh`,
        `x-shar`,
        `x-shockwave-flash`,
        `x-silverlight-app`,
        `x-sql`,
        `x-stuffit`,
        `x-stuffitx`,
        `x-subrip`,
        `x-sv4cpio`,
        `x-sv4crc`,
        `x-t3vm-image`,
        `x-tads`,
        `x-tar`,
        `x-tcl`,
        `x-tex`,
        `x-tex-tfm`,
        `x-texinfo`,
        `x-tgif`,
        `x-ustar`,
        `x-virtualbox-hdd`,
        `x-virtualbox-ova`,
        `x-virtualbox-ovf`,
        `x-virtualbox-vbox`,
        `x-virtualbox-vbox-extpack`,
        `x-virtualbox-vdi`,
        `x-virtualbox-vhd`,
        `x-virtualbox-vmdk`,
        `x-wais-source`,
        `x-web-app-manifest+json`,
        `x-www-form-urlencoded`,
        `x-x509-ca-cert`,
        `x-xfig`,
        `x-xliff+xml`,
        `x-xpinstall`,
        `x-xz`,
        `x-zmachine`,
        `x400-bp`,
        `xacml+xml`,
        `xaml+xml`,
        `xcap-att+xml`,
        `xcap-caps+xml`,
        `xcap-diff+xml`,
        `xcap-el+xml`,
        `xcap-error+xml`,
        `xcap-ns+xml`,
        `xcon-conference-info+xml`,
        `xcon-conference-info-diff+xml`,
        `xenc+xml`,
        `xhtml+xml`,
        `xhtml-voice+xml`
      )
    }
    trait application_3 {
      final lazy val `xliff+xml`: MediaType =
        new MediaType("application", "xliff+xml", Compressible, NotBinary)
      final lazy val `xml`: MediaType = new MediaType(
        "application",
        "xml",
        Compressible,
        NotBinary,
        List("xml", "xsl", "xsd", "rng"))
      final lazy val `xml-dtd`: MediaType =
        new MediaType("application", "xml-dtd", Compressible, NotBinary, List("dtd"))
      final lazy val `xml-external-parsed-entity`: MediaType =
        new MediaType("application", "xml-external-parsed-entity", Compressible, NotBinary)
      final lazy val `xml-patch+xml`: MediaType =
        new MediaType("application", "xml-patch+xml", Compressible, NotBinary)
      final lazy val `xmpp+xml`: MediaType =
        new MediaType("application", "xmpp+xml", Compressible, NotBinary)
      final lazy val `xop+xml`: MediaType =
        new MediaType("application", "xop+xml", Compressible, NotBinary, List("xop"))
      final lazy val `xproc+xml`: MediaType =
        new MediaType("application", "xproc+xml", Compressible, NotBinary, List("xpl"))
      final lazy val `xslt+xml`: MediaType =
        new MediaType("application", "xslt+xml", Compressible, NotBinary, List("xslt"))
      final lazy val `xspf+xml`: MediaType =
        new MediaType("application", "xspf+xml", Compressible, NotBinary, List("xspf"))
      final lazy val `xv+xml`: MediaType = new MediaType(
        "application",
        "xv+xml",
        Compressible,
        NotBinary,
        List("mxml", "xhvml", "xvml", "xvm"))
      final lazy val `yang`: MediaType =
        new MediaType("application", "yang", Compressible, NotBinary, List("yang"))
      final lazy val `yang-data+json`: MediaType =
        new MediaType("application", "yang-data+json", Compressible, NotBinary)
      final lazy val `yang-data+xml`: MediaType =
        new MediaType("application", "yang-data+xml", Compressible, NotBinary)
      final lazy val `yang-patch+json`: MediaType =
        new MediaType("application", "yang-patch+json", Compressible, NotBinary)
      final lazy val `yang-patch+xml`: MediaType =
        new MediaType("application", "yang-patch+xml", Compressible, NotBinary)
      final lazy val `yin+xml`: MediaType =
        new MediaType("application", "yin+xml", Compressible, NotBinary, List("yin"))
      final lazy val `zip`: MediaType =
        new MediaType("application", "zip", Uncompressible, Binary, List("zip"))
      final lazy val `zlib`: MediaType = new MediaType("application", "zlib", Compressible, NotBinary)
      final lazy val `zstd`: MediaType = new MediaType("application", "zstd", Compressible, NotBinary)
      final lazy val part_3: List[MediaType] = List(
        `xliff+xml`,
        `xml`,
        `xml-dtd`,
        `xml-external-parsed-entity`,
        `xml-patch+xml`,
        `xmpp+xml`,
        `xop+xml`,
        `xproc+xml`,
        `xslt+xml`,
        `xspf+xml`,
        `xv+xml`,
        `yang`,
        `yang-data+json`,
        `yang-data+xml`,
        `yang-patch+json`,
        `yang-patch+xml`,
        `yin+xml`,
        `zip`,
        `zlib`,
        `zstd`
      )
    }
  }
  object application
      extends application_parts.application_0
      with application_parts.application_1
      with application_parts.application_2
      with application_parts.application_3 {
    final lazy val all: List[MediaType] = Nil ++ part_0 ++ part_1 ++ part_2 ++ part_3
  }
  object audio {
    final lazy val `1d-interleaved-parityfec`: MediaType =
      new MediaType("audio", "1d-interleaved-parityfec", Compressible, Binary)
    final lazy val `32kadpcm`: MediaType = new MediaType("audio", "32kadpcm", Compressible, Binary)
    final lazy val `3gpp`: MediaType =
      new MediaType("audio", "3gpp", Uncompressible, Binary, List("3gpp"))
    final lazy val `3gpp2`: MediaType = new MediaType("audio", "3gpp2", Compressible, Binary)
    final lazy val `aac`: MediaType = new MediaType("audio", "aac", Compressible, Binary)
    final lazy val `ac3`: MediaType = new MediaType("audio", "ac3", Compressible, Binary)
    final lazy val `adpcm`: MediaType = new MediaType("audio", "adpcm", Compressible, Binary, List("adp"))
    final lazy val `amr`: MediaType = new MediaType("audio", "amr", Compressible, Binary)
    final lazy val `amr-wb`: MediaType = new MediaType("audio", "amr-wb", Compressible, Binary)
    final lazy val `amr-wb+` : MediaType = new MediaType("audio", "amr-wb+", Compressible, Binary)
    final lazy val `aptx`: MediaType = new MediaType("audio", "aptx", Compressible, Binary)
    final lazy val `asc`: MediaType = new MediaType("audio", "asc", Compressible, Binary)
    final lazy val `atrac-advanced-lossless`: MediaType =
      new MediaType("audio", "atrac-advanced-lossless", Compressible, Binary)
    final lazy val `atrac-x`: MediaType = new MediaType("audio", "atrac-x", Compressible, Binary)
    final lazy val `atrac3`: MediaType = new MediaType("audio", "atrac3", Compressible, Binary)
    final lazy val `basic`: MediaType =
      new MediaType("audio", "basic", Uncompressible, Binary, List("au", "snd"))
    final lazy val `bv16`: MediaType = new MediaType("audio", "bv16", Compressible, Binary)
    final lazy val `bv32`: MediaType = new MediaType("audio", "bv32", Compressible, Binary)
    final lazy val `clearmode`: MediaType = new MediaType("audio", "clearmode", Compressible, Binary)
    final lazy val `cn`: MediaType = new MediaType("audio", "cn", Compressible, Binary)
    final lazy val `dat12`: MediaType = new MediaType("audio", "dat12", Compressible, Binary)
    final lazy val `dls`: MediaType = new MediaType("audio", "dls", Compressible, Binary)
    final lazy val `dsr-es201108`: MediaType =
      new MediaType("audio", "dsr-es201108", Compressible, Binary)
    final lazy val `dsr-es202050`: MediaType =
      new MediaType("audio", "dsr-es202050", Compressible, Binary)
    final lazy val `dsr-es202211`: MediaType =
      new MediaType("audio", "dsr-es202211", Compressible, Binary)
    final lazy val `dsr-es202212`: MediaType =
      new MediaType("audio", "dsr-es202212", Compressible, Binary)
    final lazy val `dv`: MediaType = new MediaType("audio", "dv", Compressible, Binary)
    final lazy val `dvi4`: MediaType = new MediaType("audio", "dvi4", Compressible, Binary)
    final lazy val `eac3`: MediaType = new MediaType("audio", "eac3", Compressible, Binary)
    final lazy val `encaprtp`: MediaType = new MediaType("audio", "encaprtp", Compressible, Binary)
    final lazy val `evrc`: MediaType = new MediaType("audio", "evrc", Compressible, Binary)
    final lazy val `evrc-qcp`: MediaType = new MediaType("audio", "evrc-qcp", Compressible, Binary)
    final lazy val `evrc0`: MediaType = new MediaType("audio", "evrc0", Compressible, Binary)
    final lazy val `evrc1`: MediaType = new MediaType("audio", "evrc1", Compressible, Binary)
    final lazy val `evrcb`: MediaType = new MediaType("audio", "evrcb", Compressible, Binary)
    final lazy val `evrcb0`: MediaType = new MediaType("audio", "evrcb0", Compressible, Binary)
    final lazy val `evrcb1`: MediaType = new MediaType("audio", "evrcb1", Compressible, Binary)
    final lazy val `evrcnw`: MediaType = new MediaType("audio", "evrcnw", Compressible, Binary)
    final lazy val `evrcnw0`: MediaType = new MediaType("audio", "evrcnw0", Compressible, Binary)
    final lazy val `evrcnw1`: MediaType = new MediaType("audio", "evrcnw1", Compressible, Binary)
    final lazy val `evrcwb`: MediaType = new MediaType("audio", "evrcwb", Compressible, Binary)
    final lazy val `evrcwb0`: MediaType = new MediaType("audio", "evrcwb0", Compressible, Binary)
    final lazy val `evrcwb1`: MediaType = new MediaType("audio", "evrcwb1", Compressible, Binary)
    final lazy val `evs`: MediaType = new MediaType("audio", "evs", Compressible, Binary)
    final lazy val `fwdred`: MediaType = new MediaType("audio", "fwdred", Compressible, Binary)
    final lazy val `g711-0`: MediaType = new MediaType("audio", "g711-0", Compressible, Binary)
    final lazy val `g719`: MediaType = new MediaType("audio", "g719", Compressible, Binary)
    final lazy val `g722`: MediaType = new MediaType("audio", "g722", Compressible, Binary)
    final lazy val `g7221`: MediaType = new MediaType("audio", "g7221", Compressible, Binary)
    final lazy val `g723`: MediaType = new MediaType("audio", "g723", Compressible, Binary)
    final lazy val `g726-16`: MediaType = new MediaType("audio", "g726-16", Compressible, Binary)
    final lazy val `g726-24`: MediaType = new MediaType("audio", "g726-24", Compressible, Binary)
    final lazy val `g726-32`: MediaType = new MediaType("audio", "g726-32", Compressible, Binary)
    final lazy val `g726-40`: MediaType = new MediaType("audio", "g726-40", Compressible, Binary)
    final lazy val `g728`: MediaType = new MediaType("audio", "g728", Compressible, Binary)
    final lazy val `g729`: MediaType = new MediaType("audio", "g729", Compressible, Binary)
    final lazy val `g7291`: MediaType = new MediaType("audio", "g7291", Compressible, Binary)
    final lazy val `g729d`: MediaType = new MediaType("audio", "g729d", Compressible, Binary)
    final lazy val `g729e`: MediaType = new MediaType("audio", "g729e", Compressible, Binary)
    final lazy val `gsm`: MediaType = new MediaType("audio", "gsm", Compressible, Binary)
    final lazy val `gsm-efr`: MediaType = new MediaType("audio", "gsm-efr", Compressible, Binary)
    final lazy val `gsm-hr-08`: MediaType = new MediaType("audio", "gsm-hr-08", Compressible, Binary)
    final lazy val `ilbc`: MediaType = new MediaType("audio", "ilbc", Compressible, Binary)
    final lazy val `ip-mr_v2.5`: MediaType = new MediaType("audio", "ip-mr_v2.5", Compressible, Binary)
    final lazy val `isac`: MediaType = new MediaType("audio", "isac", Compressible, Binary)
    final lazy val `l16`: MediaType = new MediaType("audio", "l16", Compressible, Binary)
    final lazy val `l20`: MediaType = new MediaType("audio", "l20", Compressible, Binary)
    final lazy val `l24`: MediaType = new MediaType("audio", "l24", Uncompressible, Binary)
    final lazy val `l8`: MediaType = new MediaType("audio", "l8", Compressible, Binary)
    final lazy val `lpc`: MediaType = new MediaType("audio", "lpc", Compressible, Binary)
    final lazy val `melp`: MediaType = new MediaType("audio", "melp", Compressible, Binary)
    final lazy val `melp1200`: MediaType = new MediaType("audio", "melp1200", Compressible, Binary)
    final lazy val `melp2400`: MediaType = new MediaType("audio", "melp2400", Compressible, Binary)
    final lazy val `melp600`: MediaType = new MediaType("audio", "melp600", Compressible, Binary)
    final lazy val `midi`: MediaType =
      new MediaType("audio", "midi", Compressible, Binary, List("mid", "midi", "kar", "rmi"))
    final lazy val `mobile-xmf`: MediaType = new MediaType("audio", "mobile-xmf", Compressible, Binary)
    final lazy val `mp3`: MediaType = new MediaType("audio", "mp3", Uncompressible, Binary, List("mp3"))
    final lazy val `mp4`: MediaType =
      new MediaType("audio", "mp4", Uncompressible, Binary, List("m4a", "mp4a"))
    final lazy val `mp4a-latm`: MediaType = new MediaType("audio", "mp4a-latm", Compressible, Binary)
    final lazy val `mpa`: MediaType = new MediaType("audio", "mpa", Compressible, Binary)
    final lazy val `mpa-robust`: MediaType = new MediaType("audio", "mpa-robust", Compressible, Binary)
    final lazy val `mpeg`: MediaType = new MediaType(
      "audio",
      "mpeg",
      Uncompressible,
      Binary,
      List("mpga", "mp2", "mp2a", "mp3", "m2a", "m3a"))
    final lazy val `mpeg4-generic`: MediaType =
      new MediaType("audio", "mpeg4-generic", Compressible, Binary)
    final lazy val `musepack`: MediaType = new MediaType("audio", "musepack", Compressible, Binary)
    final lazy val `ogg`: MediaType =
      new MediaType("audio", "ogg", Uncompressible, Binary, List("oga", "ogg", "spx"))
    final lazy val `opus`: MediaType = new MediaType("audio", "opus", Compressible, Binary)
    final lazy val `parityfec`: MediaType = new MediaType("audio", "parityfec", Compressible, Binary)
    final lazy val `pcma`: MediaType = new MediaType("audio", "pcma", Compressible, Binary)
    final lazy val `pcma-wb`: MediaType = new MediaType("audio", "pcma-wb", Compressible, Binary)
    final lazy val `pcmu`: MediaType = new MediaType("audio", "pcmu", Compressible, Binary)
    final lazy val `pcmu-wb`: MediaType = new MediaType("audio", "pcmu-wb", Compressible, Binary)
    final lazy val `prs.sid`: MediaType = new MediaType("audio", "prs.sid", Compressible, Binary)
    final lazy val `qcelp`: MediaType = new MediaType("audio", "qcelp", Compressible, Binary)
    final lazy val `raptorfec`: MediaType = new MediaType("audio", "raptorfec", Compressible, Binary)
    final lazy val `red`: MediaType = new MediaType("audio", "red", Compressible, Binary)
    final lazy val `rtp-enc-aescm128`: MediaType =
      new MediaType("audio", "rtp-enc-aescm128", Compressible, Binary)
    final lazy val `rtp-midi`: MediaType = new MediaType("audio", "rtp-midi", Compressible, Binary)
    final lazy val `rtploopback`: MediaType = new MediaType("audio", "rtploopback", Compressible, Binary)
    final lazy val `rtx`: MediaType = new MediaType("audio", "rtx", Compressible, Binary)
    final lazy val `s3m`: MediaType = new MediaType("audio", "s3m", Compressible, Binary, List("s3m"))
    final lazy val `silk`: MediaType = new MediaType("audio", "silk", Compressible, Binary, List("sil"))
    final lazy val `smv`: MediaType = new MediaType("audio", "smv", Compressible, Binary)
    final lazy val `smv-qcp`: MediaType = new MediaType("audio", "smv-qcp", Compressible, Binary)
    final lazy val `smv0`: MediaType = new MediaType("audio", "smv0", Compressible, Binary)
    final lazy val `sp-midi`: MediaType = new MediaType("audio", "sp-midi", Compressible, Binary)
    final lazy val `speex`: MediaType = new MediaType("audio", "speex", Compressible, Binary)
    final lazy val `t140c`: MediaType = new MediaType("audio", "t140c", Compressible, Binary)
    final lazy val `t38`: MediaType = new MediaType("audio", "t38", Compressible, Binary)
    final lazy val `telephone-event`: MediaType =
      new MediaType("audio", "telephone-event", Compressible, Binary)
    final lazy val `tone`: MediaType = new MediaType("audio", "tone", Compressible, Binary)
    final lazy val `uemclip`: MediaType = new MediaType("audio", "uemclip", Compressible, Binary)
    final lazy val `ulpfec`: MediaType = new MediaType("audio", "ulpfec", Compressible, Binary)
    final lazy val `usac`: MediaType = new MediaType("audio", "usac", Compressible, Binary)
    final lazy val `vdvi`: MediaType = new MediaType("audio", "vdvi", Compressible, Binary)
    final lazy val `vmr-wb`: MediaType = new MediaType("audio", "vmr-wb", Compressible, Binary)
    final lazy val `vnd.3gpp.iufp`: MediaType =
      new MediaType("audio", "vnd.3gpp.iufp", Compressible, Binary)
    final lazy val `vnd.4sb`: MediaType = new MediaType("audio", "vnd.4sb", Compressible, Binary)
    final lazy val `vnd.audiokoz`: MediaType =
      new MediaType("audio", "vnd.audiokoz", Compressible, Binary)
    final lazy val `vnd.celp`: MediaType = new MediaType("audio", "vnd.celp", Compressible, Binary)
    final lazy val `vnd.cisco.nse`: MediaType =
      new MediaType("audio", "vnd.cisco.nse", Compressible, Binary)
    final lazy val `vnd.cmles.radio-events`: MediaType =
      new MediaType("audio", "vnd.cmles.radio-events", Compressible, Binary)
    final lazy val `vnd.cns.anp1`: MediaType =
      new MediaType("audio", "vnd.cns.anp1", Compressible, Binary)
    final lazy val `vnd.cns.inf1`: MediaType =
      new MediaType("audio", "vnd.cns.inf1", Compressible, Binary)
    final lazy val `vnd.dece.audio`: MediaType =
      new MediaType("audio", "vnd.dece.audio", Compressible, Binary, List("uva", "uvva"))
    final lazy val `vnd.digital-winds`: MediaType =
      new MediaType("audio", "vnd.digital-winds", Compressible, Binary, List("eol"))
    final lazy val `vnd.dlna.adts`: MediaType =
      new MediaType("audio", "vnd.dlna.adts", Compressible, Binary)
    final lazy val `vnd.dolby.heaac.1`: MediaType =
      new MediaType("audio", "vnd.dolby.heaac.1", Compressible, Binary)
    final lazy val `vnd.dolby.heaac.2`: MediaType =
      new MediaType("audio", "vnd.dolby.heaac.2", Compressible, Binary)
    final lazy val `vnd.dolby.mlp`: MediaType =
      new MediaType("audio", "vnd.dolby.mlp", Compressible, Binary)
    final lazy val `vnd.dolby.mps`: MediaType =
      new MediaType("audio", "vnd.dolby.mps", Compressible, Binary)
    final lazy val `vnd.dolby.pl2`: MediaType =
      new MediaType("audio", "vnd.dolby.pl2", Compressible, Binary)
    final lazy val `vnd.dolby.pl2x`: MediaType =
      new MediaType("audio", "vnd.dolby.pl2x", Compressible, Binary)
    final lazy val `vnd.dolby.pl2z`: MediaType =
      new MediaType("audio", "vnd.dolby.pl2z", Compressible, Binary)
    final lazy val `vnd.dolby.pulse.1`: MediaType =
      new MediaType("audio", "vnd.dolby.pulse.1", Compressible, Binary)
    final lazy val `vnd.dra`: MediaType =
      new MediaType("audio", "vnd.dra", Compressible, Binary, List("dra"))
    final lazy val `vnd.dts`: MediaType =
      new MediaType("audio", "vnd.dts", Compressible, Binary, List("dts"))
    final lazy val `vnd.dts.hd`: MediaType =
      new MediaType("audio", "vnd.dts.hd", Compressible, Binary, List("dtshd"))
    final lazy val `vnd.dvb.file`: MediaType =
      new MediaType("audio", "vnd.dvb.file", Compressible, Binary)
    final lazy val `vnd.everad.plj`: MediaType =
      new MediaType("audio", "vnd.everad.plj", Compressible, Binary)
    final lazy val `vnd.hns.audio`: MediaType =
      new MediaType("audio", "vnd.hns.audio", Compressible, Binary)
    final lazy val `vnd.lucent.voice`: MediaType =
      new MediaType("audio", "vnd.lucent.voice", Compressible, Binary, List("lvp"))
    final lazy val `vnd.ms-playready.media.pya`: MediaType =
      new MediaType("audio", "vnd.ms-playready.media.pya", Compressible, Binary, List("pya"))
    final lazy val `vnd.nokia.mobile-xmf`: MediaType =
      new MediaType("audio", "vnd.nokia.mobile-xmf", Compressible, Binary)
    final lazy val `vnd.nortel.vbk`: MediaType =
      new MediaType("audio", "vnd.nortel.vbk", Compressible, Binary)
    final lazy val `vnd.nuera.ecelp4800`: MediaType =
      new MediaType("audio", "vnd.nuera.ecelp4800", Compressible, Binary, List("ecelp4800"))
    final lazy val `vnd.nuera.ecelp7470`: MediaType =
      new MediaType("audio", "vnd.nuera.ecelp7470", Compressible, Binary, List("ecelp7470"))
    final lazy val `vnd.nuera.ecelp9600`: MediaType =
      new MediaType("audio", "vnd.nuera.ecelp9600", Compressible, Binary, List("ecelp9600"))
    final lazy val `vnd.octel.sbc`: MediaType =
      new MediaType("audio", "vnd.octel.sbc", Compressible, Binary)
    final lazy val `vnd.presonus.multitrack`: MediaType =
      new MediaType("audio", "vnd.presonus.multitrack", Compressible, Binary)
    final lazy val `vnd.qcelp`: MediaType = new MediaType("audio", "vnd.qcelp", Compressible, Binary)
    final lazy val `vnd.rhetorex.32kadpcm`: MediaType =
      new MediaType("audio", "vnd.rhetorex.32kadpcm", Compressible, Binary)
    final lazy val `vnd.rip`: MediaType =
      new MediaType("audio", "vnd.rip", Compressible, Binary, List("rip"))
    final lazy val `vnd.rn-realaudio`: MediaType =
      new MediaType("audio", "vnd.rn-realaudio", Uncompressible, Binary)
    final lazy val `vnd.sealedmedia.softseal.mpeg`: MediaType =
      new MediaType("audio", "vnd.sealedmedia.softseal.mpeg", Compressible, Binary)
    final lazy val `vnd.vmx.cvsd`: MediaType =
      new MediaType("audio", "vnd.vmx.cvsd", Compressible, Binary)
    final lazy val `vnd.wave`: MediaType = new MediaType("audio", "vnd.wave", Uncompressible, Binary)
    final lazy val `vorbis`: MediaType = new MediaType("audio", "vorbis", Uncompressible, Binary)
    final lazy val `vorbis-config`: MediaType =
      new MediaType("audio", "vorbis-config", Compressible, Binary)
    final lazy val `wav`: MediaType = new MediaType("audio", "wav", Uncompressible, Binary, List("wav"))
    final lazy val `wave`: MediaType = new MediaType("audio", "wave", Uncompressible, Binary, List("wav"))
    final lazy val `webm`: MediaType =
      new MediaType("audio", "webm", Uncompressible, Binary, List("weba"))
    final lazy val `x-aac`: MediaType =
      new MediaType("audio", "x-aac", Uncompressible, Binary, List("aac"))
    final lazy val `x-aiff`: MediaType =
      new MediaType("audio", "x-aiff", Compressible, Binary, List("aif", "aiff", "aifc"))
    final lazy val `x-caf`: MediaType =
      new MediaType("audio", "x-caf", Uncompressible, Binary, List("caf"))
    final lazy val `x-flac`: MediaType =
      new MediaType("audio", "x-flac", Compressible, Binary, List("flac"))
    final lazy val `x-m4a`: MediaType = new MediaType("audio", "x-m4a", Compressible, Binary, List("m4a"))
    final lazy val `x-matroska`: MediaType =
      new MediaType("audio", "x-matroska", Compressible, Binary, List("mka"))
    final lazy val `x-mpegurl`: MediaType =
      new MediaType("audio", "x-mpegurl", Compressible, Binary, List("m3u"))
    final lazy val `x-ms-wax`: MediaType =
      new MediaType("audio", "x-ms-wax", Compressible, Binary, List("wax"))
    final lazy val `x-ms-wma`: MediaType =
      new MediaType("audio", "x-ms-wma", Compressible, Binary, List("wma"))
    final lazy val `x-pn-realaudio`: MediaType =
      new MediaType("audio", "x-pn-realaudio", Compressible, Binary, List("ram", "ra"))
    final lazy val `x-pn-realaudio-plugin`: MediaType =
      new MediaType("audio", "x-pn-realaudio-plugin", Compressible, Binary, List("rmp"))
    final lazy val `x-realaudio`: MediaType =
      new MediaType("audio", "x-realaudio", Compressible, Binary, List("ra"))
    final lazy val `x-tta`: MediaType = new MediaType("audio", "x-tta", Compressible, Binary)
    final lazy val `x-wav`: MediaType = new MediaType("audio", "x-wav", Compressible, Binary, List("wav"))
    final lazy val `xm`: MediaType = new MediaType("audio", "xm", Compressible, Binary, List("xm"))
    final lazy val all: List[MediaType] = List(
      `1d-interleaved-parityfec`,
      `32kadpcm`,
      `3gpp`,
      `3gpp2`,
      `aac`,
      `ac3`,
      `adpcm`,
      `amr`,
      `amr-wb`,
      `amr-wb+`,
      `aptx`,
      `asc`,
      `atrac-advanced-lossless`,
      `atrac-x`,
      `atrac3`,
      `basic`,
      `bv16`,
      `bv32`,
      `clearmode`,
      `cn`,
      `dat12`,
      `dls`,
      `dsr-es201108`,
      `dsr-es202050`,
      `dsr-es202211`,
      `dsr-es202212`,
      `dv`,
      `dvi4`,
      `eac3`,
      `encaprtp`,
      `evrc`,
      `evrc-qcp`,
      `evrc0`,
      `evrc1`,
      `evrcb`,
      `evrcb0`,
      `evrcb1`,
      `evrcnw`,
      `evrcnw0`,
      `evrcnw1`,
      `evrcwb`,
      `evrcwb0`,
      `evrcwb1`,
      `evs`,
      `fwdred`,
      `g711-0`,
      `g719`,
      `g722`,
      `g7221`,
      `g723`,
      `g726-16`,
      `g726-24`,
      `g726-32`,
      `g726-40`,
      `g728`,
      `g729`,
      `g7291`,
      `g729d`,
      `g729e`,
      `gsm`,
      `gsm-efr`,
      `gsm-hr-08`,
      `ilbc`,
      `ip-mr_v2.5`,
      `isac`,
      `l16`,
      `l20`,
      `l24`,
      `l8`,
      `lpc`,
      `melp`,
      `melp1200`,
      `melp2400`,
      `melp600`,
      `midi`,
      `mobile-xmf`,
      `mp3`,
      `mp4`,
      `mp4a-latm`,
      `mpa`,
      `mpa-robust`,
      `mpeg`,
      `mpeg4-generic`,
      `musepack`,
      `ogg`,
      `opus`,
      `parityfec`,
      `pcma`,
      `pcma-wb`,
      `pcmu`,
      `pcmu-wb`,
      `prs.sid`,
      `qcelp`,
      `raptorfec`,
      `red`,
      `rtp-enc-aescm128`,
      `rtp-midi`,
      `rtploopback`,
      `rtx`,
      `s3m`,
      `silk`,
      `smv`,
      `smv-qcp`,
      `smv0`,
      `sp-midi`,
      `speex`,
      `t140c`,
      `t38`,
      `telephone-event`,
      `tone`,
      `uemclip`,
      `ulpfec`,
      `usac`,
      `vdvi`,
      `vmr-wb`,
      `vnd.3gpp.iufp`,
      `vnd.4sb`,
      `vnd.audiokoz`,
      `vnd.celp`,
      `vnd.cisco.nse`,
      `vnd.cmles.radio-events`,
      `vnd.cns.anp1`,
      `vnd.cns.inf1`,
      `vnd.dece.audio`,
      `vnd.digital-winds`,
      `vnd.dlna.adts`,
      `vnd.dolby.heaac.1`,
      `vnd.dolby.heaac.2`,
      `vnd.dolby.mlp`,
      `vnd.dolby.mps`,
      `vnd.dolby.pl2`,
      `vnd.dolby.pl2x`,
      `vnd.dolby.pl2z`,
      `vnd.dolby.pulse.1`,
      `vnd.dra`,
      `vnd.dts`,
      `vnd.dts.hd`,
      `vnd.dvb.file`,
      `vnd.everad.plj`,
      `vnd.hns.audio`,
      `vnd.lucent.voice`,
      `vnd.ms-playready.media.pya`,
      `vnd.nokia.mobile-xmf`,
      `vnd.nortel.vbk`,
      `vnd.nuera.ecelp4800`,
      `vnd.nuera.ecelp7470`,
      `vnd.nuera.ecelp9600`,
      `vnd.octel.sbc`,
      `vnd.presonus.multitrack`,
      `vnd.qcelp`,
      `vnd.rhetorex.32kadpcm`,
      `vnd.rip`,
      `vnd.rn-realaudio`,
      `vnd.sealedmedia.softseal.mpeg`,
      `vnd.vmx.cvsd`,
      `vnd.wave`,
      `vorbis`,
      `vorbis-config`,
      `wav`,
      `wave`,
      `webm`,
      `x-aac`,
      `x-aiff`,
      `x-caf`,
      `x-flac`,
      `x-m4a`,
      `x-matroska`,
      `x-mpegurl`,
      `x-ms-wax`,
      `x-ms-wma`,
      `x-pn-realaudio`,
      `x-pn-realaudio-plugin`,
      `x-realaudio`,
      `x-tta`,
      `x-wav`,
      `xm`
    )
  }
  object chemical {
    final lazy val `x-cdx`: MediaType =
      new MediaType("chemical", "x-cdx", Compressible, NotBinary, List("cdx"))
    final lazy val `x-cif`: MediaType =
      new MediaType("chemical", "x-cif", Compressible, NotBinary, List("cif"))
    final lazy val `x-cmdf`: MediaType =
      new MediaType("chemical", "x-cmdf", Compressible, NotBinary, List("cmdf"))
    final lazy val `x-cml`: MediaType =
      new MediaType("chemical", "x-cml", Compressible, NotBinary, List("cml"))
    final lazy val `x-csml`: MediaType =
      new MediaType("chemical", "x-csml", Compressible, NotBinary, List("csml"))
    final lazy val `x-pdb`: MediaType = new MediaType("chemical", "x-pdb", Compressible, NotBinary)
    final lazy val `x-xyz`: MediaType =
      new MediaType("chemical", "x-xyz", Compressible, NotBinary, List("xyz"))
    final lazy val all: List[MediaType] =
      List(`x-cdx`, `x-cif`, `x-cmdf`, `x-cml`, `x-csml`, `x-pdb`, `x-xyz`)
  }
  object font {
    final lazy val `collection`: MediaType =
      new MediaType("font", "collection", Compressible, NotBinary, List("ttc"))
    final lazy val `otf`: MediaType = new MediaType("font", "otf", Compressible, NotBinary, List("otf"))
    final lazy val `sfnt`: MediaType = new MediaType("font", "sfnt", Compressible, NotBinary)
    final lazy val `ttf`: MediaType = new MediaType("font", "ttf", Compressible, NotBinary, List("ttf"))
    final lazy val `woff`: MediaType =
      new MediaType("font", "woff", Compressible, NotBinary, List("woff"))
    final lazy val `woff2`: MediaType =
      new MediaType("font", "woff2", Compressible, NotBinary, List("woff2"))
    final lazy val all: List[MediaType] = List(`collection`, `otf`, `sfnt`, `ttf`, `woff`, `woff2`)
  }
  object image {
    final lazy val `aces`: MediaType = new MediaType("image", "aces", Compressible, Binary, List("exr"))
    final lazy val `apng`: MediaType =
      new MediaType("image", "apng", Uncompressible, Binary, List("apng"))
    final lazy val `avci`: MediaType = new MediaType("image", "avci", Compressible, Binary)
    final lazy val `avcs`: MediaType = new MediaType("image", "avcs", Compressible, Binary)
    final lazy val `bmp`: MediaType = new MediaType("image", "bmp", Compressible, Binary, List("bmp"))
    final lazy val `cgm`: MediaType = new MediaType("image", "cgm", Compressible, Binary, List("cgm"))
    final lazy val `dicom-rle`: MediaType =
      new MediaType("image", "dicom-rle", Compressible, Binary, List("drle"))
    final lazy val `emf`: MediaType = new MediaType("image", "emf", Compressible, Binary, List("emf"))
    final lazy val `fits`: MediaType = new MediaType("image", "fits", Compressible, Binary, List("fits"))
    final lazy val `g3fax`: MediaType = new MediaType("image", "g3fax", Compressible, Binary, List("g3"))
    final lazy val `gif`: MediaType = new MediaType("image", "gif", Uncompressible, Binary, List("gif"))
    final lazy val `heic`: MediaType = new MediaType("image", "heic", Compressible, Binary, List("heic"))
    final lazy val `heic-sequence`: MediaType =
      new MediaType("image", "heic-sequence", Compressible, Binary, List("heics"))
    final lazy val `heif`: MediaType = new MediaType("image", "heif", Compressible, Binary, List("heif"))
    final lazy val `heif-sequence`: MediaType =
      new MediaType("image", "heif-sequence", Compressible, Binary, List("heifs"))
    final lazy val `ief`: MediaType = new MediaType("image", "ief", Compressible, Binary, List("ief"))
    final lazy val `jls`: MediaType = new MediaType("image", "jls", Compressible, Binary, List("jls"))
    final lazy val `jp2`: MediaType =
      new MediaType("image", "jp2", Uncompressible, Binary, List("jp2", "jpg2"))
    final lazy val `jpeg`: MediaType =
      new MediaType("image", "jpeg", Uncompressible, Binary, List("jpeg", "jpg", "jpe"))
    final lazy val `jpm`: MediaType = new MediaType("image", "jpm", Uncompressible, Binary, List("jpm"))
    final lazy val `jpx`: MediaType =
      new MediaType("image", "jpx", Uncompressible, Binary, List("jpx", "jpf"))
    final lazy val `ktx`: MediaType = new MediaType("image", "ktx", Compressible, Binary, List("ktx"))
    final lazy val `naplps`: MediaType = new MediaType("image", "naplps", Compressible, Binary)
    final lazy val `pjpeg`: MediaType = new MediaType("image", "pjpeg", Uncompressible, Binary)
    final lazy val `png`: MediaType = new MediaType("image", "png", Uncompressible, Binary, List("png"))
    final lazy val `prs.btif`: MediaType =
      new MediaType("image", "prs.btif", Compressible, Binary, List("btif"))
    final lazy val `prs.pti`: MediaType =
      new MediaType("image", "prs.pti", Compressible, Binary, List("pti"))
    final lazy val `pwg-raster`: MediaType = new MediaType("image", "pwg-raster", Compressible, Binary)
    final lazy val `sgi`: MediaType = new MediaType("image", "sgi", Compressible, Binary, List("sgi"))
    final lazy val `svg+xml`: MediaType =
      new MediaType("image", "svg+xml", Compressible, Binary, List("svg", "svgz"))
    final lazy val `t38`: MediaType = new MediaType("image", "t38", Compressible, Binary, List("t38"))
    final lazy val `tiff`: MediaType =
      new MediaType("image", "tiff", Uncompressible, Binary, List("tif", "tiff"))
    final lazy val `tiff-fx`: MediaType =
      new MediaType("image", "tiff-fx", Compressible, Binary, List("tfx"))
    final lazy val `vnd.adobe.photoshop`: MediaType =
      new MediaType("image", "vnd.adobe.photoshop", Compressible, Binary, List("psd"))
    final lazy val `vnd.airzip.accelerator.azv`: MediaType =
      new MediaType("image", "vnd.airzip.accelerator.azv", Compressible, Binary, List("azv"))
    final lazy val `vnd.cns.inf2`: MediaType =
      new MediaType("image", "vnd.cns.inf2", Compressible, Binary)
    final lazy val `vnd.dece.graphic`: MediaType = new MediaType(
      "image",
      "vnd.dece.graphic",
      Compressible,
      Binary,
      List("uvi", "uvvi", "uvg", "uvvg"))
    final lazy val `vnd.djvu`: MediaType =
      new MediaType("image", "vnd.djvu", Compressible, Binary, List("djvu", "djv"))
    final lazy val `vnd.dvb.subtitle`: MediaType =
      new MediaType("image", "vnd.dvb.subtitle", Compressible, Binary, List("sub"))
    final lazy val `vnd.dwg`: MediaType =
      new MediaType("image", "vnd.dwg", Compressible, Binary, List("dwg"))
    final lazy val `vnd.dxf`: MediaType =
      new MediaType("image", "vnd.dxf", Compressible, Binary, List("dxf"))
    final lazy val `vnd.fastbidsheet`: MediaType =
      new MediaType("image", "vnd.fastbidsheet", Compressible, Binary, List("fbs"))
    final lazy val `vnd.fpx`: MediaType =
      new MediaType("image", "vnd.fpx", Compressible, Binary, List("fpx"))
    final lazy val `vnd.fst`: MediaType =
      new MediaType("image", "vnd.fst", Compressible, Binary, List("fst"))
    final lazy val `vnd.fujixerox.edmics-mmr`: MediaType =
      new MediaType("image", "vnd.fujixerox.edmics-mmr", Compressible, Binary, List("mmr"))
    final lazy val `vnd.fujixerox.edmics-rlc`: MediaType =
      new MediaType("image", "vnd.fujixerox.edmics-rlc", Compressible, Binary, List("rlc"))
    final lazy val `vnd.globalgraphics.pgb`: MediaType =
      new MediaType("image", "vnd.globalgraphics.pgb", Compressible, Binary)
    final lazy val `vnd.microsoft.icon`: MediaType =
      new MediaType("image", "vnd.microsoft.icon", Compressible, Binary, List("ico"))
    final lazy val `vnd.mix`: MediaType = new MediaType("image", "vnd.mix", Compressible, Binary)
    final lazy val `vnd.mozilla.apng`: MediaType =
      new MediaType("image", "vnd.mozilla.apng", Compressible, Binary)
    final lazy val `vnd.ms-modi`: MediaType =
      new MediaType("image", "vnd.ms-modi", Compressible, Binary, List("mdi"))
    final lazy val `vnd.ms-photo`: MediaType =
      new MediaType("image", "vnd.ms-photo", Compressible, Binary, List("wdp"))
    final lazy val `vnd.net-fpx`: MediaType =
      new MediaType("image", "vnd.net-fpx", Compressible, Binary, List("npx"))
    final lazy val `vnd.radiance`: MediaType =
      new MediaType("image", "vnd.radiance", Compressible, Binary)
    final lazy val `vnd.sealed.png`: MediaType =
      new MediaType("image", "vnd.sealed.png", Compressible, Binary)
    final lazy val `vnd.sealedmedia.softseal.gif`: MediaType =
      new MediaType("image", "vnd.sealedmedia.softseal.gif", Compressible, Binary)
    final lazy val `vnd.sealedmedia.softseal.jpg`: MediaType =
      new MediaType("image", "vnd.sealedmedia.softseal.jpg", Compressible, Binary)
    final lazy val `vnd.svf`: MediaType = new MediaType("image", "vnd.svf", Compressible, Binary)
    final lazy val `vnd.tencent.tap`: MediaType =
      new MediaType("image", "vnd.tencent.tap", Compressible, Binary, List("tap"))
    final lazy val `vnd.valve.source.texture`: MediaType =
      new MediaType("image", "vnd.valve.source.texture", Compressible, Binary, List("vtf"))
    final lazy val `vnd.wap.wbmp`: MediaType =
      new MediaType("image", "vnd.wap.wbmp", Compressible, Binary, List("wbmp"))
    final lazy val `vnd.xiff`: MediaType =
      new MediaType("image", "vnd.xiff", Compressible, Binary, List("xif"))
    final lazy val `vnd.zbrush.pcx`: MediaType =
      new MediaType("image", "vnd.zbrush.pcx", Compressible, Binary, List("pcx"))
    final lazy val `webp`: MediaType = new MediaType("image", "webp", Compressible, Binary, List("webp"))
    final lazy val `wmf`: MediaType = new MediaType("image", "wmf", Compressible, Binary, List("wmf"))
    final lazy val `x-3ds`: MediaType = new MediaType("image", "x-3ds", Compressible, Binary, List("3ds"))
    final lazy val `x-cmu-raster`: MediaType =
      new MediaType("image", "x-cmu-raster", Compressible, Binary, List("ras"))
    final lazy val `x-cmx`: MediaType = new MediaType("image", "x-cmx", Compressible, Binary, List("cmx"))
    final lazy val `x-freehand`: MediaType = new MediaType(
      "image",
      "x-freehand",
      Compressible,
      Binary,
      List("fh", "fhc", "fh4", "fh5", "fh7"))
    final lazy val `x-icon`: MediaType =
      new MediaType("image", "x-icon", Compressible, Binary, List("ico"))
    final lazy val `x-jng`: MediaType = new MediaType("image", "x-jng", Compressible, Binary, List("jng"))
    final lazy val `x-mrsid-image`: MediaType =
      new MediaType("image", "x-mrsid-image", Compressible, Binary, List("sid"))
    final lazy val `x-ms-bmp`: MediaType =
      new MediaType("image", "x-ms-bmp", Compressible, Binary, List("bmp"))
    final lazy val `x-pcx`: MediaType = new MediaType("image", "x-pcx", Compressible, Binary, List("pcx"))
    final lazy val `x-pict`: MediaType =
      new MediaType("image", "x-pict", Compressible, Binary, List("pic", "pct"))
    final lazy val `x-portable-anymap`: MediaType =
      new MediaType("image", "x-portable-anymap", Compressible, Binary, List("pnm"))
    final lazy val `x-portable-bitmap`: MediaType =
      new MediaType("image", "x-portable-bitmap", Compressible, Binary, List("pbm"))
    final lazy val `x-portable-graymap`: MediaType =
      new MediaType("image", "x-portable-graymap", Compressible, Binary, List("pgm"))
    final lazy val `x-portable-pixmap`: MediaType =
      new MediaType("image", "x-portable-pixmap", Compressible, Binary, List("ppm"))
    final lazy val `x-rgb`: MediaType = new MediaType("image", "x-rgb", Compressible, Binary, List("rgb"))
    final lazy val `x-tga`: MediaType = new MediaType("image", "x-tga", Compressible, Binary, List("tga"))
    final lazy val `x-xbitmap`: MediaType =
      new MediaType("image", "x-xbitmap", Compressible, Binary, List("xbm"))
    final lazy val `x-xcf`: MediaType = new MediaType("image", "x-xcf", Uncompressible, Binary)
    final lazy val `x-xpixmap`: MediaType =
      new MediaType("image", "x-xpixmap", Compressible, Binary, List("xpm"))
    final lazy val `x-xwindowdump`: MediaType =
      new MediaType("image", "x-xwindowdump", Compressible, Binary, List("xwd"))
    final lazy val all: List[MediaType] = List(
      `aces`,
      `apng`,
      `avci`,
      `avcs`,
      `bmp`,
      `cgm`,
      `dicom-rle`,
      `emf`,
      `fits`,
      `g3fax`,
      `gif`,
      `heic`,
      `heic-sequence`,
      `heif`,
      `heif-sequence`,
      `ief`,
      `jls`,
      `jp2`,
      `jpeg`,
      `jpm`,
      `jpx`,
      `ktx`,
      `naplps`,
      `pjpeg`,
      `png`,
      `prs.btif`,
      `prs.pti`,
      `pwg-raster`,
      `sgi`,
      `svg+xml`,
      `t38`,
      `tiff`,
      `tiff-fx`,
      `vnd.adobe.photoshop`,
      `vnd.airzip.accelerator.azv`,
      `vnd.cns.inf2`,
      `vnd.dece.graphic`,
      `vnd.djvu`,
      `vnd.dvb.subtitle`,
      `vnd.dwg`,
      `vnd.dxf`,
      `vnd.fastbidsheet`,
      `vnd.fpx`,
      `vnd.fst`,
      `vnd.fujixerox.edmics-mmr`,
      `vnd.fujixerox.edmics-rlc`,
      `vnd.globalgraphics.pgb`,
      `vnd.microsoft.icon`,
      `vnd.mix`,
      `vnd.mozilla.apng`,
      `vnd.ms-modi`,
      `vnd.ms-photo`,
      `vnd.net-fpx`,
      `vnd.radiance`,
      `vnd.sealed.png`,
      `vnd.sealedmedia.softseal.gif`,
      `vnd.sealedmedia.softseal.jpg`,
      `vnd.svf`,
      `vnd.tencent.tap`,
      `vnd.valve.source.texture`,
      `vnd.wap.wbmp`,
      `vnd.xiff`,
      `vnd.zbrush.pcx`,
      `webp`,
      `wmf`,
      `x-3ds`,
      `x-cmu-raster`,
      `x-cmx`,
      `x-freehand`,
      `x-icon`,
      `x-jng`,
      `x-mrsid-image`,
      `x-ms-bmp`,
      `x-pcx`,
      `x-pict`,
      `x-portable-anymap`,
      `x-portable-bitmap`,
      `x-portable-graymap`,
      `x-portable-pixmap`,
      `x-rgb`,
      `x-tga`,
      `x-xbitmap`,
      `x-xcf`,
      `x-xpixmap`,
      `x-xwindowdump`
    )
  }
  object message {
    final lazy val `cpim`: MediaType = new MediaType("message", "cpim", Compressible, NotBinary)
    final lazy val `delivery-status`: MediaType =
      new MediaType("message", "delivery-status", Compressible, NotBinary)
    final lazy val `disposition-notification`: MediaType = new MediaType(
      "message",
      "disposition-notification",
      Compressible,
      NotBinary,
      List("disposition-notification"))
    final lazy val `external-body`: MediaType =
      new MediaType("message", "external-body", Compressible, NotBinary)
    final lazy val `feedback-report`: MediaType =
      new MediaType("message", "feedback-report", Compressible, NotBinary)
    final lazy val `global`: MediaType =
      new MediaType("message", "global", Compressible, NotBinary, List("u8msg"))
    final lazy val `global-delivery-status`: MediaType =
      new MediaType("message", "global-delivery-status", Compressible, NotBinary, List("u8dsn"))
    final lazy val `global-disposition-notification`: MediaType = new MediaType(
      "message",
      "global-disposition-notification",
      Compressible,
      NotBinary,
      List("u8mdn"))
    final lazy val `global-headers`: MediaType =
      new MediaType("message", "global-headers", Compressible, NotBinary, List("u8hdr"))
    final lazy val `http`: MediaType = new MediaType("message", "http", Uncompressible, NotBinary)
    final lazy val `imdn+xml`: MediaType = new MediaType("message", "imdn+xml", Compressible, NotBinary)
    final lazy val `news`: MediaType = new MediaType("message", "news", Compressible, NotBinary)
    final lazy val `partial`: MediaType = new MediaType("message", "partial", Uncompressible, NotBinary)
    final lazy val `rfc822`: MediaType =
      new MediaType("message", "rfc822", Compressible, NotBinary, List("eml", "mime"))
    final lazy val `s-http`: MediaType = new MediaType("message", "s-http", Compressible, NotBinary)
    final lazy val `sip`: MediaType = new MediaType("message", "sip", Compressible, NotBinary)
    final lazy val `sipfrag`: MediaType = new MediaType("message", "sipfrag", Compressible, NotBinary)
    final lazy val `tracking-status`: MediaType =
      new MediaType("message", "tracking-status", Compressible, NotBinary)
    final lazy val `vnd.si.simp`: MediaType =
      new MediaType("message", "vnd.si.simp", Compressible, NotBinary)
    final lazy val `vnd.wfa.wsc`: MediaType =
      new MediaType("message", "vnd.wfa.wsc", Compressible, NotBinary, List("wsc"))
    final lazy val all: List[MediaType] = List(
      `cpim`,
      `delivery-status`,
      `disposition-notification`,
      `external-body`,
      `feedback-report`,
      `global`,
      `global-delivery-status`,
      `global-disposition-notification`,
      `global-headers`,
      `http`,
      `imdn+xml`,
      `news`,
      `partial`,
      `rfc822`,
      `s-http`,
      `sip`,
      `sipfrag`,
      `tracking-status`,
      `vnd.si.simp`,
      `vnd.wfa.wsc`
    )
  }
  object model {
    final lazy val `3mf`: MediaType = new MediaType("model", "3mf", Compressible, NotBinary)
    final lazy val `gltf+json`: MediaType =
      new MediaType("model", "gltf+json", Compressible, NotBinary, List("gltf"))
    final lazy val `gltf-binary`: MediaType =
      new MediaType("model", "gltf-binary", Compressible, NotBinary, List("glb"))
    final lazy val `iges`: MediaType =
      new MediaType("model", "iges", Uncompressible, NotBinary, List("igs", "iges"))
    final lazy val `mesh`: MediaType =
      new MediaType("model", "mesh", Uncompressible, NotBinary, List("msh", "mesh", "silo"))
    final lazy val `stl`: MediaType = new MediaType("model", "stl", Compressible, NotBinary)
    final lazy val `vnd.collada+xml`: MediaType =
      new MediaType("model", "vnd.collada+xml", Compressible, NotBinary, List("dae"))
    final lazy val `vnd.dwf`: MediaType =
      new MediaType("model", "vnd.dwf", Compressible, NotBinary, List("dwf"))
    final lazy val `vnd.flatland.3dml`: MediaType =
      new MediaType("model", "vnd.flatland.3dml", Compressible, NotBinary)
    final lazy val `vnd.gdl`: MediaType =
      new MediaType("model", "vnd.gdl", Compressible, NotBinary, List("gdl"))
    final lazy val `vnd.gs-gdl`: MediaType = new MediaType("model", "vnd.gs-gdl", Compressible, NotBinary)
    final lazy val `vnd.gs.gdl`: MediaType = new MediaType("model", "vnd.gs.gdl", Compressible, NotBinary)
    final lazy val `vnd.gtw`: MediaType =
      new MediaType("model", "vnd.gtw", Compressible, NotBinary, List("gtw"))
    final lazy val `vnd.moml+xml`: MediaType =
      new MediaType("model", "vnd.moml+xml", Compressible, NotBinary)
    final lazy val `vnd.mts`: MediaType =
      new MediaType("model", "vnd.mts", Compressible, NotBinary, List("mts"))
    final lazy val `vnd.opengex`: MediaType =
      new MediaType("model", "vnd.opengex", Compressible, NotBinary)
    final lazy val `vnd.parasolid.transmit.binary`: MediaType =
      new MediaType("model", "vnd.parasolid.transmit.binary", Compressible, NotBinary)
    final lazy val `vnd.parasolid.transmit.text`: MediaType =
      new MediaType("model", "vnd.parasolid.transmit.text", Compressible, NotBinary)
    final lazy val `vnd.rosette.annotated-data-model`: MediaType =
      new MediaType("model", "vnd.rosette.annotated-data-model", Compressible, NotBinary)
    final lazy val `vnd.usdz+zip`: MediaType =
      new MediaType("model", "vnd.usdz+zip", Uncompressible, NotBinary)
    final lazy val `vnd.valve.source.compiled-map`: MediaType =
      new MediaType("model", "vnd.valve.source.compiled-map", Compressible, NotBinary)
    final lazy val `vnd.vtu`: MediaType =
      new MediaType("model", "vnd.vtu", Compressible, NotBinary, List("vtu"))
    final lazy val `vrml`: MediaType =
      new MediaType("model", "vrml", Uncompressible, NotBinary, List("wrl", "vrml"))
    final lazy val `x3d+binary`: MediaType =
      new MediaType("model", "x3d+binary", Uncompressible, NotBinary, List("x3db", "x3dbz"))
    final lazy val `x3d+fastinfoset`: MediaType =
      new MediaType("model", "x3d+fastinfoset", Compressible, NotBinary)
    final lazy val `x3d+vrml`: MediaType =
      new MediaType("model", "x3d+vrml", Uncompressible, NotBinary, List("x3dv", "x3dvz"))
    final lazy val `x3d+xml`: MediaType =
      new MediaType("model", "x3d+xml", Compressible, NotBinary, List("x3d", "x3dz"))
    final lazy val `x3d-vrml`: MediaType = new MediaType("model", "x3d-vrml", Compressible, NotBinary)
    final lazy val all: List[MediaType] = List(
      `3mf`,
      `gltf+json`,
      `gltf-binary`,
      `iges`,
      `mesh`,
      `stl`,
      `vnd.collada+xml`,
      `vnd.dwf`,
      `vnd.flatland.3dml`,
      `vnd.gdl`,
      `vnd.gs-gdl`,
      `vnd.gs.gdl`,
      `vnd.gtw`,
      `vnd.moml+xml`,
      `vnd.mts`,
      `vnd.opengex`,
      `vnd.parasolid.transmit.binary`,
      `vnd.parasolid.transmit.text`,
      `vnd.rosette.annotated-data-model`,
      `vnd.usdz+zip`,
      `vnd.valve.source.compiled-map`,
      `vnd.vtu`,
      `vrml`,
      `x3d+binary`,
      `x3d+fastinfoset`,
      `x3d+vrml`,
      `x3d+xml`,
      `x3d-vrml`
    )
  }
  object multipart {
    final lazy val `alternative`: MediaType =
      new MediaType("multipart", "alternative", Uncompressible, NotBinary)
    final lazy val `appledouble`: MediaType =
      new MediaType("multipart", "appledouble", Compressible, NotBinary)
    final lazy val `byteranges`: MediaType =
      new MediaType("multipart", "byteranges", Compressible, NotBinary)
    final lazy val `digest`: MediaType = new MediaType("multipart", "digest", Compressible, NotBinary)
    final lazy val `encrypted`: MediaType =
      new MediaType("multipart", "encrypted", Uncompressible, NotBinary)
    final lazy val `form-data`: MediaType =
      new MediaType("multipart", "form-data", Uncompressible, NotBinary)
    final lazy val `header-set`: MediaType =
      new MediaType("multipart", "header-set", Compressible, NotBinary)
    final lazy val `mixed`: MediaType = new MediaType("multipart", "mixed", Uncompressible, NotBinary)
    final lazy val `multilingual`: MediaType =
      new MediaType("multipart", "multilingual", Compressible, NotBinary)
    final lazy val `parallel`: MediaType = new MediaType("multipart", "parallel", Compressible, NotBinary)
    final lazy val `related`: MediaType = new MediaType("multipart", "related", Uncompressible, NotBinary)
    final lazy val `report`: MediaType = new MediaType("multipart", "report", Compressible, NotBinary)
    final lazy val `signed`: MediaType = new MediaType("multipart", "signed", Uncompressible, NotBinary)
    final lazy val `vnd.bint.med-plus`: MediaType =
      new MediaType("multipart", "vnd.bint.med-plus", Compressible, NotBinary)
    final lazy val `voice-message`: MediaType =
      new MediaType("multipart", "voice-message", Compressible, NotBinary)
    final lazy val `x-mixed-replace`: MediaType =
      new MediaType("multipart", "x-mixed-replace", Compressible, NotBinary)
    final lazy val all: List[MediaType] = List(
      `alternative`,
      `appledouble`,
      `byteranges`,
      `digest`,
      `encrypted`,
      `form-data`,
      `header-set`,
      `mixed`,
      `multilingual`,
      `parallel`,
      `related`,
      `report`,
      `signed`,
      `vnd.bint.med-plus`,
      `voice-message`,
      `x-mixed-replace`
    )
  }
  object text {
    final lazy val `1d-interleaved-parityfec`: MediaType =
      new MediaType("text", "1d-interleaved-parityfec", Compressible, NotBinary)
    final lazy val `cache-manifest`: MediaType =
      new MediaType("text", "cache-manifest", Compressible, NotBinary, List("appcache", "manifest"))
    final lazy val `calendar`: MediaType =
      new MediaType("text", "calendar", Compressible, NotBinary, List("ics", "ifb"))
    final lazy val `calender`: MediaType = new MediaType("text", "calender", Compressible, NotBinary)
    final lazy val `cmd`: MediaType = new MediaType("text", "cmd", Compressible, NotBinary)
    final lazy val `coffeescript`: MediaType =
      new MediaType("text", "coffeescript", Compressible, NotBinary, List("coffee", "litcoffee"))
    final lazy val `css`: MediaType = new MediaType("text", "css", Compressible, NotBinary, List("css"))
    final lazy val `csv`: MediaType = new MediaType("text", "csv", Compressible, NotBinary, List("csv"))
    final lazy val `csv-schema`: MediaType = new MediaType("text", "csv-schema", Compressible, NotBinary)
    final lazy val `directory`: MediaType = new MediaType("text", "directory", Compressible, NotBinary)
    final lazy val `dns`: MediaType = new MediaType("text", "dns", Compressible, NotBinary)
    final lazy val `ecmascript`: MediaType = new MediaType("text", "ecmascript", Compressible, NotBinary)
    final lazy val `encaprtp`: MediaType = new MediaType("text", "encaprtp", Compressible, NotBinary)
    final lazy val `enriched`: MediaType = new MediaType("text", "enriched", Compressible, NotBinary)
    final lazy val `fwdred`: MediaType = new MediaType("text", "fwdred", Compressible, NotBinary)
    final lazy val `grammar-ref-list`: MediaType =
      new MediaType("text", "grammar-ref-list", Compressible, NotBinary)
    final lazy val `html`: MediaType =
      new MediaType("text", "html", Compressible, NotBinary, List("html", "htm", "shtml"))
    final lazy val `jade`: MediaType =
      new MediaType("text", "jade", Compressible, NotBinary, List("jade"))
    final lazy val `javascript`: MediaType = new MediaType("text", "javascript", Compressible, NotBinary)
    final lazy val `jcr-cnd`: MediaType = new MediaType("text", "jcr-cnd", Compressible, NotBinary)
    final lazy val `jsx`: MediaType = new MediaType("text", "jsx", Compressible, NotBinary, List("jsx"))
    final lazy val `less`: MediaType =
      new MediaType("text", "less", Compressible, NotBinary, List("less"))
    final lazy val `markdown`: MediaType =
      new MediaType("text", "markdown", Compressible, NotBinary, List("markdown", "md"))
    final lazy val `mathml`: MediaType =
      new MediaType("text", "mathml", Compressible, NotBinary, List("mml"))
    final lazy val `mizar`: MediaType = new MediaType("text", "mizar", Compressible, NotBinary)
    final lazy val `n3`: MediaType = new MediaType("text", "n3", Compressible, NotBinary, List("n3"))
    final lazy val `parameters`: MediaType = new MediaType("text", "parameters", Compressible, NotBinary)
    final lazy val `parityfec`: MediaType = new MediaType("text", "parityfec", Compressible, NotBinary)
    final lazy val `plain`: MediaType = new MediaType(
      "text",
      "plain",
      Compressible,
      NotBinary,
      List("txt", "text", "conf", "def", "list", "log", "in", "ini"))
    final lazy val `provenance-notation`: MediaType =
      new MediaType("text", "provenance-notation", Compressible, NotBinary)
    final lazy val `prs.fallenstein.rst`: MediaType =
      new MediaType("text", "prs.fallenstein.rst", Compressible, NotBinary)
    final lazy val `prs.lines.tag`: MediaType =
      new MediaType("text", "prs.lines.tag", Compressible, NotBinary, List("dsc"))
    final lazy val `prs.prop.logic`: MediaType =
      new MediaType("text", "prs.prop.logic", Compressible, NotBinary)
    final lazy val `raptorfec`: MediaType = new MediaType("text", "raptorfec", Compressible, NotBinary)
    final lazy val `red`: MediaType = new MediaType("text", "red", Compressible, NotBinary)
    final lazy val `rfc822-headers`: MediaType =
      new MediaType("text", "rfc822-headers", Compressible, NotBinary)
    final lazy val `richtext`: MediaType =
      new MediaType("text", "richtext", Compressible, NotBinary, List("rtx"))
    final lazy val `rtf`: MediaType = new MediaType("text", "rtf", Compressible, NotBinary, List("rtf"))
    final lazy val `rtp-enc-aescm128`: MediaType =
      new MediaType("text", "rtp-enc-aescm128", Compressible, NotBinary)
    final lazy val `rtploopback`: MediaType =
      new MediaType("text", "rtploopback", Compressible, NotBinary)
    final lazy val `rtx`: MediaType = new MediaType("text", "rtx", Compressible, NotBinary)
    final lazy val `sgml`: MediaType =
      new MediaType("text", "sgml", Compressible, NotBinary, List("sgml", "sgm"))
    final lazy val `shex`: MediaType =
      new MediaType("text", "shex", Compressible, NotBinary, List("shex"))
    final lazy val `slim`: MediaType =
      new MediaType("text", "slim", Compressible, NotBinary, List("slim", "slm"))
    final lazy val `strings`: MediaType = new MediaType("text", "strings", Compressible, NotBinary)
    final lazy val `stylus`: MediaType =
      new MediaType("text", "stylus", Compressible, NotBinary, List("stylus", "styl"))
    final lazy val `t140`: MediaType = new MediaType("text", "t140", Compressible, NotBinary)
    final lazy val `tab-separated-values`: MediaType =
      new MediaType("text", "tab-separated-values", Compressible, NotBinary, List("tsv"))
    final lazy val `troff`: MediaType = new MediaType(
      "text",
      "troff",
      Compressible,
      NotBinary,
      List("t", "tr", "roff", "man", "me", "ms"))
    final lazy val `turtle`: MediaType =
      new MediaType("text", "turtle", Compressible, NotBinary, List("ttl"))
    final lazy val `ulpfec`: MediaType = new MediaType("text", "ulpfec", Compressible, NotBinary)
    final lazy val `uri-list`: MediaType =
      new MediaType("text", "uri-list", Compressible, NotBinary, List("uri", "uris", "urls"))
    final lazy val `vcard`: MediaType =
      new MediaType("text", "vcard", Compressible, NotBinary, List("vcard"))
    final lazy val `vnd.a`: MediaType = new MediaType("text", "vnd.a", Compressible, NotBinary)
    final lazy val `vnd.abc`: MediaType = new MediaType("text", "vnd.abc", Compressible, NotBinary)
    final lazy val `vnd.ascii-art`: MediaType =
      new MediaType("text", "vnd.ascii-art", Compressible, NotBinary)
    final lazy val `vnd.curl`: MediaType =
      new MediaType("text", "vnd.curl", Compressible, NotBinary, List("curl"))
    final lazy val `vnd.curl.dcurl`: MediaType =
      new MediaType("text", "vnd.curl.dcurl", Compressible, NotBinary, List("dcurl"))
    final lazy val `vnd.curl.mcurl`: MediaType =
      new MediaType("text", "vnd.curl.mcurl", Compressible, NotBinary, List("mcurl"))
    final lazy val `vnd.curl.scurl`: MediaType =
      new MediaType("text", "vnd.curl.scurl", Compressible, NotBinary, List("scurl"))
    final lazy val `vnd.debian.copyright`: MediaType =
      new MediaType("text", "vnd.debian.copyright", Compressible, NotBinary)
    final lazy val `vnd.dmclientscript`: MediaType =
      new MediaType("text", "vnd.dmclientscript", Compressible, NotBinary)
    final lazy val `vnd.dvb.subtitle`: MediaType =
      new MediaType("text", "vnd.dvb.subtitle", Compressible, NotBinary, List("sub"))
    final lazy val `vnd.esmertec.theme-descriptor`: MediaType =
      new MediaType("text", "vnd.esmertec.theme-descriptor", Compressible, NotBinary)
    final lazy val `vnd.fly`: MediaType =
      new MediaType("text", "vnd.fly", Compressible, NotBinary, List("fly"))
    final lazy val `vnd.fmi.flexstor`: MediaType =
      new MediaType("text", "vnd.fmi.flexstor", Compressible, NotBinary, List("flx"))
    final lazy val `vnd.gml`: MediaType = new MediaType("text", "vnd.gml", Compressible, NotBinary)
    final lazy val `vnd.graphviz`: MediaType =
      new MediaType("text", "vnd.graphviz", Compressible, NotBinary, List("gv"))
    final lazy val `vnd.hgl`: MediaType = new MediaType("text", "vnd.hgl", Compressible, NotBinary)
    final lazy val `vnd.in3d.3dml`: MediaType =
      new MediaType("text", "vnd.in3d.3dml", Compressible, NotBinary, List("3dml"))
    final lazy val `vnd.in3d.spot`: MediaType =
      new MediaType("text", "vnd.in3d.spot", Compressible, NotBinary, List("spot"))
    final lazy val `vnd.iptc.newsml`: MediaType =
      new MediaType("text", "vnd.iptc.newsml", Compressible, NotBinary)
    final lazy val `vnd.iptc.nitf`: MediaType =
      new MediaType("text", "vnd.iptc.nitf", Compressible, NotBinary)
    final lazy val `vnd.latex-z`: MediaType =
      new MediaType("text", "vnd.latex-z", Compressible, NotBinary)
    final lazy val `vnd.motorola.reflex`: MediaType =
      new MediaType("text", "vnd.motorola.reflex", Compressible, NotBinary)
    final lazy val `vnd.ms-mediapackage`: MediaType =
      new MediaType("text", "vnd.ms-mediapackage", Compressible, NotBinary)
    final lazy val `vnd.net2phone.commcenter.command`: MediaType =
      new MediaType("text", "vnd.net2phone.commcenter.command", Compressible, NotBinary)
    final lazy val `vnd.radisys.msml-basic-layout`: MediaType =
      new MediaType("text", "vnd.radisys.msml-basic-layout", Compressible, NotBinary)
    final lazy val `vnd.si.uricatalogue`: MediaType =
      new MediaType("text", "vnd.si.uricatalogue", Compressible, NotBinary)
    final lazy val `vnd.sun.j2me.app-descriptor`: MediaType =
      new MediaType("text", "vnd.sun.j2me.app-descriptor", Compressible, NotBinary, List("jad"))
    final lazy val `vnd.trolltech.linguist`: MediaType =
      new MediaType("text", "vnd.trolltech.linguist", Compressible, NotBinary)
    final lazy val `vnd.wap.si`: MediaType = new MediaType("text", "vnd.wap.si", Compressible, NotBinary)
    final lazy val `vnd.wap.sl`: MediaType = new MediaType("text", "vnd.wap.sl", Compressible, NotBinary)
    final lazy val `vnd.wap.wml`: MediaType =
      new MediaType("text", "vnd.wap.wml", Compressible, NotBinary, List("wml"))
    final lazy val `vnd.wap.wmlscript`: MediaType =
      new MediaType("text", "vnd.wap.wmlscript", Compressible, NotBinary, List("wmls"))
    final lazy val `vtt`: MediaType = new MediaType("text", "vtt", Compressible, NotBinary, List("vtt"))
    final lazy val `x-asm`: MediaType =
      new MediaType("text", "x-asm", Compressible, NotBinary, List("s", "asm"))
    final lazy val `x-c`: MediaType = new MediaType(
      "text",
      "x-c",
      Compressible,
      NotBinary,
      List("c", "cc", "cxx", "cpp", "h", "hh", "dic"))
    final lazy val `x-component`: MediaType =
      new MediaType("text", "x-component", Compressible, NotBinary, List("htc"))
    final lazy val `x-fortran`: MediaType =
      new MediaType("text", "x-fortran", Compressible, NotBinary, List("f", "for", "f77", "f90"))
    final lazy val `x-gwt-rpc`: MediaType = new MediaType("text", "x-gwt-rpc", Compressible, NotBinary)
    final lazy val `x-handlebars-template`: MediaType =
      new MediaType("text", "x-handlebars-template", Compressible, NotBinary, List("hbs"))
    final lazy val `x-java-source`: MediaType =
      new MediaType("text", "x-java-source", Compressible, NotBinary, List("java"))
    final lazy val `x-jquery-tmpl`: MediaType =
      new MediaType("text", "x-jquery-tmpl", Compressible, NotBinary)
    final lazy val `x-lua`: MediaType =
      new MediaType("text", "x-lua", Compressible, NotBinary, List("lua"))
    final lazy val `x-markdown`: MediaType =
      new MediaType("text", "x-markdown", Compressible, NotBinary, List("mkd"))
    final lazy val `x-nfo`: MediaType =
      new MediaType("text", "x-nfo", Compressible, NotBinary, List("nfo"))
    final lazy val `x-opml`: MediaType =
      new MediaType("text", "x-opml", Compressible, NotBinary, List("opml"))
    final lazy val `x-org`: MediaType =
      new MediaType("text", "x-org", Compressible, NotBinary, List("org"))
    final lazy val `x-pascal`: MediaType =
      new MediaType("text", "x-pascal", Compressible, NotBinary, List("p", "pas"))
    final lazy val `x-processing`: MediaType =
      new MediaType("text", "x-processing", Compressible, NotBinary, List("pde"))
    final lazy val `x-sass`: MediaType =
      new MediaType("text", "x-sass", Compressible, NotBinary, List("sass"))
    final lazy val `x-scss`: MediaType =
      new MediaType("text", "x-scss", Compressible, NotBinary, List("scss"))
    final lazy val `x-setext`: MediaType =
      new MediaType("text", "x-setext", Compressible, NotBinary, List("etx"))
    final lazy val `x-sfv`: MediaType =
      new MediaType("text", "x-sfv", Compressible, NotBinary, List("sfv"))
    final lazy val `x-suse-ymp`: MediaType =
      new MediaType("text", "x-suse-ymp", Compressible, NotBinary, List("ymp"))
    final lazy val `x-uuencode`: MediaType =
      new MediaType("text", "x-uuencode", Compressible, NotBinary, List("uu"))
    final lazy val `x-vcalendar`: MediaType =
      new MediaType("text", "x-vcalendar", Compressible, NotBinary, List("vcs"))
    final lazy val `x-vcard`: MediaType =
      new MediaType("text", "x-vcard", Compressible, NotBinary, List("vcf"))
    final lazy val `xml`: MediaType = new MediaType("text", "xml", Compressible, NotBinary, List("xml"))
    final lazy val `xml-external-parsed-entity`: MediaType =
      new MediaType("text", "xml-external-parsed-entity", Compressible, NotBinary)
    final lazy val `yaml`: MediaType =
      new MediaType("text", "yaml", Compressible, NotBinary, List("yaml", "yml"))
    final lazy val all: List[MediaType] = List(
      `1d-interleaved-parityfec`,
      `cache-manifest`,
      `calendar`,
      `calender`,
      `cmd`,
      `coffeescript`,
      `css`,
      `csv`,
      `csv-schema`,
      `directory`,
      `dns`,
      `ecmascript`,
      `encaprtp`,
      `enriched`,
      `fwdred`,
      `grammar-ref-list`,
      `html`,
      `jade`,
      `javascript`,
      `jcr-cnd`,
      `jsx`,
      `less`,
      `markdown`,
      `mathml`,
      `mizar`,
      `n3`,
      `parameters`,
      `parityfec`,
      `plain`,
      `provenance-notation`,
      `prs.fallenstein.rst`,
      `prs.lines.tag`,
      `prs.prop.logic`,
      `raptorfec`,
      `red`,
      `rfc822-headers`,
      `richtext`,
      `rtf`,
      `rtp-enc-aescm128`,
      `rtploopback`,
      `rtx`,
      `sgml`,
      `shex`,
      `slim`,
      `strings`,
      `stylus`,
      `t140`,
      `tab-separated-values`,
      `troff`,
      `turtle`,
      `ulpfec`,
      `uri-list`,
      `vcard`,
      `vnd.a`,
      `vnd.abc`,
      `vnd.ascii-art`,
      `vnd.curl`,
      `vnd.curl.dcurl`,
      `vnd.curl.mcurl`,
      `vnd.curl.scurl`,
      `vnd.debian.copyright`,
      `vnd.dmclientscript`,
      `vnd.dvb.subtitle`,
      `vnd.esmertec.theme-descriptor`,
      `vnd.fly`,
      `vnd.fmi.flexstor`,
      `vnd.gml`,
      `vnd.graphviz`,
      `vnd.hgl`,
      `vnd.in3d.3dml`,
      `vnd.in3d.spot`,
      `vnd.iptc.newsml`,
      `vnd.iptc.nitf`,
      `vnd.latex-z`,
      `vnd.motorola.reflex`,
      `vnd.ms-mediapackage`,
      `vnd.net2phone.commcenter.command`,
      `vnd.radisys.msml-basic-layout`,
      `vnd.si.uricatalogue`,
      `vnd.sun.j2me.app-descriptor`,
      `vnd.trolltech.linguist`,
      `vnd.wap.si`,
      `vnd.wap.sl`,
      `vnd.wap.wml`,
      `vnd.wap.wmlscript`,
      `vtt`,
      `x-asm`,
      `x-c`,
      `x-component`,
      `x-fortran`,
      `x-gwt-rpc`,
      `x-handlebars-template`,
      `x-java-source`,
      `x-jquery-tmpl`,
      `x-lua`,
      `x-markdown`,
      `x-nfo`,
      `x-opml`,
      `x-org`,
      `x-pascal`,
      `x-processing`,
      `x-sass`,
      `x-scss`,
      `x-setext`,
      `x-sfv`,
      `x-suse-ymp`,
      `x-uuencode`,
      `x-vcalendar`,
      `x-vcard`,
      `xml`,
      `xml-external-parsed-entity`,
      `yaml`
    )
  }
  object video {
    final lazy val `1d-interleaved-parityfec`: MediaType =
      new MediaType("video", "1d-interleaved-parityfec", Compressible, Binary)
    final lazy val `3gpp`: MediaType =
      new MediaType("video", "3gpp", Compressible, Binary, List("3gp", "3gpp"))
    final lazy val `3gpp-tt`: MediaType = new MediaType("video", "3gpp-tt", Compressible, Binary)
    final lazy val `3gpp2`: MediaType = new MediaType("video", "3gpp2", Compressible, Binary, List("3g2"))
    final lazy val `bmpeg`: MediaType = new MediaType("video", "bmpeg", Compressible, Binary)
    final lazy val `bt656`: MediaType = new MediaType("video", "bt656", Compressible, Binary)
    final lazy val `celb`: MediaType = new MediaType("video", "celb", Compressible, Binary)
    final lazy val `dv`: MediaType = new MediaType("video", "dv", Compressible, Binary)
    final lazy val `encaprtp`: MediaType = new MediaType("video", "encaprtp", Compressible, Binary)
    final lazy val `h261`: MediaType = new MediaType("video", "h261", Compressible, Binary, List("h261"))
    final lazy val `h263`: MediaType = new MediaType("video", "h263", Compressible, Binary, List("h263"))
    final lazy val `h263-1998`: MediaType = new MediaType("video", "h263-1998", Compressible, Binary)
    final lazy val `h263-2000`: MediaType = new MediaType("video", "h263-2000", Compressible, Binary)
    final lazy val `h264`: MediaType = new MediaType("video", "h264", Compressible, Binary, List("h264"))
    final lazy val `h264-rcdo`: MediaType = new MediaType("video", "h264-rcdo", Compressible, Binary)
    final lazy val `h264-svc`: MediaType = new MediaType("video", "h264-svc", Compressible, Binary)
    final lazy val `h265`: MediaType = new MediaType("video", "h265", Compressible, Binary)
    final lazy val `iso.segment`: MediaType = new MediaType("video", "iso.segment", Compressible, Binary)
    final lazy val `jpeg`: MediaType = new MediaType("video", "jpeg", Compressible, Binary, List("jpgv"))
    final lazy val `jpeg2000`: MediaType = new MediaType("video", "jpeg2000", Compressible, Binary)
    final lazy val `jpm`: MediaType =
      new MediaType("video", "jpm", Compressible, Binary, List("jpm", "jpgm"))
    final lazy val `mj2`: MediaType =
      new MediaType("video", "mj2", Compressible, Binary, List("mj2", "mjp2"))
    final lazy val `mp1s`: MediaType = new MediaType("video", "mp1s", Compressible, Binary)
    final lazy val `mp2p`: MediaType = new MediaType("video", "mp2p", Compressible, Binary)
    final lazy val `mp2t`: MediaType = new MediaType("video", "mp2t", Compressible, Binary, List("ts"))
    final lazy val `mp4`: MediaType =
      new MediaType("video", "mp4", Uncompressible, Binary, List("mp4", "mp4v", "mpg4"))
    final lazy val `mp4v-es`: MediaType = new MediaType("video", "mp4v-es", Compressible, Binary)
    final lazy val `mpeg`: MediaType = new MediaType(
      "video",
      "mpeg",
      Uncompressible,
      Binary,
      List("mpeg", "mpg", "mpe", "m1v", "m2v"))
    final lazy val `mpeg4-generic`: MediaType =
      new MediaType("video", "mpeg4-generic", Compressible, Binary)
    final lazy val `mpv`: MediaType = new MediaType("video", "mpv", Compressible, Binary)
    final lazy val `nv`: MediaType = new MediaType("video", "nv", Compressible, Binary)
    final lazy val `ogg`: MediaType = new MediaType("video", "ogg", Uncompressible, Binary, List("ogv"))
    final lazy val `parityfec`: MediaType = new MediaType("video", "parityfec", Compressible, Binary)
    final lazy val `pointer`: MediaType = new MediaType("video", "pointer", Compressible, Binary)
    final lazy val `quicktime`: MediaType =
      new MediaType("video", "quicktime", Uncompressible, Binary, List("qt", "mov"))
    final lazy val `raptorfec`: MediaType = new MediaType("video", "raptorfec", Compressible, Binary)
    final lazy val `raw`: MediaType = new MediaType("video", "raw", Compressible, Binary)
    final lazy val `rtp-enc-aescm128`: MediaType =
      new MediaType("video", "rtp-enc-aescm128", Compressible, Binary)
    final lazy val `rtploopback`: MediaType = new MediaType("video", "rtploopback", Compressible, Binary)
    final lazy val `rtx`: MediaType = new MediaType("video", "rtx", Compressible, Binary)
    final lazy val `smpte291`: MediaType = new MediaType("video", "smpte291", Compressible, Binary)
    final lazy val `smpte292m`: MediaType = new MediaType("video", "smpte292m", Compressible, Binary)
    final lazy val `ulpfec`: MediaType = new MediaType("video", "ulpfec", Compressible, Binary)
    final lazy val `vc1`: MediaType = new MediaType("video", "vc1", Compressible, Binary)
    final lazy val `vc2`: MediaType = new MediaType("video", "vc2", Compressible, Binary)
    final lazy val `vnd.cctv`: MediaType = new MediaType("video", "vnd.cctv", Compressible, Binary)
    final lazy val `vnd.dece.hd`: MediaType =
      new MediaType("video", "vnd.dece.hd", Compressible, Binary, List("uvh", "uvvh"))
    final lazy val `vnd.dece.mobile`: MediaType =
      new MediaType("video", "vnd.dece.mobile", Compressible, Binary, List("uvm", "uvvm"))
    final lazy val `vnd.dece.mp4`: MediaType =
      new MediaType("video", "vnd.dece.mp4", Compressible, Binary)
    final lazy val `vnd.dece.pd`: MediaType =
      new MediaType("video", "vnd.dece.pd", Compressible, Binary, List("uvp", "uvvp"))
    final lazy val `vnd.dece.sd`: MediaType =
      new MediaType("video", "vnd.dece.sd", Compressible, Binary, List("uvs", "uvvs"))
    final lazy val `vnd.dece.video`: MediaType =
      new MediaType("video", "vnd.dece.video", Compressible, Binary, List("uvv", "uvvv"))
    final lazy val `vnd.directv.mpeg`: MediaType =
      new MediaType("video", "vnd.directv.mpeg", Compressible, Binary)
    final lazy val `vnd.directv.mpeg-tts`: MediaType =
      new MediaType("video", "vnd.directv.mpeg-tts", Compressible, Binary)
    final lazy val `vnd.dlna.mpeg-tts`: MediaType =
      new MediaType("video", "vnd.dlna.mpeg-tts", Compressible, Binary)
    final lazy val `vnd.dvb.file`: MediaType =
      new MediaType("video", "vnd.dvb.file", Compressible, Binary, List("dvb"))
    final lazy val `vnd.fvt`: MediaType =
      new MediaType("video", "vnd.fvt", Compressible, Binary, List("fvt"))
    final lazy val `vnd.hns.video`: MediaType =
      new MediaType("video", "vnd.hns.video", Compressible, Binary)
    final lazy val `vnd.iptvforum.1dparityfec-1010`: MediaType =
      new MediaType("video", "vnd.iptvforum.1dparityfec-1010", Compressible, Binary)
    final lazy val `vnd.iptvforum.1dparityfec-2005`: MediaType =
      new MediaType("video", "vnd.iptvforum.1dparityfec-2005", Compressible, Binary)
    final lazy val `vnd.iptvforum.2dparityfec-1010`: MediaType =
      new MediaType("video", "vnd.iptvforum.2dparityfec-1010", Compressible, Binary)
    final lazy val `vnd.iptvforum.2dparityfec-2005`: MediaType =
      new MediaType("video", "vnd.iptvforum.2dparityfec-2005", Compressible, Binary)
    final lazy val `vnd.iptvforum.ttsavc`: MediaType =
      new MediaType("video", "vnd.iptvforum.ttsavc", Compressible, Binary)
    final lazy val `vnd.iptvforum.ttsmpeg2`: MediaType =
      new MediaType("video", "vnd.iptvforum.ttsmpeg2", Compressible, Binary)
    final lazy val `vnd.motorola.video`: MediaType =
      new MediaType("video", "vnd.motorola.video", Compressible, Binary)
    final lazy val `vnd.motorola.videop`: MediaType =
      new MediaType("video", "vnd.motorola.videop", Compressible, Binary)
    final lazy val `vnd.mpegurl`: MediaType =
      new MediaType("video", "vnd.mpegurl", Compressible, Binary, List("mxu", "m4u"))
    final lazy val `vnd.ms-playready.media.pyv`: MediaType =
      new MediaType("video", "vnd.ms-playready.media.pyv", Compressible, Binary, List("pyv"))
    final lazy val `vnd.nokia.interleaved-multimedia`: MediaType =
      new MediaType("video", "vnd.nokia.interleaved-multimedia", Compressible, Binary)
    final lazy val `vnd.nokia.mp4vr`: MediaType =
      new MediaType("video", "vnd.nokia.mp4vr", Compressible, Binary)
    final lazy val `vnd.nokia.videovoip`: MediaType =
      new MediaType("video", "vnd.nokia.videovoip", Compressible, Binary)
    final lazy val `vnd.objectvideo`: MediaType =
      new MediaType("video", "vnd.objectvideo", Compressible, Binary)
    final lazy val `vnd.radgamettools.bink`: MediaType =
      new MediaType("video", "vnd.radgamettools.bink", Compressible, Binary)
    final lazy val `vnd.radgamettools.smacker`: MediaType =
      new MediaType("video", "vnd.radgamettools.smacker", Compressible, Binary)
    final lazy val `vnd.sealed.mpeg1`: MediaType =
      new MediaType("video", "vnd.sealed.mpeg1", Compressible, Binary)
    final lazy val `vnd.sealed.mpeg4`: MediaType =
      new MediaType("video", "vnd.sealed.mpeg4", Compressible, Binary)
    final lazy val `vnd.sealed.swf`: MediaType =
      new MediaType("video", "vnd.sealed.swf", Compressible, Binary)
    final lazy val `vnd.sealedmedia.softseal.mov`: MediaType =
      new MediaType("video", "vnd.sealedmedia.softseal.mov", Compressible, Binary)
    final lazy val `vnd.uvvu.mp4`: MediaType =
      new MediaType("video", "vnd.uvvu.mp4", Compressible, Binary, List("uvu", "uvvu"))
    final lazy val `vnd.vivo`: MediaType =
      new MediaType("video", "vnd.vivo", Compressible, Binary, List("viv"))
    final lazy val `vp8`: MediaType = new MediaType("video", "vp8", Compressible, Binary)
    final lazy val `webm`: MediaType =
      new MediaType("video", "webm", Uncompressible, Binary, List("webm"))
    final lazy val `x-f4v`: MediaType = new MediaType("video", "x-f4v", Compressible, Binary, List("f4v"))
    final lazy val `x-fli`: MediaType = new MediaType("video", "x-fli", Compressible, Binary, List("fli"))
    final lazy val `x-flv`: MediaType =
      new MediaType("video", "x-flv", Uncompressible, Binary, List("flv"))
    final lazy val `x-m4v`: MediaType = new MediaType("video", "x-m4v", Compressible, Binary, List("m4v"))
    final lazy val `x-matroska`: MediaType =
      new MediaType("video", "x-matroska", Uncompressible, Binary, List("mkv", "mk3d", "mks"))
    final lazy val `x-mng`: MediaType = new MediaType("video", "x-mng", Compressible, Binary, List("mng"))
    final lazy val `x-ms-asf`: MediaType =
      new MediaType("video", "x-ms-asf", Compressible, Binary, List("asf", "asx"))
    final lazy val `x-ms-vob`: MediaType =
      new MediaType("video", "x-ms-vob", Compressible, Binary, List("vob"))
    final lazy val `x-ms-wm`: MediaType =
      new MediaType("video", "x-ms-wm", Compressible, Binary, List("wm"))
    final lazy val `x-ms-wmv`: MediaType =
      new MediaType("video", "x-ms-wmv", Uncompressible, Binary, List("wmv"))
    final lazy val `x-ms-wmx`: MediaType =
      new MediaType("video", "x-ms-wmx", Compressible, Binary, List("wmx"))
    final lazy val `x-ms-wvx`: MediaType =
      new MediaType("video", "x-ms-wvx", Compressible, Binary, List("wvx"))
    final lazy val `x-msvideo`: MediaType =
      new MediaType("video", "x-msvideo", Compressible, Binary, List("avi"))
    final lazy val `x-sgi-movie`: MediaType =
      new MediaType("video", "x-sgi-movie", Compressible, Binary, List("movie"))
    final lazy val `x-smv`: MediaType = new MediaType("video", "x-smv", Compressible, Binary, List("smv"))
    final lazy val all: List[MediaType] = List(
      `1d-interleaved-parityfec`,
      `3gpp`,
      `3gpp-tt`,
      `3gpp2`,
      `bmpeg`,
      `bt656`,
      `celb`,
      `dv`,
      `encaprtp`,
      `h261`,
      `h263`,
      `h263-1998`,
      `h263-2000`,
      `h264`,
      `h264-rcdo`,
      `h264-svc`,
      `h265`,
      `iso.segment`,
      `jpeg`,
      `jpeg2000`,
      `jpm`,
      `mj2`,
      `mp1s`,
      `mp2p`,
      `mp2t`,
      `mp4`,
      `mp4v-es`,
      `mpeg`,
      `mpeg4-generic`,
      `mpv`,
      `nv`,
      `ogg`,
      `parityfec`,
      `pointer`,
      `quicktime`,
      `raptorfec`,
      `raw`,
      `rtp-enc-aescm128`,
      `rtploopback`,
      `rtx`,
      `smpte291`,
      `smpte292m`,
      `ulpfec`,
      `vc1`,
      `vc2`,
      `vnd.cctv`,
      `vnd.dece.hd`,
      `vnd.dece.mobile`,
      `vnd.dece.mp4`,
      `vnd.dece.pd`,
      `vnd.dece.sd`,
      `vnd.dece.video`,
      `vnd.directv.mpeg`,
      `vnd.directv.mpeg-tts`,
      `vnd.dlna.mpeg-tts`,
      `vnd.dvb.file`,
      `vnd.fvt`,
      `vnd.hns.video`,
      `vnd.iptvforum.1dparityfec-1010`,
      `vnd.iptvforum.1dparityfec-2005`,
      `vnd.iptvforum.2dparityfec-1010`,
      `vnd.iptvforum.2dparityfec-2005`,
      `vnd.iptvforum.ttsavc`,
      `vnd.iptvforum.ttsmpeg2`,
      `vnd.motorola.video`,
      `vnd.motorola.videop`,
      `vnd.mpegurl`,
      `vnd.ms-playready.media.pyv`,
      `vnd.nokia.interleaved-multimedia`,
      `vnd.nokia.mp4vr`,
      `vnd.nokia.videovoip`,
      `vnd.objectvideo`,
      `vnd.radgamettools.bink`,
      `vnd.radgamettools.smacker`,
      `vnd.sealed.mpeg1`,
      `vnd.sealed.mpeg4`,
      `vnd.sealed.swf`,
      `vnd.sealedmedia.softseal.mov`,
      `vnd.uvvu.mp4`,
      `vnd.vivo`,
      `vp8`,
      `webm`,
      `x-f4v`,
      `x-fli`,
      `x-flv`,
      `x-m4v`,
      `x-matroska`,
      `x-mng`,
      `x-ms-asf`,
      `x-ms-vob`,
      `x-ms-wm`,
      `x-ms-wmv`,
      `x-ms-wmx`,
      `x-ms-wvx`,
      `x-msvideo`,
      `x-sgi-movie`,
      `x-smv`
    )
  }
  object x_conference {
    final lazy val `x-cooltalk`: MediaType =
      new MediaType("x-conference", "x-cooltalk", Compressible, NotBinary, List("ice"))
    final lazy val all: List[MediaType] = List(`x-cooltalk`)
  }
  object x_shader {
    final lazy val `x-fragment`: MediaType =
      new MediaType("x-shader", "x-fragment", Compressible, NotBinary)
    final lazy val `x-vertex`: MediaType = new MediaType("x-shader", "x-vertex", Compressible, NotBinary)
    final lazy val all: List[MediaType] = List(`x-fragment`, `x-vertex`)
  }
}
