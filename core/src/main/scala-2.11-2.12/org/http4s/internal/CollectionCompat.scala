...
private[http4s] object CollectionCompat {
  ...
  def pairsToMultiParams[K, V](map: collection.Seq[(K, Option[V])]): immutable.Map[K, immutable.Seq[V]] = {
  ...
  def mapValues[K, A, B](map: collection.Map[K, A])(f: A => B): immutable.Map[K, B] =
