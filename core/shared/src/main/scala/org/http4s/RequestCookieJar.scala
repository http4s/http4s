...
class RequestCookieJar private (val cookies: List[RequestCookie]) extends AnyVal {
  ...
  /** Filters this map by retaining only keys satisfying a predicate.
    ...
    * @return
    *   an immutable map consisting only of those key value pairs of this map where the key
    *   satisfies the predicate `p`. The resulting map wraps the original map without copying any
    *   elements.
    */
