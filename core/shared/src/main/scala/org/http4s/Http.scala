...
object Http {
  ...
  /** Lifts a function into an [[Http]] kleisli. The application of `run` is suspended in `F` to
  ...
  /** Transforms an [[Http]] on its input. The application of the transformed function is suspended
    * in `F` to permit more efficient combination of routes via `SemigroupK`.
    *
    * @tparam F
    *   the effect of the [[Response]] returned by the [[Http]]
    * @tparam G
