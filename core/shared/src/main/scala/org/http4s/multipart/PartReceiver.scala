...
trait PartReceiver[F[_], A] {
  ...
  def preprocess(transformPartEntity: Entity[F] => Entity[F]): PartReceiver[F, A] =
    part => this.receive(part.copy(entity = transformPartEntity(part.entity)))
...
object PartReceiver extends PartReceiverPlatform {
  ...
  private def partBodyExceedsMaximumLengthError = InvalidMessageBodyFailure(
    "Part body exceeds maximum length"
  )

  private def limitPartSize[F[_]](
      maxPartSizeBytes: Long
  )(implicit F: ApplicativeError[F, Throwable]): Entity[F] => Entity[F] = {
    case Entity.Empty => Entity.Empty
    case e @ Entity.Strict(bytes) =>
      if (bytes.length <= maxPartSizeBytes) e
      else Entity.stream(Stream.raiseError(partBodyExceedsMaximumLengthError))

    case Entity.Streamed(s, _) =>
      def go(s: Stream[F, Byte], accumSize: Long): Pull[F, Byte, Unit] = s.pull.uncons.flatMap {
        case Some((chunk, tail)) =>
          ...
          else Pull.raiseError[F](partBodyExceedsMaximumLengthError)
        case None =>
          Pull.done
      }

      Entity.stream(go(s, 0L).stream)
  }
