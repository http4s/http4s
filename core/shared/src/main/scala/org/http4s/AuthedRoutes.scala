...
object AuthedRoutes {
  ...
  /** Lifts a function into an [[AuthedRoutes]]. The application of `run` is suspended in `F` to
  ...
  /** Lifts a partial function into an [[AuthedRoutes]]. The application of the partial function is
    ...
    * @return
    *   An [[AuthedRoutes]] that returns some [[Response]] in an `OptionT[F, *]` wherever `pf` is
