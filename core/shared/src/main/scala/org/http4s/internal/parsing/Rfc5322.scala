...
object Rfc5322 {
  ...
  val comment: Parser[Unit] = Parser.defer(char('(') ~ (FWS.?.with1 ~ ccontent).backtrack.rep0 ~ FWS.? ~ char(')')).void
  ...
  val `angle-addr`: Parser[String] = `addr-spec`.between(char('<'), char('>')).between(CFWS.?, CFWS.?)
