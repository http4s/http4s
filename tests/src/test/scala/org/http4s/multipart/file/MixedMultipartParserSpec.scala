...
object MixedMultipartParserSpec extends Specification {
  ...
  def unspool(
    ...
    if (str.isEmpty) {
      ...
      Stream.emits(str.getBytes(charset))
    } else {
  ...
  "form streaming mixed parsing" should {
    Fragments.foreach(List(1, 2, 3, 5, 8, 13, 21, 987)) { chunkSize =>
      s"produce the body from a single part with chunk size ${chunkSize}" in {
        ...
        val results =
          unspool(input, chunkSize).through(MultipartParser.parseStreamedFile[IO](boundary))
    ...
    "produce the body from a single part that doesn't start with a \r\n" in {
      ...
      val input = ruinDelims(unprocessedInput)
      val results =
        unspool(input, 15).through(MultipartParser.parseStreamedFile[IO](boundary))
      val multipartMaterialized = results.compile.last.map(_.get).unsafeRunSync()
    ...
    "parse utf8 headers properly" in {
      ...
      val results =
        unspool(input, 15, StandardCharsets.UTF_8)
          .through(MultipartParser.parseStreamedFile[IO](boundary))
      val multipartMaterialized = results.compile.last.map(_.get).unsafeRunSync()
    ...
    "fail if the header is too large" in {
      ...
      val results =
        unspool(input, 15).through(MultipartParser.parseStreamedFile[IO](boundary, maxSize))
    ...
    "produce the body from a single part input of one chunk" in {
      ...
      val results = unspool(input).through(MultipartParser.parseStreamedFile[IO](boundary))
      val multipartMaterialized = results.compile.last.map(_.get).unsafeRunSync()
      val headers = multipartMaterialized.parts.foldLeft(Headers.empty)(_ ++ _.headers)
      val bodies = multipartMaterialized.parts
        .foldLeft(Stream.empty.covary[IO]: Stream[IO, Byte])(_ ++ _.body)
    ...
    "parse uneven input properly" in {
      ...
      val results = unprocessed.through(MultipartParser.parseStreamedFile[IO](boundary))
      val multipartMaterialized = results.compile.last.map(_.get).unsafeRunSync()
      val bodies = multipartMaterialized
        .parts(1)
        .body
    ...
    "produce the correct headers from a two part input" in {
      ...
      val results = unspool(input).through(MultipartParser.parseStreamedFile[IO](boundaryTest))
    ...
    "parse parts lazily" in {
      ...
      val boundaryTest = Boundary("RU(_9F(PcJK5+JMOPCAF6Aj4iSXvpJkWy):6s)YU0")
      val results =
        unspool(input).through(MultipartParser.parseToPartsStreamedFile[IO](boundaryTest))
    ...
    "produce a temp file when exceeding allocation limit" in {
      val unprocessedInput =
        """
          |--_5PHqf8_Pl1FCzBuT5o_mVZg36k67UYI
          |Content-Disposition: form-data; name="upload"; filename="integration.txt"
      ...
      multipartMaterialized.parts match {
        case Vector(f @ FilePart(headers, _, filePath), BasicPart(_, _)) =>
          ...
          //File was successfully created for the part
          file.isFile must_== true
          ...
          //Running it through "body" automatically deletes the temp file
          f.body
            .through(text.utf8Decode[IO])
            .compile
            .foldMonoid
            .unsafeRunSync() must_== expected
          file.exists() must_== false

        case _ =>
          ko("Multipart deserialized into the wrong parts")
      }
    }
  }
}
