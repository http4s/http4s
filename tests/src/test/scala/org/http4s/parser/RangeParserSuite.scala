...
class RangeParserSuite extends Http4sSuite {
  ...
  test("RangeParser should parse string") {
    val headers = Seq(
      "bytes=0-500" -> Range(RangeUnit.Bytes, SubRange(0, 500)),
      "bytes=0-499,500-999,1000-1500" -> Range(
        RangeUnit.Bytes,
        SubRange(0, 499),
        SubRange(500, 999),
        SubRange(1000, 1500)),
      "page=0-100" -> Range(RangeUnit("page"), SubRange(0, 100)),
      "bytes=10-" -> Range(10),
      "bytes=-90" -> Range(-90)
    )

    headers.foreach { case (str, expected) =>
      assertEquals(Range.parse(str), Right(expected))
    }

    assert(Range.parse("bytes=10").isLeft)
  }
