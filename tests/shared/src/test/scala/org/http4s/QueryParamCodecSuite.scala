...
class QueryParamCodecSuite extends Http4sSuite with QueryParamCodecInstances {
  ...
  checkAll(
    "Instant QueryParamCodec",
    QueryParamCodecLaws[Instant](ArbitraryInstant, implicitly, implicitly, implicitly))
  checkAll(
    "LocalDate QueryParamCodec",
    QueryParamCodecLaws[LocalDate](ArbitraryLocalDate, implicitly, implicitly, implicitly))
  ...
  if (!sys.props.get("java.specification.version").contains("1.8")) {
    ...
    checkAll(
      "ZonedDateTime QueryParamCodec",
      QueryParamCodecLaws[ZonedDateTime](
        ArbitraryZonedDateTime,
        implicitly,
        implicitly,
        implicitly))
  }
...
trait QueryParamCodecInstances {
  ...
  implicit val ArbitraryZonedDateTime: Arbitrary[ZonedDateTime] = {
    ...
    Arbitrary(
      for {
        instant <- ArbitraryInstant.arbitrary // re-use `Gen` from `Arbitrary[Instant]` here
