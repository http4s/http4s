...
class MultipartReceiverSuite extends Http4sSuite {
  ...
  test("`MultipartReceiver.at(...).once` should fail if the given field is missing") {
    ...
    val requestWithoutFoo = multipartRequest(Part.formData("not-foo", "bar"))
  ...
  test(
    ...
    val maliciousRequest =
      multipartRequest(Part.fileData("not-foo", "infinite.bin", Entity.stream(infiniteBytes)))
  ...
  test(
    ...
    val maliciousRequest = multipartRequest(
      Part.fileData(
        "foo",
        "infinite.txt",
        Entity.stream(infiniteText.through(fs2.text.utf8.encode)),
      )
    )
  ...
  test(
    ...
    val requestWithMultipleFoos = multipartRequest(
      Part.formData("foo", "bar1"),
      Part.formData("foo", "bar2"),
    )
  ...
  test(
    ...
    val requestWithFoo = multipartRequest(Part.formData("foo", "bar"))
  ...
  test(
    ...
    val requestWithFooAndOthers = multipartRequest(
      Part.formData("bing", "bong"),
      Part.formData("ding", "dong"),
      Part.fileData(
        "file",
        "file.txt",
        Entity.stream(infiniteText.take(512).through(fs2.text.utf8.encode)),
      ),
      Part.formData("foo", "bar"), // <- here it is!
      Part.formData("baz", "biff"),
    )
  ...
  test(
    ...
    val requestWithManyFoos = multipartRequest(
      Part.formData("foo", "bar1"),
      Part.formData("foo", "bar2"),
      Part.formData("not-foo", "bar3"),
      Part.formData("foo", "bar4"),
    )
  ...
  test(
    ...
    val requestWithInfiniteFoo = multipartRequest(
      Part.fileData("foo", "foo.bin", Entity.stream(infiniteBytes))
    )
  ...
  test(
    ...
    val requestWithFoo = multipartRequest(
      Part.fileData("foo", "foo.bin", Entity.stream(infiniteBytes.take(512)))
    )
  ...
  test(
    ...
    val arbitraryRequest = multipartRequest(
      ...
      Part.fileData("c", "c.bin", Entity.stream(infiniteBytes.take(512))),
      Part.fileData("d", "d.bin", Entity.stream(infiniteBytes.take(256))),
    )
  ...
  locally {
    ...
    test(
      ...
      val badFoo = Part
        .formData("foo", "")
        .copy(entity = Entity.stream(infiniteText.take(8192).through(fs2.text.utf8.encode)))
      val badBar =
        Part
          .formData("bar", "")
          .copy(entity = Entity.stream(infiniteText.through(fs2.text.utf8.encode)))
      val badFile =
        Part.fileData[IO]("file", "upload.bin", Entity.stream(infiniteBytes.take(65535)))

      val goodFoo = Part.formData("foo", "hello")
      val goodBar = Part.formData("bar", "world")
      val goodFile =
        Part.fileData[IO]("file", "upload.bin", Entity.stream(infiniteBytes.take(1024)))
    ...
    test(
      ...
      val requestIO = multipartRequest(
        Part.formData("foo", "hello"),
        Part.formData("bar", "world"),
        Part.fileData[IO](
          "file",
          "upload.bin",
          Entity.stream(Stream.emit("file content here").through(fs2.text.utf8.encode[IO])),
        ),
        Part.formData("a", "Hey!"),
        Part.formData("b", "I'm here"),
        Part.formData("c", "to distract"),
        Part.formData("d", "you!"),
      )
  ...
  test(
    ...
    val parts =
      Iterator.from(0).take(10).map(i => Part.formData(i.toString, "whatever")).toVector
