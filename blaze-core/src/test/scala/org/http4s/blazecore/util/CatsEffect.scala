...
trait CatsEffect {
  ...
  implicit def effectAsResult[F[_]: Async, R](implicit R: AsResult[R], D: Dispatcher[F]): AsResult[F[R]] = new AsResult[F[R]] {
    def asResult(t: => F[R]): Result = {
      R.asResult(D.unsafeRunTimed(t, Timeout))
    }
  }

  implicit def resourceAsResult[F[_]: Async, R](implicit R: AsResult[R], D: Dispatcher[F]): AsResult[Resource[F,R]] = new AsResult[Resource[F,R]]{
