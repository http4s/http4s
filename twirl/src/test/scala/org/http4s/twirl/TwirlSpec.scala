...
class TwirlSpec extends Http4sSpec {
  ...
  "HTML encoder" should {
    "return Content-Type text/html with proper charset" in prop { implicit cs: Charset =>
      ...
      headers.get(`Content-Type`) must_== Some(`Content-Type`(MediaType.text.`text/html`, Some(cs)))
    }
    ...
    "return Content-Type application/javascript with proper charset" in prop {
      implicit cs: Charset =>
        ...
        headers.get(`Content-Type`) must_== Some(
          `Content-Type`(MediaType.application.`application/javascript`, Some(cs)))
    }
  ...
  "Text encoder" should {
    "return Content-Type text/plain with proper charset" in prop { implicit cs: Charset =>
      ...
      headers.get(`Content-Type`) must_== Some(
        `Content-Type`(MediaType.text.`text/plain`, Some(cs)))
    }
  ...
  "XML encoder" should {
    "return Content-Type application/xml with proper charset" in prop { implicit cs: Charset =>
      ...
      headers.get(`Content-Type`) must_== Some(
        `Content-Type`(MediaType.application.`application/xml`, Some(cs)))
    }
